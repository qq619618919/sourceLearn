...
/**
 * // TODO_MA 注释： ContainerManager 用来管理zookeeper container类型的节点，
 * // TODO_MA 注释： container 类型的节点作为容器借来用来存放别的节点，当一个 container 类型节点的所有子节点都被删除之后，
 * // TODO_MA 注释： ContainerManager 会按照固定的检查周期去找到这些空的 container 节点然后把他们删除掉
 * Manages cleanup of container ZNodes. This class is meant to only be run from the leader.
 * There's no harm in running from followers/observers but that will be extra work that's not needed.
 * Once started, it periodically checks container nodes that have a cversion &gt; 0 and have no children.
 * A delete is attempted on the node. The result of the delete is unimportant.
...
public class ContainerManager {
    ...
    public ContainerManager(ZKDatabase zkDb, RequestProcessor requestProcessor, int checkIntervalMs, int maxPerMinute,
        ...
        LOG.info("Using checkIntervalMs={} maxPerMinute={} maxNeverUsedIntervalMs={}", checkIntervalMs, maxPerMinute,
