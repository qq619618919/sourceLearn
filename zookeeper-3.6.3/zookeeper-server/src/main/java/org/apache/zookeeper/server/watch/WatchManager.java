...
public class WatchManager implements IWatchManager {
     ...
     *  注释： ZNode path 对应的一堆 Watcher
     */
    private final Map<String, Set<Watcher>> watchTable = new HashMap<>();

    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： Watcher 对应的一堆 ZNode Path
     */
    ...
    public WatcherOrBitSet triggerWatch(String path, EventType type, WatcherOrBitSet supress) {
        ...
        // TODO_MA 马中华 注释： 封装一个 WatchedEvent
        WatchedEvent e = new WatchedEvent(type, KeeperState.SyncConnected, path);

        // TODO_MA 马中华 注释： 需要触发的 Watcher 集合
        ...
        synchronized (this) {
            for (String localPath : pathParentIterator.asIterable()) {
                ...
                // TODO_MA 注释： 找到该 path 节点对应的所有 Watcher 集合
                Set<Watcher> thisWatchers = watchTable.get(localPath);
                if (thisWatchers == null || thisWatchers.isEmpty()) {
                    continue;
                }

                // TODO_MA 注释： 遍历每个 Watcher
