...
public class JettyAdminServer implements AdminServer {
    ...
    public JettyAdminServer(String address, int port, int timeout, String commandUrl, int httpVersion,
            boolean portUnification, boolean forceHttps) throws IOException, GeneralSecurityException {
        ...
        if(!portUnification && !forceHttps) {
            ...
            try(QuorumX509Util x509Util = new QuorumX509Util()) {
                ...
                if(forceHttps) {
                    ...
                    connector = new ServerConnector(server, new UnifiedConnectionFactory(sslContextFactory,
                            HttpVersion.fromVersion(httpVersion).asString()), new HttpConnectionFactory(config));
                }
            }
        }
    ...
    public void start() throws AdminServerException {
        try {
            ...
            String message = String
                    .format("Problem starting AdminServer on address %s, port %d and command URL %s", address, port,
                            commandUrl);
    ...
    public void shutdown() throws AdminServerException {
        try {
            ...
            String message = String
                    .format("Problem stopping AdminServer on address %s, port %d and command URL %s", address, port,
                            commandUrl);
            throw new AdminServerException(message, e);
        }
    }
