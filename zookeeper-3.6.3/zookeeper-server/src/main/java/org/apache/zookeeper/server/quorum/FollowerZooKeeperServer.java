...
public class FollowerZooKeeperServer extends LearnerZooKeeperServer {
    ...
    protected void setupRequestProcessors() {
        ...
        // TODO_MA 注释： 第三个： FinalRequestProcessor
        RequestProcessor finalProcessor = new FinalRequestProcessor(this);

        // TODO_MA 注释： 第二个： CommitProcessor
        commitProcessor = new CommitProcessor(finalProcessor, Long.toString(getServerId()), true,
                getZooKeeperServerListener());
        commitProcessor.start();

        // TODO_MA 注释： 第一个： FollowerRequestProcessor
        firstProcessor = new FollowerRequestProcessor(this, commitProcessor);
        ((FollowerRequestProcessor) firstProcessor).start();

        // TODO_MA 注释： SyncRequestProcessor
    ...
    public void commit(long zxid) {
        ...
        if(firstElementZxid != zxid) {
            LOG.error("Committing zxid 0x" + Long.toHexString(zxid) + " but next pending txn 0x" + Long
                    .toHexString(firstElementZxid));
    ...
    public synchronized void sync() {
        ...
        // TODO_MA 注释： 从队列中，获取同步请求
        Request r = pendingSyncs.remove();

        // TODO_MA 注释： 发送同步请求给 Leader
