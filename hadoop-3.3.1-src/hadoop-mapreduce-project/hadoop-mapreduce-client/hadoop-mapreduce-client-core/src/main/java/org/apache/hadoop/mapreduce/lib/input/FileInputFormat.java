...
public abstract class FileInputFormat<K, V> extends InputFormat<K, V> {
    ...
    protected List<FileStatus> listStatus(JobContext job) throws IOException {
        ...
        if (numThreads == 1) {
            ...
            try {
                LocatedFileStatusFetcher locatedFileStatusFetcher = new LocatedFileStatusFetcher(job.getConfiguration(), dirs,
                        recursive, inputFilter, true
                );
     ...
     * @throws IOException
     * // TODO_MA 马中华 注释： 假设现在有 300M 的数据，3个block ， 3个split
     * // TODO_MA 马中华 注释： 假设现在有 260M 的数据，3个bllck（128 128 4），  2个切片（128M  132M）
     */
    public List<InputSplit> getSplits(JobContext job) throws IOException {
         ...
         *  注释：
         *  getFormatMinSplitSize() = 1， mapreduce.input.fileinputformat.split.minsize = 1
         *  1、minSize = 1
         *  2、maxSize = Long.MAX_VALUE = mapreduce.input.fileinputformat.split.maxsize
         */
        long minSize = Math.max(getFormatMinSplitSize(), getMinSplitSize(job));
        long maxSize = getMaxSplitSize(job);

        // generate splits
        // TODO_MA 马中华 注释： 这就是个容器，存储每个逻辑切片
        ...
        boolean ignoreDirs = !getInputDirRecursive(job) && job.getConfiguration()
                .getBoolean(INPUT_DIR_NONRECURSIVE_IGNORE_SUBDIRS, false);
        ...
        for (FileStatus file : files) {
            ...
            if (length != 0) {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 如果配置了文件可切割，则进行逻辑切片，否则单独成为一个切片
                 *  如果你想让一个 文件单独启动一个 Task ，直接让 isSplitable 方法返回false 即可
                 */
                if (isSplitable(job, path)) {
                    // TODO_MA 马中华 注释： 这个是获取这个file 的指定数据块大小, 默认 128M
                    long blockSize = file.getBlockSize();
                    // TODO_MA 马中华 注释： 通过计算拿到 blockSize, minSize, maxSize 中的中间值作为 切片大小
                    // TODO_MA 马中华 注释： splitsize = 256M,  0-256  256-512, ....
                    // TODO_MA 马中华 注释： splitsize = 默认情况下的 128M
                    // TODO_MA 马中华 注释： 是不是可以认为 一个数据块 一个 Task？ 本来是可以的，但是作者思考的更多，做了优化
                    // TODO_MA 马中华 注释： 优化：在某些极端的情况下，可以少启动一个 MapTask
                    // TODO_MA 马中华 注释： 本来要启动 100001 ==> 100000
                     ...
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： 具体的切片工作机制！
                     *  bytesRemaining 现在文件中剩下的未切片的数据大小
                     *  splitSize 切片大小 = 128M
                     *  SPLIT_SLOP = 1.1
                     *  假设最后两个数据块分别为： a(128M)  和   b(>12.8M  <12.8M>)
                     *  结论：
                     *  1、如果 最后两个数据块的 总大小 大于 128M 的 1.1 倍，则最后两个数据块都单独形成一个逻辑切片
                     *  2、如果 最后两个数据块的 总大小 小于 128M 的 1.1 倍，a 和 b 就形成一个逻辑切片
                     */
                    while (((double) bytesRemaining) / splitSize > SPLIT_SLOP) {
                        int blkIndex = getBlockIndex(blkLocations, length - bytesRemaining);
                        // TODO_MA 马中华 注释： 执行这儿的时候，把剩下数据的前面 128M 单独形成一个逻辑切片，然后又剩下一些数据
                    ...
                    // TODO_MA 马中华 注释： 剩下的数据，单独成为一个数据块！
                    if (bytesRemaining != 0) {
                        int blkIndex = getBlockIndex(blkLocations, length - bytesRemaining);
                ...
                // TODO_MA 马中华 注释： 当前这个文件单独形成一个逻辑切片
                else { // not splitable
                    if (LOG.isDebugEnabled()) {
            ...
            // TODO_MA 马中华 注释： 如果文件长度为0，那么构建一个 空 Split
            else {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "Total # of splits generated by getSplits: " + splits.size() + ", TimeTaken: " + sw.now(TimeUnit.MILLISECONDS));
        }
        return splits;
    }
