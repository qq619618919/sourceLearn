...
public class JobClient extends CLI implements AutoCloseable {
    ...
    private long retryInterval = MRJobConfig.DEFAULT_MR_CLIENT_JOB_RETRY_INTERVAL;
    ...
    static class NetworkedJob implements RunningJob {
         ...
         * has completely forgotten about the job.
         * (eg, 24 hours after the job completes.)
         */
        public NetworkedJob(JobStatus status, Cluster cluster) throws IOException {
            this(status, cluster, new JobConf(status.getJobFile()));
        }

        private NetworkedJob(JobStatus status, Cluster cluster, JobConf conf) throws IOException {
        ...
        public synchronized void setJobPriority(String priority) throws IOException {
            try {
                job.setPriority(org.apache.hadoop.mapreduce.JobPriority.valueOf(priority));
            } catch (InterruptedException ie) {
                throw new IOException(ie);
            }
        }
        ...
        public synchronized TaskCompletionEvent[] getTaskCompletionEvents(int startFrom) throws IOException {
            try {
                org.apache.hadoop.mapreduce.TaskCompletionEvent[] acls = job.getTaskCompletionEvents(startFrom, 10);
            ...
            return clientUgi.doAs(new PrivilegedExceptionAction<ClusterStatus>() {
                public ClusterStatus run() throws IOException, InterruptedException {
                    ...
                    return new ClusterStatus(metrics.getTaskTrackerCount(), metrics.getBlackListedTaskTrackerCount(),
                            cluster.getTaskTrackerExpiryInterval(), metrics.getOccupiedMapSlots(), metrics.getOccupiedReduceSlots(),
                            metrics.getMapSlotCapacity(), metrics.getReduceSlotCapacity(), cluster.getJobTrackerStatus(),
                            metrics.getDecommissionedTaskTrackerCount(), metrics.getGrayListedTaskTrackerCount()
                    );
                }
            });
        } catch (InterruptedException ie) {
            ...
            return clientUgi.doAs(new PrivilegedExceptionAction<ClusterStatus>() {
                public ClusterStatus run() throws IOException, InterruptedException {
                    ...
                    return new ClusterStatus(arrayToStringList(cluster.getActiveTaskTrackers()),
                            arrayToBlackListInfo(cluster.getBlackListedTaskTrackers()), cluster.getTaskTrackerExpiryInterval(),
    ...
    public JobStatus[] getAllJobs() throws IOException {
        try {
            org.apache.hadoop.mapreduce.JobStatus[] jobs = clientUgi.doAs(
                    new PrivilegedExceptionAction<org.apache.hadoop.mapreduce.JobStatus[]>() {
    ...
    protected long getCounter(org.apache.hadoop.mapreduce.Counters cntrs, String counterGroupName,
    ...
    public QueueAclsInfo[] getQueueAclsForCurrentUser() throws IOException {
        try {
            org.apache.hadoop.mapreduce.QueueAclsInfo[] acls = clientUgi.doAs(
                    new PrivilegedExceptionAction<org.apache.hadoop.mapreduce.QueueAclsInfo[]>() {
    ...
    public long renewDelegationToken(
            Token<DelegationTokenIdentifier> token) throws InvalidToken, IOException, InterruptedException {
