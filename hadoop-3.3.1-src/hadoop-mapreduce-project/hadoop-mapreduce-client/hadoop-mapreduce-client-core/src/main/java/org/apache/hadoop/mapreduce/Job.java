...
public class Job extends JobContextImpl implements JobContext, AutoCloseable {
    ...
    @Private
    @VisibleForTesting
    JobSubmitter getJobSubmitter(FileSystem fs, ClientProtocol submitClient) throws IOException {
    ...
    public void submit() throws IOException, InterruptedException, ClassNotFoundException {
         ...
         *  注释： 也是初始化 cluster
         *  因为有两种情况需要考虑：
         *  1、通过 shell 命令提交， 在这之前，已经初始化得到 CLuster 对象了。
         *  2、本地调试： MR程序中的最后一句代码就是： job.waitForCompetetion(){job.submit()}
         *      就在这儿初始化 Cluster, 就是为了拿到 RM RPC 代理
         */
        connect();

        // TODO_MA 马中华 注释： JobSubmitter Job 提交器
    ...
    public boolean monitorAndPrintJob() throws IOException, InterruptedException {
        ...
        while (!isComplete() || !reportedAfterCompletion) {
            ...
            String report = (" map " + StringUtils.formatPercent(mapProgress(), 0) + " reduce " + StringUtils.formatPercent(
                    reduceProgress(), 0)
            );
    ...
    public static int getProgressPollInterval(Configuration conf) {
        ...
        if (progMonitorPollIntervalMillis < 1) {
            LOG.warn(
                    PROGRESS_MONITOR_POLL_INTERVAL_KEY + " has been set to an invalid value; " + " replacing with " + DEFAULT_MONITOR_POLL_INTERVAL);
    ...
    public static int getCompletionPollInterval(Configuration conf) {
        ...
        if (completionPollIntervalMillis < 1) {
            LOG.warn(
                    COMPLETION_POLL_INTERVAL_KEY + " has been set to an invalid value; " + "replacing with " + DEFAULT_COMPLETION_POLL_INTERVAL);
