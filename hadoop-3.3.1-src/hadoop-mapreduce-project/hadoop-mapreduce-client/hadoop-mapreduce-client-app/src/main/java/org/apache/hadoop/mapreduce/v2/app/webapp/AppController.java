...
public class AppController extends Controller implements AMParams {
    ...
    public void info() {
        ...
        info("Application Master Overview").__("Application ID:", info.getId()).__("Application Name:", info.getName())
                .__("User:", info.getUser()).__("Started on:", Times.format(info.getStartTime()))
                .__("Elasped: ", org.apache.hadoop.util.StringUtils.formatTime(info.getElapsedTime()));
    ...
    public void requireJob() {
        ...
        if (!checkAccess(job)) {
            ...
            throw new RuntimeException(
                    "Access denied: User " + request().getRemoteUser() + " does not have permission to view job " + $(JOB_ID));
        }
    }
