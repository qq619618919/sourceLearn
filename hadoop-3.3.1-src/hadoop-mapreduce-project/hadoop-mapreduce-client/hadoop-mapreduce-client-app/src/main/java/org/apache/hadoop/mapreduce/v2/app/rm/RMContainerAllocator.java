...
public class RMContainerAllocator extends RMContainerRequestor implements ContainerAllocator {
    ...
    static {
        ...
        PRIORITY_OPPORTUNISTIC_MAP = RecordFactoryProvider.getRecordFactory(null).newRecordInstance(Priority.class);
    ...
    public RMContainerAllocator(ClientService clientService, AppContext context, AMPreemptionPolicy preemptionPolicy) {
        ...
        // TODO_MA 马中华 注释： RMContainerAllocator 是 RMContainerRequestor 的子类
        // TODO_MA 马中华 注释： 是 RMCommunicator 的子类
    ...
    protected void serviceInit(Configuration conf) throws Exception {
        ...
        reducerUnconditionalPreemptionDelayMs = 1000 * conf.getInt(MRJobConfig.MR_JOB_REDUCER_UNCONDITIONAL_PREEMPT_DELAY_SEC,
                MRJobConfig.DEFAULT_MR_JOB_REDUCER_UNCONDITIONAL_PREEMPT_DELAY_SEC
        );
        reducerNoHeadroomPreemptionDelayMs = conf.getInt(MRJobConfig.MR_JOB_REDUCER_PREEMPT_DELAY_SEC,
        ...
        this.scheduledRequests.setNumOpportunisticMapsPercent(
                conf.getInt(MRJobConfig.MR_NUM_OPPORTUNISTIC_MAPS_PERCENT, MRJobConfig.DEFAULT_MR_NUM_OPPORTUNISTIC_MAPS_PERCENT));
        LOG.info(
                this.scheduledRequests.getNumOpportunisticMapsPercent() + "% of the mappers will be scheduled using OPPORTUNISTIC containers");
    }
            ...
            public void run() {
                ...
                while (!stopped.get() && !Thread.currentThread().isInterrupted()) {
                    ...
                    try {
                         ...
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 2、处理事件
                         *  接收到事件之后执行处理，根据事件的类型，构建不同的 Contianer 申请请求
                         *  计入到不同的队列
                         */
                        handleEvent(event);
                    } catch (Throwable t) {
    ...
    protected synchronized void heartbeat() throws Exception {
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释：
         *  1、flink： allocateSlotAndDepoly()
         *      1、申请 slot
         *      2、完成 slot 和 vertex 的映射
         *      3、部署 Task
         */
        ...
        if (recalculateReduceSchedule) {
            ...
            if (!reducerPreempted) {
                // Only schedule new reducers if no reducer preemption happens for this heartbeat
                scheduleReduces(getJob().getTotalMaps(), completedMaps, scheduledRequests.maps.size(),
                        scheduledRequests.reduces.size(), assignedRequests.maps.size(), assignedRequests.reduces.size(),
    ...
    protected synchronized void handleEvent(ContainerAllocatorEvent event) {
        ...
        if (event.getType() == ContainerAllocator.EventType.CONTAINER_REQ) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 申请 ReduceTask 需要的 Container
             */
            else {
    ...
    private void handleReduceContainerRequest(ContainerRequestEvent reqEvent) {
        ...
        if (reduceResourceRequest.equals(Resources.none())) {
            ...
            eventHandler.handle(new JobHistoryEvent(jobId,
                    new NormalizedResourceEvent(org.apache.hadoop.mapreduce.TaskType.REDUCE, reduceResourceRequest.getMemorySize())
            ));
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： ReduceTask 的 Ccontainer 申请请求先加入到 pendingReduces 队列，等待 MapTask 都执行完之后，才调用
         */
        if (reduceContainerRequestAccepted) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 对于 ReduceTask 来说，构建了 ContainerRequest 是放入到 pendingReduces
             *  等待队列，得等待 MapTask 的所有 ContainerRequest 都匹配完了，才能拔 pendingReduces
             *  的请求，移入到 scheduleRequetes 队列！
             */
    ...
    private void handleMapContainerRequest(ContainerRequestEvent reqEvent) {
        ...
        if (mapResourceRequest.equals(Resources.none())) {
            ...
            eventHandler.handle(new JobHistoryEvent(jobId,
                    new NormalizedResourceEvent(org.apache.hadoop.mapreduce.TaskType.MAP, mapResourceRequest.getMemorySize())
            ));
        ...
        if (mapContainerRequestAccepted) {
            ...
            // TODO_MA 马中华 注释： MapTask 的 Contianer 的申请请求丢入到这个队列
    ...
    private void preemptReducer(int hangingMapRequests) {
        ...
        int preemptionReduceNumForPreemptionLimit = ResourceCalculatorUtils.divideAndCeilContainers(
                Resources.multiply(getResourceLimit(), maxReducePreemptionLimit), reduceResourceRequest,
                getSchedulerResourceTypes()
        );
        int preemptionReduceNumForAllMaps = ResourceCalculatorUtils.divideAndCeilContainers(
                Resources.multiply(mapResourceRequest, hangingMapRequests), reduceResourceRequest, getSchedulerResourceTypes());
    ...
    private int getNumHangingRequests(long allocationDelayThresholdMs, Map<TaskAttemptId, ContainerRequest> requestMap) {
    ...
    @Private
    public void scheduleReduces(int totalMaps, int completedMaps, int scheduledMaps, int scheduledReduces, int assignedMaps,
                                int assignedReduces, Resource mapResourceReqt, Resource reduceResourceReqt, int numPendingReduces,
        ...
        int rampUp = ResourceCalculatorUtils.computeAvailableContainers(
                Resources.subtract(finalReduceResourceLimit, netScheduledReduceResource), reduceResourceReqt,
                getSchedulerResourceTypes()
        );
    ...
    private List<Container> getResources() throws Exception {
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 1、 发起 RPC 请求向 RM 申请 Container
             */
            ...
            throw new RMContainerAllocationException(
                    "Resource Manager doesn't recognize AttemptId: " + this.getContext().getApplicationAttemptId(), e);
        ...
        // TODO_MA 马中华 注释： 3 返回 Container 列表
                ...
                for (Map.Entry<TaskAttemptId, Container> entry : taskSet.entrySet()) {
                    ...
                    if (unusableNodes.contains(taskAttemptNodeId)) {
                        ...
                        eventHandler.handle(new TaskAttemptKillEvent(tid,
                                "TaskAttempt killed because it ran on unusable node" + taskAttemptNodeId, rescheduleNextAttempt
                        ));
                    }
                }
            }
    ...
    class ScheduledRequests {
         ...
         */
        @VisibleForTesting
        final LinkedHashMap<TaskAttemptId, ContainerRequest> reduces = new LinkedHashMap<TaskAttemptId, ContainerRequest>();
        ...
        private void assign(List<Container> allocatedContainers) {
            ...
            while (it.hasNext()) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "Assigning container " + allocated.getId() + " with priority " + allocated.getPriority() + " to NM " + allocated.getNodeId());
                }
                ...
                if (PRIORITY_FAST_FAIL_MAP.equals(priority) || PRIORITY_MAP.equals(priority) || PRIORITY_OPPORTUNISTIC_MAP.equals(
                        priority)) {
                    if (ResourceCalculatorUtils.computeAvailableContainers(allocatedResource, mapResourceRequest,
                            getSchedulerResourceTypes()
                    ) <= 0 || maps.isEmpty()) {
                        LOG.info(
                                "Cannot assign container " + allocated + " for a map as either " + " container memory less than required " + mapResourceRequest + " or no pending map tasks - maps.isEmpty=" + maps.isEmpty());
                        isAssignable = false;
                    }
                } else if (PRIORITY_REDUCE.equals(priority)) {
                    if (ResourceCalculatorUtils.computeAvailableContainers(allocatedResource, reduceResourceRequest,
                            getSchedulerResourceTypes()
                    ) <= 0 || (reducePending <= 0)) {
                        LOG.info(
                                "Cannot assign container " + allocated + " for a reduce as either " + " container memory less than required " + reduceResourceRequest + " or no pending reduce tasks.");
                        isAssignable = false;
                    } else {
                        reducePending--;
                    }
                ...
                if (isNodeBlacklisted(allocatedHost)) {
                    ...
                    LOG.info(
                            "Got allocated container on a blacklisted " + " host " + allocatedHost + ". Releasing container " + allocated);
                    ...
                    if (toBeReplacedReq != null) {
                        ...
                        /*************************************************
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 之前初始化的时候，创建一个数据结构： AssignRequests 对象，内部维护了两个map
                         *  如果完成匹配了，就登记在册！
                         */
                        ...
                        LOG.info(
                                "Could not map allocated container to a valid request." + " Releasing allocated container " + allocated);
                    }
        ...
        private void containerAssigned(Container allocated, ContainerRequest assigned) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： TaskAttemptEventType.TA_ASSIGNED 事件
             *  Container 从 unassign 变成 assigned
             */
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Assigned container (" + allocated + ") " + " to task " + assigned.attemptID + " on node " + allocated.getNodeId()
                                .toString());
            }
        }
        ...
        private ContainerRequest getContainerReqToReplace(Container allocated) {
            ...
            if (PRIORITY_FAST_FAIL_MAP.equals(priority)) {
                ...
                return toBeReplaced;
            } else if (PRIORITY_MAP.equals(priority) || PRIORITY_OPPORTUNISTIC_MAP.equals(priority)) {
    ...
    class AssignedRequests {
         ...
         */
        @VisibleForTesting
        final LinkedHashMap<TaskAttemptId, Container> maps = new LinkedHashMap<TaskAttemptId, Container>();
    ...
    private class ScheduleStats {
        ...
        public void log(String msgPrefix) {
            LOG.info(
                    msgPrefix + "PendingReds:" + numPendingReduces + " ScheduledMaps:" + numScheduledMaps + " ScheduledReds:" + numScheduledReduces + " AssignedMaps:" + numAssignedMaps + " AssignedReds:" + numAssignedReduces + " CompletedMaps:" + numCompletedMaps + " CompletedReds:" + numCompletedReduces + " ContAlloc:" + numContainersAllocated + " ContRel:" + numContainersReleased + " HostLocal:" + hostLocalAssigned + " RackLocal:" + rackLocalAssigned);
        }
    }
    ...
    static class PreemptionContext extends AMPreemptionPolicy.Context {
        ...
        public List<Container> getContainers(TaskType t) {
            if (TaskType.REDUCE.equals(t)) return new ArrayList<Container>(reqs.reduces.values());
            if (TaskType.MAP.equals(t)) return new ArrayList<Container>(reqs.maps.values());
            return null;
        }
