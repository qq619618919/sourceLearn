...
public class ResourceUtils {
    ...
    private static final Pattern RESOURCE_NAME_PATTERN = Pattern.compile(
            "^(((\\p{Alnum}([\\p{Alnum}-]*\\p{Alnum})?\\.)*" + "\\p{Alnum}([\\p{Alnum}-]*\\p{Alnum})?)/)?\\p{Alpha}([\\w.-]*)$");
            ...
            if (newDefinedRI != null) {
                ...
                if (!expectedUnit.equals(actualUnit) || !expectedType.equals(actualType)) {
                    throw new YarnRuntimeException(
                            "Defined mandatory resource type=" + key + " inside resource-types.xml, however its type or " + "unit is conflict to mandatory resource types, expected type=" + expectedType + ", unit=" + expectedUnit + "; actual type=" + actualType + " actual unit=" + actualUnit);
                }
            }
        }
    }
    ...
    private static void addMandatoryResources(Map<String, ResourceInformation> res) {
        ...
        if (!res.containsKey(MEMORY)) {
            LOG.debug("Adding resource type - name = {}, units = {}, type = {}", MEMORY, ResourceInformation.MEMORY_MB.getUnits(),
        ...
        if (!res.containsKey(VCORES)) {
            LOG.debug("Adding resource type - name = {}, units = {}, type = {}", VCORES, ResourceInformation.VCORES.getUnits(),
                    ResourceTypes.COUNTABLE
            );
    ...
    private static void setAllocationForMandatoryResources(Map<String, ResourceInformation> res, Configuration conf) {
        ...
        mem.setMinimumAllocation(getAllocation(conf, YarnConfiguration.RESOURCE_TYPES + "." + mem.getName() + MINIMUM_ALLOCATION,
                YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_MB, YarnConfiguration.DEFAULT_RM_SCHEDULER_MINIMUM_ALLOCATION_MB
        ));
        mem.setMaximumAllocation(getAllocation(conf, YarnConfiguration.RESOURCE_TYPES + "." + mem.getName() + MAXIMUM_ALLOCATION,
        ...
        cpu.setMinimumAllocation(getAllocation(conf, YarnConfiguration.RESOURCE_TYPES + "." + cpu.getName() + MINIMUM_ALLOCATION,
                YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_VCORES,
                YarnConfiguration.DEFAULT_RM_SCHEDULER_MINIMUM_ALLOCATION_VCORES
        ));
        cpu.setMaximumAllocation(getAllocation(conf, YarnConfiguration.RESOURCE_TYPES + "." + cpu.getName() + MAXIMUM_ALLOCATION,
    ...
    private static long getAllocation(Configuration conf, String resourceTypesKey, String schedulerKey, long schedulerDefault) {
        ...
        if (value == -1) {
            LOG.debug(
                    "Mandatory Resource '{}' is not " + "configured in resource-types config file. Setting allocation " + "specified using '{}'",
                    resourceTypesKey, schedulerKey
            );
    ...
    @VisibleForTesting
    static void validateNameOfResourceNameAndThrowException(String resourceName) throws YarnRuntimeException {
    ...
    private static Map<String, ResourceInformation> getResourceInformationMapFromConfig(Configuration conf) {
        ...
        if (resourceNames != null && resourceNames.length != 0) {
            for (String resourceName : resourceNames) {
                ...
                String resourceTypeName = conf.get(YarnConfiguration.RESOURCE_TYPES + "." + resourceName + TYPE,
                        ResourceTypes.COUNTABLE.toString()
                );
                Long minimumAllocation = conf.getLong(YarnConfiguration.RESOURCE_TYPES + "." + resourceName + MINIMUM_ALLOCATION,
                        0L
                );
                Long maximumAllocation = conf.getLong(YarnConfiguration.RESOURCE_TYPES + "." + resourceName + MAXIMUM_ALLOCATION,
                        Long.MAX_VALUE
                );
                if (resourceName == null || resourceName.isEmpty() || resourceUnits == null || resourceTypeName == null) {
                    throw new YarnRuntimeException(
                            "Incomplete configuration for resource type '" + resourceName + "'. One of name, units or type is configured incorrectly.");
                }
                ...
                LOG.info(
                        "Adding resource type - name = " + resourceName + ", units = " + resourceUnits + ", type = " + resourceTypeName);
                ...
                resourceInformationMap.put(resourceName,
                        ResourceInformation.newInstance(resourceName, resourceUnits, 0L, resourceType, minimumAllocation,
                                maximumAllocation, resourceTagSet, null
                        )
                );
            }
        }
    ...
    public static String[] parseResourceValue(String resourceValue) {
        ...
        if (StringUtils.isAlpha(units) || units.equals("")) {
            ...
            throw new IllegalArgumentException(
                    "Units '" + units + "'" + " contains non alphabet characters, which is not allowed.");
        }
    }
    ...
    private static Map<String, ResourceInformation> initializeNodeResourceInformation(Configuration conf) {
        ...
        // TODO_MA 马中华 注释：  node-resources.xml
    ...
    public static List<ResourceTypeInfo> getResourcesTypeInfo() {
        ...
        for (ResourceInformation resourceInfo : resourcesInfo) {
            array.add(
                    ResourceTypeInfo.newInstance(resourceInfo.getName(), resourceInfo.getUnits(), resourceInfo.getResourceType()));
        }
        return array;
    }
    ...
    public static void reinitializeResources(List<ResourceTypeInfo> resourceTypeInfo) {
        ...
        for (ResourceTypeInfo resourceType : resourceTypeInfo) {
            resourceInformationMap.put(resourceType.getName(),
                    ResourceInformation.newInstance(resourceType.getName(), resourceType.getDefaultUnit(),
    ...
    public static List<ResourceInformation> getRequestedResourcesFromConfig(Configuration configuration, String prefix) {
        ...
        Map<String, String> customResourcesMap = configuration.getValByRegex(
                "^" + Pattern.quote(prefix) + YARN_IO_OPTIONAL + "[^.]+$");
    ...
    public static void areMandatoryResourcesAvailable(Resource res) {
        ...
        if (memoryResourceInformation != null) {
            ...
            if (value > Integer.MAX_VALUE) {
                throw new IllegalArgumentException(
                        "Value '" + value + "' for " + "resource memory is more than the maximum for an integer.");
            }
            if (value == 0) {
        ...
        if (vcoresResourceInformation != null) {
            ...
            if (value > Integer.MAX_VALUE) {
                throw new IllegalArgumentException(
                        "Value '" + value + "' for resource" + " vcores is more than the maximum for an integer.");
            }
            if (value == 0) {
    ...
    @InterfaceAudience.Private
    @InterfaceStability.Unstable
    public static Resource createResourceFromString(String resourceStr, List<ResourceTypeInfo> resourceTypeInfos) {
    ...
    private static void validateResourceTypes(Iterable<String> resourceNames,
                                              List<ResourceTypeInfo> resourceTypeInfos) throws ResourceNotFoundException {
        for (String resourceName : resourceNames) {
