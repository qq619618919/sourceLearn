...
public class RpcServerFactoryPBImpl implements RpcServerFactory {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(RpcServerFactoryPBImpl.class);
    ...
    public Server getServer(Class<?> protocol, Object instance, InetSocketAddress addr, Configuration conf,
        ...
        if (constructor == null) {
            ...
            try {
                 ...
                 *  注释： 通过反射构建 ApplicationMasterProtocol 的 RPCServer 实例
                 */
                ...
                throw new YarnRuntimeException(
                        "Could not find constructor with params: " + Long.TYPE + ", " + InetSocketAddress.class + ", " + Configuration.class,
                        e
                );
            }
        }
        ...
        try {
            return createServer(pbProtocol, addr, conf, secretManager, numHandlers, (BlockingService) method.invoke(null, service),
                    portRangeConfig
            );
        } catch (InvocationTargetException e) {
            throw new YarnRuntimeException(e);
    ...
    private Server createServer(Class<?> pbProtocol, InetSocketAddress addr, Configuration conf,
        ...
        RPC.Server server = new RPC.Builder(conf).setProtocol(pbProtocol).setInstance(blockingService)
                ...
                .setSecretManager(secretManager).setPortRangeConfig(portRangeConfig).build();
