...
public class TestRMEmbeddedElector extends ClientBaseWithFixes {
	 ...
	 */
	private void testCallbackSynchronization(
			SyncTestType type) throws IOException, InterruptedException, TimeoutException {
	...
	private void testCallbackSynchronizationActive(AdminService as,
			ActiveStandbyElectorBasedElectorService ees) throws IOException, InterruptedException, TimeoutException {
		ees.becomeActive();

		GenericTestUtils.waitFor(() -> transitionToActiveCounter.get() >= 1, 500, 10 * 1000);
	...
	private void testCallbackSynchronizationStandby(AdminService as,
		...
		GenericTestUtils.waitFor(() -> transitionToStandbyCounter.get() >= 1, 500, 10 * 1000);
