...
public class FairScheduler extends AbstractYarnScheduler<FSAppAttempt, FSSchedulerNode> {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(FairScheduler.class);
    private static final Logger STATE_DUMP_LOG = LoggerFactory.getLogger(FairScheduler.class.getName() + ".statedump");
    ...
    public boolean isAtLeastReservationThreshold(ResourceCalculator resourceCalculator, Resource resource) {
        return Resources.greaterThanOrEqual(resourceCalculator, getClusterResource(), resource, reservationThreshold);
    }
    ...
    private void validateConf(FairSchedulerConfiguration config) {
        ...
        if (minMem < 0 || minMem > maxMem) {
            throw new YarnRuntimeException(
                    "Invalid resource scheduler memory" + " allocation configuration: " + YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_MB + "=" + minMem + ", " + YarnConfiguration.RM_SCHEDULER_MAXIMUM_ALLOCATION_MB + "=" + maxMem + ".  Both values must be greater than or equal to 0" + "and the maximum allocation value must be greater than or equal to" + "the minimum allocation value.");
        }

        long incrementMem = config.getIncrementAllocation().getMemorySize();
        if (incrementMem <= 0) {
            throw new YarnRuntimeException(
                    "Invalid resource scheduler memory" + " allocation configuration: " + FairSchedulerConfiguration.RM_SCHEDULER_INCREMENT_ALLOCATION_MB + "=" + incrementMem + ". Values must be greater than 0.");
        }
        ...
        if (minVcores < 0 || minVcores > maxVcores) {
            throw new YarnRuntimeException(
                    "Invalid resource scheduler vcores" + " allocation configuration: " + YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_VCORES + "=" + minVcores + ", " + YarnConfiguration.RM_SCHEDULER_MAXIMUM_ALLOCATION_VCORES + "=" + maxVcores + ".  Both values must be greater than or equal to 0" + "and the maximum allocation value must be greater than or equal to" + "the minimum allocation value.");
        }

        int incrementVcore = config.getIncrementAllocation().getVirtualCores();
        if (incrementVcore <= 0) {
            throw new YarnRuntimeException(
                    "Invalid resource scheduler vcores" + " allocation configuration: " + FairSchedulerConfiguration.RM_SCHEDULER_INCREMENT_ALLOCATION_VCORES + "=" + incrementVcore + ". Values must be greater than 0.");
        }
    }
    ...
    private void dumpSchedulerState() {
        ...
        STATE_DUMP_LOG.debug(
                "FairScheduler state: Cluster Capacity: " + clusterResource + "  Allocations: " + rootMetrics.getAllocatedResources() + "  Availability: " + Resource.newInstance(
    ...
    protected void addApplication(ApplicationId applicationId, String queueName, String user, boolean isAppRecovering,
        ...
        if (!isAppRecovering && placementContext == null) {
            String message = "Reject application " + applicationId + " submitted by user " + user + " application rejected by placement rules.";
        ...
        try {
            ...
            FSLeafQueue queue = queueMgr.getLeafQueue(queueName, true, applicationId);
            if (queue == null) {
                if (!isAppRecovering) {
                    rejectApplicationWithMessage(applicationId, queueName + " is not a leaf queue");
                    return;
                }
            ...
            if (!isAppRecovering) {
                ...
                if (!queue.hasAccess(QueueACL.SUBMIT_APPLICATIONS, userUgi) && !queue.hasAccess(QueueACL.ADMINISTER_QUEUE,
                        userUgi
                )) {
                    String msg = "User " + user + " does not have permission to submit " + applicationId + " to queue " + queueName;
            ...
            if (rmApp != null) {
                ...
                LOG.error("Couldn't find RM app for " + applicationId + " to set queue name on");
            }
            ...
            if (!isAppRecovering && rmApp != null && rmApp.getAMResourceRequests() != null) {
                ...
                if (!invalidAMResourceRequests.isEmpty()) {
                    String msg = String.format(
                            "Cannot submit application %s to queue %s because " + "it has zero amount of resource for a requested " + "resource! Invalid requested AM resources: %s, " + "maximum queue resources: %s",
                            applicationId, queueName, invalidAMResourceRequests, queue.getMaxShare()
                    );
                    rejectApplicationWithMessage(applicationId, msg);
                    queue.removeAssignedApp(applicationId);
                    return;
            ...
            SchedulerApplication<FSAppAttempt> application = new SchedulerApplication<>(queue, user);
            applications.put(applicationId, application);
            queue.getMetrics().submitApp(user);

            LOG.info(
                    "Accepted application " + applicationId + " from user: " + user + ", in queue: " + queueName + ", currently num of applications: " + applications.size());
            if (isAppRecovering) {
     ...
     */
    protected void addApplicationAttempt(ApplicationAttemptId applicationAttemptId, boolean transferStateFromPreviousAttempt,
    ...
    protected void completedContainerInternal(RMContainer rmContainer, ContainerStatus containerStatus,
        ...
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Application attempt " + application.getApplicationAttemptId() + " released container " + container.getId() + " on node: " + (node == null ? nodeID : node) + " with event: " + event);
            }
        } finally {
            writeLock.unlock();
        }
    ...
    public Resource getNormalizedResource(Resource requestedResource, Resource maxResourceCapability) {
        return SchedulerUtils.getNormalizedResource(requestedResource, DOMINANT_RESOURCE_CALCULATOR, minimumAllocation,
                maxResourceCapability, incrAllocation
        );
    }
    ...
    public Allocation allocate(ApplicationAttemptId appAttemptId, List<ResourceRequest> ask,
        ...
        if (!application.getApplicationAttemptId().equals(appAttemptId)) {
            LOG.error("Calling allocate on previous or removed " + "or non existent application attempt " + appAttemptId);
            return EMPTY_ALLOCATION;
        }
        ...
        if (!invalidAsks.isEmpty()) {
            throw new SchedulerInvalidResoureRequestException(String.format(
                    "Resource request is invalid for application %s because queue %s " + "has 0 amount of resource for a resource type! " + "Validation result: %s",
        ...
        try {
            if (!ask.isEmpty()) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "allocate: pre-update" + " applicationAttemptId=" + appAttemptId + " application=" + application.getApplicationId());
                }
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "allocate: post-update" + " applicationAttemptId=" + appAttemptId + " #ask=" + ask.size() + " reservation= " + application.getCurrentReservation());
        ...
        return new Allocation(newlyAllocatedContainers, headroom, preemptionContainerIds, null, null, updatedNMTokens, null,
                null, application.pullNewlyPromotedContainers(), application.pullNewlyDemotedContainers(),
                previousAttemptContainers, null
        );
    }

    private List<MaxResourceValidationResult> validateResourceRequests(List<ResourceRequest> requests, FSLeafQueue queue) {
        ...
        for (ResourceRequest resourceRequest : requests) {
            ...
            MaxResourceValidationResult validationResult = SchedulerUtils.validateResourceRequestsAgainstQueueMaxResource(
                    resourceRequest, queue.getMaxShare());
            if (!validationResult.isValid()) {
                validationResults.add(validationResult);
                LOG.warn(String.format(
                        "Queue %s cannot handle resource request" + "because it has zero available amount of resource " + "for a requested resource type, " + "so the resource request is ignored!" + " Requested resources: %s, " + "maximum queue resources: %s",
            ...
            case APP_ADDED:
                ...
                if (queueName != null) {
                    addApplication(appAddedEvent.getApplicationId(), queueName, appAddedEvent.getUser(),
                            appAddedEvent.getIsAppRecovering(), appAddedEvent.getPlacementContext()
                    );
                }
                break;
            case APP_REMOVED:
    ...
    private void initScheduler(Configuration conf) throws IOException {
        ...
        try {
            ...
            if (updateInterval < 0) {
                ...
                LOG.warn(
                        FairSchedulerConfiguration.UPDATE_INTERVAL_MS + " is invalid, so using default value " + FairSchedulerConfiguration.DEFAULT_UPDATE_INTERVAL_MS + " ms instead");
            }
            ...
            if (continuousSchedulingEnabled && !migration) {
                ...
                LOG.warn(
                        "Continuous scheduling is turned ON. It is deprecated " + "because it can cause scheduler slowness due to locking issues. " + "Schedulers should use assignmultiple as a replacement.");
    ...
    public void serviceInit(Configuration conf) throws Exception {
        ...
        noTerminalRuleCheck = migration && conf.getBoolean(FairSchedulerConfiguration.NO_TERMINAL_RULE_CHECK, false);
    ...
    private void applyChildDefaults() {
        ...
        for (FSQueue queue : queues) {
            ...
            if ((queue.getParent() != null) && !configuredLeafQueues.contains(
                    queue.getName()) && !configuredParentQueues.contains(queue.getName())) {
        ...
        while (cur != lowestCommonAncestor) {
            ...
            if (cur.getNumRunnableApps() == cur.getMaxRunningApps()) {
                throw new YarnException(
                        "Moving app attempt " + appAttId + " to queue " + queueName + " would violate queue maxRunningApps constraints on" + " queue " + cur.getQueueName());
            }

            // maxShare
            if (!Resources.fitsIn(Resources.add(cur.getResourceUsage(), consumption), cur.getMaxShare())) {
                throw new YarnException(
                        "Moving app attempt " + appAttId + " to queue " + queueName + " would violate queue maxShare constraints on" + " queue " + cur.getQueueName());
            }
    ...
    private void executeMove(SchedulerApplication<FSAppAttempt> app, FSAppAttempt attempt, FSLeafQueue oldQueue,
        ...
        if (wasRunnable && !nowRunnable) {
            throw new YarnException(
                    "Should have already verified that app " + attempt.getApplicationId() + " would be runnable in new queue");
        }
    ...
    public void removeQueue(String queueName) throws YarnException {
        ...
        if (reservationQueue != null) {
            if (!queueMgr.removeLeafQueue(queueName)) {
                throw new YarnException(
                        "Could not remove queue " + queueName + " as " + "its either not a leaf queue or its not empty");
            }
        }
    }
    ...
    public Priority updateApplicationPriority(Priority newPriority, ApplicationId applicationId,
        ...
        throw new YarnException("Update application priority is not supported in Fair Scheduler");
    }
