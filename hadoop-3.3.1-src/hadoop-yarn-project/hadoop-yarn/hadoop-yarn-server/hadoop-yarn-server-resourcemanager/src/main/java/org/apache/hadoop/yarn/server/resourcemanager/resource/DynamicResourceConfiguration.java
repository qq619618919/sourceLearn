...
public class DynamicResourceConfiguration extends Configuration {
    ...
    public int getVcoresPerNode(String node) {
        int vcoresPerNode = getInt(getNodePrefix(node) + VCORES, YarnConfiguration.DEFAULT_NM_VCORES);
        return vcoresPerNode;
    }

    public void setVcoresPerNode(String node, int vcores) {
        setInt(getNodePrefix(node) + VCORES, vcores);
        LOG.debug("DRConf - setVcoresPerNode: nodePrefix={}, vcores={}", getNodePrefix(node), vcores);

    }

    public int getMemoryPerNode(String node) {
        int memoryPerNode = getInt(getNodePrefix(node) + MEMORY, YarnConfiguration.DEFAULT_NM_PMEM_MB);
        return memoryPerNode;
    }

    public void setMemoryPerNode(String node, int memory) {
        setInt(getNodePrefix(node) + MEMORY, memory);
        LOG.debug("DRConf - setMemoryPerNode: nodePrefix={}, memory={}", getNodePrefix(node), memory);

    }

    public int getOverCommitTimeoutPerNode(String node) {
        int overCommitTimeoutPerNode = getInt(getNodePrefix(node) + OVERCOMMIT_TIMEOUT,
    ...
    public void setOverCommitTimeoutPerNode(String node, int overCommitTimeout) {
        ...
        LOG.debug("DRConf - setOverCommitTimeoutPerNode: nodePrefix={}," + " overCommitTimeout={}", getNodePrefix(node),
                overCommitTimeout
        );
    }
