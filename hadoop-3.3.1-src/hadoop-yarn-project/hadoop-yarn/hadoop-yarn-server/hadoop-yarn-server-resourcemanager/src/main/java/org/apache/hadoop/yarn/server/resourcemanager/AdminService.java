...
public class AdminService extends CompositeService implements HAServiceProtocol, ResourceManagerAdministrationProtocol {
    ...
    public void serviceInit(Configuration conf) throws Exception {
        autoFailoverEnabled = rm.getRMContext().isHAEnabled() && HAUtil.isAutomaticFailoverEnabled(conf);

        masterServiceBindAddress = conf.getSocketAddr(YarnConfiguration.RM_BIND_HOST, YarnConfiguration.RM_ADMIN_ADDRESS,
    ...
    protected void startServer() throws Exception {
        ...
        this.server = (Server) rpc.getServer(ResourceManagerAdministrationProtocol.class, this, masterServiceBindAddress,
                conf, null, conf.getInt(YarnConfiguration.RM_ADMIN_CLIENT_THREAD_COUNT,
        ...
        if (conf.getBoolean(CommonConfigurationKeysPublic.HADOOP_SECURITY_AUTHORIZATION, false)) {
            refreshServiceAcls(getConfiguration(conf, YarnConfiguration.HADOOP_POLICY_CONFIGURATION_FILE),
        ...
        this.server.start();
        conf.updateConnectAddr(YarnConfiguration.RM_BIND_HOST, YarnConfiguration.RM_ADMIN_ADDRESS,
                YarnConfiguration.DEFAULT_RM_ADMIN_ADDRESS, server.getListenerAddress()
        );
    }
            ...
            case REQUEST_BY_USER:
                if (autoFailoverEnabled) {
                    throw new AccessControlException(
                            "Manual failover for this ResourceManager is disallowed, " + "because automatic failover is enabled.");
                }
                break;
            case REQUEST_BY_USER_FORCED:
                if (autoFailoverEnabled) {
                    LOG.warn(
                            "Allowing manual failover from " + org.apache.hadoop.ipc.Server.getRemoteAddress() + " even though automatic failover is enabled, because the user " + "specified the force flag");
                }
                break;
            case REQUEST_BY_ZKFC:
                if (!autoFailoverEnabled) {
                    throw new AccessControlException(
                            "Request from ZK failover controller at " + org.apache.hadoop.ipc.Server.getRemoteAddress() + " denied " + "since automatic failover is not enabled");
                }
                break;
        }
    }
    ...
    public synchronized void transitionToActive(HAServiceProtocol.StateChangeRequestInfo reqInfo) throws IOException {
        ...
        try {
            ...
            rm.getRMContext().getDispatcher().getEventHandler()
                    .handle(new RMFatalEvent(RMFatalEventType.TRANSITION_TO_ACTIVE_FAILED, e,
                            "failure to refresh configuration settings"
                    ));
        ...
        try {
            ...
            RMAuditLogger.logFailure(user.getShortUserName(), "transitionToActive", "", "RM",
    ...
    public synchronized void transitionToStandby(HAServiceProtocol.StateChangeRequestInfo reqInfo) throws IOException {
        ...
        try {
            ...
            RMAuditLogger.logFailure(user.getShortUserName(), "transitionToStandby", "", "RM",
    ...
    @Override
    public synchronized void transitionToObserver(StateChangeRequestInfo reqInfo) throws IOException {
        ...
        try {
            if (isSchedulerMutable()) {
                throw new IOException(
                        "Scheduler configuration is mutable. " + operation + " is not allowed in this scenario.");
            }
            refreshQueues();
    ...
    public void refreshQueues() throws IOException, YarnException {
        ...
        rm.getRMContext().getScheduler().reinitialize(conf, this.rm.getRMContext());
        ...
        try {
            ...
            switch (request.getDecommissionType()) {
                case NORMAL:
                    ...
                    break;
                case GRACEFUL:
                    rm.getRMContext().getNodesListManager().refreshNodesGracefully(conf, request.getDecommissionTimeout());
                    break;
                case FORCEFUL:
    ...
    public RefreshSuperUserGroupsConfigurationResponse refreshSuperUserGroupsConfiguration(
        ...
        checkRMStatus(user.getShortUserName(), operation, "refresh super-user-groups.");
    ...
    private void refreshSuperUserGroupsConfiguration() throws IOException, YarnException {
        ...
        Configuration conf = getConfiguration(new Configuration(false), YarnConfiguration.CORE_SITE_CONFIGURATION_FILE,
    ...
    private void refreshUserToGroupsMappings() throws IOException, YarnException {
        Groups.getUserToGroupsMappingService(
                getConfiguration(new Configuration(false), YarnConfiguration.CORE_SITE_CONFIGURATION_FILE)).refresh();
    }
    ...
    public RefreshServiceAclsResponse refreshServiceAcls(
        ...
        if (!getConfig().getBoolean(CommonConfigurationKeysPublic.HADOOP_SECURITY_AUTHORIZATION, false)) {
            throw RPCUtil.getRemoteException(new IOException(
                    "Service Authorization (" + CommonConfigurationKeysPublic.HADOOP_SECURITY_AUTHORIZATION + ") not enabled."));
        }
    ...
    private void refreshActiveServicesAcls() throws IOException, YarnException {
        PolicyProvider policyProvider = RMPolicyProvider.getInstance();
        Configuration conf = getConfiguration(new Configuration(false), YarnConfiguration.HADOOP_POLICY_CONFIGURATION_FILE);
        rm.getRMContext().getClientRMService().refreshServiceAcls(conf, policyProvider);
        rm.getRMContext().getApplicationMasterService().refreshServiceAcls(conf, policyProvider);
        rm.getRMContext().getResourceTrackerService().refreshServiceAcls(conf, policyProvider);
    }
    ...
    public UpdateNodeResourceResponse updateNodeResource(
        ...
        for (NodeId nodeId : nodeIds) {
            ...
            if (node == null) {
                LOG.error(
                        "Resource update get failed on all nodes due to change " + "resource on an unrecognized node: " + nodeId);
        ...
        for (Map.Entry<NodeId, ResourceOption> entry : nodeResourceMap.entrySet()) {
            ...
            if (node == null) {
                ...
                LOG.info(
                        "Update resource on node(" + node.getNodeID() + ") with resource(" + newResourceOption.toString() + ")");
    ...
    public RefreshNodesResourcesResponse refreshNodesResources(
            RefreshNodesResourcesRequest request) throws YarnException, StandbyException {
        ...
        try {
            ...
            InputStream drInputStream = this.rm.getRMContext().getConfigurationProvider()
                    .getConfigurationInputStream(configuration, YarnConfiguration.DR_CONFIGURATION_FILE);
            ...
            Configuration ysconf = getConfiguration(new Configuration(false),
                    YarnConfiguration.YARN_SITE_CONFIGURATION_FILE
            );
            this.rm.getRMContext().getResourceTrackerService().updateHeartBeatConfiguration(ysconf);
    ...
    private synchronized Configuration getConfiguration(Configuration conf,
        ...
        for (String confFileName : confFileNames) {
            InputStream confFileInputStream = this.rm.getRMContext().getConfigurationProvider()
    ...
    void refreshAll() throws ServiceFailedException {
        try {
            ...
            if (isSchedulerMutable()) {
                try {
                    ((MutableConfScheduler) rm.getRMContext().getScheduler()).getMutableConfProvider()
    ...
    public AddToClusterNodeLabelsResponse addToClusterNodeLabels(
        ...
        try {
            rm.getRMContext().getNodeLabelManager().addToCluserNodeLabels(request.getNodeLabels());
            RMAuditLogger.logSuccess(user.getShortUserName(), operation, "AdminService");
            return response;
        } catch (IOException ioe) {
            throw logAndWrapException(ioe, user.getShortUserName(), operation, msg);
    ...
    public RemoveFromClusterNodeLabelsResponse removeFromClusterNodeLabels(
        ...
        RemoveFromClusterNodeLabelsResponse response = recordFactory.newRecordInstance(
                RemoveFromClusterNodeLabelsResponse.class);
        try {
            rm.getRMContext().getNodeLabelManager().removeFromClusterNodeLabels(request.getNodeLabels());
            RMAuditLogger.logSuccess(user.getShortUserName(), operation, "AdminService");
            return response;
        } catch (IOException ioe) {
            throw logAndWrapException(ioe, user.getShortUserName(), operation, msg);
    ...
    public ReplaceLabelsOnNodeResponse replaceLabelsOnNode(
        ...
        if (request.getFailOnUnknownNodes()) {
            ...
            for (NodeId requestedNode : request.getNodeToLabels().keySet()) {
                ...
                if (requestedNode.getPort() != 0) {
                    if (rm.getRMContext().getRMNodes().containsKey(requestedNode) || rm.getRMContext().getInactiveRMNodes()
                            .containsKey(requestedNode)) {
                        isKnown = true;
                    }
                } else {
                    ...
                    if (!isKnown) {
                        for (NodeId knownNode : rm.getRMContext().getInactiveRMNodes().keySet()) {
                            if (knownNode.getHost().equals(requestedNode.getHost())) {
                                isKnown = true;
                                break;
                            }
            ...
            if (!unknownNodes.isEmpty()) {
                RMAuditLogger.logFailure(user.getShortUserName(), operation, "", "AdminService",
                        "Failed to replace labels as there are unknown nodes:" + Arrays.toString(unknownNodes.toArray())
                );
                throw RPCUtil.getRemoteException(new IOException(
                        "Failed to replace labels as there are unknown nodes:" + Arrays.toString(unknownNodes.toArray())));
            }
        ...
        try {
            rm.getRMContext().getNodeLabelManager().replaceLabelsOnNode(request.getNodeToLabels());
            RMAuditLogger.logSuccess(user.getShortUserName(), operation, "AdminService");
            return response;
        } catch (IOException ioe) {
            throw logAndWrapException(ioe, user.getShortUserName(), operation, msg);
    ...
    private YarnException logAndWrapException(Exception exception, String user, String operation,
                                              String msg) throws YarnException {
    ...
    public CheckForDecommissioningNodesResponse checkForDecommissioningNodes(
            CheckForDecommissioningNodesRequest checkForDecommissioningNodesRequest) throws IOException, YarnException {
        ...
        Set<NodeId> decommissioningNodes = rm.getRMContext().getNodesListManager().checkForDecommissioningNodes();
        RMAuditLogger.logSuccess(user.getShortUserName(), operation, "AdminService");
        CheckForDecommissioningNodesResponse response = recordFactory.newRecordInstance(
                CheckForDecommissioningNodesResponse.class);
    ...
    public NodesToAttributesMappingResponse mapAttributesToNodes(
        ...
        NodeAttributesManager nodeAttributesManager = rm.getRMContext().getNodeAttributesManager();
        try {
            Map<String, Set<NodeAttribute>> nodeAttributeMapping = validateAndFetch(nodesToAttributes, failOnUnknownNodes);
            switch (request.getOperation()) {
                ...
                case REMOVE:
                    ...
                    break;
                case REPLACE:
                    nodeAttributesManager.replaceNodeAttributes(NodeAttribute.PREFIX_CENTRALIZED, nodeAttributeMapping);
                    break;
                default:
                    throw new IOException(
                            "Invalid operation " + request.getOperation() + " specified in the mapAttributesToNodes request ");
        ...
        RMAuditLogger.logSuccess(user.getShortUserName(), operation, "AdminService");
        return recordFactory.newRecordInstance(NodesToAttributesMappingResponse.class);
    }

    private void validateAttributesExists(List<NodeToAttributes> nodesToAttributes) throws IOException {
        NodeAttributesManager nodeAttributesManager = rm.getRMContext().getNodeAttributesManager();
        for (NodeToAttributes nodeToAttrs : nodesToAttributes) {
            ...
            Set<NodeAttribute> attrs = nodeAttributesManager.getAttributesForNode(hostname).keySet();
            ...
            for (NodeAttribute nodeAttr : attributes) {
                if (!attrs.contains(nodeAttr)) {
                    throw new IOException(
                            "Node attribute [" + nodeAttr.getAttributeKey() + "] doesn't exist on node " + nodeToAttrs.getNode());
                }
            }
        }
    }
     ...
     *           has a wrong prefix
     */
    private Map<String, Set<NodeAttribute>> validateAndFetch(List<NodeToAttributes> nodesToAttributesMapping,
        ...
        for (NodeToAttributes nodeToAttributes : nodesToAttributesMapping) {
            ...
            if (!nodeAttributes.stream().allMatch(nodeAttribute -> NodeAttribute.PREFIX_CENTRALIZED.equals(
                    nodeAttribute.getAttributeKey().getAttributePrefix()))) {
                throw new IOException(
                        "Invalid Attribute Mapping for the node " + node + ". Prefix should be " + NodeAttribute.PREFIX_CENTRALIZED);
            }
     ...
     */
    private boolean validateForInvalidNode(String node, boolean failOnUnknownNodes) {
