...
public class ResourceManager extends CompositeService implements Recoverable, ResourceManagerMXBean {
    ...
    protected void serviceInit(Configuration conf) throws Exception {
        ...
        // load core-site.xml
        // TODO_MA 马中华 注释： 解析 core-site.xml 配置文件
        loadConfigurationXml(YarnConfiguration.CORE_SITE_CONFIGURATION_FILE);

        // Do refreshSuperUserGroupsConfiguration with loaded core-site.xml
        // Or use RM specific configurations to overwrite the common ones first if they exist
        RMServerUtils.processRMProxyUsersConf(conf);
        ProxyUsers.refreshSuperUserGroupsConfiguration(this.conf);

        // load yarn-site.xml
        // TODO_MA 马中华 注释： 解析 yarn-site.xml 配置文件
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释：  初始化 AsycDispatcher，并且加入 ServiceList
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： AdminService 为管理员提供了一套独立的服务接口
         */
        ...
        if (this.rmContext.isHAEnabled()) {
            ...
            if (HAUtil.isAutomaticFailoverEnabled(conf) && HAUtil.isAutomaticFailoverEmbedded(conf)) {
                // TODO_MA 马中华 注释： 创建选举代理，进行选举
                // TODO_MA 马中华 注释： ActiveStandbyElectorBasedElectorService
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建 Active ResourceManager 的相关服务
         */
        createAndInitActiveServices(false);

        // TODO_MA 马中华 注释： WebApp 默认端口： 8090
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 调用各种 service 实例的 serviceInit() 方法之心 service 的初始化
         */
    ...
    public ZKCuratorManager createAndStartZKManager(Configuration config) throws IOException {
        ...
        if (HAUtil.isHAEnabled(config) && HAUtil.getConfValueForRMInstance(YarnConfiguration.ZK_RM_STATE_STORE_ROOT_NODE_ACL,
                config
        ) == null) {
    ...
    protected EventHandler<SchedulerEvent> createSchedulerEventDispatcher() {
        ...
        int threadMonitorRate = conf.getInt(YarnConfiguration.YARN_DISPATCHER_CPU_MONITOR_SAMPLES_PER_MIN,
    ...
    protected Dispatcher createDispatcher() {
        ...
        GenericEventTypeMetrics genericEventTypeMetrics = GenericEventTypeMetricsManager.create(dispatcher.getName(),
                NodesListManagerEventType.class
        );
        // We can add more
    ...
    protected ResourceScheduler createScheduler() {
        // TODO_MA 马中华 注释： 默认实现： CapacityScheduler
        ...
        try {
            ...
            if (ResourceScheduler.class.isAssignableFrom(schedulerClazz)) {
                ...
                throw new YarnRuntimeException(
                        "Class: " + schedulerClassName + " not instance of " + ResourceScheduler.class.getCanonicalName());
            }
    ...
    protected ReservationSystem createReservationSystem() {
        ...
        try {
            ...
            if (ReservationSystem.class.isAssignableFrom(reservationClazz)) {
                ...
                throw new YarnRuntimeException(
                        "Class: " + reservationClassName + " not instance of " + ReservationSystem.class.getCanonicalName());
            }
        } catch (ClassNotFoundException e) {
    ...
    protected SystemServiceManager createServiceManager() {
        ...
        try {
            ...
            if (SystemServiceManager.class.isAssignableFrom(schedulerClazz)) {
                ...
                throw new YarnRuntimeException(
                        "Class: " + schedulerClassName + " not instance of " + SystemServiceManager.class.getCanonicalName());
            }
        } catch (ClassNotFoundException e) {
    ...
    protected SystemMetricsPublisher createSystemMetricsPublisher() {
        ...
        if (YarnConfiguration.timelineServiceV1Enabled(conf) && YarnConfiguration.systemMetricsPublisherEnabled(conf)) {
        ...
        if (YarnConfiguration.timelineServiceV2Enabled(conf) && YarnConfiguration.systemMetricsPublisherEnabled(conf)) {
            ...
            SystemMetricsPublisher publisherV2 = new TimelineServiceV2Publisher(rmContext.getRMTimelineCollectorManager());
    ...
    protected static void validateConfigs(Configuration conf) {
        ...
        if (globalMaxAppAttempts <= 0) {
            throw new YarnRuntimeException(
                    "Invalid global max attempts configuration" + ", " + YarnConfiguration.GLOBAL_RM_AM_MAX_ATTEMPTS + "=" + globalMaxAppAttempts + ", it should be a positive integer.");
        }
        ...
        if (expireIntvl < heartbeatIntvl) {
            throw new YarnRuntimeException(
                    "Nodemanager expiry interval should be no" + " less than heartbeat interval, " + YarnConfiguration.RM_NM_EXPIRY_INTERVAL_MS + "=" + expireIntvl + ", " + YarnConfiguration.RM_NM_HEARTBEAT_INTERVAL_MS + "=" + heartbeatIntvl);
        }
    }
    ...
    public class RMActiveServices extends CompositeService {
        ...
        protected void serviceInit(Configuration configuration) throws Exception {
            ...
            String[] amsProcessorList = conf.getStrings(YarnConfiguration.RM_APPLICATION_MASTER_SERVICE_PROCESSORS);
    ...
    @Private
    private class SchedulerEventDispatcher extends EventDispatcher<SchedulerEvent> {

        private final Thread eventProcessorMonitor;

        SchedulerEventDispatcher(String name, int samplesPerMin) {
            super(scheduler, name);
            this.eventProcessorMonitor = new Thread(new EventProcessorMonitor(getEventProcessorId(), samplesPerMin));
        ...
        private final class EventProcessorMonitor implements Runnable {
            ...
            EventProcessorMonitor(long id, int samplesPerMin) {
                ...
                if (clusterMetrics != null && tmxb != null && tmxb.isThreadCpuTimeSupported()) {
                    this.run = true;
        ...
        public void run() {
            ...
            if (rmContext.isHAEnabled()) {
                try {
                    ...
                    /*************************************************
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： elector = ActiveStandbyElector
                     *  2、加入选举
                     */
            ...
            if (rmApp != null) {
                ...
                if (rmAppAttempt != null) {
                    ...
                    if (previousFailedAttempt != null) {
                        try {
                            LOG.debug("Event {} handled by {}", event.getType(), previousFailedAttempt);
                            previousFailedAttempt.handle(event);
                        } catch (Throwable t) {
                            LOG.error(
                                    "Error in handling event type " + event.getType() + " for applicationAttempt " + appAttemptId + " with " + previousFailedAttempt,
                                    t
                            );
                        }
                    } else {
    ...
    protected void startWepApp() {
        ...
        Builder<ResourceManager> builder = WebApps.$for("cluster", ResourceManager.class, this, "ws").with(conf)
                ...
                .withCSRFProtection(YarnConfiguration.RM_CSRF_PREFIX).withXFSProtection(YarnConfiguration.RM_XFS_PREFIX)
                .at(webAppAddress);
        ...
        if (WebAppUtils.getResolvedRMWebAppURLWithoutScheme(conf).equals(proxyHostAndPort)) {
            ...
            builder.withAttribute(WebAppProxy.PROXY_CA, rmContext.getProxyCAManager().getProxyCA());
    ...
    protected void serviceStart() throws Exception {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 如果不是 HA 集群，则直接成为 active
         *  如果是非 HA 的模式，则直接成为 active，如果是 HA ，之前启动的 StandByTransitionRunnable 会进行选举
         */
        if (!this.rmContext.isHAEnabled()) {
            transitionToActive();
        }
        ...
        try {
            ...
            // TODO_MA 马中华 注释： 正常启动
                ...
                ShutdownHookManager.get()
                        .addShutdownHook(new CompositeServiceShutdownHook(resourceManager), SHUTDOWN_HOOK_PRIORITY);

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 其实是调用 RM 的 serviceInit()
                 */
                resourceManager.init(conf);

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 其实是调用 RM 的 serviceStart()
                 */
    ...
    static void deleteRMConfStore(Configuration conf) throws Exception {
        ...
        String confProviderStr = conf.get(YarnConfiguration.SCHEDULER_CONFIGURATION_STORE_CLASS,
