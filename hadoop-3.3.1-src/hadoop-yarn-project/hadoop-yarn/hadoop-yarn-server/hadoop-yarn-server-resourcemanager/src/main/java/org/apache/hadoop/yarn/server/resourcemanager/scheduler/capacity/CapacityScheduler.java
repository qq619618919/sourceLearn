...
public class CapacityScheduler extends AbstractYarnScheduler<FiCaSchedulerApp, FiCaSchedulerNode> implements PreemptableResourceScheduler, CapacitySchedulerContext, Configurable, ResourceAllocationCommitter, MutableConfScheduler {
    ...
    void initScheduler(Configuration configuration) throws IOException, YarnException {
        writeLock.lock();
        try {
            String confProviderStr = configuration.get(YarnConfiguration.SCHEDULER_CONFIGURATION_STORE_CLASS,
            ...
            if (this.calculator instanceof DefaultResourceCalculator && ResourceUtils.getNumberOfKnownResourceTypes() > 2) {
                throw new YarnRuntimeException(
                        "RM uses DefaultResourceCalculator which" + " used only memory as resource-type but invalid resource-types" + " specified " + ResourceUtils.getResourceTypes() + ". Use" + " DominantResourceCalculator instead to make effective use of" + " these resource-types");
            }
            ...
            if (scheduleAsynchronously) {
                ...
                asyncMaxPendingBacklogs = this.conf.getInt(
                        CapacitySchedulerConfiguration.SCHEDULE_ASYNCHRONOUSLY_MAXIMUM_PENDING_BACKLOGS,
                        CapacitySchedulerConfiguration.DEFAULT_SCHEDULE_ASYNCHRONOUSLY_MAXIMUM_PENDING_BACKLOGS
                );
            }
            ...
            if (rmContext.getMultiNodeSortingManager() != null) {
                rmContext.getMultiNodeSortingManager()
                        .registerMultiNodePolicyNames(multiNodePlacementEnabled, this.conf.getMultiNodePlacementPolicies());
            }

            LOG.info(
                    "Initialized CapacityScheduler with " + "calculator=" + getResourceCalculator().getClass() + ", " + "minimumAllocation=<" + getMinimumResourceCapability() + ">, " + "maximumAllocation=<" + getMaximumResourceCapability() + ">, " + "asynchronousScheduling=" + scheduleAsynchronously + ", " + "asyncScheduleInterval=" + asyncScheduleInterval + "ms" + ",multiNodePlacementEnabled=" + multiNodePlacementEnabled);
        } finally {
            writeLock.unlock();
        }
    }
    ...
    private static boolean shouldSkipNodeSchedule(FiCaSchedulerNode node, CapacityScheduler cs, boolean printVerboseLog) {
        ...
        if (timeElapsedFromLastHeartbeat > cs.nmHeartbeatInterval * 2) {
            if (printVerboseLog && LOG.isDebugEnabled()) {
                LOG.debug(
                        "Skip scheduling on node because it haven't heartbeated for " + timeElapsedFromLastHeartbeat / 1000.0f + " secs");
            }
            return true;
        }
        return false;
            ...
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    if (!runSchedules.get()) {
                        ...
                        if (cs.getAsyncSchedulingPendingBacklogs() > cs.asyncMaxPendingBacklogs) {
                            Thread.sleep(1);
                        } else {
                            schedule(cs);
    ...
    static class ResourceCommitterService extends Thread {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    ResourceCommitRequest<FiCaSchedulerApp, FiCaSchedulerNode> request = backlogs.take();
                    cs.writeLock.lock();
                    try {
    ...
    public void updatePlacementRules() throws IOException {
        ...
        Collection<String> placementRuleStrs = conf.getStringCollection(YarnConfiguration.QUEUE_PLACEMENT_RULES);
        List<PlacementRule> placementRules = new ArrayList<>();
        Set<String> distinguishRuleSet = CapacitySchedulerConfigValidator.validatePlacementRules(placementRuleStrs);
        ...
        for (String placementRuleStr : placementRuleStrs) {
            switch (placementRuleStr) {
                ...
                default:
                    ...
                    try {
                        PlacementRule rule = PlacementFactory.getPlacementRule(placementRuleStr, conf);
                        if (null != rule) {
                            try {
    ...
    private void addApplicationOnRecovery(ApplicationId applicationId, String queueName, String user, Priority priority,
        ...
        try {
            ...
            CSQueue queue = getOrCreateQueueFromPlacementContext(applicationId, user, queueName, placementContext, true);
                ...
                if (!getConfiguration().shouldAppFailFast(getConfig())) {
                    this.rmContext.getDispatcher().getEventHandler()
                            .handle(new RMAppEvent(applicationId, RMAppEventType.KILL,
                                    "Application killed on recovery as it" + " was submitted to queue " + queueName + " which no longer exists after restart."
                            ));
                    return;
                } else {
            ...
            if (!(queue instanceof LeafQueue)) {
                ...
                if (!getConfiguration().shouldAppFailFast(getConfig())) {
                    this.rmContext.getDispatcher().getEventHandler()
                            .handle(new RMAppEvent(applicationId, RMAppEventType.KILL,
                                    "Application killed on recovery as it was " + "submitted to queue " + queueName + " which is no longer a leaf queue after restart."
                            ));
                    return;
                } else {
    ...
    private CSQueue getOrCreateQueueFromPlacementContext(ApplicationId applicationId, String user, String queueName,
                                                         ApplicationPlacementContext placementContext, boolean isRecovery) {
                ...
                try {
                    ...
                    if (isRecovery) {
                        if (!getConfiguration().shouldAppFailFast(getConfig())) {
                            ...
                            this.rmContext.getDispatcher().getEventHandler()
                                    .handle(new RMAppEvent(applicationId, RMAppEventType.KILL,
                                            "Application killed on recovery" + " as it was submitted to queue " + queueName + " which could not be auto-created"
                                    ));
                        } else {
                        ...
                        final String message = "Application " + applicationId + " submission by user : " + user + " to  queue : " + queueName + " failed : " + e.getMessage();
                        this.rmContext.getDispatcher().getEventHandler()
    ...
    private void addApplication(ApplicationId applicationId, String queueName, String user, Priority priority,
        ...
        try {
            ...
            CSQueue queue = getOrCreateQueueFromPlacementContext(applicationId, user, queueName, placementContext, false);

            if (queue == null) {
                String message;
                if (isAmbiguous(queueName)) {
                    message = "Application " + applicationId + " submitted by user " + user + " to ambiguous queue: " + queueName + " please use full queue path instead.";
                } else {
                    message = "Application " + applicationId + " submitted by user " + user + " to unknown queue: " + queueName;
                }
            ...
            if (!(queue instanceof LeafQueue)) {
                ...
                this.rmContext.getDispatcher().getEventHandler()
                        .handle(new RMAppEvent(applicationId, RMAppEventType.APP_REJECTED, message));
                return;
            } else if (queue instanceof AutoCreatedLeafQueue && queue.getParent() instanceof ManagedParentQueue) {

                //If queue already exists and auto-queue creation was not required,
                //placement context should not be null
                if (placementContext == null) {
                    String message = "Application " + applicationId + " submission by user : " + user + " to specified queue : " + queueName + "  is prohibited. " + "Verify automatic queue mapping for user exists in " + QUEUE_MAPPING;
                    this.rmContext.getDispatcher().getEventHandler()
                            .handle(new RMAppEvent(applicationId, RMAppEventType.APP_REJECTED, message));
                    return;
                ...
                } else if (!queue.getParent().getQueueShortName()
                        .equals(placementContext.getParentQueue()) && !queue.getParent().getQueuePath()
                        .equals(placementContext.getParentQueue())) {
                    String message = "Auto created Leaf queue " + placementContext.getQueue() + " " + "already exists under queue : " + queue.getParent()
                            .getQueueShortName() + ". But Queue mapping configuration " + CapacitySchedulerConfiguration.QUEUE_MAPPING + " has been " + "updated to a different parent queue : " + placementContext.getParentQueue() + " for the specified user : " + user;
                    this.rmContext.getDispatcher().getEventHandler()
                            .handle(new RMAppEvent(applicationId, RMAppEventType.APP_REJECTED, message));
                    return;
                }
            ...
            try {
                priority = workflowPriorityMappingsMgr.mapWorkflowPriorityForApp(applicationId, queue, user, priority);
            ...
            try {
                ...
                this.rmContext.getDispatcher().getEventHandler()
                        .handle(new RMAppEvent(applicationId, RMAppEventType.APP_REJECTED, ace.toString()));
                return;
            }
            // update the metrics
            queue.getMetrics().submitApp(user);
    ...
    private void addApplicationAttempt(ApplicationAttemptId applicationAttemptId, boolean transferStateFromPreviousAttempt,
        ...
        try {
            ...
            FiCaSchedulerApp attempt = new FiCaSchedulerApp(applicationAttemptId, application.getUser(), queue,
                    queue.getAbstractUsersManager(), rmContext, application.getPriority(), isAttemptRecovering,
                    activitiesManager
            );
            ...
            LOG.info(
                    "Added Application Attempt " + applicationAttemptId + " to scheduler from user " + application.getUser() + " in queue " + queue.getQueuePath());
    ...
    private void doneApplicationAttempt(ApplicationAttemptId applicationAttemptId, RMAppAttemptState rmAppAttemptFinalState,
    ...
    private void normalizeSchedulingRequests(List<SchedulingRequest> asks) {
        ...
        for (SchedulingRequest ask : asks) {
            ...
            if (sizing != null && sizing.getResources() != null) {
                sizing.setResources(getNormalizedResource(sizing.getResources(), maxAllocation));
            }
        }
    }
    ...
    public Allocation allocate(ApplicationAttemptId applicationAttemptId, List<ResourceRequest> ask,
        ...
        if (!application.getApplicationAttemptId().equals(applicationAttemptId)) {
            LOG.error(
                    "Calling allocate on previous or removed " + "or non existent application attempt " + applicationAttemptId);
            return EMPTY_ALLOCATION;
        }
        ...
        try {
            ...
            allocation = application.getAllocation(getResourceCalculator(), getClusterResource(),
        ...
        LOG.debug("Allocation for application {} : {} with cluster resource : {}", applicationAttemptId, allocation,
                getClusterResource()
        );
        return allocation;
    }
    ...
    protected void nodeUpdate(RMNode rmNode) {
        ...
        if (!scheduleAsynchronously) {
            ...
            try {
                ...
                // work
                updateSchedulerHealth(lastNodeUpdateTime, rmNode.getNodeID(), CSAssignment.NULL_ASSIGNMENT);
    ...
    private void updateLabelsOnNode(NodeId nodeId, Set<String> newLabels) {
        ...
        for (RMContainer rmContainer : node.getCopiedListOfRunningContainers()) {
            ...
            if (null != application) {
                ...
                LOG.warn(
                        "There's something wrong, some RMContainers running on" + " a node, but we cannot find SchedulerApplicationAttempt " + "for it. Node=" + node.getNodeID() + " applicationAttemptId=" + rmContainer.getApplicationAttemptId());
                continue;
            }
        }
    ...
    private void updateSchedulerHealth(long now, NodeId nodeId, CSAssignment assignment) {
        List<AssignmentInformation.AssignmentDetails> allocations = assignment.getAssignmentInformation()
                .getAllocationDetails();
        List<AssignmentInformation.AssignmentDetails> reservations = assignment.getAssignmentInformation()
                .getReservationDetails();
        // Get nodeId from allocated container if incoming argument is null.
        NodeId updatedNodeid = (nodeId == null) ? allocations.get(allocations.size() - 1).rmContainer.getNodeId() : nodeId;
        ...
        if (!allocations.isEmpty()) {
            ...
            schedulerHealth.updateAllocation(now, updatedNodeid, allocatedContainerId, allocatedQueue);
        }
        if (!reservations.isEmpty()) {
            ...
            schedulerHealth.updateReservation(now, updatedNodeid, reservedContainerId, reservedQueue);
        }
        ...
        schedulerHealth.updateSchedulerRunDetails(now, assignment.getAssignmentInformation().getAllocated(),
    ...
    private boolean canAllocateMore(CSAssignment assignment, int offswitchCount, int assignedContainers) {
        ...
        if (assignment == null || Resources.equals(assignment.getResource(), Resources.none())) {
            return false;
        }
        ...
        return assignMultipleEnabled && (maxAssignPerHeartbeat == -1 || assignedContainers < maxAssignPerHeartbeat);
    }
    ...
    private CandidateNodeSet<FiCaSchedulerNode> getCandidateNodeSet(FiCaSchedulerNode node) {
        ...
        if (multiNodePlacementEnabled) {
            ...
            List<FiCaSchedulerNode> nodes = nodeTracker.getNodesPerPartition(node.getPartition());
            if (nodes != null && !nodes.isEmpty()) {
                nodes.forEach(n -> nodesByPartition.put(n.getNodeID(), n));
                candidates = new SimpleCandidateNodeSet<FiCaSchedulerNode>(nodesByPartition, node.getPartition());
            }
        }
        return candidates;
    }
    ...
    private void allocateContainersToNode(NodeId nodeId, boolean withNodeHeartbeat) {
        ...
        if (null != node) {
            ...
            CandidateNodeSet<FiCaSchedulerNode> candidates = getCandidateNodeSet(node);
            CSAssignment assignment = allocateContainersToNode(candidates, withNodeHeartbeat);
            ...
            if (null != assignment && withNodeHeartbeat) {
                ...
                while (canAllocateMore(assignment, offswitchCount, assignedContainers)) {
                    ...
                    if (null != assignment && Resources.greaterThan(calculator, getClusterResource(),
                ...
                if (offswitchCount >= offswitchPerHeartbeatLimit) {
                    LOG.debug("Assigned maximum number of off-switch containers: {}," + " assignments so far: {}",
                            offswitchCount, assignment
                    );
                }
            }
        }
     ...
     */
    private CSAssignment allocateContainerOnSingleNode(CandidateNodeSet<FiCaSchedulerNode> candidates,
                                                       FiCaSchedulerNode node, boolean withNodeHeartbeat) {
        LOG.debug("Trying to schedule on node: {}, available: {}", node.getNodeName(), node.getUnallocatedResource());
        ...
        if (getNode(node.getNodeID()) != node) {
            ...
            ActivitiesLogger.QUEUE.recordQueueActivity(activitiesManager, node, "", getRootQueue().getQueuePath(),
                    ActivityState.REJECTED, ActivityDiagnosticConstant.INIT_CHECK_SINGLE_NODE_REMOVED
            );
        ...
        if (reservedContainer != null) {
            ...
            LOG.debug("Skipping scheduling since node {} is reserved by" + " application {}", node.getNodeID(),
        ...
        if (calculator.computeAvailableContainers(
                Resources.add(node.getUnallocatedResource(), node.getTotalKillableResources()), minimumAllocation) <= 0) {
            LOG.debug(
                    "This node " + node.getNodeID() + " doesn't have sufficient " + "available or preemptible resource for minimum allocation");
            ActivitiesLogger.QUEUE.recordQueueActivity(activitiesManager, node, "", getRootQueue().getQueuePath(),
                    ActivityState.REJECTED, ActivityDiagnosticConstant.INIT_CHECK_SINGLE_NODE_RESOURCE_INSUFFICIENT
            );
            ActivitiesLogger.NODE.finishSkippedNodeAllocation(activitiesManager, node);
            return null;
        }
    ...
    private void allocateFromReservedContainer(FiCaSchedulerNode node, boolean withNodeHeartbeat,
                                               RMContainer reservedContainer) {
        FiCaSchedulerApp reservedApplication = getCurrentAttemptForContainer(reservedContainer.getContainerId());
        if (reservedApplication == null) {
            LOG.error(
                    "Trying to schedule for a finished app, please double check. nodeId=" + node.getNodeID() + " container=" + reservedContainer.getContainerId());
            return;
        }

        // Try to fulfill the reservation
        LOG.debug("Trying to fulfill reservation for application {} on node: {}", reservedApplication.getApplicationId(),
                node.getNodeID()
        );

        LeafQueue queue = ((LeafQueue) reservedApplication.getQueue());
        CSAssignment assignment = queue.assignContainers(getClusterResource(), new SimpleCandidateNodeSet<>(node),
                // TODO, now we only consider limits for parent for non-labeled
                // resources, should consider labeled resources as well.
                new ResourceLimits(labelManager.getResourceByLabel(RMNodeLabelsManager.NO_LABEL, getClusterResource())),
        ...
        if (assignment.isFulfilledReservation()) {
            ...
            ActivitiesLogger.QUEUE.recordQueueActivity(activitiesManager, node, queue.getParent().getQueuePath(),
                    queue.getQueuePath(), ActivityState.ACCEPTED, ActivityDiagnosticConstant.EMPTY
            );
            ActivitiesLogger.NODE.finishAllocatedNodeAllocation(activitiesManager, node, reservedContainer.getContainerId(),
                    AllocationState.ALLOCATED_FROM_RESERVED
            );
        } else if (assignment.getAssignmentInformation().getNumReservations() > 0) {
            ActivitiesLogger.QUEUE.recordQueueActivity(activitiesManager, node, queue.getParent().getQueuePath(),
                    queue.getQueuePath(), ActivityState.RE_RESERVED, ActivityDiagnosticConstant.EMPTY
            );
            ActivitiesLogger.NODE.finishAllocatedNodeAllocation(activitiesManager, node, reservedContainer.getContainerId(),
    ...
    private CSAssignment allocateOrReserveNewContainers(CandidateNodeSet<FiCaSchedulerNode> candidates,
                                                        boolean withNodeHeartbeat) {
        CSAssignment assignment = getRootQueue().assignContainers(getClusterResource(), candidates,
                new ResourceLimits(labelManager.getResourceByLabel(candidates.getPartition(), getClusterResource())),
                SchedulingMode.RESPECT_PARTITION_EXCLUSIVITY
        );
        ...
        if (StringUtils.equals(candidates.getPartition(), RMNodeLabelsManager.NO_LABEL)) {
            return null;
        }

        // Only do non-exclusive allocation when the node-label supports that
        try {
            if (rmContext.getNodeLabelManager().isExclusiveNodeLabel(candidates.getPartition())) {
                return null;
            }
        } catch (IOException e) {
            LOG.warn("Exception when trying to get exclusivity of node label=" + candidates.getPartition(), e);
            return null;
        }

        // Try to use NON_EXCLUSIVE
        assignment = getRootQueue().assignContainers(getClusterResource(), candidates,
                // TODO, now we only consider limits for parent for non-labeled
                // resources, should consider labeled resources as well.
                new ResourceLimits(labelManager.getResourceByLabel(RMNodeLabelsManager.NO_LABEL, getClusterResource())),
    ...
    private CSAssignment allocateContainersOnMultiNodes(CandidateNodeSet<FiCaSchedulerNode> candidates) {
        ...
        if (getRootQueue().getQueueCapacities()
                .getUsedCapacity(candidates.getPartition()) >= 1.0f && preemptionManager.getKillableResource(
                CapacitySchedulerConfiguration.ROOT, candidates.getPartition()) == Resources.none()) {
            ...
            LOG.debug("This partition '{}' doesn't have available or " + "killable resource", candidates.getPartition());
            ActivitiesLogger.QUEUE.recordQueueActivity(activitiesManager, null, "", getRootQueue().getQueuePath(),
                    ActivityState.REJECTED, ActivityDiagnosticConstant.INIT_CHECK_PARTITION_RESOURCE_INSUFFICIENT
            );
            ActivitiesLogger.NODE.finishSkippedNodeAllocation(activitiesManager, null);
            return null;
        }
    ...
    CSAssignment allocateContainersToNode(CandidateNodeSet<FiCaSchedulerNode> candidates, boolean withNodeHeartbeat) {
        ...
        if (!multiNodePlacementEnabled) {
            ...
            assignment = allocateContainerOnSingleNode(candidates, node, withNodeHeartbeat);
            ActivitiesLogger.NODE.finishNodeUpdateRecording(activitiesManager, node.getNodeID(), candidates.getPartition());
        } else {
            ActivitiesLogger.NODE.startNodeUpdateRecording(activitiesManager, ActivitiesManager.EMPTY_NODE_ID);
            assignment = allocateContainersOnMultiNodes(candidates);
            ActivitiesLogger.NODE.finishNodeUpdateRecording(activitiesManager, ActivitiesManager.EMPTY_NODE_ID,
                    candidates.getPartition()
            );
        }

        if (assignment != null && assignment.getAssignmentInformation() != null && assignment.getAssignmentInformation()
                .getNumAllocations() > 0) {
    ...
    public void handle(SchedulerEvent event) {
        switch (event.getType()) {
                ...
                if (queueName != null) {
                    if (!appAddedEvent.getIsAppRecovering()) {
                        addApplication(appAddedEvent.getApplicationId(), queueName, appAddedEvent.getUser(),
                                appAddedEvent.getApplicatonPriority(), appAddedEvent.getPlacementContext()
                        );
                    } else {
            ...
            case MANAGE_QUEUE: {
                ...
                try {
                    final List<QueueManagementChange> queueManagementChanges = queueManagementChangeEvent.getQueueManagementChanges();
                    ...
                    LOG.error(
                            "Queue Management Change event cannot be applied for " + "parent queue : " + parentQueue.getQueuePath(),
                            sde
                    );
                } catch (IOException ioe) {
                    LOG.error(
    ...
    private void updateNodeAttributes(NodeAttributesUpdateSchedulerEvent attributeUpdateEvent) {
        writeLock.lock();
        try {
            for (Entry<String, Set<NodeAttribute>> entry : attributeUpdateEvent.getUpdatedNodeToAttributes().entrySet()) {
    ...
    private void refreshLabelToNodeCache(Set<String> updateLabels) {
        Map<String, Set<NodeId>> labelMapping = labelManager.getLabelsToNodes(updateLabels);
    ...
    private void addNode(RMNode nodeManager) {
        ...
        try {
            ...
            if (rmContext.getNodeAttributesManager() != null) {
                rmContext.getNodeAttributesManager().refreshNodeAttributesToScheduler(schedulerNode.getNodeID());
            }
    ...
    private void removeNode(RMNode nodeInfo) {
        ...
        try {
            ...
            for (RMContainer container : runningContainers) {
                super.completedContainer(container, SchedulerUtils.createAbnormalContainerStatus(container.getContainerId(),
    ...
    protected void completedContainerInternal(RMContainer rmContainer, ContainerStatus containerStatus,
        ...
        if (null == node) {
            LOG.info(
                    "Container " + container + " of" + " removed node " + container.getNodeId() + " completed with event " + event);
            return;
        }
    ...
    public void markContainerForKillable(RMContainer killableContainer) {
        writeLock.lock();
        try {
            LOG.debug("{}: container {}", SchedulerEventType.MARK_CONTAINER_FOR_KILLABLE, killableContainer);

            if (!isLazyPreemptionEnabled) {
                super.completedContainer(killableContainer,
                        SchedulerUtils.createPreemptedContainerStatus(killableContainer.getContainerId(),
                                SchedulerUtils.PREEMPTED_CONTAINER
                        ), RMContainerEventType.KILL
                );
            } else {
                FiCaSchedulerNode node = getSchedulerNode(killableContainer.getAllocatedNode());
                ...
                if (null != application) {
                    ...
                    getPreemptionManager().addKillableContainer(
                            new KillableContainer(killableContainer, node.getPartition(), leafQueuePath));
                }
            }
        } finally {
            writeLock.unlock();
    ...
    private void markContainerForNonKillable(RMContainer nonKillableContainer) {
        writeLock.lock();
        try {
            LOG.debug("{}: container {}", SchedulerEventType.MARK_CONTAINER_FOR_NONKILLABLE, nonKillableContainer);

            FiCaSchedulerNode node = getSchedulerNode(nonKillableContainer.getAllocatedNode());
            ...
            if (null != application) {
                ...
                getPreemptionManager().removeKillableContainer(
                        new KillableContainer(nonKillableContainer, node.getPartition(), leafQueuePath));
            }
        } finally {
            writeLock.unlock();
        }
    ...
    public boolean checkAccess(UserGroupInformation callerUGI, QueueACL acl, String queueName) {
        ...
        if (queueName.startsWith("root.")) {
            ...
            while (queue == null) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Queue {} does not exist, checking parent {}", queueName, parentName);
                }
        ...
        if (queue == null) {
            LOG.debug("ACL not found for queue access-type {} for queue {}", acl, queueName);
            return false;
        }
    ...
    public boolean isSystemAppsLimitReached() {
        if (getRootQueue().getNumApplications() < conf.getMaximumSystemApplications()) {
            return false;
        }
        return true;
    }
    ...
    private String resolveReservationQueueName(String queueName, ApplicationId applicationId, ReservationId reservationID,
        ...
        try {
            ...
            if (!(AbstractAutoCreatedLeafQueue.class.isAssignableFrom(q.getClass()))) {
                throw new SchedulerDynamicEditException(
                        "The queue that we are asked " + "to remove (" + queueName + ") is not a AutoCreatedLeafQueue or ReservationQueue");
            }
            ...
            if (disposableLeafQueue.getNumApplications() > 0) {
                throw new SchedulerDynamicEditException(
                        "The queue " + queueName + " is not empty " + disposableLeafQueue.getApplications()
                                .size() + " active apps " + disposableLeafQueue.getPendingApplications()
                                .size() + " pending apps");
            }
    ...
    public void addQueue(Queue queue) throws SchedulerDynamicEditException, IOException {
        ...
        try {
            ...
            } else if (!(AbstractAutoCreatedLeafQueue.class.isAssignableFrom(queue.getClass()))) {
                throw new SchedulerDynamicEditException(
                        "Queue is not an implementation of " + "AbstractAutoCreatedLeafQueue : " + queue.getClass());
            }

            AbstractAutoCreatedLeafQueue newQueue = (AbstractAutoCreatedLeafQueue) queue;

            if (newQueue.getParent() == null || !(AbstractManagedParentQueue.class.isAssignableFrom(
                    newQueue.getParent().getClass())
            )) {
                throw new SchedulerDynamicEditException(
                        "ParentQueue for " + newQueue + " is not properly set" + " (should be set and be a PlanQueue or ManagedParentQueue)");
            }
    ...
    public void setEntitlement(String inQueue, QueueEntitlement entitlement) throws YarnException {
        ...
        try {
            ...
            if (!(AbstractAutoCreatedLeafQueue.class.isAssignableFrom(queue.getClass()))) {
                throw new SchedulerDynamicEditException(
                        "Entitlement can not be" + " modified dynamically since queue " + inQueue + " is not a AutoCreatedLeafQueue");
            }

            if (parent == null || !(AbstractManagedParentQueue.class.isAssignableFrom(parent.getClass()))) {
                throw new SchedulerDynamicEditException(
                        "The parent of AutoCreatedLeafQueue " + inQueue + " must be a PlanQueue/ManagedParentQueue");
            }
            ...
            LOG.info(
                    "Set entitlement for AutoCreatedLeafQueue " + inQueue + "  to " + queue.getCapacity() + " request was (" + entitlement.getCapacity() + ")");
        } finally {
            writeLock.unlock();
        }
    }
    ...
    private void checkQueuePartition(FiCaSchedulerApp app, LeafQueue dest) throws YarnException {
        ...
        for (String label : appLabelexpressions) {
            if (!SchedulerUtils.checkQueueLabelExpression(targetqueuelabels, label, null)) {
                nonAccessiblelabels.add(label);
            }
        }
        if (nonAccessiblelabels.size() > 0) {
            throw new YarnException(
                    "Specified queue=" + dest.getQueuePath() + " can't satisfy following " + "apps label expressions =" + nonAccessiblelabels + " accessible node labels =" + targetqueuelabels);
        }
    }
    ...
    public Resource getMaximumResourceCapability(String queueName) {
        ...
        Resource clusterMaxAllocationConsiderNodeMax = getMaximumResourceCapability();

        return Resources.componentwiseMin(queueMaxAllocation, clusterMaxAllocationConsiderNodeMax);
    }
    ...
    @Override
    public Priority checkAndGetApplicationPriority(Priority priorityRequestedByApp, UserGroupInformation user,
        ...
        try {
            ...
            if (null == appPriority) {
                ...
                appPriority = this.appPriorityACLManager.getDefaultPriority(normalizeQueueName(queuePath), user);
                ...
                if (null == appPriority) {
                    appPriority = this.queueManager.getDefaultPriorityForQueue(normalizeQueueName(queuePath));
                }

                LOG.info(
                        "Application '" + applicationId + "' is submitted without priority " + "hence considering default queue/cluster priority: " + appPriority.getPriority());
            }
            ...
            if (!appPriorityACLManager.checkAccess(user, normalizeQueueName(queuePath), appPriority)) {
                throw new YarnException(new AccessControlException(
                        "User " + user + " does not have permission to submit/update " + applicationId + " for " + appPriority));
            }

            LOG.info(
                    "Priority '" + appPriority.getPriority() + "' is acceptable in queue : " + queuePath + " for application: " + applicationId);
    ...
    @Override
    public Priority updateApplicationPriority(Priority newPriority, ApplicationId applicationId,
        ...
        try {
            ...
            if (application == null) {
                throw new YarnException(
                        "Application '" + applicationId + "' is not present, hence could not change priority.");
            }
            ...
            ApplicationStateData appState = ApplicationStateData.newInstance(rmApp.getSubmitTime(), rmApp.getStartTime(),
                    rmApp.getApplicationSubmissionContext(), rmApp.getUser(), rmApp.getCallerContext()
            );
            ...
            LOG.info(
                    "Priority '" + appPriority + "' is updated in queue :" + rmApp.getQueue() + " for application: " + applicationId + " for the user: " + rmApp.getUser());
            return appPriority;
        } finally {
            writeLock.unlock();
        }
    ...
    private SchedulerContainer<FiCaSchedulerApp, FiCaSchedulerNode> getSchedulerContainer(RMContainer rmContainer,
                                                                                          boolean allocated) {
        if (null == rmContainer) {
            return null;
        }
    ...
    private List<SchedulerContainer<FiCaSchedulerApp, FiCaSchedulerNode>> getSchedulerContainersToRelease(
        ...
        if (csAssignment.getContainersToKill() != null && !csAssignment.getContainersToKill().isEmpty()) {
            ...
            for (RMContainer rmContainer : csAssignment.getContainersToKill()) {
                SchedulerContainer schedulerContainer = getSchedulerContainer(rmContainer, false);
        ...
        if (csAssignment.getExcessReservation() != null) {
            ...
            SchedulerContainer schedulerContainer = getSchedulerContainer(csAssignment.getExcessReservation(), false);
            if (schedulerContainer != null) {
                list.add(schedulerContainer);
            }
        }
    ...
    public void submitResourceCommitRequest(Resource cluster, CSAssignment csAssignment) {
        ResourceCommitRequest<FiCaSchedulerApp, FiCaSchedulerNode> request = createResourceCommitRequest(csAssignment);
    ...
    @Override
    public boolean attemptAllocationOnNode(SchedulerApplicationAttempt appAttempt, SchedulingRequest schedulingRequest,
        ...
        if (schedulingRequest.getResourceSizing() != null) {
            ...
            if (!appAttempt.isStopped()) {
                ResourceCommitRequest<FiCaSchedulerApp, FiCaSchedulerNode> resourceCommitRequest = createResourceCommitRequest(
                        appAttempt, schedulingRequest, schedulerNode);

                // Validate placement constraint is satisfied before
                // committing the request.
                try {
                    if (!PlacementConstraintsUtil.canSatisfyConstraints(appAttempt.getApplicationId(), schedulingRequest,
                            schedulerNode, rmContext.getPlacementConstraintManager(), rmContext.getAllocationTagsManager()
                    )) {
                        LOG.info(
                                "Failed to allocate container for application " + appAttempt.getApplicationId() + " on node " + schedulerNode.getNodeName() + " because this allocation violates the" + " placement constraint.");
                        return false;
                    }
    ...
    private ResourceCommitRequest<FiCaSchedulerApp, FiCaSchedulerNode> createResourceCommitRequest(
            SchedulerApplicationAttempt appAttempt, SchedulingRequest schedulingRequest, SchedulerNode schedulerNode) {
        ...
        if (Resources.greaterThan(calculator, getClusterResource(), resource, Resources.none())) {
            ContainerId cId = ContainerId.newContainerId(appAttempt.getApplicationAttemptId(),
                    appAttempt.getAppSchedulingInfo().getNewContainerId()
            );
            Container container = BuilderUtils.newContainer(cId, schedulerNode.getNodeID(), schedulerNode.getHttpAddress(),
                    resource, schedulingRequest.getPriority(), null, ExecutionType.GUARANTEED,
                    schedulingRequest.getAllocationRequestId()
            );
            RMContainer rmContainer = new RMContainerImpl(container, SchedulerRequestKey.extractFrom(container),
                    appAttempt.getApplicationAttemptId(), container.getNodeId(), appAttempt.getUser(), rmContext, false
            );
            ((RMContainerImpl) rmContainer).setAllocationTags(new HashSet<>(schedulingRequest.getAllocationTags()));

            SchedulerContainer<FiCaSchedulerApp, FiCaSchedulerNode> schedulerContainer = getSchedulerContainer(rmContainer,
                    true
            );
            if (schedulerContainer == null) {
                allocated = null;
            } else {
                allocated = new ContainerAllocationProposal<>(schedulerContainer, null, null, NodeType.NODE_LOCAL,
                        NodeType.NODE_LOCAL, SchedulingMode.RESPECT_PARTITION_EXCLUSIVITY, resource
                );
            }
        }

        if (null != allocated) {
            List<ContainerAllocationProposal<FiCaSchedulerApp, FiCaSchedulerNode>> allocationsList = new ArrayList<>();
    ...
    @VisibleForTesting
    public ResourceCommitRequest<FiCaSchedulerApp, FiCaSchedulerNode> createResourceCommitRequest(
            CSAssignment csAssignment) {
        ContainerAllocationProposal<FiCaSchedulerApp, FiCaSchedulerNode> allocated = null;
        ...
        if (Resources.greaterThan(calculator, getClusterResource(), csAssignment.getResource(), Resources.none())) {
            // Allocated something
            List<AssignmentInformation.AssignmentDetails> allocations = csAssignment.getAssignmentInformation()
                    .getAllocationDetails();
            if (!allocations.isEmpty()) {
                RMContainer rmContainer = allocations.get(0).rmContainer;
                SchedulerContainer<FiCaSchedulerApp, FiCaSchedulerNode> schedulerContainer = getSchedulerContainer(
                        rmContainer, true);
                if (schedulerContainer == null) {
                    allocated = null;
                    ...
                    allocated = new ContainerAllocationProposal<>(schedulerContainer,
                            ...
                            getSchedulerContainer(csAssignment.getFulfilledReservedContainer(), false),
                            csAssignment.getType(), csAssignment.getRequestLocalityType(),
                            csAssignment.getSchedulingMode() != null ? csAssignment.getSchedulingMode() : SchedulingMode.RESPECT_PARTITION_EXCLUSIVITY,
            ...
            // Reserved something
            List<AssignmentInformation.AssignmentDetails> reservation = csAssignment.getAssignmentInformation()
                    .getReservationDetails();
    ...
    public boolean tryCommit(Resource cluster, ResourceCommitRequest r, boolean updatePending) {
        ...
        boolean updateUnconfirmedAllocatedResource = request.getContainersToAllocate() != null && !request.getContainersToAllocate()
                .isEmpty();
        ...
        if (request.anythingAllocatedOrReserved()) {
            ...
            if (!request.getContainersToRelease().isEmpty()) {
                attemptId = request.getContainersToRelease().get(0).getSchedulerApplicationAttempt()
        ...
        if (attemptId != null) {
            ...
            if (app != null && attemptId.equals(app.getApplicationAttemptId())) {
                ...
                LOG.debug("Allocation proposal accepted={}, proposal={}", isSuccess, request);
    ...
    public long checkAndGetApplicationLifetime(String queueName, long lifetimeRequestedByApp) {
        ...
        try {
            ...
            if (maximumApplicationLifetime <= 0) {
                return (lifetimeRequestedByApp <= 0) ? defaultApplicationLifetime : lifetimeRequestedByApp;
            }
    ...
    private LeafQueue autoCreateLeafQueue(ApplicationPlacementContext placementContext) throws IOException, YarnException {
        ...
        if (!StringUtils.isEmpty(parentQueueName)) {
            ...
            if (parentQueue != null && conf.isAutoCreateChildQueueEnabled(parentQueue.getQueuePath())) {
                ...
                throw new SchedulerDynamicEditException(
                        "Could not auto-create leaf queue for " + leafQueueName + ". Queue mapping specifies an invalid parent queue " + "which does not exist " + parentQueueName);
            }
        } else {
            throw new SchedulerDynamicEditException(
                    "Could not auto-create leaf queue for " + leafQueueName + ". Queue mapping does not specify" + " which parent queue it needs to be created under.");
        }
