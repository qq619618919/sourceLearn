...
public class ContainerScheduler extends AbstractService implements EventHandler<ContainerSchedulerEvent> {
    ...
    private final Map<ContainerId, Container> oppContainersToKill = new HashMap<>();
    ...
    public ContainerScheduler(Context context, AsyncDispatcher dispatcher, NodeManagerMetrics metrics) {
        this(context, dispatcher, metrics, context.getConf().getInt(YarnConfiguration.NM_OPPORTUNISTIC_CONTAINERS_MAX_QUEUE_LENGTH,
    ...
    public void serviceInit(Configuration conf) throws Exception {
        ...
        this.usePauseEventForPreemption = conf.getBoolean(YarnConfiguration.NM_CONTAINER_QUEUING_USE_PAUSE_FOR_PREEMPTION,
    ...
    /**
     * // TODO_MA 马中华 注释： 存在 NodeManager 中用来做 Container 调度的。
     * // TODO_MA 马中华 注释： ContainerSchedulerEventType.RECOVERY_COMPLETED
        ...
        if (updateEvent.isResourceChange()) {
            if (runningContainers.containsKey(containerId)) {
                this.utilizationTracker.subtractContainerResource(
                        new ContainerImpl(getConfig(), null, null, null, null, updateEvent.getOriginalToken(), context));
                this.utilizationTracker.addContainerResources(updateEvent.getContainer());
                getContainersMonitor().handle(
                        new ChangeMonitoringContainerResourceEvent(containerId, updateEvent.getUpdatedToken().getResource()));
            }
        }
     ...
     */
    public void recoverActiveContainer(Container container, RecoveredContainerState rcs) {
    ...
    private void startPendingContainers(boolean forceStartGuaranteedContaieners) {
         ...
         *  注释：
         */
        boolean resourcesAvailable = startContainers(queuedGuaranteedContainers.values(), forceStartGuaranteedContaieners);
    ...
    private boolean enqueueContainer(Container container) {
        ...
        if (isGuaranteedContainer) {
            ...
            if (queuedOpportunisticContainers.size() < maxOppQueueLength) {
                ...
                container.sendKillEvent(ContainerExitStatus.KILLED_BY_CONTAINER_SCHEDULER,
    ...
    private void reclaimOpportunisticContainerResources(Container container) {
        ...
        for (Container contToReclaim : extraOppContainersToReclaim) {
            ...
            LOG.info("Container {} will be {} to start the " + "execution of guaranteed container {}.",
                    contToReclaim.getContainerId(), preemptionAction, container.getContainerId()
            );

            if (usePauseEventForPreemption) {
                contToReclaim.sendPauseEvent("Container Paused to make room for Guaranteed Container");
            } else {
                contToReclaim.sendKillEvent(ContainerExitStatus.KILLED_BY_CONTAINER_SCHEDULER,
    ...
    private ResourceUtilization resourcesToFreeUp(ContainerId containerToStartId) {
        ...
        ResourceUtilization resourceAllocationToFreeUp = ResourceUtilization.newInstance(
        ...
        for (Container container : queuedGuaranteedContainers.values()) {
            ContainersMonitor.increaseResourceUtilization(getContainersMonitor(), resourceAllocationToFreeUp,
        ...
        for (Container container : oppContainersToKill.values()) {
            ContainersMonitor.decreaseResourceUtilization(getContainersMonitor(), resourceAllocationToFreeUp,
                    container.getResource()
            );
        }
    ...
    public void updateQueuingLimit(ContainerQueuingLimit limit) {
        ...
        if ((queuingLimit.getMaxQueueLength() > -1) && (queuingLimit.getMaxQueueLength() < queuedOpportunisticContainers.size())) {
            dispatcher.getEventHandler()
                    .handle(new ContainerSchedulerEvent(null, ContainerSchedulerEventType.SHED_QUEUED_CONTAINERS));
        }
    }
        ...
        while (containerIter.hasNext()) {
            ...
            if (container.getContainerState() != ContainerState.PAUSED) {
                if (numAllowed <= 0) {
                    ...
                    LOG.info("Opportunistic container {} will be killed to meet NM queuing" + " limits.",
