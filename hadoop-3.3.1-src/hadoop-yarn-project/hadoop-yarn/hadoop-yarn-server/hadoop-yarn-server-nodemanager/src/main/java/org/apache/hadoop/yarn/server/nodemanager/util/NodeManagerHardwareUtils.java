...
public class NodeManagerHardwareUtils {
    ...
    private static boolean isHardwareDetectionEnabled(Configuration conf) {
        return conf.getBoolean(YarnConfiguration.NM_ENABLE_HARDWARE_CAPABILITY_DETECTION,
    ...
    public static int getNodeCPUs(ResourceCalculatorPlugin plugin, Configuration conf) {
        ...
        boolean countLogicalCores = conf.getBoolean(YarnConfiguration.NM_COUNT_LOGICAL_PROCESSORS_AS_CORES,
    ...
    public static int getNodeCpuPercentage(Configuration conf) {
        int nodeCpuPercentage = Math.min(conf.getInt(YarnConfiguration.NM_RESOURCE_PERCENTAGE_PHYSICAL_CPU_LIMIT,
    ...
    private static int getVCoresInternal(ResourceCalculatorPlugin plugin, Configuration conf) {
        ...
        if (cores == -1) {
            ...
            float multiplier = conf.getFloat(YarnConfiguration.NM_PCORES_VCORES_MULTIPLIER,
    ...
    private static long getContainerMemoryMBInternal(ResourceCalculatorPlugin plugin, Configuration conf) {
        ...
        if (memoryMb == -1) {
            ...
            if (containerPhysicalMemoryMB <= 0) {
                LOG.error(
                        "Calculated memory for YARN containers is too low." + " Node memory is " + physicalMemoryMB + " MB, system reserved memory is " + reservedMemoryMB + " MB.");
            }
