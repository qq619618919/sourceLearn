...
public class NodeResourceMonitorImpl extends AbstractService implements NodeResourceMonitor {
    ...
    final static Logger LOG = LoggerFactory.getLogger(NodeResourceMonitorImpl.class);
    ...
    private ResourceUtilization nodeUtilization = ResourceUtilization.newInstance(0, 0, 0f, customResources);
    ...
    protected void serviceInit(Configuration conf) throws Exception {
        this.monitoringInterval = conf.getLong(YarnConfiguration.NM_RESOURCE_MON_INTERVAL_MS,
        ...
        if (nmContext.getResourcePluginManager() != null) {
            this.gpuResourcePlugin = (GpuResourcePlugin) nmContext.getResourcePluginManager().getNameToPlugins()
    ...
    private boolean isEnabled() {
        ...
        if (resourceCalculatorPlugin == null) {
            LOG.info("ResourceCalculatorPlugin is unavailable on this system. " + this.getClass()
                    .getName() + " is disabled.");
            return false;
        }
        return true;
    ...
    private class MonitoringThread extends Thread {
        ...
        public void run() {
            ...
            // TODO_MA 马中华 注释： 每隔 monitoringInterval 时间做资源检查
            while (true) {
                ...
                try {
                    if (gpuNodeResourceUpdateHandler != null) {
                        totalNodeGpuUtilization = gpuNodeResourceUpdateHandler.getTotalNodeGpuUtilization();
                    }
                } catch (Exception e) {
                    LOG.error("Get Node GPU Utilization error: " + e);
                }

                customResources.put(ResourceInformation.GPU_URI, totalNodeGpuUtilization);
                nodeUtilization = ResourceUtilization.newInstance((int) (pmem >> 20), // B -> MB
                        (int) (vmem >> 20), // B -> MB
