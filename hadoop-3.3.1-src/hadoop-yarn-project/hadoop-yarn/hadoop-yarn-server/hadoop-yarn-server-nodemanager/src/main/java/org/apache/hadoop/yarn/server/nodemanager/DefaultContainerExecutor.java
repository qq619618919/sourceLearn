...
public class DefaultContainerExecutor extends ContainerExecutor {
    ...
    public int launchContainer(ContainerStartContext ctx) throws IOException, ConfigurationException {
        ...
        if (Shell.WINDOWS && sb.getWrapperScriptPath().toString().length() > WIN_MAX_PATH) {
            throw new IOException(String.format(
                    "Cannot launch container using script at path %s, because it exceeds " + "the maximum supported path length of %d characters.  Consider " + "configuring shorter directories in %s.",
                    sb.getWrapperScriptPath(), WIN_MAX_PATH, YarnConfiguration.NM_LOCAL_DIRS
            ));
        }
    ...
    protected LocalWrapperScriptBuilder getLocalWrapperScriptBuilder(String containerIdStr, Path containerWorkDir) {
        return Shell.WINDOWS ? new WindowsLocalWrapperScriptBuilder(containerIdStr,
                containerWorkDir
    ...
    protected abstract class LocalWrapperScriptBuilder {
        ...
        public void writeLocalWrapperScript(Path launchDst, Path pidFile) throws IOException {
            try (DataOutputStream out = lfs.create(wrapperScriptPath,
    ...
    private final class UnixLocalWrapperScriptBuilder extends LocalWrapperScriptBuilder {
        ...
        private void writeSessionScript(Path launchDst, Path pidFile) throws IOException {
            try (DataOutputStream out = lfs.create(sessionScriptPath,
                    EnumSet.of(CREATE, OVERWRITE)
            ); PrintStream pout = new PrintStream(out, false, "UTF-8")) {
    ...
    void createUserLocalDirs(List<String> localDirs, String user) throws IOException {
        ...
        if (!userDirStatus) {
            throw new IOException(
                    "Not able to initialize user directories " + "in any of the configured local directories for user " + user);
        }
    }
    ...
    void createUserCacheDirs(List<String> localDirs, String user) throws IOException {
        ...
        if (!appcacheDirStatus) {
            throw new IOException(
                    "Not able to initialize app-cache directories " + "in any of the configured local directories for user " + user);
        }
        if (!distributedCacheDirStatus) {
            throw new IOException(
                    "Not able to initialize distributed-cache directories " + "in any of the configured local directories for user " + user);
        }
    }
    ...
    void createAppDirs(List<String> localDirs, String user, String appId) throws IOException {
        ...
        if (!initAppDirStatus) {
            throw new IOException(
                    "Not able to initialize app directories " + "in any of the configured local directories for app " + appId.toString());
        }
    }
    ...
    void createAppLogDirs(String appId, List<String> logDirs, String user) throws IOException {
        ...
        if (!appLogDirStatus) {
            throw new IOException(
                    "Not able to initialize app-log directories " + "in any of the configured local directories for app " + appId);
        }
    }
    ...
    void createContainerLogDirs(String appId, String containerId, List<String> logDirs, String user) throws IOException {
        ...
        if (!containerLogDirStatus) {
            throw new IOException(
                    "Not able to initialize container-log directories " + "in any of the configured local directories for container " + containerId);
        }
    }
    ...
    public String getLogDirPermissions() {
        if (this.logDirPermissions == null) {
            this.logDirPermissions = getConf().get(YarnConfiguration.NM_DEFAULT_CONTAINER_EXECUTOR_LOG_DIRS_PERMISSIONS,
