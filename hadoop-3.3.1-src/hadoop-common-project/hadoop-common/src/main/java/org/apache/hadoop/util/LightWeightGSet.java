...
/**
 * // TODO_MA 马中华 注释： 一个低内存占用 {@link GSet} 实现，它使用一个数组来存储元素和链接列表来解决冲突。
 * A low memory footprint {@link GSet} implementation,
 * which uses an array for storing the elements
 * and linked lists for collision resolution.
 *
 * // TODO_MA 马中华 注释： 不会执行重新哈希。因此，永远不会调整内部数组的大小。
 * No rehash will be performed.
 * Therefore, the internal array will never be resized.
 *
 * // TODO_MA 马中华 注释： 此类不支持空元素。
 * This class does not support null element.
 *
 * // TODO_MA 马中华 注释： 这个类不是线程安全的。
...
public class LightWeightGSet<K, E extends K> implements GSet<K, E> {
    ...
    public LightWeightGSet(final int recommended_length) {
        ...
        // TODO_MA 马中华 注释： 1、验证是否在 1 到 2^30 之间.
        // TODO_MA 马中华 注释： 2、处理成一个大于等于且是2的倍数的actual值
        final int actual = actualArrayLength(recommended_length);
        if (LOG.isDebugEnabled()) {
            LOG.debug("recommended=" + recommended_length + ", actual=" + actual);
        }
        // TODO_MA 马中华 注释： 初始化一个 LinkedElement 数组
    ...
    public E put(final E element) {
        ...
        // find index
        // TODO_MA 马中华 注释： hash 取模
        final int index = getIndex(element);

        // remove if it already exists
        // TODO_MA 马中华 注释： 移除存在的
    ...
    public String toString() {
        ...
        b.append("(size=").append(size).append(String.format(", %08x", hash_mask)).append(", modification=").append(modification)
    ...
    static int computeCapacity(long maxMemory, double percentage, String mapName) {
        ...
        LOG.info(percentage + "% max memory " + StringUtils.TraditionalBinaryPrefix.long2String(maxMemory, "B",
                1) + " = " + StringUtils.TraditionalBinaryPrefix.long2String((long) percentMemory, "B", 1));
