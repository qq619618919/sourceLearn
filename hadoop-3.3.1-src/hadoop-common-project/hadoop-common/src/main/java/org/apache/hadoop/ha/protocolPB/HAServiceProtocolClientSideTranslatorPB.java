...
public class HAServiceProtocolClientSideTranslatorPB implements HAServiceProtocol, Closeable, ProtocolTranslator {
    ...
    public HAServiceProtocolClientSideTranslatorPB(InetSocketAddress addr, Configuration conf, SocketFactory socketFactory,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： HAServiceProtocol 就是 ZKFC 和 anmenode 之间的通信协议
         *  rpcProxy 就是 namenodeRpcServer 的代理
         */
    ...
    public void transitionToActive(StateChangeRequestInfo reqInfo) throws IOException {
        try {
            TransitionToActiveRequestProto req = TransitionToActiveRequestProto.newBuilder().setReqInfo(convert(reqInfo))
                    .build();
    ...
    public void transitionToStandby(StateChangeRequestInfo reqInfo) throws IOException {
        try {
            TransitionToStandbyRequestProto req = TransitionToStandbyRequestProto.newBuilder().setReqInfo(convert(reqInfo))
                    .build();
    ...
    public void transitionToObserver(StateChangeRequestInfo reqInfo) throws IOException {
        try {
            TransitionToObserverRequestProto req = TransitionToObserverRequestProto.newBuilder().setReqInfo(convert(reqInfo))
                    .build();
