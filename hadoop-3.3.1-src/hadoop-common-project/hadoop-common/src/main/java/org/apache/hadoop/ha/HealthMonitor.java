...
public class HealthMonitor {
    ...
    private List<ServiceStateCallback> serviceStateCallbacks = Collections.synchronizedList(
            new LinkedList<ServiceStateCallback>());
    ...
    HealthMonitor(Configuration conf, HAServiceTarget target) {
        ...
        // TODO_MA 马中华 注释： ha.health-monitor.check-interval.ms = 1000ms
    ...
    private void doHealthChecks() throws InterruptedException {
        while (shouldRun) {
            ...
            // TODO_MA 马中华 注释： 休息一段时间
            Thread.sleep(checkIntervalMillis);
        }
    }

    private boolean isHealthCheckFailedException(Throwable t) {
        return ((t instanceof HealthCheckFailedException) || (t instanceof RemoteException && ((RemoteException) t).unwrapRemoteException(
                HealthCheckFailedException.class) instanceof HealthCheckFailedException
        )
        );
    }
    ...
    private class MonitorDaemon extends Daemon {
        ...
        public void run() {
            ...
            // TODO_MA 马中华 注释： while 循环
            while (shouldRun) {
                try {
                    /*************************************************
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： 首先建立和 NameNode 的连接
                     */
                    loopUntilConnected();

                    /*************************************************
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： 发送 RPC 请求让 NameNode 做健康检查
                     */
                    doHealthChecks();
