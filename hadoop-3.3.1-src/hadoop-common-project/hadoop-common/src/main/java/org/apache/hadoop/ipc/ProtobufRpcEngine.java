...
public class ProtobufRpcEngine implements RpcEngine {
    ...
    public static class Server extends ProtobufRpcEngine2.Server {
        ...
        public Server(Class<?> protocolClass, Object protocolImpl, Configuration conf, String bindAddress, int port,
                      ...
                      SecretManager<? extends TokenIdentifier> secretManager, String portRangeConfig,
                      AlignmentContext alignmentContext) throws IOException {
            super(protocolClass, protocolImpl, conf, bindAddress, port, numHandlers, numReaders, queueSizePerHandler,
                    verbose, secretManager, portRangeConfig, alignmentContext
            );
        }
         ...
         */
        static RpcWritable processCall(RPC.Server server, String connectionProtocolName, RpcWritable.Buffer request,
            ...
            try {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 通过反射调用 服务实例的 服务方法，获取执行结果
                 *  1、service 这个是服务端对应的 服务实例
                 *  2、methodDescriptor 想要调动的 服务实例的 方法
                 *  3、param 方法的参数
                 */
                result = service.callBlockingMethod(methodDescriptor, null, param);

//                Response response(result) = connect(hostname, port)
//                .sendRequest(Request(ServerClass.class, "serverMethod", Param(参数)))
