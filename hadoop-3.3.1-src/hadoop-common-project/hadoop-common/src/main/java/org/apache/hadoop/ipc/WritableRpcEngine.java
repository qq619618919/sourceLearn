...
public class WritableRpcEngine implements RpcEngine {
	...
	private static synchronized void initialize() {
		org.apache.hadoop.ipc.Server
				.registerProtocolEngine(RPC.RpcKind.RPC_WRITABLE, Invocation.class, new Server.WritableRpcInvoker());
		isInitialized = true;
	}
	...
	private static class Invocation implements Writable, Configurable {
		...
		public String toString() {
			...
			buffer.append(")").append(", rpc version=" + rpcVersion).append(", client version=" + clientVersion)
	...
	private static class Invoker implements RpcInvocationHandler {
		...
		public Invoker(Class<?> protocol, InetSocketAddress address, UserGroupInformation ticket, Configuration conf,
				SocketFactory factory, int rpcTimeout, AtomicBoolean fallbackToSimpleAuth,
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			try {
				value = (ObjectWritable) client
						.call(RPC.RpcKind.RPC_WRITABLE, new Invocation(method, args), remoteId, fallbackToSimpleAuth,
								alignmentContext);
			} finally {
				if(traceScope != null)
					traceScope.close();
	...
	public <T> ProtocolProxy<T> getProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr,
		...
		T proxy = (T) Proxy.newProxyInstance(protocol.getClassLoader(), new Class[]{protocol},
				new Invoker(protocol, addr, ticket, conf, factory, rpcTimeout, fallbackToSimpleAuth, alignmentContext));
	...
	public RPC.Server getServer(Class<?> protocolClass, Object protocolImpl, String bindAddress, int port,
		...
		return new Server(protocolClass, protocolImpl, conf, bindAddress, port, numHandlers, numReaders,
				queueSizePerHandler, verbose, secretManager, portRangeConfig, alignmentContext);
	}
	...
	public static class Server extends RPC.Server {
		 ...
		 * @param port          the port to listen for connections on
		 */
		public Server(Class<?> protocolClass, Object protocolImpl, Configuration conf, String bindAddress,
			...
			if(protocolClass == null) { // derive protocol from impl
				...
				if(!protocolClass.isAssignableFrom(protocolImpl.getClass())) {
					throw new IOException(
							"protocolClass " + protocolClass + " is not implemented by protocolImpl which is of class " + protocolImpl
									.getClass());
				}
		...
		static class WritableRpcInvoker implements RpcInvoker {
			...
			public Writable call(org.apache.hadoop.ipc.RPC.Server server, String protocolName, Writable rpcRequest,
				...
				if(call.getRpcVersion() != writableRpcVersion) {
					...
					throw new RpcServerException("WritableRpc version mismatch, client side version=" + call
							.getRpcVersion() + ", server side version=" + writableRpcVersion);
				}
				...
				try {
					Method method = protocolImpl.protocolClass
							.getMethod(call.getMethodName(), call.getParameterClasses());
