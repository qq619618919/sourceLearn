...
/**
 * // TODO_MA 马中华 注释： 通过运行 shell 脚本来杀死 namenode
...
public class ShellCommandFencer extends Configured implements FenceMethod {
    ...
    private String parseArgs(HAServiceProtocol.HAServiceState state, String cmd) {
        ...
        if (args.length > 2) {
            throw new IllegalArgumentException(
                    "Expecting arguments size of at most " + "two, getting " + Arrays.asList(args));
        }
        if (HAServiceProtocol.HAServiceState.ACTIVE.equals(state)) {
            ...
            throw new IllegalArgumentException("Unexpected HA service state:" + state);
        }
    }
    ...
    private void addTargetInfoAsEnvVars(HAServiceTarget target, Map<String, String> environment) {
        ...
        if (targetState == null || HAServiceProtocol.HAServiceState.ACTIVE.equals(targetState)) {
            ...
            throw new IllegalArgumentException("Unexpected HA service state:" + targetState);
        }
