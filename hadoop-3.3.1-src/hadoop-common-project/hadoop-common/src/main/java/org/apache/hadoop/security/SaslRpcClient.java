...
public class SaslRpcClient {
    ...
    private SaslClient createSaslClient(SaslAuth authType) throws SaslException, IOException {
        ...
        switch (method) {
            ...
            case KERBEROS: {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "RPC Server's Kerberos principal name for protocol=" + protocol.getCanonicalName() + " is " + serverPrincipal);
                }
                break;
            }
            default:
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "Creating SASL " + mechanism + "(" + method + ") " + " client to authenticate to service at " + saslServerName);
        }
        return saslFactory.createSaslClient(new String[]{mechanism}, saslUser, saslProtocol, saslServerName, saslProperties,
                saslCallback
        );
    }
    ...
    String getServerPrincipal(SaslAuth authType) throws IOException {
        ...
        if (serverKeyPattern != null && !serverKeyPattern.isEmpty()) {
            ...
            if (!pattern.matcher(serverPrincipal).matches()) {
                throw new IllegalArgumentException(
                        String.format("Server has invalid Kerberos principal: %s," + " doesn't match the pattern: %s",
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "getting serverKey: " + serverKey + " conf value: " + conf.get(serverKey) + " principal: " + confPrincipal);
            }
            ...
            if (!serverPrincipal.equals(confPrincipal)) {
                throw new IllegalArgumentException(
                        String.format("Server has invalid Kerberos principal: %s, expecting: %s", serverPrincipal, confPrincipal));
            }
        }
