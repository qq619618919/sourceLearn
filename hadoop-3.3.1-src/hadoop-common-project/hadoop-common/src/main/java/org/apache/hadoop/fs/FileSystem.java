...
public abstract class FileSystem extends Configured implements Closeable, DelegationTokenIssuer, PathCapabilities {
    ...
    protected URI canonicalizeUri(URI uri) {
        if (uri.getPort() == -1 && getDefaultPort() > 0) {
            ...
            try {
                uri = new URI(uri.getScheme(), uri.getUserInfo(), uri.getHost(), getDefaultPort(), uri.getPath(), uri.getQuery(),
                        uri.getFragment()
                );
        ...
        if (thisScheme.equalsIgnoreCase(thatScheme)) {// schemes match
            ...
            if (uri != null) {
                ...
                if (thisAuthority == thatAuthority ||       // authorities match
                        (thisAuthority != null && thisAuthority.equalsIgnoreCase(thatAuthority))) return;
            }
        }
    ...
    public FSDataOutputStream create(Path f, Progressable progress) throws IOException {
        return create(f, true, getConf().getInt(IO_FILE_BUFFER_SIZE_KEY, IO_FILE_BUFFER_SIZE_DEFAULT), getDefaultReplication(f),
    ...
    public FSDataOutputStream create(Path f, boolean overwrite, int bufferSize, short replication, long blockSize,
         ...
         *  注释：
         */
        return this.create(f, FsCreateModes.applyUMask(FsPermission.getFileDefault(), FsPermission.getUMask(getConf())), overwrite,
     ...
     */
    public abstract FSDataOutputStream create(Path f, FsPermission permission, boolean overwrite, int bufferSize, short replication,
     ...
     * @see #setPermission(Path, FsPermission)
     */
    public FSDataOutputStream create(Path f, FsPermission permission, EnumSet<CreateFlag> flags, int bufferSize, short replication,
                                     long blockSize, Progressable progress) throws IOException {
     ...
     */
    public FSDataOutputStream create(Path f, FsPermission permission, EnumSet<CreateFlag> flags, int bufferSize, short replication,
                                     long blockSize, Progressable progress, ChecksumOpt checksumOpt) throws IOException {
     ...
     */
    @Deprecated
    protected FSDataOutputStream primitiveCreate(Path f, FsPermission absolutePermission, EnumSet<CreateFlag> flag, int bufferSize,
        ...
        return this.create(f, absolutePermission, flag.contains(CreateFlag.OVERWRITE), bufferSize, replication, blockSize,
                progress
        );
    }
    ...
    public FSDataOutputStream createNonRecursive(Path f, FsPermission permission, boolean overwrite, int bufferSize,
        ...
        return createNonRecursive(f, permission,
                overwrite ? EnumSet.of(CreateFlag.CREATE, CreateFlag.OVERWRITE) : EnumSet.of(CreateFlag.CREATE), bufferSize,
    ...
    public ContentSummary getContentSummary(Path f) throws IOException {
        ...
        for (FileStatus s : listStatus(f)) {
            ...
            ContentSummary c = s.isDirectory() ? getContentSummary(s.getPath()) : new ContentSummary.Builder().length(length)
    ...
    public FileStatus[] globStatus(Path pathPattern) throws IOException {
        return Globber.createGlobber(this).withPathPattern(pathPattern).withPathFiltern(DEFAULT_FILTER).withResolveSymlinks(true)
                .build().glob();
    }
    ...
    public void close() throws IOException {
        debugLogFileSystemClose("close", "Key: " + key + "; URI: " + getUri() + "; Object Identity Hash: " + Integer.toHexString(
    ...
    static void checkAccessPermissions(FileStatus stat, FsAction mode) throws AccessControlException, IOException {
        ...
        throw new AccessControlException(
                String.format("Permission denied: user=%s, path=\"%s\":%s:%s:%s%s", user, stat.getPath(), stat.getOwner(),
                        stat.getGroup(), stat.isDirectory() ? "d" : "-", perm
                ));
    }
    ...
    public void createSymlink(final Path target, final Path link,
                              final boolean createParent) throws AccessControlException, FileAlreadyExistsException, FileNotFoundException, ParentNotDirectoryException, UnsupportedFileSystemException, IOException {
     ...
     * @throws IOException see specific implementation
     */
    public FileStatus getFileLinkStatus(
            final Path f) throws AccessControlException, FileNotFoundException, UnsupportedFileSystemException, IOException {
        // Supporting filesystems should override this method
    ...
    private static void loadFileSystems() {
        ...
        synchronized (FileSystem.class) {
            if (!FILE_SYSTEMS_LOADED) {
                 ...
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 总共有 8 种
                 *  1、viewfs        ViewFileSystem
                 *  2、swebhdfs      SWebHdfsFileSystem
                 *  3、file          LocalFileSystem
                 *  4、har           HarFileSystem
                 *  5、http          HttpFileSystem
                 *  6、hdfs          DistributedFileSystem
                 *  7、webhdfs       WebHdfsFileSystem
                 *  8、https         HttpsFileSystem
                 */
    ...
    public static Class<? extends FileSystem> getFileSystemClass(String scheme, Configuration conf) throws IOException {
        ...
        if (conf != null) {
            // TODO_MA 马中华 注释： key = fs.hdfs.impl
    ...
    private static FileSystem createFileSystem(URI uri, Configuration conf) throws IOException {
        ...
        try (TraceScope scope = tracer.newScope("FileSystem#createFileSystem"); DurationInfo ignored = new DurationInfo(LOGGER,
                false, "Creating FS %s", uri
        )) {
            scope.addKVAnnotation("scheme", uri.getScheme());

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 通过反射创建 FileSystem 实例
             */
    ...
    static final class Cache {
        ...
        private FileSystem getInternal(URI uri, Configuration conf, Key key) throws IOException {
            ...
            try {
                // TODO_MA 马中华 注释： 尝试从缓存中，获取，如果存在，则直接返回
                // TODO_MA 马中华 注释： 如果不存在，则创建
                // See if FS was instantiated by another thread while waiting for the permit.
                synchronized (this) {
                    fs = map.get(key);
                }
                if (fs != null) {
                ...
                final long timeout = conf.getTimeDuration(SERVICE_SHUTDOWN_TIMEOUT, SERVICE_SHUTDOWN_TIMEOUT_DEFAULT,
                ...
                synchronized (this) { // lock on the Cache object
                    ...
                    if (oldfs != null) {
                        ...
                        if (map.isEmpty() && !ShutdownHookManager.get().isShutdownInProgress()) {
                            ShutdownHookManager.get().addShutdownHook(clientFinalizer, SHUTDOWN_HOOK_PRIORITY, timeout,
                                    ShutdownHookManager.TIME_UNIT_DEFAULT
                            );
                        }
        ...
        static class Key {
            ...
            public boolean equals(Object obj) {
                ...
                if (obj instanceof Key) {
                    Key that = (Key) obj;
                    return isEqual(this.scheme, that.scheme) && isEqual(this.authority, that.authority) && isEqual(this.ugi,
                            that.ugi
    ...
    protected static FSDataOutputStreamBuilder createDataOutputStreamBuilder(@Nonnull final FileSystem fileSystem,
    ...
    private static final class FileSystemDataOutputStreamBuilder extends FSDataOutputStreamBuilder<FSDataOutputStream, FileSystemDataOutputStreamBuilder> {
        ...
        public FSDataOutputStream build() throws IOException {
            ...
            if (getFlags().contains(CreateFlag.CREATE) || getFlags().contains(CreateFlag.OVERWRITE)) {
                if (isRecursive()) {
                    return getFS().create(getPath(), getPermission(), getFlags(), getBufferSize(), getReplication(), getBlockSize(),
                            getProgress(), getChecksumOpt()
                    );
                } else {
                    return getFS().createNonRecursive(getPath(), getPermission(), getFlags(), getBufferSize(), getReplication(),
                            getBlockSize(), getProgress()
                    );
                }
     ...
     */
    protected CompletableFuture<FSDataInputStream> openFileWithOptions(final Path path,
     ...
     */
    protected CompletableFuture<FSDataInputStream> openFileWithOptions(final PathHandle pathHandle,
                                                                       final OpenFileParameters parameters) throws IOException {
    ...
    @InterfaceStability.Unstable
    protected static FSDataInputStreamBuilder createDataInputStreamBuilder(@Nonnull final FileSystem fileSystem,
                                                                           @Nonnull final Path path) {
    ...
    private static class FSDataInputStreamBuilder extends FutureDataInputStreamBuilderImpl implements FutureDataInputStreamBuilder {
        ...
        public CompletableFuture<FSDataInputStream> build() throws IOException {
            ...
            OpenFileParameters parameters = new OpenFileParameters().withMandatoryKeys(getMandatoryKeys()).withOptions(getOptions())
                    .withBufferSize(getBufferSize()).withStatus(super.getStatus());  // explicit to avoid IDE warnings
