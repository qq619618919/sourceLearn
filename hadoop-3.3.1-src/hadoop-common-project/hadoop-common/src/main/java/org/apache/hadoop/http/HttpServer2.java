...
public final class HttpServer2 implements FilterContainer {
    ...
    public static class Builder {
        ...
        private void loadSSLConfiguration() throws IOException {
            ...
            needsClientAuth = sslConf.getBoolean(SSLFactory.SSL_SERVER_NEED_CLIENT_AUTH,
        ...
        public HttpServer2 build() throws IOException {
            ...
            if (this.securityEnabled && !this.conf.get(authFilterConfigurationPrefix + "type")
        ...
        private ServerConnector createHttpsChannelConnector(Server server, HttpConfiguration httpConfig) {
            ...
            long storesReloadInterval = conf.getLong(FileBasedKeyStoresFactory.SSL_STORES_RELOAD_INTERVAL_TPL_KEY,
    ...
    private void addPrometheusServlet(Configuration conf) {
        prometheusSupport = conf.getBoolean(CommonConfigurationKeysPublic.HADOOP_PROMETHEUS_ENABLED,
    ...
    private static FilterInitializer[] getFilterInitializers(Configuration conf) {
        ...
        if (classList.contains(AuthenticationFilterInitializer.class) && classList.contains(
                ProxyUserAuthenticationFilterInitializer.class)) {
    ...
    protected void addDefaultApps(ContextHandlerCollection parent, final String appDir,
        ...
        boolean logsEnabled = conf.getBoolean(CommonConfigurationKeys.HADOOP_HTTP_LOGS_ENABLED,
        ...
        for (int i = 0; i < servletMappings.length; i++) {
            if (servletMappings[i].containsPathSpec(pathSpec)) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "Found existing " + servletMappings[i].getServletName() + " servlet at path " + pathSpec + "; will replace mapping" + " with " + holder.getName() + " servlet");
                }
    ...
    public void addInternalServlet(String name, String pathSpec, Class<? extends HttpServlet> clazz,
        ...
        for (int i = 0; i < servletMappings.length; i++) {
            if (servletMappings[i].containsPathSpec(pathSpec)) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "Found existing " + servletMappings[i].getServletName() + " servlet at path " + pathSpec + "; will replace mapping" + " with " + sh.getName() + " servlet");
                }
                ServletMapping[] newServletMappings = ArrayUtil.removeFromArray(servletMappings, servletMappings[i]);
                webAppContext.getServletHandler().setServletMappings(newServletMappings);
                break;
     ...
     */
    public static void defineFilter(ServletContextHandler ctx, String name, String classname, Map<String, String> parameters,
                                    String[] urls) {
        FilterHolder filterHolder = getFilterHolder(name, classname, parameters);
    ...
    private void initSpnego(Configuration conf, String hostName, String usernameConfKey,
                            String keytabConfKey) throws IOException {
    ...
    public void stop() throws Exception {
        ...
        if (this.configurationChangeMonitor.isPresent()) {
            try {
                ...
                LOG.error("Error while canceling configuration monitoring timer for webapp" + webAppContext.getDisplayName(),
                        e
                );
    ...
    public String toString() {
        ...
        StringBuilder sb = new StringBuilder("HttpServer (").append(
                isAlive() ? STATE_DESCRIPTION_ALIVE : STATE_DESCRIPTION_NOT_LIVE).append("), listening at:");
