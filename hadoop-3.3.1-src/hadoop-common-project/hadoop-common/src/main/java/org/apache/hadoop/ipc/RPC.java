...
public class RPC {
     ...
     * @throws IOException if the far end through a RemoteException
     */
    public static <T> ProtocolProxy<T> waitForProtocolProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr,
                                                            Configuration conf) throws IOException {
     ...
     * talking to a server at the named address.
     * @param <T>*/
    public static <T> T getProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr, UserGroupInformation ticket,
                                 Configuration conf, SocketFactory factory) throws IOException {
    ...
    public static void stopProxy(Object proxy) {
        ...
        throw new HadoopIllegalArgumentException(
                "Cannot close proxy - is not Closeable or " + "does not provide closeable invocation handler " + proxy.getClass());
    }
...
//            RPC.setProtocolEngine(conf, ZKFCProtocolPB.class, ProtobufRpcEngine2.class);
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： Hadoop RPC Server 的代码入口
             */
            return getProtocolEngine(this.protocol, this.conf)

                    // TODO_MA 马中华 注释： 获取和构建 Server 端
                    .getServer(this.protocol, this.instance, this.bindAddress, this.port, this.numHandlers, this.numReaders,
                            this.queueSizePerHandler, this.verbose, this.conf, this.secretManager, this.portRangeConfig,
    ...
    public abstract static class Server extends org.apache.hadoop.ipc.Server {
        ...
        private static final Pattern COMPLEX_SERVER_NAME_PATTERN = Pattern.compile(
                "(?:[^\\$]*\\$)*([A-Za-z][^\\$]+)(?:\\$\\d+)?");
        ...
        void registerProtocolAndImpl(RpcKind rpcKind, Class<?> protocolClass, Object protocolImpl) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "RpcKind = " + rpcKind + " Protocol Name = " + protocolName + " version=" + version + " ProtocolImpl=" + protocolImpl.getClass()
