...
public class DomainSocket implements Closeable {
    ...
    private static native int receiveFileDescriptors0(int fd, FileDescriptor[] descriptors, byte[] buf, int offset,
                                                      int length) throws IOException;
        ...
        public int read(ByteBuffer dst) throws IOException {
            ...
            try {
                ...
                if (dst.isDirect()) {
                    ...
                    nread = DomainSocket.readArray0(DomainSocket.this.fd, dst.array(), dst.position() + dst.arrayOffset(),
                            dst.remaining()
                    );
                } else {
                    throw new AssertionError(
                            "we don't support " + "using ByteBuffers that aren't either direct or backed by " + "arrays");
                }
                if (nread > 0) {
