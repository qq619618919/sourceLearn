...
public class ProtobufRpcEngine2 implements RpcEngine {
    ...
    static void registerProtocolEngine() {
        if (Server.getRpcInvoker(RPC.RpcKind.RPC_PROTOCOL_BUFFER) == null) {
            org.apache.hadoop.ipc.Server.registerProtocolEngine(RPC.RpcKind.RPC_PROTOCOL_BUFFER,
                    ProtobufRpcEngine2.RpcProtobufRequest.class, new Server.ProtoBufRpcInvoker()
            );
        }
    }
    ...
    public <T> ProtocolProxy<T> getProxy(Class<T> protocol, long clientVersion, InetSocketAddress addr,
         ...
         *  注释： 当需要发送一个 RPC 请求的时候，就会调用 Invoker.invoke() 方法
         *  Invoker + RpcCall 其实和 我做的抽象叫做 Request 有点类似
         *  -
         *  完整抽象：
         *  proxy.method() 内部其实就是调用 InvocationHandler 的 invoke()
         *  这是 JDK 动态代理机制的工作机制
         *  - 层级总结：
         *  1、proxy.method()
         *  2、invoker.invoke()
    ...
    protected static class Invoker implements RpcInvocationHandler {
        ...
        protected Invoker(Class<?> protocol, InetSocketAddress addr, UserGroupInformation ticket, Configuration conf,
                          SocketFactory factory, int rpcTimeout, RetryPolicy connectionRetryPolicy,
                          AtomicBoolean fallbackToSimpleAuth, AlignmentContext alignmentContext) throws IOException {
            // TODO_MA 马中华 注释： 内部封装了一个 RPC 客户端
            this(protocol,
                    Client.ConnectionId.getConnectionId(addr, protocol, ticket, rpcTimeout, connectionRetryPolicy, conf),
                    conf, factory
            );
         ...
         */
        protected Invoker(Class<?> protocol, Client.ConnectionId connId, Configuration conf, SocketFactory factory) {
        ...
        public Message invoke(Object proxy, final Method method, Object[] args) throws ServiceException {
            ...
            if (args.length != 2) { // RpcController + Message
                throw new ServiceException(
                        "Too many or few parameters for request. Method: [" + method.getName() + "]" + ", Expected: 2, Actual: " + args.length);
            }
            if (args[1] == null) {
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace(Thread.currentThread()
                        .getId() + ": Call -> " + remoteId + ": " + method.getName() + " {" + TextFormat.shortDebugString(
                        (Message) args[1]) + "}");
            }
            ...
            try {
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 通过客户端发送 call 请求，内部就是发送 RPC 请求
                 */
                val = (RpcWritable.Buffer) client.call(RPC.RpcKind.RPC_PROTOCOL_BUFFER,
                        constructRpcRequest(method, theRequest), remoteId, fallbackToSimpleAuth, alignmentContext
                );

            } catch (Throwable e) {
                if (LOG.isTraceEnabled()) {
                    LOG.trace(Thread.currentThread()
                            .getId() + ": Exception <- " + remoteId + ": " + method.getName() + " {" + e + "}");
                }
                if (traceScope != null) {
        ...
        private Message getReturnMessage(final Method method, final RpcWritable.Buffer buf) throws ServiceException {
            ...
            try {
                ...
                if (LOG.isTraceEnabled()) {
                    LOG.trace(Thread.currentThread()
                            .getId() + ": Response <- " + remoteId + ": " + method.getName() + " {" + TextFormat.shortDebugString(
                            returnMessage) + "}");
                }
    ...
    public RPC.Server getServer(Class<?> protocol, Object protocolImpl, String bindAddress, int port, int numHandlers,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 注意 numHandlers, numReaders 这两个参数，很重要
         *  就是 Handler 和 Reader 线程的数量
         */
        return new Server(protocol, protocolImpl, conf, bindAddress, port, numHandlers, numReaders, queueSizePerHandler,
                verbose, secretManager, portRangeConfig, alignmentContext
        );
    }
    ...
    public static class Server extends RPC.Server {
        ...
        static class ProtoBufRpcInvoker implements RpcInvoker {
             ...
             * it is.</li>
             * </ol>
             */ public Writable call(RPC.Server server, String connectionProtocolName, Writable writableRequest,
                 ...
                 *  注释：
                 */
                return call(server, connectionProtocolName, request, receiveTime, methodName, declaringClassProtoName,
                        clientVersion
                );
            }

            @SuppressWarnings("deprecation")
            protected Writable call(RPC.Server server, String connectionProtocolName, RpcWritable.Buffer request,
                                    long receiveTime, String methodName, String declaringClassProtoName,
                                    long clientVersion) throws Exception {
                if (server.verbose) {
                ...
                if (protocolImpl.isShadedPBImpl()) {
                    return call(server, connectionProtocolName, request, methodName, protocolImpl);
                }
                 ...
                 *  注释： 处理 Call
                 */
                // Legacy protobuf implementation. Handle using legacy (Non-shaded) protobuf classes.
                return ProtobufRpcEngine.Server.processCall(server, connectionProtocolName, request, methodName,
                        protocolImpl
                );
            }

            private RpcWritable call(RPC.Server server, String connectionProtocolName, RpcWritable.Buffer request,
