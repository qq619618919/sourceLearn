...
public abstract class Server {
    ...
    public static void registerProtocolEngine(RPC.RpcKind rpcKind, Class<? extends Writable> rpcRequestWrapperClass,
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "rpcKind=" + rpcKind + ", rpcRequestWrapperClass=" + rpcRequestWrapperClass + ", rpcInvoker=" + rpcInvoker);
        }
    }

    public Class<? extends Writable> getRpcRequestWrapper(RpcKindProto rpcKind) {
        if (rpcRequestClass != null) return rpcRequestClass;
    ...
    void logSlowRpcCalls(String methodName, Call call, ProcessingDetails details) {
        ...
        if ((rpcMetrics.getProcessingSampleCount() > minSampleSize) && (processingTime > threeSigma)) {
            LOG.warn("Slow RPC : {} took {} {} to process from client {}," + " the processing detail is {}", methodName,
                    processingTime, RpcMetrics.TIMEUNIT, call, details.toString()
            );
    ...
    static Class<? extends RpcScheduler> getSchedulerClass(String prefix, Configuration conf) {
        ...
        // TODO_MA 马中华 注释： 默认实现： DefaultRpcScheduler
    ...
    public static class Call implements Schedulable, PrivilegedExceptionAction<Void> {
        ...
        Call() {
            this(RpcConstants.INVALID_CALL_ID, RpcConstants.INVALID_RETRY_COUNT, RPC.RpcKind.RPC_BUILTIN,
    ...
    private class RpcCall extends Call {
        ...
        RpcCall(Connection connection, int id, int retryCount, Writable param, RPC.RpcKind kind, byte[] clientId,
        ...
        public Void run() throws Exception {
            ...
            try {
                 ...
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 执行 RPC 请求的处理
                 *  1、call 方法执行 RPC 请求的真正处理，内部就是通过反射调用服务实例的对应同名方法，得到结果
                 *  2、value 就是 call 方法的执行结果
                 *  补充一点：
                 *  value 会被存储在 call 中， 所以只要 call 中的 Writable rv 不为空，则表示这个 call 已经处理过了
                 *  再补充一点：
                 *  这个 call 就会被丢到对应的 connection 对象的 响应队列中
                 */
            ...
            if (!isResponseDeferred()) {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： RpcCall 对象的 Writable rv 成员变量保存处理结果
                 */
                setResponseFields(value, responseParams);

                // TODO_MA 马中华 注释： 到这儿位置，Handler 线程的工作就完成了
        ...
        // TODO_MA 马中华 注释： response 就是当前 RpcCall 的 rv 成员变量
        // TODO_MA 马中华 注释： 把 Writable 的 rv 转换成 ByteBuffer 的 rpcResponse
        ...
        void doResponse(Throwable t, RpcStatusProto status) throws IOException {
            ...
            if (t != null) {
                ...
                setupResponse(call, status, RpcErrorCodeProto.ERROR_RPC_SERVER, null, t.getClass().getName(),
                 ...
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 设置响应
                 *  将 ByteBuffer 格式的 响应结果 设置到 RpcCall 的 rpcResponse 成员变量上保存
                 */
                setupResponse(call, call.responseParams.returnStatus, call.responseParams.detailedErr, call.rv,
                        call.responseParams.errorClass, call.responseParams.error
                );
            }

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 通过链接对象，给客户端返回该 RpcCall 请求处理的 结果
             */
        ...
        private void sendDeferedResponse() {
            try {
                ...
                LOG.error("Failed to send deferred response. ThreadName=" + Thread.currentThread()
        ...
        public void setDeferredResponse(Writable response) {
            if (this.connection.getServer().running) {
                try {
                    ...
                    LOG.error("Failed to setup deferred successful response. ThreadName=" + Thread.currentThread()
                            .getName() + ", Call=" + this);
                    return;
                }
                sendDeferedResponse();
        ...
        public void setDeferredError(Throwable t) {
            if (this.connection.getServer().running) {
                ...
                try {
                    ...
                    setupResponse(this, responseParams.returnStatus, responseParams.detailedErr, null,
                            responseParams.errorClass, responseParams.error
                    );
                } catch (IOException e) {
                    // For synchronous calls, application code is done once it has
                    // returned from a method. It does not expect to receive an error.
                    // This is equivalent to what happens in synchronous calls when the
                    // response cannot be sent.
                    LOG.error("Failed to setup deferred error response. ThreadName=" + Thread.currentThread()
                            .getName() + ", Call=" + this);
                }
                sendDeferedResponse();
            }
    ...
    private class Listener extends Thread {
        ...
        private boolean reuseAddr = conf.getBoolean(CommonConfigurationKeysPublic.IPC_SERVER_REUSEADDR_KEY,
        ...
        Listener(int port) throws IOException {
            ...
            // TODO_MA 马中华 注释： 启动 NIO 服务端，绑定端口
            ...
            // TODO_MA 马中华 注释： NIO 服务端 注册 OP_ACCEPT 请求，等待 NIO 客户端的链接请求发送过来
        ...
        private class Reader extends Thread {
            ...
            Reader(String name) throws IOException {
                super(name);
                // TODO_MA 马中华 注释： 维护待服务的 Connection
            ...
            private synchronized void doRunLoop() {
                while (running) {
                    ...
                    try {
                        // TODO_MA 马中华 注释： 给每一个 SocketChannel 注册 OP_READ 事件
            ...
            public void addConnection(Connection conn) throws InterruptedException {
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： Reader 线程其实服务的 Connection 有很多的
                 *  Reader 线程维护了一个队列，用来保存服务对象 Connection
                 */
        ...
        // TODO_MA 马中华 注释： Listener 的 run() 方法
        @Override
        public void run() {
            ...
            // TODO_MA 马中华 注释： 如果退出这个循环了，表示该 Server 不运行了。
            while (running) {
                SelectionKey key = null;
                try {
                    getSelector().select();
                    // TODO_MA 马中华 注释： SelectionKey = Channel + OP_APPECT
            ...
            // TODO_MA 马中华 注释： 当 Server 不运行了，则各种组件回收
            synchronized (this) {
                try {
                    acceptChannel.close();
                    selector.close();
        ...
        void doAccept(SelectionKey key) throws InterruptedException, IOException, OutOfMemoryError {
            ...
            // TODO_MA 马中华 注释： 这就是存储于在 服务端里面的 客户端抽象
            ...
            while ((channel = server.accept()) != null) {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 通过轮询获取到一个 Reader 线程
                 */
                Reader reader = getReader();

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： SocketChannel 注册到 Connection
                 *  1、创建 Connection，包装了 SocketChannel
                 *  2、注册 SocketChannel
                 */
                ...
                // TODO_MA 马中华 注释： SelectionKey 绑定 Connection
                key.attach(c);  // so closeCurrentConnection can get the object

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： Reader 线程和 Connection 绑定
                 */
                reader.addConnection(c);

                // TODO_MA 马中华 注释： 最终的结果，完成了链接之后
                // TODO_MA 马中华 注释： 把 SocketChannel 和 Reader 和 Connection 三方绑一一关系
                // TODO_MA 马中华 注释： SelectionKey 完成和上述三个对象的绑定

                // TODO_MA 马中华 注释： 问题：此时已经完成了链接，肯定就是等待客户端发送数据过来
                // TODO_MA 马中华 注释： 但是此时，并没有发现给 SocketChannel 注册 OP_READ 事件

                // TODO_MA 马中华 注释： 一定在 Reader 线程的 run() 方法中
            }
        }

        // TODO_MA 马中华 注释：  Connection + SocketChannel + Reader + SlectionKey
        void doRead(SelectionKey key) throws InterruptedException {
            ...
            try {
                ...
                LOG.info(Thread.currentThread()
                        .getName() + ": readAndProcess from client " + c + " threw exception [" + e + "]", e);
    ...
    private class Responder extends Thread {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 大体上，Call 对象的响应结果的返回的流程：
         *  1、如果 responseQueue 的长度为 1，也就表示不忙，通过同步方式，直接返回响应给客户端，如果忙，加入到队列就完事
         *  2、调用 processResponse 开始返回：
         *      1、先尝试返回数据，如果没有一次性把所有数据返回给客户端
         *      2、把这个 RpcCall 继续加入到 responseQueue 的头部
         *      3、注册一个 OP_WRITE 事件， Responder 线程会响应这个事件， 然后继续从 responseQueue 中获取头部 RpcCall 来返回数据给客户端
         */
        ...
        // Remove calls that have been pending in the responseQueue for a long time.
                    ...
                    if (!call.rpcResponse.hasRemaining()) {
                        ...
                        if (LOG.isDebugEnabled()) {
                            LOG.debug(Thread.currentThread()
                                    .getName() + ": responding to " + call + " Wrote " + numBytes + " bytes.");
                        }
                    }
                    // TODO_MA 马中华 注释： 如果一次没写完，则加入 RpcCall 到响应队列中
                    else {
                        ...
                        if (LOG.isDebugEnabled()) {
                            LOG.debug(Thread.currentThread()
                                    .getName() + ": responding to " + call + " Wrote partial " + numBytes + " bytes.");
                        }
                    }
        ...
        void doRespond(RpcCall call) throws IOException {
            synchronized (call.connection.responseQueue) {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 加入到 Connection 的响应队列
                 */
                call.connection.responseQueue.addLast(call);

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 如果响应队列中，只有刚才加入进去的 需响应的 RpcCall, 则证明比较闲，
                 *  那么使用 同步方式来进行 响应
                 *  如果队列大于1，则表示队列中，还有未响应的 RpcCall 则通过异步方式来进行响应
                 */
                if (call.connection.responseQueue.size() == 1) {

                    // TODO_MA 马中华 注释： 同步方式处理
                    processResponse(call.connection.responseQueue, true);
                } else {
                    // TODO_MA 马中华 注释： 如果确实要走这个 else
                    // TODO_MA 马中华 注释： 表示 Responder 线程，正在为这个 Connection 上的其他的 RpcCall 做响应返回
                    // TODO_MA 马中华 注释： 此时加进去队列的 RpcCall 乖乖等待就行
                }

                // TODO_MA 马中华 注释： 1、如果 Connection 不忙，则通过同步方式来返回响应
                // TODO_MA 马中华 注释： 2、如果 Connection 繁忙，则通过异步方式来返回
            }
        }
    ...
    public class Connection {
        ...
        private void saslReadAndProcess(
                RpcWritable.Buffer buffer) throws RpcServerException, IOException, InterruptedException {
            final RpcSaslProto saslMessage = getMessage(RpcSaslProto.getDefaultInstance(), buffer);
            switch (saslMessage.getState()) {
                case WRAP: {
                    if (!saslContextEstablished || !useWrap) {
                        throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_INVALID_RPC_HEADER,
        ...
        private void saslProcess(RpcSaslProto saslMessage) throws RpcServerException, IOException, InterruptedException {
            ...
            try {
                try {
                    ...
                    AUDITLOG.warn(
                            AUTH_FAILED_FOR + this.toString() + ":" + attemptingUser + " (" + e.getLocalizedMessage() + ") with true cause: (" + tce.getLocalizedMessage() + ")");
                    throw tce;
                }

                if (saslServer != null && saslServer.isComplete()) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug("SASL server context established. Negotiated QoP is " + saslServer.getNegotiatedProperty(
                                Sasl.QOP));
                    }
         ...
         * @throws InterruptedException
         */
        private RpcSaslProto processSaslMessage(
                RpcSaslProto saslMessage) throws SaslException, IOException, AccessControlException, InterruptedException {
                    ...
                    if (!negotiateResponse.getAuthsList().contains(clientSaslAuth)) {
                        if (sentNegotiate) {
                            throw new AccessControlException(
                                    clientSaslAuth.getMethod() + " authentication is not enabled." + "  Available:" + enabledAuthMethods);
                        }
                        saslResponse = buildSaslNegotiateResponse();
                        break;
                    }
        ...
        private RpcSaslProto processSaslToken(RpcSaslProto saslMessage) throws SaslException {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Have read input token of size " + saslToken.length + " for processing by saslServer.evaluateResponse()");
            }
        ...
        private RpcSaslProto buildSaslResponse(SaslState state, byte[] replyToken) {
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Will send " + state + " token of size " + ((replyToken != null) ? replyToken.length : null) + " from saslServer.");
            }
        ...
        private void doSaslReply(Exception ioe) throws IOException {
            setupResponse(authFailedCall, RpcStatusProto.FATAL, RpcErrorCodeProto.FATAL_UNAUTHORIZED, null,
            ...
            while (!shouldClose()) { // stop if a fatal response has been sent.
                ...
                if (!connectionHeaderRead) {
                    ...
                    if (!RpcConstants.HEADER.equals(dataLengthBuffer)) {
                        LOG.warn("Incorrect RPC Header length from {}:{} " + "expected length: {} got length: {}",
                    ...
                    if (version != CURRENT_VERSION) {
                        ...
                        LOG.warn(
                                "Version mismatch from " + hostAddress + ":" + remotePort + " got version " + version + " expected version " + CURRENT_VERSION);
                        setupBadVersionResponse(version);
                        return -1;
                    }
                 ...
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 读取数据
                 *  这个 ByteBuffer data 就是客户端发送过来的数据
                 */
        ...
        private void setupBadVersionResponse(int clientVersion) throws IOException {
            ...
            if (clientVersion >= 9) {
                ...
                setupResponse(fakeCall, RpcStatusProto.FATAL, RpcErrorCodeProto.FATAL_VERSION_MISMATCH, null,
            ...
            if (authProtocol == AuthProtocol.NONE) {
                ...
                if ((protocolUser != null) && (!protocolUser.getUserName().equals(user.getUserName()))) {
                    if (authMethod == AuthMethod.TOKEN) {
                        ...
                        throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_UNAUTHORIZED, new AccessControlException(
                                "Authenticated user (" + user + ") doesn't match what the client claims to be (" + protocolUser + ")"));
                    } else {
        ...
        private void processOneRpc(ByteBuffer bb) throws IOException, InterruptedException {
            ...
            try {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug(Thread.currentThread()
                            .getName() + ": processOneRpc from client " + this + " threw exception [" + rse + "]");
                }
                // use the wrapped exception if there is one.
                Throwable t = (rse.getCause() != null) ? rse.getCause() : rse;
                final RpcCall call = new RpcCall(this, callId, retry);
                // TODO_MA 马中华 注释： 准备异常响应
                setupResponse(call, rse.getRpcStatusProto(), rse.getRpcErrorCodeProto(), null, t.getClass().getName(),
                        t.getMessage()
                );
                // TODO_MA 马中华 注释： 发送异常响应
                sendResponse(call);
            }
        }
        ...
        private void processRpcRequest(RpcRequestHeaderProto header,
                                       RpcWritable.Buffer buffer) throws RpcServerException, InterruptedException {
            ...
            try { //Read the rpc request
                ...
                LOG.warn(
                        "Unable to read call parameters for client " + getHostAddress() + "on connection protocol " + this.protocolName + " for rpcKind " + header.getRpcKind(),
                        t
                );
            ...
            if (header.hasTraceInfo()) {
                if (tracer != null) {
                    ...
                    // trace
                    SpanId parentSpanId = new SpanId(header.getTraceInfo().getTraceId(),
            ...
            if (header.hasCallerContext()) {
                callerContext = new CallerContext.Builder(header.getCallerContext().getContext()).setSignature(
                        header.getCallerContext().getSignature().toByteArray()).build();
            }
             ...
             *  注释： 封装 RpcCall 实例
             */
            RpcCall call = new RpcCall(this, header.getCallId(), header.getRetryCount(), rpcRequest,
                    ProtoUtil.convert(header.getRpcKind()), header.getClientId().toByteArray(), traceScope, callerContext
            );
            ...
            try {
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 丢入 队列 管理器，之后由  Handler 来完成消费
                 */
        ...
        private void authorizeConnection() throws RpcServerException {
            try {
                ...
                LOG.info(
                        "Connection from " + this + " for protocol " + connectionContext.getProtocol() + " is unauthorized for user " + user);
        ...
        <T extends Message> T getMessage(Message message, RpcWritable.Buffer buffer) throws RpcServerException {
            try {
                ...
                throw new FatalRpcServerException(RpcErrorCodeProto.FATAL_DESERIALIZING_REQUEST,
    ...
    private void internalQueueCall(Call call, boolean blocking) throws IOException, InterruptedException {
        try {
             ...
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 加入队列
             *  注意： callQueue 的消费者，就是 Handler 线程
             */
            ...
            while (running) {
                ...
                try {
                    ...
                    if (running) {                          // unexpected -- log it
                        ...
                        if (traceScope != null) {
                            traceScope.getSpan()
                                    .addTimelineAnnotation("unexpectedly interrupted: " + StringUtils.stringifyException(e));
                        }
                    }
                } catch (Exception e) {
    ...
    protected Server(String bindAddress, int port, Class<? extends Writable> rpcRequestClass, int handlerCount,
        ...
        // TODO_MA 马中华 注释： Reader 线程的个数，默认是1 个
        if (numReaders != -1) {
         ...
         *  注释： 队列管理器
         */
        this.callQueue = new CallQueueManager<Call>(getQueueClass(prefix, conf), getSchedulerClass(prefix, conf),
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 链接管理器 初始化
         *  维护映射关系:  SocketChannel + Connection + SelectionKey + Reader
         */
        ...
        this.setLogSlowRPC(conf.getBoolean(CommonConfigurationKeysPublic.IPC_SERVER_LOG_SLOW_RPC,
     ...
     * @throws IOException
     */
    private void setupResponse(RpcCall call, RpcStatusProto status, RpcErrorCodeProto erCode, Writable rv, String errorClass,
        ...
        if (status == RpcStatusProto.SUCCESS) {
            ...
            try {
                ...
                setupResponse(call, RpcStatusProto.ERROR, RpcErrorCodeProto.ERROR_SERIALIZING_RESPONSE, null,
                        t.getClass().getName(), StringUtils.stringifyException(t)
                );
                return;
            }
        } else { // Rpc Failure
     ...
     * @throws IOException
     */
    private void setupResponseOldVersionFatal(ByteArrayOutputStream response, RpcCall call, Writable rv, String errorClass,
                                              String error) throws IOException {
    ...
    private void wrapWithSasl(RpcCall call) throws IOException {
        if (call.connection.saslServer != null) {
            ...
            RpcSaslProto saslMessage = RpcSaslProto.newBuilder().setState(SaslState.WRAP)
                    .setToken(ByteString.copyFrom(token)).build();
    ...
    private class ConnectionManager {
        ...
        ConnectionManager() {
            ...
            // create a set with concurrency -and- a thread-safe iterator, add 2 for listener and idle closer threads
            // TODO_MA 马中华 注释： 链接管理器
            this.connections = Collections.newSetFromMap(
                    new ConcurrentHashMap<Connection, Boolean>(maxQueueSize, 0.75f, readThreads + 2));
        ...
        Connection register(SocketChannel channel, int ingressPort, boolean isOnAuxiliaryPort) {
            ...
            // TODO_MA 马中华 注释： 创建一个链接，其实就是一个线程，用来给这个客户端进行服务
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Server connection from " + connection + "; # active connections: " + size() + "; # queued calls: " + callQueue.size());
            }
            return connection;
        }
        ...
        boolean close(Connection connection) {
            ...
            if (exists) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug(Thread.currentThread()
                            .getName() + ": disconnecting client " + connection + ". Number of active connections: " + size());
                }
        ...
        synchronized void closeIdle(boolean scanAll) {
            ...
            for (Connection connection : connections) {
                ...
                if (connection.isIdle() && connection.getLastContact() < minLastContact && close(
                        connection) && !scanAll && (++closed == maxIdleToClose)) {
                    break;
                }
            }
        }
