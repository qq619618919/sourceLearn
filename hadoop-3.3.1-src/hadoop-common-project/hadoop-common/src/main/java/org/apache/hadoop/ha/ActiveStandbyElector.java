...
public class ActiveStandbyElector implements StatCallback, StringCallback {
    ...
    public ActiveStandbyElector(String zookeeperHostPorts, int zookeeperSessionTimeout, String parentZnodeName,
                                List<ACL> acl, List<ZKAuthInfo> authInfo, ActiveStandbyElectorCallback app,
                                int maxRetryNum) throws IOException, HadoopIllegalArgumentException, KeeperException {
    ...
    public ActiveStandbyElector(String zookeeperHostPorts, int zookeeperSessionTimeout, String parentZnodeName,
        ...
        // TODO_MA 马中华 注释： HDFS: parentZnodeName = /hadoop-ha/nameservice
        // TODO_MA 马中华 注释： YARN: parentZnodeName = /yarn-leader-election/nameservice
        znodeWorkingDir = parentZnodeName;
        // TODO_MA 马中华 注释： hdfs = /hadoop-ha/nameservice/ActiveStandbyElectorLock
        // TODO_MA 马中华 注释： yarn = /hadoop-ha/clusterID/ActiveStandbyElectorLock
        zkLockFilePath = znodeWorkingDir + "/" + LOCK_FILENAME;
        // TODO_MA 马中华 注释： HDFS: /hadoop-ha/nameservice/ActiveBreadCrumb
        // TODO_MA 马中华 注释： YARN: /hadoop-ha/clusterID/ActiveBreadCrumb
    ...
    public synchronized void ensureParentZNode() throws IOException, InterruptedException, KeeperException {
        ...
        // TODO_MA 马中华 注释： 逐级创建， znodeWorkingDir = /hadoop-ha/nameservice
    ...
    public synchronized void processResult(int rc, String path, Object ctx, String name) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "CreateNode result: " + rc + " for path: " + path + " connectionState: " + zkConnectionState + "  for " + this);
        }
        ...
        if (isSuccess(code)) {
            ...
            // TODO_MA 马中华 注释： 尝试成为 active
            if (becomeActive()) {
                // 注册锁节点的监听
                monitorActiveStatus();
            } else {
                // 如果切换成为 active 没有成功，则再次尝试竞争成为 active
                reJoinElectionAfterFailureToBecomeActive();
            }
            // 如果切换成为 active 成功，则该方法返回
            return;
        }

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 如果锁节点存在了，则证明是别人创建的
         */
        if (isNodeExists(code)) {
            ...
            // TODO_MA 马中华 注释： 注册监听， 监听 NodeDeleted 事件
            monitorActiveStatus();
            return;
        }
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释：
         *  如果不是  节点创建成功  和  节点已经存在  两种情况，则重试
         */
        if (shouldRetry(code)) {
            // TODO_MA 马中华 注释： 如果 重试计数器 < 重试最大次数 = 3
            if (createRetryCount < maxRetryNum) {
                LOG.debug("Retrying createNode createRetryCount: " + createRetryCount);
                // TODO_MA 马中华 注释： 重试次数 +1
                ++createRetryCount;
                // TODO_MA 马中华 注释： 重试创建锁节点
                createLockNodeAsync();
                return;
            }
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 重试也没有成功，则报错了
         *  尝试3次做选举没有成功，那么启动一个线程不停的尝试！  为什么这么做?
         *  因为如果你在这里不停的重试直到成功，则把主线程给阻塞了。
         */
        fatalError(errorMessage);
    }

    /**
     * // TODO_MA 马中华 注释： 监视器的 Zookeeper 回调的接口实现（存在）
    ...
    public synchronized void processResult(int rc, String path, Object ctx, Stat stat) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "StatNode result: " + rc + " for path: " + path + " connectionState: " + zkConnectionState + " for " + this);
        }
     ...
     * 2、path 事件对应的节点路径
     * 3、event  事件
     * 万一： event.getEventType() = NodeDeleted 事件
     */
    synchronized void processWatchEvent(ZooKeeper zk, WatchedEvent event) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "Watcher event type: " + eventType + " with state:" + event.getState() + " for path:" + event.getPath() + " connectionState: " + zkConnectionState + " for " + this);
        }

        // TODO_MA 马中华 注释： 针对事件的 State 执行处理
        // TODO_MA 马中华 注释： 为了确保链接正常
        ...
        // TODO_MA 马中华 注释： 针对事件类型执行处理
        String path = event.getPath();
        if (path != null) {
            switch (eventType) {

                // TODO_MA 马中华 注释： 锁节点被删除
                case NodeDeleted:
    ...
    protected synchronized ZooKeeper connectToZooKeeper() throws IOException, KeeperException {
        ...
        // TODO_MA 马中华 注释： 绑定 监听器 和 ZK 客户端
    ...
    protected ZooKeeper createZooKeeper() throws IOException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： watcher = WatcherWithClientRef
         */
    ...
    private void joinElectionInternal() {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建锁节点 = /yarn-leader-election/yarn330ha/ActiveStandbyElectorLock
         */
        createLockNodeAsync();
    }
    ...
    private boolean becomeActive() {
        ...
        try {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 首先确保 old active 死掉
             *  总共有三种方式： FenceMethod 的三个子类
             *  1、SshFencedByTcpPort: 通过 fuser 命令来杀死 namenode 进程
             *  2、ShellCommandFencer： 通过配置一个脚本来运行杀死 namenode   power off  shultdown
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 发送 RPC 请求让 amNeNode 切换状态为 active
             *  1、appClient = hdfs
             *  2、appClient = YARN
             */
    ...
    private void writeBreadCrumbNode(Stat oldBreadcrumbStat) throws KeeperException, InterruptedException {
        ...
        if (oldBreadcrumbStat == null) {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 注意创建的是持久节点！意思是就算 old active 死掉了，这个znode节点也不会被删除，
             *  所以当 new active 上线的时候，可以找到 old active 是谁，然后去确保他死掉！
             */
        ...
        try {
            ...
            if (!Arrays.equals(data, appData)) {
                throw new IllegalStateException(
                        "We thought we were active, but in fact " + "the active znode had the wrong data: " + StringUtils.byteToHexString(
                                data) + " (stat=" + stat + ")");
            }
    ...
    private Stat fenceOldActive() throws InterruptedException, KeeperException {
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 从面包屑中，获取数据，看看是不是还是 old active 的
             */
    ...
    private void createLockNodeAsync() {
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建锁节点
         *  1、节点的类型，也就是锁节点，是临时节点，好处：zkfc 一死掉，则锁节点就 zk 自动删掉！
         *  2、注意倒数第二个参数；我此时是去创建一个 znode 节点，只要请求成功，则调用 this.processResult()
         *      1、创建节点成功
         *      2、创建节点失败 = 节点已经存在了  你去晚了
         *  zkLockFilePath = /yarn-leader-election/yarn330ha/ActiveStandbyElectorLock
         */
        zkClient.create(zkLockFilePath, appData, zkAcl, CreateMode.EPHEMERAL, this, zkClient);
        // TODO_MA 马中华 注释： 当我尝试创建 锁节点 成功了，则回到用 this.processReuslt() 尝试切换状态为 active
    }
    ...
    private String createWithRetries(final String path, final byte[] data, final List<ACL> acl,
                                     final CreateMode mode) throws InterruptedException, KeeperException {
    ...
    public String toString() {
        return "elector id=" + System.identityHashCode(
                this) + " appData=" + ((appData == null) ? "null" : StringUtils.byteToHexString(appData)
        ) + " cb=" + appClient;
    }
