...
public class RetryPolicies {
    ...
    public static final RetryPolicy failoverOnNetworkException(RetryPolicy fallbackPolicy, int maxFailovers, long delayMillis,
                                                               long maxDelayBase) {
        return new FailoverOnNetworkExceptionRetry(fallbackPolicy, maxFailovers, delayMillis, maxDelayBase);
    }

    public static final RetryPolicy failoverOnNetworkException(RetryPolicy fallbackPolicy, int maxFailovers, int maxRetries,
    ...
    public static class MultipleLinearRandomRetry implements RetryPolicy {
        ...
        public static MultipleLinearRandomRetry parseCommaSeparatedString(String s) {
            ...
            if (elements.length % 2 != 0) {
                LOG.warn(
                        "Illegal value: the number of elements in \"" + s + "\" is " + elements.length + " but an even number of elements is expected.");
                return null;
            }
        ...
        private static int parsePositiveInt(final String[] elements, final int i, final String originalString) {
            ...
            if (n <= 0) {
                LOG.warn(
                        "The value " + n + " <= 0: it is parsed from the string \"" + s + "\" which is the index " + i + " element in \"" + originalString + "\"");
                return -1;
            }
            return n;
        }
    ...
    static class FailoverOnNetworkExceptionRetry implements RetryPolicy {
        ...
        public FailoverOnNetworkExceptionRetry(RetryPolicy fallbackPolicy, int maxFailovers, int maxRetries, long delayMillis,
                                               long maxDelayBase) {
        ...
        public RetryAction shouldRetry(Exception e, int retries, int failovers, boolean isIdempotentOrAtMostOnce) throws Exception {
            ...
            if (e instanceof ConnectException || e instanceof EOFException || e instanceof NoRouteToHostException || e instanceof UnknownHostException || e instanceof StandbyException || e instanceof ConnectTimeoutException || shouldFailoverOnException(
                    e)) {
