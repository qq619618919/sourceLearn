...
public class NetworkTopology {
    ...
    public static NetworkTopology getInstance(Configuration conf, InnerNode.Factory factory) {
        NetworkTopology nt = ReflectionUtils.newInstance(
                conf.getClass(CommonConfigurationKeysPublic.NET_TOPOLOGY_IMPL_KEY, NetworkTopology.class,
    ...
    public void add(Node node) {
        ...
        try {
            ...
            if ((depthOfAllLeaves != -1) && (depthOfAllLeaves != newDepth)) {
                LOG.error("Error: can't add leaf node {} at depth {} to topology:{}\n", NodeBase.getPath(node), newDepth,
                        this
                );
                throw new InvalidTopologyException("Failed to add " + NodeBase.getPath(
    ...
    public Node getNode(String loc) {
        ...
        try {
            loc = NodeBase.normalize(loc);
            if (!NodeBase.ROOT.equals(loc)) loc = loc.substring(1);
    ...
    protected Node chooseRandom(final String scope, String excludedScope, final Collection<Node> excludedNodes) {
        ...
        if (numOfDatanodes <= 0) {
            LOG.debug("Failed to find datanode (scope=\"{}\" excludedScope=\"{}\")." + " numOfDatanodes={}", scope,
                    excludedScope, numOfDatanodes
            );
            return null;
        }
        ...
        if (excludedScope == null) {
            ...
            try {
                 ...
                 *  注释：
                 */
                availableNodes = countNumOfAvailableNodes(scope, excludedNodes) - countNumOfAvailableNodes(excludedScope,
                        excludedNodes
                );
            } finally {
                netlock.readLock().unlock();
        ...
        LOG.debug(
                "Choosing random from {} available nodes on node {}," + " scope={}, excludedScope={}, excludeNodes={}. numOfDatanodes={}.",
                availableNodes, innerNode, scope, excludedScope, excludedNodes, numOfDatanodes
        );
        Node ret = null;
        if (availableNodes > 0) {
     ...
     */
    private Node chooseRandom(final InnerNode parentNode, final Node excludedScopeNode, final Collection<Node> excludedNodes,
        ...
        if (ret == null && lastValidNode != null) {
            LOG.error(
                    "BUG: Found lastValidNode {} but not nth valid node. " + "parentNode={}, excludedScopeNode={}, excludedNodes={}, " + "totalInScopeNodes={}, availableNodes={}, nthValidToReturn={}.",
                    lastValidNode, parentNode, excludedScopeNode, excludedNodes, totalInScopeNodes, availableNodes,
                    nthValidToReturn
            );
            ret = lastValidNode;
        }
        ...
        try {
            if (excludedNodes != null) {
                for (Node node : excludedNodes) {
                    ...
                    if ((NodeBase.getPath(node) + NodeBase.PATH_SEPARATOR_STR).startsWith(
    ...
    private static String normalizeNetworkLocationPath(String path) {
        ...
        if (path.charAt(0) != PATH_SEPARATOR) {
            throw new IllegalArgumentException(
                    "Network Location" + "path doesn't start with " + PATH_SEPARATOR + ": " + path);
        }
     ...
     *     that point from outside to help sort the same distance.
     */
    public <T extends Node> void sortByDistanceUsingNetworkLocation(Node reader, T[] nodes, int activeLen,
                                                                    Consumer<List<T>> secondarySort) {
     ...
     */
    private <T extends Node> void sortByDistance(Node reader, T[] nodes, int activeLen, Consumer<List<T>> secondarySort,
                                                 boolean nonDataNodeReader) {
        /** Sort weights for the nodes array */
        TreeMap<Integer, List<T>> weightedNodeTree = new TreeMap<>();
        int nWeight;
        for (int i = 0; i < activeLen; i++) {
            ...
            weightedNodeTree.computeIfAbsent(nWeight, k -> new ArrayList<>(1)).add(nodes[i]);
        }
        int idx = 0;
