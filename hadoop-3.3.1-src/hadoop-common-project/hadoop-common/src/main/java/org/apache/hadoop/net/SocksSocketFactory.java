...
public class SocksSocketFactory extends SocketFactory implements Configurable {
    ...
    @Override
    public Socket createSocket(String host, int port, InetAddress localHostAddr,
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof SocksSocketFactory)) return false;
        final SocksSocketFactory other = (SocksSocketFactory) obj;
        if (proxy == null) {
            if (other.proxy != null) return false;
        } else if (!proxy.equals(other.proxy)) return false;
        return true;
    }
    ...
    private void setProxy(String proxyStr) {
        ...
        if (strs.length != 2) throw new RuntimeException("Bad SOCKS proxy parameter: " + proxyStr);
        String host = strs[0];
