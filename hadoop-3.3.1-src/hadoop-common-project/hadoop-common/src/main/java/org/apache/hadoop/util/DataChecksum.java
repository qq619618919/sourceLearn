...
public class DataChecksum implements Checksum {
    ...
    public enum Type {
        ...
        CRC32C(CHECKSUM_CRC32C, 4),    // TODO_MA 马中华 注释： 默认使用
    ...
    public static DataChecksum newDataChecksum(Type type, int bytesPerChecksum) {
        ...
        switch (type) {
            ...
            // TODO_MA 马中华 注释： 默认使用 java.util.zip.CRC32C
            case CRC32C:
    ...
    public static DataChecksum newDataChecksum(byte[] bytes, int offset) throws IOException {
        if (offset < 0 || bytes.length < offset + getChecksumHeaderSize()) {
            throw new InvalidChecksumSizeException(
                    "Could not create DataChecksum " + " from the byte array of length " + bytes.length + " and offset " + offset);
        }
        ...
        if (csum == null) {
            throw new InvalidChecksumSizeException(
                    ("Could not create DataChecksum " + " from the byte array of length " + bytes.length + " and bytesPerCheckSum of " + bytesPerChecksum));
        }
        return csum;
    }
    ...
    public static DataChecksum newDataChecksum(DataInputStream in) throws IOException {
        ...
        if (summer == null) {
            throw new InvalidChecksumSizeException(
                    "Could not create DataChecksum " + "of type " + type + " with bytesPerChecksum " + bpc);
        }
        return summer;
    }
    ...
    public void verifyChunkedSums(ByteBuffer data, ByteBuffer checksums, String fileName, long basePos) throws ChecksumException {
        ...
        if (data.hasArray() && checksums.hasArray()) {
            ...
            if (NativeCrc32.isAvailable()) {
                NativeCrc32.verifyChunkedSumsByteArray(bytesPerChecksum, type.id, checksums.array(), crcsOffset, data.array(),
                        dataOffset, data.remaining(), fileName, basePos
                );
            } else {
                verifyChunked(type, summer, data.array(), dataOffset, data.remaining(), bytesPerChecksum, checksums.array(),
                        crcsOffset, fileName, basePos
                );
            }
            return;
        }
    ...
    static void verifyChunked(final Type type, final Checksum algorithm, final ByteBuffer data, final int bytesPerCrc,
                              final ByteBuffer crcs, final String filename, final long basePos) throws ChecksumException {
     ...
     */
    static void verifyChunked(final Type type, final Checksum algorithm, final byte[] data, final int dataOffset,
                              final int dataLength, final int bytesPerCrc, final byte[] crcs, final int crcsOffset,
    ...
    private static void throwChecksumException(Type type, Checksum algorithm, String filename, long errPos, int expected,
                                               int computed) throws ChecksumException {
        throw new ChecksumException(
                "Checksum " + type + " not matched for file " + filename + " at position " + errPos + String.format(
                        ": expected=%X but computed=%X", expected, computed) + ", algorithm=" + algorithm.getClass()
                        .getSimpleName(), errPos);
    }
    ...
    public void calculateChunkedSums(ByteBuffer data, ByteBuffer checksums) {
        ...
        if (data.hasArray() && checksums.hasArray()) {
            calculateChunkedSums(data.array(), data.arrayOffset() + data.position(), data.remaining(), checksums.array(),
        ...
        static {
            ...
            try {
                newCRC32C = MethodHandles.publicLookup()
                        .findConstructor(Class.forName("java.util.zip.CRC32C"), MethodType.methodType(void.class));
