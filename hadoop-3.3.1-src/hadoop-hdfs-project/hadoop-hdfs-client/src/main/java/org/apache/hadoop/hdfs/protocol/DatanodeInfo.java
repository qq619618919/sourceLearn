...
public class DatanodeInfo extends DatanodeID implements Node {
    ...
    public enum AdminStates {
        // TODO_MA 马中华 注释： 正常服务中
        NORMAL("In Service"),
        // TODO_MA 马中华 注释： 退役进行中
        DECOMMISSION_INPROGRESS("Decommission In Progress"),
        // TODO_MA 马中华 注释： 已退役
        DECOMMISSIONED("Decommissioned"),
        // TODO_MA 马中华 注释： 进入维护中
    ...
    /** Constructor. */
    private DatanodeInfo(final String ipAddr, final String hostName, final String datanodeUuid, final int xferPort,
                         final int infoPort, final int infoSecurePort, final int ipcPort, final long capacity,
                         final long dfsUsed, final long nonDfsUsed, final long remaining, final long blockPoolUsed,
                         final long cacheCapacity, final long cacheUsed, final long lastUpdate,
                         final long lastUpdateMonotonic, final int xceiverCount, final String networkLocation,
                         final AdminStates adminState, final String upgradeDomain, final long lastBlockReportTime,
    ...
    public void setLastUpdate(long lastUpdate) {
        // TODO_MA 马中华 注释： 记录这个 datanode 的最近一次心跳时间！
        // TODO_MA 马中华 注释： 验活！
        // TODO_MA 马中华 注释： 标准： 连续10次心跳没有收到的话，就认为有问题了
        // TODO_MA 马中华 注释： 启动检查机制！
        // TODO_MA 马中华 注释： 每 5 分钟检查一次，最多检查两次
        // TODO_MA 马中华 注释： NN如何判断 DN死亡： 如果发现某个 dn 连续10次心跳没有被收到，并且启动了两次检查也没有检测到 datanode
        this.lastUpdate = lastUpdate;
    }
    ...
    public String getDatanodeReport() {
        ...
        buffer.append("Configured Capacity: ").append(c).append(" (").append(StringUtils.byteDesc(c)).append(")")
                .append("\n").append("DFS Used: ").append(u).append(" (").append(StringUtils.byteDesc(u)).append(")")
                .append("\n").append("Non DFS Used: ").append(nonDFSUsed).append(" (")
                .append(StringUtils.byteDesc(nonDFSUsed)).append(")").append("\n").append("DFS Remaining: ").append(r)
                .append(" (").append(StringUtils.byteDesc(r)).append(")").append("\n").append("DFS Used%: ")
                .append(percent2String(usedPercent)).append("\n").append("DFS Remaining%: ")
                .append(percent2String(remainingPercent)).append("\n").append("Configured Cache Capacity: ").append(cc)
                .append(" (").append(StringUtils.byteDesc(cc)).append(")").append("\n").append("Cache Used: ").append(cu)
                .append(" (").append(StringUtils.byteDesc(cu)).append(")").append("\n").append("Cache Remaining: ")
                .append(cr).append(" (").append(StringUtils.byteDesc(cr)).append(")").append("\n").append("Cache Used%: ")
                .append(percent2String(cacheUsedPercent)).append("\n").append("Cache Remaining%: ")
                .append(percent2String(cacheRemainingPercent)).append("\n").append("Xceivers: ").append(getXceiverCount())
                .append("\n").append("Last contact: ").append(new Date(lastUpdate)).append("\n")
                .append("Last Block Report: ").append(lastBlockReportTime != 0 ? new Date(lastBlockReportTime) : "Never")
    ...
    public String dumpDatanode() {
        ...
        buffer.append(" ").append(c).append("(").append(StringUtils.byteDesc(c)).append(")").append(" ").append(u)
                .append("(").append(StringUtils.byteDesc(u)).append(")").append(" ").append(percent2String(usedPercent))
                .append(" ").append(r).append("(").append(StringUtils.byteDesc(r)).append(")").append(" ").append(cc)
                .append("(").append(StringUtils.byteDesc(cc)).append(")").append(" ").append(cu).append("(")
                .append(StringUtils.byteDesc(cu)).append(")").append(" ").append(percent2String(cacheUsedPercent))
                .append(" ").append(cr).append("(").append(StringUtils.byteDesc(cr)).append(")").append(" ")
    ...
    public static class DatanodeInfoBuilder {
        ...
        public DatanodeInfo build() {
            return new DatanodeInfo(ipAddr, hostName, datanodeUuid, xferPort, infoPort, infoSecurePort, ipcPort, capacity,
                    dfsUsed, nonDfsUsed, remaining, blockPoolUsed, cacheCapacity, cacheUsed, lastUpdate, lastUpdateMonotonic,
                    xceiverCount, location, adminState, upgradeDomain, lastBlockReportTime, lastBlockReportMonotonic,
                    numBlocks
            );
        }
    }
