...
public class DFSClient implements java.io.Closeable, RemotePeerFactory, DataEncryptionKeyFactory, KeyProviderTokenIssuer {
     ...
     *  注释： Client 和 NameNode 的通信协议
     *  服务端中服务实例： NameNodeRpcServer implements ClientProtocol
     */
    ...
    public DFSClient(URI nameNodeUri, ClientProtocol rpcNamenode, Configuration conf,
        ...
        this.dtpReplaceDatanodeOnFailureReplication = (short) conf.getInt(
                HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.MIN_REPLICATION,
                HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.MIN_REPLICATION_DEFAULT
        );
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "Sets " + HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.MIN_REPLICATION + " to " + dtpReplaceDatanodeOnFailureReplication);
        }
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构建一个 DFSClient 的名字
         */
        this.clientName = "DFSClient_" + dfsClientConf.getTaskId() + "_" + ThreadLocalRandom.current()
                .nextInt() + "_" + Thread.currentThread().getId();
        int numResponseToDrop = conf.getInt(DFS_CLIENT_TEST_DROP_NAMENODE_RESPONSE_NUM_KEY,
                DFS_CLIENT_TEST_DROP_NAMENODE_RESPONSE_NUM_DEFAULT
        );

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 通信协议 ClientProtocol
         */
        ...
        if (numResponseToDrop > 0) {
            ...
            LOG.warn(
                    DFS_CLIENT_TEST_DROP_NAMENODE_RESPONSE_NUM_KEY + " is set to " + numResponseToDrop + ", this hacked client will proactively drop responses");
            proxyInfo = NameNodeProxiesClient.createProxyWithLossyRetryHandler(conf, nameNodeUri, ClientProtocol.class,
                    numResponseToDrop, nnFallbackToSimpleAuth
            );
        }

        // TODO_MA 马中华 注释： 如果存在，则直接获取
        if (proxyInfo != null) {
        ...
        if (LOG.isDebugEnabled() && 0 != localInterfaces.length) {
            LOG.debug("Using local interfaces [" + Joiner.on(',').join(localInterfaces) + "] with addresses [" + Joiner.on(',')
                    .join(localInterfaceAddrs) + "]");
        }

        Boolean readDropBehind = (conf.get(DFS_CLIENT_CACHE_DROP_BEHIND_READS) == null) ? null : conf.getBoolean(
        ...
        this.serverDefaultsValidityPeriod = conf.getTimeDuration(DFS_CLIENT_SERVER_DEFAULTS_VALIDITY_PERIOD_MS_KEY,
                DFS_CLIENT_SERVER_DEFAULTS_VALIDITY_PERIOD_MS_DEFAULT, TimeUnit.MILLISECONDS
        );
        ...
        if (clientRunning && !isFilesBeingWrittenEmpty()) {
            try {
                ...
                if (elapsed > dfsClientConf.getleaseHardLimitPeriod()) {
                    LOG.warn(
                            "Failed to renew lease for " + clientName + " for " + (elapsed / 1000) + " seconds (>= hard-limit =" + (dfsClientConf.getleaseHardLimitPeriod() / 1000) + " seconds.) " + "Closing all files being written ...",
                            e
                    );
        ...
        for (; ; ) {
            ...
            if (out != null) {
                try {
                    ...
                    LOG.error("Failed to " + (abort ? "abort" : "close") + " file: " + out.getSrc() + " with inode: " + inodeId,
                            ie
                    );
                }
            }
        ...
        private static ClientProtocol getNNProxy(Token<DelegationTokenIdentifier> token, Configuration conf) throws IOException {
            ...
            if (HAUtilClient.isTokenForLogicalUri(token) && !HAUtilClient.isLogicalUri(conf, uri)) {
                ...
                throw new IOException(
                        "Unable to map logical nameservice URI '" + uri + "' to a NameNode. Local configuration does not have " + "a failover proxy provider configured.");
            }

            ProxyAndInfo<ClientProtocol> info = NameNodeProxiesClient.createProxyWithClientProtocol(conf, uri, null);
            assert info.getDelegationTokenService()
                    .equals(token.getService()) : "Returned service '" + info.getDelegationTokenService()
                    .toString() + "' doesn't match expected service '" + token.getService().toString() + "'";
    ...
    public HdfsDataOutputStream createWrappedOutputStream(DFSOutputStream dfsos, FileSystem.Statistics statistics,
        ...
        if (feInfo != null) {
            ...
            try (TraceScope ignored = tracer.newScope("decryptEDEK")) {
                LOG.debug("Start decrypting EDEK for file: {}, output stream: 0x{}", dfsos.getSrc(),
                        Integer.toHexString(dfsos.hashCode())
                );
                decrypted = HdfsKMSUtil.decryptEncryptedDataEncryptionKey(feInfo, getKeyProvider());
                LOG.debug("Decrypted EDEK for file: {}, output stream: 0x{}", dfsos.getSrc(),
                        Integer.toHexString(dfsos.hashCode())
                );
            }
            final CryptoOutputStream cryptoOut = new CryptoOutputStream(dfsos, codec, decrypted.getMaterial(), feInfo.getIV(),
                    startPos
            );
    ...
    public OutputStream create(String src, boolean overwrite, short replication, long blockSize, Progressable progress,
        ...
        return create(src, FsPermission.getFileDefault(),
                overwrite ? EnumSet.of(CreateFlag.CREATE, CreateFlag.OVERWRITE) : EnumSet.of(CreateFlag.CREATE), replication,
    ...
    public DFSOutputStream create(String src, FsPermission permission, EnumSet<CreateFlag> flag, boolean createParent,
         ...
         *  注释：
         */
        return create(src, permission, flag, createParent, replication, blockSize, progress, buffersize, checksumOpt, favoredNodes,
                null
        );
    }
    ...
    public DFSOutputStream create(String src, FsPermission permission, EnumSet<CreateFlag> flag, boolean createParent,
                                  short replication, long blockSize, Progressable progress, int buffersize, ChecksumOpt checksumOpt,
                                  InetSocketAddress[] favoredNodes, String ecPolicyName) throws IOException {
    ...
    public DFSOutputStream create(String src, FsPermission permission, EnumSet<CreateFlag> flag, boolean createParent,
                                  short replication, long blockSize, Progressable progress, int buffersize, ChecksumOpt checksumOpt,
                                  InetSocketAddress[] favoredNodes, String ecPolicyName, String storagePolicy) throws IOException {
        checkOpen();
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： result = DFSOutputStream
         */
        final DFSOutputStream result = DFSOutputStream.newStreamForCreate(this, src, masked, flag, createParent, replication,
                blockSize, progress,
                // TODO_MA 马中华 注释： 注意这个参数
                dfsClientConf.createChecksum(checksumOpt), getFavoredNodesStr(favoredNodes), ecPolicyName, storagePolicy
        );
        ...
        // TODO_MA 马中华 注释： DFSOutputStream
        return result;
    }
     ...
     */
    public DFSOutputStream primitiveCreate(String src, FsPermission absPermission, EnumSet<CreateFlag> flag, boolean createParent,
                                           short replication, long blockSize, Progressable progress, int buffersize,
                                           ChecksumOpt checksumOpt) throws IOException {
        checkOpen();
        ...
        if (result == null) {
            ...
            result = DFSOutputStream.newStreamForCreate(this, src, absPermission, flag, createParent, replication, blockSize,
                    progress, checksum, null, null, null
            );
        }
        beginFileLease(result.getFileId(), result);
        return result;
    ...
    public void createSymlink(String target, String link, boolean createParent) throws IOException {
        ...
        try (TraceScope ignored = newPathTraceScope("createSymlink", target)) {
            ...
            throw re.unwrapRemoteException(AccessControlException.class, FileAlreadyExistsException.class,
                    ...
                    DSQuotaExceededException.class, QuotaByStorageTypeExceededException.class, UnresolvedPathException.class,
    ...
    private DFSOutputStream callAppend(String src, EnumSet<CreateFlag> flag, Progressable progress,
        ...
        try {
             ...
             *  注释：
             */
            return DFSOutputStream.newStreamForAppend(this, src, flag, progress, blkWithStatus.getLastBlock(), status,
            ...
            throw re.unwrapRemoteException(AccessControlException.class, FileNotFoundException.class, SafeModeException.class,
                    DSQuotaExceededException.class, QuotaByStorageTypeExceededException.class, UnsupportedOperationException.class,
    ...
    public boolean rename(String src, String dst) throws IOException {
        ...
        try (TraceScope ignored = newSrcDstTraceScope("rename", src, dst)) {
            ...
            throw re.unwrapRemoteException(AccessControlException.class, NSQuotaExceededException.class,
                    DSQuotaExceededException.class, QuotaByStorageTypeExceededException.class, UnresolvedPathException.class,
                    SnapshotAccessControlException.class, ParentNotDirectoryException.class
            );
        }
    }
    ...
    public void rename(String src, String dst, Options.Rename... options) throws IOException {
        ...
        try (TraceScope ignored = newSrcDstTraceScope("rename2", src, dst)) {
            ...
            throw re.unwrapRemoteException(AccessControlException.class, DSQuotaExceededException.class,
                    QuotaByStorageTypeExceededException.class, FileAlreadyExistsException.class, FileNotFoundException.class,
                    ParentNotDirectoryException.class, SafeModeException.class, NSQuotaExceededException.class,
                    UnresolvedPathException.class, SnapshotAccessControlException.class
            );
        }
    }
    ...
    public boolean delete(String src, boolean recursive) throws IOException {
        ...
        try (TraceScope ignored = newPathTraceScope("delete", src)) {
            ...
            throw re.unwrapRemoteException(AccessControlException.class, FileNotFoundException.class, SafeModeException.class,
                    UnresolvedPathException.class, SnapshotAccessControlException.class, PathIsNotEmptyDirectoryException.class
            );
        }
    }
    ...
    private FileChecksum getFileChecksumInternal(String src, long length, ChecksumCombineMode combineMode) throws IOException {
        ...
        maker = ecPolicy != null ? new FileChecksumHelper.StripedFileNonStripedChecksumComputer(src, length, blockLocations,
                namenode, this, ecPolicy, combineMode
        ) : new FileChecksumHelper.ReplicatedFileChecksumComputer(src, length, blockLocations, namenode, this, combineMode);
    ...
    protected IOStreamPair connectToDN(DatanodeInfo dn, int timeout, Token<BlockTokenIdentifier> blockToken) throws IOException {
        return DFSUtilClient.connectToDN(dn, timeout, conf, saslClient, socketFactory, getConf().isConnectToDnViaHostname(), this,
                blockToken
        );
    }
    ...
    protected Type inferChecksumTypeByReading(LocatedBlock lb, DatanodeInfo dn) throws IOException {
        ...
        try {
            new Sender((DataOutputStream) pair.out).readBlock(lb.getBlock(), lb.getBlockToken(), clientName, 0, 1, true,
     ...
     */
    public SnapshotDiffReportListing getSnapshotDiffReportListing(String snapshotDir, String fromSnapshot, String toSnapshot,
    ...
    public boolean primitiveMkdir(String src, FsPermission absPermission, boolean createParent) throws IOException {
        ...
        try (TraceScope ignored = tracer.newScope("mkdir")) {
            ...
            throw re.unwrapRemoteException(AccessControlException.class, InvalidPathException.class,
                    ...
                    QuotaByStorageTypeExceededException.class, UnresolvedPathException.class, SnapshotAccessControlException.class
            );
        }
    }
    ...
    QuotaUsage getQuotaUsage(String src) throws IOException {
        ...
        try (TraceScope ignored = newPathTraceScope("getQuotaUsage", src)) {
            ...
            IOException ioe = re.unwrapRemoteException(AccessControlException.class, FileNotFoundException.class,
                    UnresolvedPathException.class, RpcNoSuchMethodException.class
            );
            if (ioe instanceof RpcNoSuchMethodException) {
                LOG.debug(
                        "The version of namenode doesn't support getQuotaUsage API." + " Fall back to use getContentSummary API.");
