...
public class PacketHeader {
    ...
    // TODO_MA 马中华 注释： PKT_LENGTHS_LEN = 4+2 = 6b
    public static final int PKT_LENGTHS_LEN = Ints.BYTES + Shorts.BYTES;

    // TODO_MA 马中华 注释： MAX_PROTO_SIZE = 27b
    ...
    public PacketHeader(int packetLen, long offsetInBlock, long seqno, boolean lastPacketInBlock, int dataLen, boolean syncBlock) {
        ...
        PacketHeaderProto.Builder builder = PacketHeaderProto.newBuilder().setOffsetInBlock(offsetInBlock).setSeqno(seqno)
                .setLastPacketInBlock(lastPacketInBlock).setDataLen(dataLen);
