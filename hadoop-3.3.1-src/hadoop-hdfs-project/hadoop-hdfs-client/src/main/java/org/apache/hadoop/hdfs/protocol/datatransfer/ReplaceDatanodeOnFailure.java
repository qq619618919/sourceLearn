...
public class ReplaceDatanodeOnFailure {
     ...
     *   Add a new datanode only if r >= 3 and either (1) floor(r/2) >= n or (2) the block is hflushed/appended.
     */
    private static final Condition CONDITION_DEFAULT = new Condition() {
        @Override
        public boolean satisfy(final short replication, final DatanodeInfo[] existings, final int n, final boolean isAppend,
                               final boolean isHflushed) {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 副本数 >= 3 的情况下， 如果 pipline 中剩余 datanode 不足 一半
             */
    ...
    public void checkEnabled() {
        if (policy == Policy.DISABLE) {
            throw new UnsupportedOperationException(
                    "This feature is disabled.  Please refer to " + HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.ENABLE_KEY + " configuration property.");
        }
    }
    ...
    public static ReplaceDatanodeOnFailure get(final Configuration conf) {
        ...
        final boolean bestEffort = conf.getBoolean(HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.BEST_EFFORT_KEY,
    ...
    private static Policy getPolicy(final Configuration conf) {
        final boolean enabled = conf.getBoolean(HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.ENABLE_KEY,
        ...
        final String policy = conf.get(HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.POLICY_KEY,
        ...
        throw new HadoopIllegalArgumentException(
                "Illegal configuration value for " + HdfsClientConfigKeys.BlockWrite.ReplaceDatanodeOnFailure.POLICY_KEY + ": " + policy);
    }
