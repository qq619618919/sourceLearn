...
public class BlockReaderFactory implements ShortCircuitReplicaCreator {
    ...
    public BlockReader build() throws IOException {
        ...
        try {
            ...
            if (scConf.isDomainSocketDataTraffic()) {
                ...
                if (reader != null) {
                    LOG.trace("{}: returning new remote block reader using UNIX domain " + "socket on {}", this,
                            pathInfo.getPath()
                    );
                    return reader;
                }
        ...
        for (Class<? extends ReplicaAccessorBuilder> cls : clses) {
            try {
                ...
                ReplicaAccessor accessor = builder.setAllowShortCircuitReads(allowShortCircuitLocalReads)
                        .setBlock(block.getBlockId(), block.getBlockPoolId()).setGenerationStamp(block.getGenerationStamp())
                        .setBlockAccessToken(tokenBytes).setClientName(clientName).setConfiguration(configuration)
                        .setFileName(fileName).setVerifyChecksum(verifyChecksum).setVisibleLength(visibleLength).build();
    ...
    private BlockReader getLegacyBlockReaderLocal() throws IOException {
        ...
        if (!DFSUtilClient.isLocalAddress(inetSocketAddress)) {
            LOG.trace("{}: can't construct BlockReaderLocalLegacy because the address" + "{} is not local", this,
                    inetSocketAddress
            );
            return null;
        }
        if (clientContext.getDisableLegacyBlockReaderLocal()) {
            PerformanceAdvisory.LOG.debug(
                    "{}: can't construct " + "BlockReaderLocalLegacy because " + "disableLegacyBlockReaderLocal is set.", this);
            return null;
        }
        IOException ioe;
        try {
            return BlockReaderLocalLegacy.newBlockReader(conf, userGroupInformation, configuration, fileName, block, token,
    ...
    private BlockReader getBlockReaderLocal() throws IOException {
        ...
        if (!pathInfo.getPathState().getUsableForShortCircuit()) {
            PerformanceAdvisory.LOG.debug("{}: {} is not usable for short circuit; " + "giving up on BlockReaderLocal.", this,
                    pathInfo
            );
            return null;
        }
        ...
        if (exc != null) {
            LOG.trace("{}: got InvalidToken exception while trying to construct " + "BlockReaderLocal via {}", this,
                    pathInfo.getPath()
            );
            throw exc;
        }
        if (info.getReplica() == null) {
            PerformanceAdvisory.LOG.debug(
                    "{}: failed to get " + "ShortCircuitReplica. Cannot construct " + "BlockReaderLocal via {}", this,
                    pathInfo.getPath()
            );
            return null;
        }
        return new BlockReaderLocal.Builder(conf.getShortCircuitConf()).setFilename(fileName).setBlock(block)
                .setStartOffset(startOffset).setShortCircuitReplica(info.getReplica()).setVerifyChecksum(verifyChecksum)
    ...
    public ShortCircuitReplicaInfo createShortCircuitReplicaInfo() {
        ...
        while (true) {
            ...
            try {
                ...
                slot = cache.allocShmSlot(datanode, peer, usedPeer, new ExtendedBlockId(block.getBlockId(), block.getBlockPoolId()),
                        clientName
                );
                if (usedPeer.booleanValue()) {
                    LOG.trace("{}: allocShmSlot used up our previous socket {}.  " + "Allocating a new one...", this,
                ...
                if (curPeer.fromCache) {
                    ...
                    LOG.warn(
                            this + ": I/O error requesting file descriptors.  " + "Disabling domain socket " + peer.getDomainSocket(),
                            e
                    );
                    IOUtilsClient.cleanupWithLogger(LOG, peer);
        ...
        switch (resp.getStatus()) {
                ...
                try {
                    if (fis[0] == null || fis[1] == null) {
                        throw new IOException(
                                "the datanode " + datanode + " failed to " + "pass a file descriptor (might have reached open file limit).");
                    }
            ...
            case ERROR_UNSUPPORTED:
                if (!resp.hasShortCircuitAccessVersion()) {
                    LOG.warn(
                            "short-circuit read access is disabled for " + "DataNode " + datanode + ".  reason: " + resp.getMessage());
                    clientContext.getDomainSocketFactory().disableShortCircuitForPath(pathInfo.getPath());
                } else {
                    LOG.warn(
                            "short-circuit read access for the file " + fileName + " is disabled for DataNode " + datanode + ".  reason: " + resp.getMessage());
                }
                return null;
            ...
            default:
                ...
                LOG.warn(
                        "{}: unknown response code {} while attempting to set up " + "short-circuit access. {}. Short-circuit read for " + "DataNode {} is {} based on {}.",
                        this, resp.getStatus(), resp.getMessage(), datanode, disableMsg,
        ...
        while (true) {
            ...
            try {
                ...
                if (curPeer.fromCache) {
                    ...
                    // We temporarily disable the domain socket path for a few minutes in
                    // this case, to prevent wasting more time on it.
                    LOG.warn("I/O error constructing remote block reader.  Disabling " + "domain socket " + peer.getDomainSocket(),
                            ioe
                    );
                    clientContext.getDomainSocketFactory().disableDomainSocketPath(pathInfo.getPath());
                    return null;
    ...
    private BlockReader getRemoteBlockReader(Peer peer) throws IOException {
         ...
         *  注释：
         */
        return BlockReaderRemote.newBlockReader(fileName, block, token, startOffset, length, verifyChecksum, clientName, peer,
                datanode, clientContext.getPeerCache(), cachingStrategy, networkDistance, configuration
        );
    }
