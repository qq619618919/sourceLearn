...
public class BlockMetadataHeader {
    ...
    public static DataChecksum readDataChecksum(final DataInputStream metaIn, final Object name) throws IOException {
        ...
        if (header.getVersion() != VERSION) {
            LOG.warn(
                    "Unexpected meta-file version for " + name + ": version in file is " + header.getVersion() + " but expected version is " + VERSION);
        }
    ...
    public static BlockMetadataHeader readHeader(DataInputStream in) throws IOException {
        try {
            ...
            throw new CorruptMetaHeaderException(
                    "EOF while reading header from meta" + ". The meta file may be truncated or corrupt", eof);
        }
    }
