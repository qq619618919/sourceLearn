...
public class LeaseRenewer {
    ...
    @VisibleForTesting
    /** Is the daemon running? */ public synchronized boolean isRunning() {
        ...
        for (long lastRenewed = Time.monotonicNow(); !Thread.interrupted(); Thread.sleep(getSleepPeriod())) {
            ...
            if (elapsed >= getRenewalTime()) {
                try {
                    ...
                    // TODO_MA 马中华 注释： 更新时间记录
                    lastRenewed = Time.monotonicNow();
                } catch (SocketTimeoutException ie) {
                    LOG.warn(
                            "Failed to renew lease for " + clientsString() + " for " + (elapsed / 1000) + " seconds.  Aborting ...",
                            ie
                    );
                    ...
                    LOG.warn(
                            "Failed to renew lease for " + clientsString() + " for " + (elapsed / 1000) + " seconds.  Will retry shortly ...",
                            ie
                    );
                }
            }
            ...
            synchronized (this) {
                ...
                // registered with this renewer, stop the daemon after the grace period.
