 ...
 *  注释： 保存所有等待复制的数据块副本集合.它维护了一个优先级队列 priorityQueues。
...
class LowRedundancyBlocks implements Iterable<BlockInfo> {
    ...
    // TODO_MA 马中华 注释： 保存需要立刻备份的数据块。
    // TODO_MA 马中华 注释： 这个数据块只有一个副本， 或者这个数据块没有活着的副本并且有一个拷贝所在的Datanode正处于离线中。
    // TODO_MA 马中华 注释： 这种类型的数据块很有可能丢失
    /** The queue with the highest priority: {@value} */
    static final int QUEUE_HIGHEST_PRIORITY = 0;

    // TODO_MA 马中华 注释： 保存副本数极低的数据块， 当实际副本数与期望副本数的比例小于1:3时， 加入这个队列。
    /** The queue for blocks that are way below their expected value : {@value} */
    static final int QUEUE_VERY_LOW_REDUNDANCY = 1;
    /**
     * // TODO_MA 马中华 注释： 保存正处于备份中的数据块， 但该数据块的副本数并未达到优先级1队列中的比例
     * The queue for "normally" without sufficient redundancy blocks : {@value}.
     */
    static final int QUEUE_LOW_REDUNDANCY = 2;

    // TODO_MA 马中华 注释： 数据块的副本数量已经足够， 但是数据块副本的分布不是很好， 如果一个机架或者交换机宕机很有可能造成数据块完全丢失。
    ...
    // TODO_MA 马中华 注释： 保存已经损坏的数据块， 就是该数据块对应的所有副本都损坏了。
    // TODO_MA 马中华 注释： 这里的策略是将损坏的数据块放入这个队列中， 对没有损坏的副本给予更高的优先级
    /** The queue for corrupt blocks: {@value} */
    static final int QUEUE_WITH_CORRUPT_BLOCKS = 4;

    // TODO_MA 马中华 注释： 维护了 LEVEL 个队列
    ...
    private int getPriority(BlockInfo block, int curReplicas, int readOnlyReplicas, int outOfServiceReplicas, int expectedReplicas) {
        ...
        if (curReplicas >= expectedReplicas) {
            ...
            // TODO_MA 马中华 注释： 数据块的副本数量足够， 但是没有均匀地分布在不同的机架上， 返回优先级3
    ...
    private int getPriorityContiguous(int curReplicas, int readOnlyReplicas, int outOfServiceReplicas, int expectedReplicas) {
        if (curReplicas == 0) {
            // TODO_MA 马中华 注释： 当前数据块没有有效的副本， 并且有些副本所在的Datanode正处于离线中， 优先级 0
            ...
            // TODO_MA 马中华 注释： 当前数据块的所有副本都是损坏的， 优先级4
            //all we have are corrupt blocks
            return QUEUE_WITH_CORRUPT_BLOCKS;
        }

        // TODO_MA 马中华 注释： 只有一个有效副本， 数据块可能随时丢失， 优先级0
        ...
        // TODO_MA 马中华 注释： 当前副本数小于期望副本数的三分之一， 优先级1
    ...
    synchronized boolean add(BlockInfo block, int curReplicas, int readOnlyReplicas, int outOfServiceReplicas, int expectedReplicas) {
        ...
        if (add(block, priLevel, expectedReplicas)) {
            NameNode.blockStateChangeLog.debug(
                    "BLOCK* NameSystem.LowRedundancyBlock.add: {}" + " has only {} replicas and need {} replicas so is added to" + " neededReconstructions at priority level {}",
    ...
    boolean remove(BlockInfo block, int priLevel, int oldExpectedReplicas) {
        if (priLevel >= 0 && priLevel < LEVEL && priorityQueues.get(priLevel).remove(block)) {
            NameNode.blockStateChangeLog.debug("BLOCK* NameSystem.LowRedundancyBlock.remove: Removing block {}" + " from priority queue {}",
                    block, priLevel);
            ...
            for (int i = 0; i < LEVEL; i++) {
                if (i != priLevel && priorityQueues.get(i).remove(block)) {
                    NameNode.blockStateChangeLog.debug(
                            "BLOCK* NameSystem.LowRedundancyBlock.remove: Removing block" + " {} from priority queue {}", block, i);
     ...
     *        from before
     */
    synchronized void update(BlockInfo block, int curReplicas, int readOnlyReplicas, int outOfServiceReplicas, int curExpectedReplicas,
        ...
        if (NameNode.stateChangeLog.isDebugEnabled()) {
            NameNode.stateChangeLog.debug(
                    "LowRedundancyBlocks.update " + block + " curReplicas " + curReplicas + " curExpectedReplicas " + curExpectedReplicas + " oldReplicas " + oldReplicas + " oldExpectedReplicas  " + oldExpectedReplicas + " curPri  " + curPri + " oldPri  " + oldPri);
        }
        ...
        if (add(block, curPri, curExpectedReplicas)) {
            NameNode.blockStateChangeLog.debug(
                    "BLOCK* NameSystem.LowRedundancyBlock.update: {} has only {} " + "replicas and needs {} replicas so is added to " + "neededReconstructions at priority level {}",
