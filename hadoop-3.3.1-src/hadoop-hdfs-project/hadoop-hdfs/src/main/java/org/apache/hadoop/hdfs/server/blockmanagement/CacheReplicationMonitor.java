    ...
    public void run() {
        ...
        try {
            ...
            while (true) {
                ...
                LOG.debug("Scanned {} directive(s) and {} block(s) in {} millisecond(s).", scannedDirectives, scannedBlocks,
    ...
    private void rescanFile(CacheDirective directive, INodeFile file) {
        ...
        if (pool.getBytesNeeded() > pool.getLimit()) {
            LOG.debug(
                    ...
                    directive.getId(), file.getFullPathName(), pool.getPoolName(), pool.getBytesNeeded(), pool.getLimit()
            );
            return;
        }
            ...
            for (Iterator<CachedBlock> it = dn.getPendingCached().iterator(); it.hasNext(); ) {
                ...
                if (blockInfo == null) {
                    ...
                    LOG.debug(
                            ...
                            cblock.getBlockId(), dn.getDatanodeUuid()
                    );
                    continue;
                }
                if (blockInfo.getNumBytes() > remaining) {
                    LOG.debug(
                            "Block {}: removing from PENDING_CACHED for node {} " + "because it cannot fit in remaining cache size {}.",
        ...
        for (Iterator<CachedBlock> cbIter = cachedBlocks.iterator(); cbIter.hasNext(); ) {
            ...
            if (numCached >= neededCached) {
                ...
                for (Iterator<DatanodeDescriptor> iter = pendingCached.iterator(); iter.hasNext(); ) {
                    ...
                    iter.remove();
                    LOG.trace(
                            "Block {}: removing from PENDING_CACHED for node {} " + "because we already have {} cached replicas and we only" + " need {}",
            ...
            if (numCached < neededCached) {
                ...
                for (Iterator<DatanodeDescriptor> iter = pendingUncached.iterator(); iter.hasNext(); ) {
                    ...
                    iter.remove();
                    LOG.trace(
                            "Block {}: removing from PENDING_UNCACHED for node {} " + "because we only have {} cached replicas and we need " + "{}",
                            cblock.getBlockId(), datanode.getDatanodeUuid(), numCached, neededCached
                    );
                }
            }
            ...
            if ((neededCached == 0) && pendingUncached.isEmpty() && pendingCached.isEmpty()) {
                ...
                LOG.trace(
                        "Block {}: removing from cachedBlocks, since neededCached " + "== 0, and pendingUncached and pendingCached are empty.",
                        cblock.getBlockId()
                );
                cbIter.remove();
            }
