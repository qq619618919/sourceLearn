...
class JournaledEditsCache {
    ...
    JournaledEditsCache(Configuration conf) {
        ...
        if (capacity > 0.9 * Runtime.getRuntime().maxMemory()) {
            Journal.LOG.warn(String.format(
                    "Cache capacity is set at %d bytes but " + "maximum JVM memory is only %d bytes. It is recommended that you " + "decrease the cache size or increase the heap size.",
    ...
    int retrieveEdits(long requestedStartTxn, int maxTxns, List<ByteBuffer> outputBuffers) throws IOException {
        ...
        try (AutoCloseableLock l = readLock.acquire()) {
            ...
            Iterator<Map.Entry<Long, byte[]>> incrBuffIter = dataMap.tailMap(dataMap.floorKey(requestedStartTxn), true)
                    .entrySet().iterator();
    ...
    void storeEdits(byte[] inputData, long newStartTxn, long newEndTxn, int newLayoutVersion) {
        if (newStartTxn < 0 || newEndTxn < newStartTxn) {
            Journal.LOG.error(String.format("Attempted to cache data of length %d " + "with newStartTxn %d and newEndTxn %d",
        ...
        try (AutoCloseableLock l = writeLock.acquire()) {
            if (newLayoutVersion != layoutVersion) {
                try {
                    ...
                    Journal.LOG.error(String.format(
                            "Unable to save new edits [%d, %d] " + "due to exception when updating to new layout version %d",
                ...
                Journal.LOG.error(String.format(
                        "Edits cache is out of sync; " + "looked for next txn id at %d but got start txn id for " + "cache put request at %d. Reinitializing at new request.",
            ...
            if (inputData.length > capacity) {
                initialize(INVALID_TXN_ID);
                Journal.LOG.warn(String.format(
                        "A single batch of edits was too " + "large to fit into the cache: startTxn = %d, endTxn = %d, " + "input length = %d. The capacity of the cache (%s) must be " + "increased for it to work properly (current capacity %d)." + "Cache is now empty.",
                        newStartTxn, newEndTxn, inputData.length, DFSConfigKeys.DFS_JOURNALNODE_EDIT_CACHE_SIZE_KEY, capacity
                ));
                return;
            }
            if (dataMap.isEmpty()) {
    ...
    private void updateLayoutVersion(int newLayoutVersion, long newStartTxn) throws IOException {
        StringBuilder logMsg = new StringBuilder().append("Updating edits cache to use layout version ")
                .append(newLayoutVersion).append(" starting from txn ID ").append(newStartTxn);
    ...
    private CacheMissException getCacheMissException(long requestedTxnId) {
        if (lowestTxnId == INVALID_TXN_ID) {
            return new CacheMissException(0,
                    "Cache is empty; either it was never " + "written to or the last write overflowed the cache capacity."
            );
        } else if (requestedTxnId < initialTxnId) {
            return new CacheMissException(initialTxnId - requestedTxnId,
                    "Cache started at txn ID %d but requested txns starting at %d.", initialTxnId, requestedTxnId
            );
        } else {
            return new CacheMissException(lowestTxnId - requestedTxnId,
                    "Oldest txn ID available in the cache is %d, but requested txns " + "starting at %d. The cache size (%s) may need to be increased " + "to hold more transactions (currently %d bytes containing %d " + "transactions)",
                    lowestTxnId, requestedTxnId, DFSConfigKeys.DFS_JOURNALNODE_EDIT_CACHE_SIZE_KEY, capacity,
