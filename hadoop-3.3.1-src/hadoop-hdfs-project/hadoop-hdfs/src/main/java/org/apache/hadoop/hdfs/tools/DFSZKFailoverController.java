...
public class DFSZKFailoverController extends ZKFailoverController {
    ...
    protected HAServiceTarget dataToTarget(byte[] data) {
        ...
        if (!addressFromProtobuf.equals(ret.getAddress())) {
            throw new RuntimeException(
                    "Mismatched address stored in ZK for " + ret + ": Stored protobuf was " + proto + ", address from our own " + "configuration for this NameNode was " + ret.getAddress());
        }
    ...
    protected byte[] targetToData(HAServiceTarget target) {
        ...
        return ActiveNodeInfo.newBuilder().setHostname(addr.getHostName()).setPort(addr.getPort()).setZkfcPort(target.getZKFCAddress().getPort())
                .setNameserviceId(localNNTarget.getNameServiceId()).setNamenodeId(localNNTarget.getNameNodeId()).build().toByteArray();
    }
    ...
    public static void main(String args[]) throws Exception {
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 构建一个 ZKFC 实例对象： DFSZKFailoverController
             */
