...
class BPServiceActor implements Runnable {
    ...
    // TODO_MA 马中华 注释： 对应的 NameNode 的地址
    final InetSocketAddress nnAddr;

    // TODO_MA 马中华 注释： 对应的 NameNode 的状态
    ...
    BPServiceActor(String serviceId, String nnId, InetSocketAddress nnAddr, InetSocketAddress lifelineNnAddr,
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 文件数据块增量汇报管理器
         *  datanode 在运行过程中，由于 客户端会不断地上传数据过来，所以在一段时间里，这个 datanode 所维持的 block 会增多
         *  关于 数据块汇报有两种方式：
         *  1、增量汇报
         *  2、全量汇报
         */
        ...
        // TODO_MA 马中华 注释： 包装一个工具类，用来计算 心跳 或者 块汇报的时间
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 异步处理命令的 CommandProcessingThread。
         *  namenode 给 datanode 发送命令：
         *  1、首先namenode 要干什么的时候，首先是生成要执行的命令。
         *  2、当 datanode 过来心跳 和 数据快汇报的时候，就会返回要执行的命令给对应的 datanode
         *  这里启动的这个 CommandProcessingThread 就是用来去执行这些接收到的命令
         */
    ...
    private void checkNNVersion(NamespaceInfo nsInfo) throws IncorrectVersionException {
        ...
        // TODO_MA 马中华 注释： 如果 namenode 的版本和 datanode 的版本不匹配
        String dnVersion = VersionInfo.getVersion();
        if (!nnVersion.equals(dnVersion)) {
            LOG.info(
                    "Reported NameNode version '" + nnVersion + "' does not match " + "DataNode version '" + dnVersion + "' but is within acceptable " + "limits. Note: This is normal during a rolling upgrade.");
        }
    }
    ...
    private void connectToNNAndHandshake() throws IOException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 链接 NameNode，获取 NameNode 通信代理
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： DataNode 从 NameNode 处获取 NameSpace 的消息
         */
        ...
        // TODO_MA 马中华 注释： 确认 DataNode 本地的 BlockPool 是否存在，如果不存在，则进行初始化
        // TODO_MA 马中华 注释： 重点操作：初始化本地 BlockPool
    ...
    List<DatanodeCommand> blockReport(long fullBrLeaseId) throws IOException {
        ...
        // or we will report an RBW replica after the BlockReport already reports a FINALIZED one.
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 发送增量块汇报
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 存储目录的集合拿到， Volume = 文件夹
         *  生成一个全量汇报 报告单！  FLink slot 的状态汇报： SlotReport
         */
        ...
        boolean useBlocksBuffer = bpRegistration.getNamespaceInfo()
                .isCapabilitySupported(NamespaceInfo.Capability.STORAGE_BLOCK_REPORT_BUFFERS);
        blockReportSizes.clear();
        try {

            // TODO_MA 马中华 注释： 如果总数据块小于这个阈值，则发送全量块汇报
            if (totalBlockCount < dnConf.blockReportSplitThreshold) {
                // Below split threshold, send all reports in a single message.

                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： DataNode 给 NameNode 发送全量块汇报
                 *  一个 datanode 管理的数据块的个数多不多？
                 *  128M  1G = 8个块
                 *  1T = 1024G = 1024 * 8 = 8100
                 *  挂在了 10T 磁盘，但是装满 50% = 4 * 8100 = 44000 左右数据块
                 *  -
                 *  由于 全量汇报，携带的信息有点多，频繁做汇报，会给 namenode 造成极大压力
                 *  每 6 个小时做一次！
                 */
            ...
            LOG.info((success ? "S" : "Uns") + "uccessfully sent block report 0x" + Long.toHexString(
                    reportId) + " to namenode: " + nnAddr + ",  containing " + reports.length + " storage report(s), of which we sent " + numReportsSent + "." + " The reports had " + totalBlockCount + " total blocks and used " + numRPCs + " RPC(s). This took " + brCreateCost + " msecs to generate and " + brSendCost + " msecs for RPC and NN processing." + " Got back " + ((nCmds == 0) ? "no commands" : ((nCmds == 1) ? "one command: " + cmds.get(
                    0) : (nCmds + " commands: " + Joiner.on("; ").join(cmds))
            )
            ) + ".");
        }

        // TODO_MA 马中华 注释： 记录刚刚块汇报的时间
        scheduler.updateLastBlockReportTime(monotonicNow());

        // TODO_MA 马中华 注释： 计算得到下一个块汇报的时间
    ...
    DatanodeCommand cacheReport() throws IOException {
        ...
        if (startTime - lastCacheReport > dnConf.cacheReportInterval) {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "CacheReport of " + blockIds.size() + " block(s) took " + createCost + " msecs to generate and " + sendCost + " msecs for RPC and NN processing");
            }
        }
        return cmd;
    }
    ...
    HeartbeatResponse sendHeartBeat(boolean requestBlockReportLease) throws IOException {
        ...
        // TODO_MA 马中华 注释： 计算得到下一次心跳时间
        scheduler.scheduleNextHeartbeat();

        // TODO_MA 马中华 注释： 获取存储报告
        ...
        final SlowPeerReports slowPeers = outliersReportDue && dn.getPeerMetrics() != null ? SlowPeerReports.create(
                dn.getPeerMetrics().getOutliers()) : SlowPeerReports.EMPTY_REPORT;
        final SlowDiskReports slowDisks = outliersReportDue && dn.getDiskMetrics() != null ? SlowDiskReports.create(
                dn.getDiskMetrics().getDiskOutliersStats()) : SlowDiskReports.EMPTY_REPORT;

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： DataNode 的 BPServiceActor 给 NameNode 发送心跳
         */
        HeartbeatResponse response = bpNamenode.sendHeartbeat(bpRegistration, reports, dn.getFSDataset().getCacheCapacity(),
                dn.getFSDataset().getCacheUsed(), dn.getXmitsInProgress(), dn.getActiveTransferThreadCount(),
    ...
    private void handleRollingUpgradeStatus(HeartbeatResponse resp) throws IOException {
        ...
        if (rollingUpgradeStatus != null && rollingUpgradeStatus.getBlockPoolId().compareTo(bpos.getBlockPoolId()) != 0) {
            // Can this ever occur?
            LOG.error(
                    "Invalid BlockPoolId " + rollingUpgradeStatus.getBlockPoolId() + " in HeartbeatResponse. Expected " + bpos.getBlockPoolId());
        } else {
    ...
    private void offerService() throws Exception {
        LOG.info(
                "For namenode " + nnAddr + " using" + " BLOCKREPORT_INTERVAL of " + dnConf.blockReportInterval + "msecs" + " CACHEREPORT_INTERVAL of " + dnConf.cacheReportInterval + "msecs" + " Initial delay: " + dnConf.initialBlockReportDelayMs + "msecs" + "; heartBeatInterval=" + dnConf.heartBeatInterval + (lifelineSender != null ? "; lifelineIntervalMs=" + dnConf.getLifelineIntervalMs() : ""));
        ...
        while (shouldRun()) {
            try {
                ...
                // TODO_MA 马中华 注释： 计算是否该心跳了
                final boolean sendHeartbeat = scheduler.isHeartbeatDue(startTime);

                // TODO_MA 马中华 注释： 如果该心跳了，则发送心跳 RPC 请求给 NameNode
                ...
                if (sendHeartbeat) {
                    ...
                    if (!dn.areHeartbeatsDisabledForTests()) {
                        ...
                        // TODO_MA 马中华 注释： 处理心跳响应
                        assert resp != null;
                        if (resp.getFullBlockReportLeaseId() != 0) {
                            if (fullBlockReportLeaseId != 0) {
                                LOG.warn(nnAddr + " sent back a full block report lease " + "ID of 0x" + Long.toHexString(
                                        resp.getFullBlockReportLeaseId()) + ", but we already have a lease ID of 0x" + Long.toHexString(
                                        fullBlockReportLeaseId) + ". " + "Overwriting old lease ID.");
                            }
                        ...
                        /*************************************************
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 将心跳得到的 namenode 的命令加入到 待处理队列中
                         */
                ...
                // TODO_MA 马中华 注释： 将块汇报接收到的响应中的 来自于 namenode 的命令，加入到处理线程中的队列中
                ...
                // TODO_MA 马中华 注释： 没有工作要做；睡眠直到心跳计时器结束或工作到达，然后再次迭代。
                ibrManager.waitTillNextIBR(scheduler.getHeartbeatWaitTime());

            } catch (RemoteException re) {
                String reClass = re.getClassName();
                if (UnregisteredNodeException.class.getName().equals(reClass) || DisallowedDatanodeException.class.getName()
                        .equals(reClass) || IncorrectVersionException.class.getName().equals(reClass)) {
    ...
    void register(NamespaceInfo nsInfo) throws IOException {
        ...
        // TODO_MA 马中华 注释： 创建注册对象
        DatanodeRegistration newBpRegistration = bpos.createRegistration();

        LOG.info(this + " beginning handshake with NN");

        // TODO_MA 马中华 注释： 如果注册报错，则进行 1s 重试
        while (shouldRun()) {
            try {
                // Use returned registration from namenode with updated fields
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： bpNamenode 这个就是通信协议的代理对象
                 *  DataNode 和 NameNode 之间的通信协议
                 */
                newBpRegistration = bpNamenode.registerDatanode(newBpRegistration);
                // TODO_MA 马中华 注释： 这个方法的底层，就是要发送一个网络请求给 NameNode
                // TODO_MA 马中华 注释： bpNamenode 的初始化
                // TODO_MA 马中华 注释： 这个方法的调用的底层实现

                newBpRegistration.setNamespaceInfo(nsInfo);

                // TODO_MA 马中华 注释： 记录注册响应
            ...
            // TODO_MA 马中华 注释： 如果注册报错了，会有重试。 每隔一秒钟，重试一次
            // Try again in a second
    ...
    public void run() {
        ...
        try {
            ...
            // TODO_MA 马中华 注释： 重试到链接 namenode 成功为止
            while (true) {
                // init stuff
                try {
                    // setup storage
            ...
            // TODO_MA 马中华 注释： 核心工作
            // TODO_MA 马中华 注释： 除非这个 datanode 一直正常运行，否则就退出了这个 while
            while (shouldRun()) {
                try {
                    /*************************************************
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： DataNode 执行向 NameNode 的心跳和块汇报
                     *  1、心跳
                     *  2、数据块汇报
                     */
                    offerService();
    ...
    private final class LifelineSender implements Runnable, Closeable {
        ...
        private void sendLifelineIfDue() throws IOException {
            ...
            if (dn.areHeartbeatsDisabledForTests()) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "Skipping sending lifeline for " + BPServiceActor.this + ", because heartbeats are disabled for tests.");
                }
                return;
            }
            sendLifeline();
        ...
        private void sendLifeline() throws IOException {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "Sending lifeline with " + reports.length + " storage " + " reports from service actor: " + BPServiceActor.this);
            }
            VolumeFailureSummary volumeFailureSummary = dn.getFSDataset().getVolumeFailureSummary();
            int numFailedVolumes = volumeFailureSummary != null ? volumeFailureSummary.getFailedStorageLocations().length : 0;
            lifelineNamenode.sendLifeline(bpRegistration, reports, dn.getFSDataset().getCacheCapacity(),
    ...
    /**
     * // TODO_MA 马中华 注释： 包装用于调度心跳和块报告的时间戳计算的实用程序类。
     * Utility class that wraps the timestamp computations for scheduling heartbeats and block reports.
     */
    static class Scheduler {
        ...
        // TODO_MA 马中华 注释： 第一个参数： 心跳时间间隔：dfs.heartbeat.interval = 3s
        // TODO_MA 马中华 注释： 第三个参数： dfs.blockreport.intervalMsec = 6 * 60 * 60 * 1000 = 6h
        Scheduler(long heartbeatIntervalMs, long lifelineIntervalMs, long blockReportIntervalMs,
        ...
        long scheduleNextHeartbeat() {
            ...
            nextHeartbeatTime = monotonicNow() + heartbeatIntervalMs;  // TODO_MA 马中华 注释： 3s
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 计算下一次心跳时间
             */
            ...
            if (cmds != null) {
                ...
                for (DatanodeCommand cmd : cmds) {
                    try {
                        ...
                        String reClass = re.getClassName();
                        if (UnregisteredNodeException.class.getName()
                                .equals(reClass) || DisallowedDatanodeException.class.getName()
                                .equals(reClass) || IncorrectVersionException.class.getName().equals(reClass)) {
