...
/**
 * // TODO_MA 马中华 注释： 包含从给定路径解析的 INode 信息。
...
public class INodesInPath {
            ...
            if (!isRef && isDir && dir.isWithSnapshot()) {
                ...
                if (!isSnapshot) {
                    ...
                    if (snapshotId == CURRENT_STATE_ID || // no snapshot in dst tree of rename
                            (dstSnapshotId != CURRENT_STATE_ID && dstSnapshotId >= snapshotId)) { // the above scenario
    ...
    private static boolean shouldUpdateLatestId(int sid, int snapshotId) {
        return snapshotId == CURRENT_STATE_ID || (sid != CURRENT_STATE_ID && ID_INTEGER_COMPARATOR.compare(snapshotId,
                sid
        ) < 0
        );
    }
    ...
    private String toString(boolean vaildateObject) {
        ...
        final StringBuilder b = new StringBuilder(getClass().getSimpleName()).append(": path = ").append(getPath())
                .append("\n  inodes = ");
        if (inodes == null) {
            b.append("null");
