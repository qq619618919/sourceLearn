...
/**
 * // TODO_MA 马中华 注释： 用于存储副本的基础卷。
 * // TODO_MA 马中华 注释： 管理 Datanode 一个存储目录下的所有数据块
 * // TODO_MA 马中华 注释： 由于一个存储目录可以存储多个块池的数据块，
 * // TODO_MA 马中华 注释： 所以 FsVolumelmpl 会持有这个存储目录中保存的所有块池的 BlockPoolSlice 对象。
 *
...
public class FsVolumeImpl implements FsVolumeSpi {
     ...
     * dfs.datanode.fsdatasetcache.max.threads.per.volume) to limit resource contention.
     */
    protected ThreadPoolExecutor cacheExecutor;

    FsVolumeImpl(FsDatasetImpl dataset, String storageID, StorageDirectory sd, FileIoProvider fileIoProvider,
                 Configuration conf) throws IOException {
        // outside tests, usage created in ReservedSpaceCalculator.Builder
        this(dataset, storageID, sd, fileIoProvider, conf, null);
    }

    FsVolumeImpl(FsDatasetImpl dataset, String storageID, StorageDirectory sd, FileIoProvider fileIoProvider, Configuration conf,
                 DF usage) throws IOException {
        ...
        if (this.usage != null) {
            ...
            boolean fixedSizeVolume = conf.getBoolean(DFSConfigKeys.DFS_DATANODE_FIXED_VOLUME_SIZE_KEY,
    ...
    protected ThreadPoolExecutor initializeCacheExecutor(File parent) {
        ...
        final int maxNumThreads = dataset.datanode.getConf()
                .getInt(DFSConfigKeys.DFS_DATANODE_FSDATASETCACHE_MAX_THREADS_PER_VOLUME_KEY,
        ...
        ThreadFactory workerFactory = new ThreadFactoryBuilder().setDaemon(true)
                .setNameFormat("FsVolumeImplWorker-" + escapedPath + "-%d").build();
        ...
        if (FsDatasetImpl.LOG.isDebugEnabled()) {
            if (reference.getReferenceCount() <= 0) {
                FsDatasetImpl.LOG.debug(
                        "Decrease reference count <= 0 on " + this + Joiner.on("\n").join(Thread.currentThread().getStackTrace()));
            }
        }
        checkReference();
    ...
    private class BlockIteratorImpl implements FsVolumeSpi.BlockIterator {
        ...
        private String getNextSubDir(String prev, File dir) throws IOException {
            ...
            LOG.trace("getNextSubDir({}, {}): picking next subdirectory {} within {}", storageID, bpid, nextSubDir,
                    dir.getAbsolutePath()
            );
            return nextSubDir;
        }
        ...
        private List<String> getSubdirEntries() throws IOException {
            ...
            if (cache != null) {
                ...
                if (delta < maxStalenessMs) {
                    ...
                    LOG.trace("getSubdirEntries({}, {}): purging entries cache for {} " + "after {} ms.", storageID, bpid,
                            state.curFinalizedSubDir, delta
                    );
                    cache = null;
                }
            }
            File dir = Paths.get(bpidDir.getAbsolutePath(), "current", "finalized", state.curFinalizedDir, state.curFinalizedSubDir)
                    .toFile();
                ...
                while (true) {
                    ...
                    if (entries != null) {
                        ...
                        if (state.curEntry == null) {
                            LOG.trace("nextBlock({}, {}): advancing from {} to next " + "subdirectory.", storageID, bpid,
                            ...
                            if (!expectedBlockDir.equals(actualBlockDir)) {
                                LOG.error(
                                        ...
                                        storageID, bpid, block.getBlockId(), expectedBlockDir.getPath(), actualBlockDir.getPath()
                                );
                                continue;
                            }
        ...
        private File getBlockFile(String bpid, ExtendedBlock blk) throws IOException {
            return new File(
                    DatanodeUtil.idToBlockDir(getFinalizedDir(bpid), blk.getBlockId()).toString() + "/" + blk.getBlockName());
        }
        ...
        public void save() throws IOException {
            ...
            try (BufferedWriter writer = new BufferedWriter(
                    new OutputStreamWriter(fileIoProvider.getFileOutputStream(FsVolumeImpl.this, getTempSaveFile()), "UTF-8"))) {
            ...
            fileIoProvider.move(FsVolumeImpl.this, getTempSaveFile().toPath(), getSaveFile().toPath(),
    ...
    ReplicaInfo addFinalizedBlock(String bpid, Block b, ReplicaInfo replicaInfo, long bytesReserved) throws IOException {
        ...
        return new ReplicaBuilder(ReplicaState.FINALIZED).setBlock(replicaInfo).setFsVolume(this)
                .setDirectoryToUse(dest.getParentFile()).setLastPartialChunkChecksum(checksum).build();
    }
    ...
    void addBlockPool(String bpid, Configuration c, Timer timer) throws IOException {
        ...
        // TODO_MA 马中华 注释： current 目录下创建 bp 快池文件夹
    ...
    void deleteBPDirectories(String bpid, boolean force) throws IOException {
        ...
        if (force) {
            ...
            if (!DatanodeUtil.dirNoFilesRecursive(this, finalizedDir, fileIoProvider) || !fileIoProvider.fullyDelete(this,
                    finalizedDir
            )) {
                throw new IOException("Failed to delete " + finalizedDir);
            }
            if (lazypersistDir.exists() && ((!DatanodeUtil.dirNoFilesRecursive(this, lazypersistDir,
                    fileIoProvider
    ...
    public ReplicaInPipeline append(String bpid, ReplicaInfo replicaInfo, long newGS, long estimateBlockLen) throws IOException {
        ...
        LocalReplicaInPipeline newReplicaInfo = new ReplicaBuilder(ReplicaState.RBW).setBlockId(replicaInfo.getBlockId())
                .setLength(replicaInfo.getNumBytes()).setGenerationStamp(newGS).setFsVolume(this)
                .setDirectoryToUse(newBlkFile.getParentFile()).setWriterThread(Thread.currentThread())
    ...
    public ReplicaInPipeline createRbw(ExtendedBlock b) throws IOException {
        ...
        LocalReplicaInPipeline newReplicaInfo = new ReplicaBuilder(ReplicaState.RBW).setBlockId(b.getBlockId())
                .setGenerationStamp(b.getGenerationStamp()).setFsVolume(this).setDirectoryToUse(f.getParentFile())
                .setBytesToReserve(b.getNumBytes()).buildLocalReplicaInPipeline();
        return newReplicaInfo;
    }
    ...
    public ReplicaInPipeline convertTemporaryToRbw(ExtendedBlock b, ReplicaInfo temp) throws IOException {
        ...
        // create RBW
        final LocalReplicaInPipeline rbw = new ReplicaBuilder(ReplicaState.RBW).setBlockId(blockId).setLength(numBytes)
                .setGenerationStamp(expectedGs).setFsVolume(this).setDirectoryToUse(dest.getParentFile())
                .setWriterThread(Thread.currentThread()).setBytesToReserve(0).buildLocalReplicaInPipeline();
    ...
    public ReplicaInPipeline createTemporary(ExtendedBlock b) throws IOException {
        ...
        LocalReplicaInPipeline newReplicaInfo = new ReplicaBuilder(ReplicaState.TEMPORARY).setBlockId(b.getBlockId())
                .setGenerationStamp(b.getGenerationStamp()).setDirectoryToUse(f.getParentFile())
                .setBytesToReserve(b.getLocalBlock().getNumBytes()).setFsVolume(this).buildLocalReplicaInPipeline();
        return newReplicaInfo;
    }

    public ReplicaInPipeline updateRURCopyOnTruncate(ReplicaInfo rur, String bpid, long newBlockId, long recoveryId,
        ...
        LocalReplicaInPipeline newReplicaInfo = new ReplicaBuilder(ReplicaState.RBW).setBlockId(newBlockId)
                .setGenerationStamp(recoveryId).setFsVolume(this).setDirectoryToUse(blockFile.getParentFile())
                .setBytesToReserve(newlength).buildLocalReplicaInPipeline();
    ...
    private File[] copyReplicaWithNewBlockIdAndGS(ReplicaInfo replicaInfo, String bpid, long newBlkId,
        ...
        return FsDatasetImpl.copyBlockFiles(replicaInfo, dstMetaFile, dstBlockFile, true, DFSUtilClient.getSmallBufferSize(conf),
                conf
        );
    }
    ...
    public enum BlockDirFilter implements FilenameFilter {
        ...
        @Override
        public boolean accept(File dir, String name) {
            return name.startsWith(DataStorage.BLOCK_SUBDIR_PREFIX) || name.startsWith(
                    DataStorage.STORAGE_DIR_FINALIZED) || name.startsWith(Block.BLOCK_FILE_PREFIX);
        }
    }

    private void compileReport(File bpFinalizedDir, File dir, Collection<ScanInfo> report,
                               ReportCompiler reportCompiler) throws InterruptedException {
    ...
    private void verifyFileLocation(File actualBlockFile, File bpFinalizedDir, long blockId) {
        ...
        if (actualBlockDir.compareTo(expectedBlockDir) != 0) {
            LOG.warn(
                    "Block: " + blockId + " found in invalid directory.  Expected directory: " + expectedBlockDir + ".  Actual directory: " + actualBlockDir);
        }
    }

    public ReplicaInfo moveBlockToTmpLocation(ExtendedBlock block, ReplicaInfo replicaInfo, int smallBufferSize,
                                              Configuration conf) throws IOException {

        File[] blockFiles = FsDatasetImpl.copyBlockFiles(block.getBlockId(), block.getGenerationStamp(), replicaInfo,
                getTmpDir(block.getBlockPoolId()), replicaInfo.isOnTransientStorage(), smallBufferSize, conf
        );

        ReplicaInfo newReplicaInfo = new ReplicaBuilder(ReplicaState.TEMPORARY).setBlockId(replicaInfo.getBlockId())
                .setGenerationStamp(replicaInfo.getGenerationStamp()).setFsVolume(this)
                .setDirectoryToUse(blockFiles[0].getParentFile()).setBytesToReserve(0).build();
        newReplicaInfo.setNumBytes(blockFiles[1].length());
        return newReplicaInfo;
    }

    public File[] copyBlockToLazyPersistLocation(String bpId, long blockId, long genStamp, ReplicaInfo replicaInfo,
        ...
        File[] targetFiles = FsDatasetImpl.copyBlockFiles(blockId, genStamp, replicaInfo, lazyPersistDir, true, smallBufferSize,
                conf
        );
        return targetFiles;
    }
    ...
    public void resolveDuplicateReplicas(String bpid, ReplicaInfo memBlockInfo, ReplicaInfo diskBlockInfo,
