...
public class JournalSet implements JournalManager {
    ...
    private static final Comparator<EditLogInputStream> LOCAL_LOG_PREFERENCE_COMPARATOR = Comparator.comparing(
            EditLogInputStream::isLocalLog).reversed();

    public static final Comparator<EditLogInputStream> EDIT_LOG_INPUT_STREAM_COMPARATOR = Comparator.comparing(
            EditLogInputStream::getFirstTxId).thenComparing(EditLogInputStream::getLastTxId);
    ...
    static class JournalAndStream implements CheckableNameNodeResource {
        ...
        public void startLogSegment(long txId, int layoutVersion) throws IOException {
             ...
             *  注释： 生成 inprogress editlog 文件的 EditLogFileOutputStream 输出流
             *  1、对于本地来说，对于 FileJournalManager 来说，创建的 Stream = EditLogFileOutputStream
             *  2、对于本地来说，对于 QuorumJournalManager 来说，创建的 Stream = QuorumOutputStream
             */
    ...
    public EditLogOutputStream startLogSegment(final long txId, final int layoutVersion) throws IOException {
        ...
        // TODO_MA 马中华 注释： 遍历 JournalSet 中的每个 JournalAndStream 生成 EditLogFileOutputStream
        ...
        // TODO_MA 马中华 注释： 返回的是 JournalSetOutputStream
    ...
    public void finalizeLogSegment(final long firstTxId, final long lastTxId) throws IOException {
        ...
        // TODO_MA 马中华 注释： 遍历 JournalSet 中的每个 JournalAndStream 生成 输出流
        mapJournalsAndReportErrors(new JournalClosure() {
            @Override
            public void apply(JournalAndStream jas) throws IOException {
                if (jas.isActive()) {
    ...
    public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxId, boolean inProgressOk,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 遍历不同的 JournalManager 生成 InputStream
         */
            ...
            if (acc.isEmpty()) {
                ...
                if (accFirstTxId == elis.getFirstTxId()) {
                    ...
                    throw new RuntimeException(
                            "sorted set invariants violated!  " + "Got stream with first txid " + elis.getFirstTxId() + ", but the last firstTxId was " + accFirstTxId);
                }
            }
        }
        if (!acc.isEmpty()) {
    ...
    private void mapJournalsAndReportErrors(JournalClosure closure, String status) throws IOException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 遍历 JournalSet 中的每个 JournalAndStream 生成 输出流
         *  1、FileJournalManager -> 把数据写到本地的文件系统
         *  2、QuorumJounalManager -> 把数据写到journalnode里面
         */
        for (JournalAndStream jas : journals) {
            try {
                closure.apply(jas);
            ...
            mapJournalsAndReportErrors(new JournalClosure() {
                ...
                public void apply(JournalAndStream jas) throws IOException {
                    if (jas.isActive()) {
                        /*************************************************
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： jas 中的成员 JournalManger 有两种类型:
                         *  1、对于本地来说，对于 FileJournalManager 来说，创建的 Stream = EditLogFileOutputStream
