...
public class CheckpointConf {
    ...
    public CheckpointConf(Configuration conf) {
        ...
        // TODO_MA 马中华 注释： dfs.namenode.checkpoint.check.period = 60s
        checkpointCheckPeriod = conf.getTimeDuration(DFS_NAMENODE_CHECKPOINT_CHECK_PERIOD_KEY, DFS_NAMENODE_CHECKPOINT_CHECK_PERIOD_DEFAULT,
                TimeUnit.SECONDS);

        // TODO_MA 马中华 注释： dfs.namenode.checkpoint.period = 3600s
    ...
    private static void warnForDeprecatedConfigs(Configuration conf) {
        for (String key : ImmutableList.of("fs.checkpoint.size", "dfs.namenode.checkpoint.size")) {
            if (conf.get(key) != null) {
                LOG.warn(
                        "Configuration key " + key + " is deprecated! Ignoring..." + " Instead please specify a value for " + DFS_NAMENODE_CHECKPOINT_TXNS_KEY);
            }
        }
    }
    ...
    public long getCheckPeriod() {
        // TODO_MA 马中华 注释： checkpointCheckPeriod = dfs.namenode.checkpoint.check.period = 60s
        // TODO_MA 马中华 注释： checkpointPeriod = dfs.namenode.checkpoint.period = 3600s
