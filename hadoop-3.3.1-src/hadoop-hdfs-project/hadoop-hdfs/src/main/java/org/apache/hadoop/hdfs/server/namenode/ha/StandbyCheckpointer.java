...
public class StandbyCheckpointer {
    ...
    private void doCheckpoint() throws InterruptedException, IOException {
        ...
        try {
            ...
            // TODO_MA 马中华 注释： 上一次做 checkpoint 的 txid
            long prevCheckpointTxId = img.getStorage().getMostRecentCheckpointTxId();
            // TODO_MA 马中华 注释： 这一次做 checkpoint 的 txid
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 让 namenode 的每个数据存储目录，都生成一个新的 fsimage 文件
             */
            ...
            if (outputDir != null && !outputDir.isEmpty()) {
                try {
                    ...
                    LOG.warn("Exception encountered while saving legacy OIV image; " + "continuing with other checkpointing steps",
                            ioe
                    );
                }
            }
        ...
        // See HDFS-4816
        ExecutorService executor = new ThreadPoolExecutor(0, activeNNAddresses.size(), 100, TimeUnit.MILLISECONDS,
                new LinkedBlockingQueue<Runnable>(activeNNAddresses.size()), uploadThreadFactory
        );
            ...
            if (shouldUpload) {
                Future<TransferFsImage.TransferResult> upload = executor.submit(new Callable<TransferFsImage.TransferResult>() {
                    ...
                    public TransferFsImage.TransferResult call() throws IOException, InterruptedException {
                        ...
                        /*************************************************
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 上传最新生成的 fsimage 文件到 active namenode
                         */
                        return TransferFsImage.uploadImageFromStorage(activeNNAddress, conf, namesystem.getFSImage().getStorage(),
