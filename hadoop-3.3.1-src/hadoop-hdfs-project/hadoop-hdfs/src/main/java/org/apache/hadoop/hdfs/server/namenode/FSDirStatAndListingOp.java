...
class FSDirStatAndListingOp {
    static DirectoryListing getListingInt(FSDirectory fsd, FSPermissionChecker pc, final String srcArg, byte[] startAfter,
     ...
     */
    static HdfsFileStatus getFileInfo(FSDirectory fsd, FSPermissionChecker pc, String srcArg, boolean resolveLink,
     ...
     * @throws IOException
     */
    static GetBlockLocationsResult getBlockLocations(FSDirectory fsd, FSPermissionChecker pc, String src, long offset, long length,
        ...
        try {
            ...
            final long fileSize = iip.isSnapshot() ? inode.computeFileSize(
                    iip.getPathSnapshotId()) : inode.computeFileSizeNotIncludingLastUcBlock();
     ...
     */
    private static DirectoryListing getListing(FSDirectory fsd, INodesInPath iip, byte[] startAfter,
                                               boolean needLocation) throws IOException {
        ...
        try {
            ...
            if (!targetNode.isDirectory()) {
                ...
                return new DirectoryListing(
                        new HdfsFileStatus[]{createFileStatus(fsd, iip, null, parentStoragePolicy, needLocation, false)}, 0);
            }
            ...
            for (int i = 0; i < numOfListing && locationBudget > 0; i++) {
                ...
                byte childStoragePolicy = !child.isSymlink() ? getStoragePolicyID(child.getLocalStoragePolicyID(),
                        parentStoragePolicy
                ) : parentStoragePolicy;
     ...
     *         or null if file not found
     */
    static HdfsFileStatus getFileInfo(FSDirectory fsd, INodesInPath iip, boolean includeStoragePolicy, boolean needLocation,
                                      boolean needBlockToken) throws IOException {
        fsd.readLock();
        try {
            final INode node = iip.getLastINode();
    ...
    static HdfsFileStatus getFileInfo(FSDirectory fsd, INodesInPath iip, boolean needLocation,
                                      boolean needBlockToken) throws IOException {
        fsd.readLock();
        try {
     ...
     */
    private static HdfsFileStatus createFileStatus(FSDirectory fsd, INodesInPath iip, INode child, byte storagePolicy,
                                                   boolean needLocation, boolean needBlockToken) throws IOException {
        ...
        if (node.isFile()) {
            ...
            if (needLocation) {
                ...
                loc = fsd.getBlockManager()
                        .createLocatedBlocks(fileNode.getBlocks(snapshot), fileSize, isUc, 0L, size, needBlockToken, inSnapshot,
                                feInfo, ecPolicy
                        );
                if (loc == null) {
    ...
    private static HdfsFileStatus createFileStatus(long length, boolean isdir, int replication, long blocksize, long mtime,
                                                   long atime, FsPermission permission, EnumSet<HdfsFileStatus.Flags> flags,
        ...
        return new HdfsFileStatus.Builder().length(length).isdir(isdir).replication(replication).blocksize(blocksize).mtime(mtime)
                .atime(atime).perm(permission).flags(flags).owner(owner).group(group).symlink(symlink).path(path).fileId(fileId)
                .children(childrenNum).feInfo(feInfo).storagePolicy(storagePolicy).ecPolicy(ecPolicy).locations(locations).build();
    }

    private static ContentSummary getContentSummaryInt(FSDirectory fsd, FSPermissionChecker pc,
        ...
        try {
            ...
            if (targetNode.isDirectory()) {
                ...
                if (feature != null) {
                    ...
                    usage = new QuotaUsage.Builder().fileAndDirectoryCount(counts.getNameSpace()).quota(quotas.getNameSpace())
                            .spaceConsumed(counts.getStorageSpace()).spaceQuota(quotas.getStorageSpace())
                            .typeConsumed(counts.getTypeSpaces().asArray()).typeQuota(quotas.getTypeSpaces().asArray()).build();
                }
            }
            return usage;
        } finally {
