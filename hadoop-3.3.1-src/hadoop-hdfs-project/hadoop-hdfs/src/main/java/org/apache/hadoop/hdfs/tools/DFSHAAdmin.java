...
public class DFSHAAdmin extends HAAdmin {
    ...
    private final static Map<String, UsageInfo> USAGE_DFS_ONLY = ImmutableMap.<String, UsageInfo>builder()
            .put("-transitionToObserver", new UsageInfo("<serviceId>", "Transitions the service into Observer state"))
            .put("-failover", new UsageInfo("[--" + FORCEFENCE + "] [--" + FORCEACTIVE + "] " + "<serviceId> <serviceId>",
                    "Failover from the first service to the second.\n" + "Unconditionally fence services if the --" + FORCEFENCE + " option is used.\n" + "Try to failover to the target service " + "even if it is not ready if the " + "--" + FORCEACTIVE + " option is used."
            )).build();

    private final static Map<String, UsageInfo> USAGE_DFS_MERGED = ImmutableSortedMap.<String, UsageInfo>naturalOrder()
            .putAll(USAGE).putAll(USAGE_DFS_ONLY).build();
    ...
    // TODO_MA 马中华 注释： 执行命令： hdfs haadmin -transitionToActive nn1　
    @Override
    protected int runCmd(String[] argv) throws Exception {
        ...
        */
         ...
         *  注释： 如果不是 -transitionToObserver 或者 -failover
         *  也就是说是： -transitionToActive 或者 -transitionToStandby 等
         */
    ...
    private int failover(CommandLine cmd) throws IOException, ServiceFailedException {
        ...
        if (fromNode.isAutoFailoverEnabled()) {
            ...
            try {
                ...
                errOut.println(
                        "Failover command is not supported with " + "auto-failover enabled: " + e.getLocalizedMessage());
                return -1;
            }
        }
