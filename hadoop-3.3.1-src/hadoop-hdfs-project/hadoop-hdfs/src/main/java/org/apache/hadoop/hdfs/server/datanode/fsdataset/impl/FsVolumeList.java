 ...
 *  注释： Datanode可以定义多个存储目录，
 *  每个存储目录下的数据块是使用一个 FsVolumelmpl 对象管理的，
 *  所以 Datanode 定义了 FsVolumeList 类保存 Datanode 上所有的 FsVolumelmpl 对象，
 *  FsVolumeList 对 FsDatasetlmpl 提供类似磁盘的服务。
 */
class FsVolumeList {

    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 每个 FsVolumeImpl 就是对一个 存储目录的 管理的抽象
     *  FsVolumeList 就是一堆 FsVolumeImpl 的一个整体抽象
     */
    ...
    private final Map<StorageLocation, VolumeFailureInfo> volumeFailureInfos = Collections.synchronizedMap(
    ...
    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： FsVolumeImpl 选择策略，用来决定到底把 Block 存储在那个 目录=FsVolumeImpl
     */
    ...
    void getAllVolumesMap(final String bpid, final ReplicaMap volumeMap,
                ...
                public void run() {
                    try (FsVolumeReference ref = v.obtainReference()) {
                        ...
                        FsDatasetImpl.LOG.info(
                                "Time to add replicas to map for block pool" + " " + bpid + " on volume " + v + ": " + timeTaken + "ms");
        ...
        FsDatasetImpl.LOG.info(
                "Total time to add all replicas to map for block pool " + bpid + ": " + totalTimeTaken + "ms");
