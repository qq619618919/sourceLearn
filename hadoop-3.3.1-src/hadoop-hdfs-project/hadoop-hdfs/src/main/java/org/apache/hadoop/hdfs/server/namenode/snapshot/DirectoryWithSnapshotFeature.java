...
public class DirectoryWithSnapshotFeature implements INode.Feature {
    ...
    public static class DirectoryDiff extends AbstractINodeDiff<INodeDirectory, INodeDirectoryAttributes, DirectoryDiff> {
        ...
        DirectoryDiff(int snapshotId, INodeDirectoryAttributes snapshotINode, DirectoryDiff posteriorDiff, int childrenSize,
                      List<INode> createdList, List<INode> deletedList, boolean isSnapshotRoot) {
        ...
        @Override
        void combinePosteriorAndCollectBlocks(final INode.ReclaimContext reclaimContext, final INodeDirectory currentDir,
            ...
            return new ReadOnlyList<INode>() {
                ...
                private List<INode> initChildren() {
                    if (children == null) {
                        ...
                        List<DirectoryDiff> diffList = directoryDiffList.getDiffListBetweenSnapshots(diffIndex,
    ...
    public static class DirectoryDiffList extends AbstractINodeDiffList<INodeDirectory, INodeDirectoryAttributes, DirectoryDiff> {
        ...
        INodeDirectoryAttributes createSnapshotCopy(INodeDirectory currentDir) {
            return currentDir.isQuotaSet() ? new INodeDirectoryAttributes.CopyWithQuota(
                    currentDir) : new INodeDirectoryAttributes.SnapshotCopy(currentDir);
        }
     ...
     */
    public static void destroyDstSubtree(INode.ReclaimContext reclaimContext, INode inode, final int snapshot,
                                         final int prior) {
     ...
     */
    private static void cleanDeletedINode(INode.ReclaimContext reclaimContext, INode inode, final int post,
                                          final int prior) {
    ...
    public INode getChild(INodeDirectory currentINode, byte[] name, int snapshotId) {
        final DirectoryDiff diff = diffs.getDiffById(snapshotId);
        return diff != null ? diff.getChild(name, true, currentINode) : currentINode.getChild(name,
    ...
    public void cleanDirectory(INode.ReclaimContext reclaimContext, final INodeDirectory currentINode, final int snapshot,
                               int prior) {
            ...
            if (prior != NO_SNAPSHOT_ID) {
                ...
                if (priorDiff != null && priorDiff.getSnapshotId() == prior) {
                    ...
                    if (priorCreated != null) {
                        ...
                        final boolean destroy = currentINode.isLastReference() && currentINode.getDiffs()
                                .getLastSnapshotId() == prior && !currentINode.isInCurrentState();
