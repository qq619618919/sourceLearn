...
public class SecondaryNameNode implements Runnable, SecondaryNameNodeInfoMXBean {
    ...
    public static InetSocketAddress getHttpAddress(Configuration conf) {
        // TODO_MA 马中华 注释： dfs.namenode.secondary.http-address = 0.0.0.0:9868
    ...
    private void initialize(final Configuration conf, CommandLineOpts commandLineOpts) throws IOException {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 链接 NameNode，通过 NamenodeProtocol 通信协议来进行通信
         *  当满足条件需要做 元数据 hceckpint 的时候， SN 会发送一个 RPC 请求给 NameNode
         *  让 NN 去 rollLog （edits_inprogress_txid 变成一个 edits_starttxid_endtxid）
         *  SN 和 NN 建立了链接
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 构建 FSNamesystem 用来完成 元数据的 checkpoint
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： Secondary NameNode 的 checkpoint 相关参数
         */
        ...
        LOG.info(
                "Checkpoint Period   :" + checkpointConf.getPeriod() + " secs " + "(" + checkpointConf.getPeriod() / 60 + " min)");
    ...
    public void doWork() {
        ...
        // TODO_MA 马中华 注释： 不停的执行条件判断！  每隔 60s 检查一下是否满足条件来执行 checkpoint
        while (shouldRun) {

            // TODO_MA 马中华 注释： 每隔 1 分钟 做一次检查，是否要执行 doCheckpoint 动作
            try {
                ...
                public Boolean run() throws Exception {
                    ...
                    if (sig.mostRecentCheckpointTxId == dstImage.getStorage().getMostRecentCheckpointTxId()) {
                        ...
                        MD5Hash downloadedHash = TransferFsImage.downloadImageToStorage(nnHostPort,
                                sig.mostRecentCheckpointTxId, dstImage.getStorage(), true, false
                        );
    ...
    public void startInfoServer() throws IOException {
        ...
        // TODO_MA 马中华 注释： http 端口： dfs.namenode.secondary.http-address = 0.0.0.0:9868
        final InetSocketAddress httpAddr = getHttpAddress(conf);

        // TODO_MA 马中华 注释： https 端口： dfs.namenode.secondary.https-address = 0.0.0.0:9869
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 总的说来，构建一个 HttpServer2，内部绑定 ImageServlet 用来处理相关请求
         */
    ...
    public boolean doCheckpoint() throws IOException {
        ...
        // TODO_MA 马中华 注释： 构建 NNStorage
        NNStorage dstStorage = checkpointImage.getStorage();

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第一步： 发送 RPC 请求给 NameNode ,做  RollLog 动作
        ...
        boolean isSameCluster = (dstStorage.versionSupportsFederation(NameNodeLayoutVersion.FEATURES) && sig.isSameCluster(
                checkpointImage)
        ) || (!dstStorage.versionSupportsFederation(NameNodeLayoutVersion.FEATURES) && sig.namespaceIdMatches(
                checkpointImage)
        );
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第二步： 从 namenode 下载 fsimage 和 editlog
         */
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第三步： 合并 fsimage 和 editlog
         *  构造了一个最新的： FSDirectory
         */
        try {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 第四步： 将合并得到的最新 fsimage文件 上传给 NameNode
         */
        // Upload the new image into the NameNode.
        // Then tell the Namenode to make this new uploaded image as the most current image.
            ...
            switch (opts.getCommand()) {
                ...
                case CHECKPOINT:
                    ...
                    if (count > checkpointConf.getTxnCount() || opts.shouldForceCheckpoint()) {
                        ...
                        System.err.println(
                                "EditLog size " + count + " transactions is " + "smaller than configured checkpoint " + "interval " + checkpointConf.getTxnCount() + " transactions.");
    ...
    public static void main(String[] argv) throws Exception {
        ...
        try {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： SecondaryNameNode 有两种运行模式：
             *  1、命令模式：运行完命令之后退出
             *  2、守护模式：运行成为一个守护进程，定期执行 checkpoint
             */
             ...
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 第二种模式： 守护模式
             *  根据条件判断来执行 doCheckpoint()
             */
            else {
                // mode 2
                // TODO_MA 马中华 注释： 启动 HttpServer WebUI
                secondary.startInfoServer();
                // TODO_MA 马中华 注释： 启动 Checkpoint 线程
        ...
        private static class CheckpointLogPurger implements LogsPurgeable {
            ...
            public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxId, boolean inProgressOk,
                ...
                while (iter.hasNext()) {
                    ...
                    FileJournalManager.addStreamsToCollectionFromFiles(editFiles, streams, fromTxId, Long.MAX_VALUE,
                            inProgressOk
                    );
                }
            }
    ...
    void doMerge(CheckpointSignature sig, RemoteEditLogManifest manifest, boolean loadImage, FSImage dstImage,
