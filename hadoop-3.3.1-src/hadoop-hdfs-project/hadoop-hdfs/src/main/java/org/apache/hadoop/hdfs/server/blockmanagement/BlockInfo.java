...
public abstract class BlockInfo extends Block implements LightWeightGSet.LinkedElement {
    ...
    /**
     * // TODO_MA 马中华 注释： 当客户端对数据块进行追加写操作时会调用这个方法，也就是将该 BlockInfo 对应的数据块状态变为了构建中状态
     * // TODO_MA 马中华 注释： 它将一个 BlockInfo 对象转换为 BlockUnderConstructionFeature 对象
    ...
    public void convertToBlockUnderConstruction(BlockUCState s, DatanodeStorageInfo[] targets) {
        if (isComplete()) {
            // TODO_MA 马中华 注释： 将该 BlockInfo 对应的数据块状态变为了构建中状态
