...
public class INodeFile extends INodeWithAdditionalFields implements INodeFileAttributes, BlockCollection {
        ...
        static long getBlockLayoutRedundancy(BlockType blockType, Short replication, Byte erasureCodingPolicyID) {
            ...
            switch (blockType) {
                ...
                case CONTIGUOUS:
                    if (erasureCodingPolicyID != REPLICATION_POLICY_ID) {
                        throw new IllegalArgumentException(
                                String.format("Illegal EC policy 0x%02x for CONTIGUOUS block type", erasureCodingPolicyID));
                    }
     ...
     *  注释： 该节点对应文件的块信息
     *  为什么用数组来表示：因为数据块是有顺序的？需要维护这个顺序关系
     *  BlockInfo 数据块的元数据
     */
    private BlockInfo[] blocks;

    INodeFile(long id, byte[] name, PermissionStatus permissions, long mtime, long atime, BlockInfo[] blklist, short replication,
        ...
        for (int i = 0; i < blocks.length; i++) {
            ...
            if (err != null) {
                throw new IllegalStateException(
                        String.format("Unexpected block state: " + "%s, file=%s (%s), blocks=%s (i=%s)", err, this,
    ...
    public short getPreferredBlockReplication() {
        ...
        Preconditions.checkNotNull(ecPolicy,
                "Could not find EC policy with ID 0x" + StringUtils.byteToHexString(getErasureCodingPolicyID())
        );
    ...
    public byte getStoragePolicyID() {
        ...
        if (isStriped() && id != BLOCK_STORAGE_POLICY_ID_UNSPECIFIED && !ErasureCodingPolicyManager.checkStoragePolicySuitableForECStripedMode(
                id)) {
            id = HdfsConstants.BLOCK_STORAGE_POLICY_ID_UNSPECIFIED;
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                        "The current effective storage policy id : " + id + " is not suitable for striped mode EC file : " + getName() + ". So, just returning unspecified storage policy id");
            }
        }
    ...
    @Override
    public final QuotaCounts computeQuotaUsage(BlockStoragePolicySuite bsps, byte blockStoragePolicyId, boolean useCache,
                                               int lastSnapshotId) {
        final QuotaCounts counts = new QuotaCounts.Builder().nameSpace(1).build();

        final BlockStoragePolicy bsp = (blockStoragePolicyId == BLOCK_STORAGE_POLICY_ID_UNSPECIFIED) ? null : bsps.getPolicy(
