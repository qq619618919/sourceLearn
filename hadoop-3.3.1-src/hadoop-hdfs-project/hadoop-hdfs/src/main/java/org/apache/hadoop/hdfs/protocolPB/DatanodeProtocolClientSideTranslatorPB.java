...
public class DatanodeProtocolClientSideTranslatorPB implements ProtocolMetaInterface, DatanodeProtocol, Closeable {
    ...
    public DatanodeProtocolClientSideTranslatorPB(InetSocketAddress nameNodeAddr, Configuration conf) throws IOException {
        ...
        // TODO_MA 马中华 注释： 设置 RPC 引擎实现 和 通信协议
         ...
         *  注释：
         *  1、表面意义： 获取 DataNode 和 NameNode 通信的 NameNode 代理
         *  2、在 rpcProxy 对象的内部，其实包装了一个 RPC Client
         */
    ...
    private static DatanodeProtocolPB createNamenode(InetSocketAddress nameNodeAddr, Configuration conf,
         ...
         *  注释：
         */
        return RPC.getProxy(DatanodeProtocolPB.class, RPC.getProtocolVersion(DatanodeProtocolPB.class), nameNodeAddr, ugi,
    ...
    public DatanodeRegistration registerDatanode(DatanodeRegistration registration) throws IOException {
        ...
        // TODO_MA 马中华 注释： 生成一个 Reuqest 的 Builder
        ...
        try {
             ...
             *  1、参数： Request
             *  2、返回值： Response
             *  具体的通信，是通过 网络通信 + Protobuf 来联合完成的
             *  -
             *  发送 RPC 请求给 namenode ==> NameNodeRpcServer
             */
    ...
    @Override
    public HeartbeatResponse sendHeartbeat(DatanodeRegistration registration, StorageReport[] reports, long cacheCapacity,
                                           long cacheUsed, int xmitsInProgress, int xceiverCount, int failedVolumes,
        ...
        // TODO_MA 马中华 注释： 接收和处理 NameNode 返回回来的需要执行的命令
    ...
    public DatanodeCommand blockReport(DatanodeRegistration registration, String poolId, StorageBlockReport[] reports,
        ...
        for (StorageBlockReport r : reports) {
            StorageBlockReportProto.Builder reportBuilder = StorageBlockReportProto.newBuilder()
                    .setStorage(PBHelperClient.convert(r.getStorage()));
    ...
    @Override
    public DatanodeCommand cacheReport(DatanodeRegistration registration, String poolId,
    ...
    public void errorReport(DatanodeRegistration registration, int errorCode, String msg) throws IOException {
        ErrorReportRequestProto req = ErrorReportRequestProto.newBuilder().setRegistartion(PBHelper.convert(registration))
    ...
    public void commitBlockSynchronization(ExtendedBlock block, long newgenerationstamp, long newlength, boolean closeFile,
        ...
        CommitBlockSynchronizationRequestProto.Builder builder = CommitBlockSynchronizationRequestProto.newBuilder()
                .setBlock(PBHelperClient.convert(block)).setNewGenStamp(newgenerationstamp).setNewLength(newlength)
