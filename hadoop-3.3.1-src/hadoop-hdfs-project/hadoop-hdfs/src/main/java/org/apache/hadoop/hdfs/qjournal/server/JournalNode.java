...
public class JournalNode implements Tool, Configurable, JournalNodeMXBean {
    ...
    synchronized Journal getOrCreateJournal(String jid, String nameServiceId, StartupOption startOpt) throws IOException {
        ...
        if (journal == null) {
            ...
            if (conf.getBoolean(DFSConfigKeys.DFS_JOURNALNODE_ENABLE_SYNC_KEY,
        ...
        } else if (journalSyncersById.get(jid) != null && !journalSyncersById.get(jid)
                .isJournalSyncerStarted() && !journalsById.get(jid)
                .getTriedJournalSyncerStartedwithnsId() && nameServiceId != null) {
            startSyncer(journal, jid, nameServiceId);
        }
    ...
    public void setConf(Configuration conf) {
        ...
        if (nameserviceIds.size() < 2) {
            ...
            // TODO_MA 马中华 注释： dfs.journalnode.edits.dir.nameService = 自己配置
            ...
            if (journalNodeDir == null) {
                // TODO_MA 马中华 注释： 默认：/tmp/hadoop/dfs/journalnode/
        ...
        if (this.tracer == null) {
            this.tracer = new Tracer.Builder("JournalNode").conf(TraceUtils.wrapHadoopConf("journalnode.htrace", conf))
                    .build();
        }
    }
    ...
    public void start() throws IOException {
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 首先初始化 JourNalNode 的本地文件夹
             *  举例： /home/bigdata/data/journaldata/hadoop330ha
             */
            for (File journalDir : localDir) {
                validateAndCreateJournalDir(journalDir);
            }

            // TODO_MA 马中华 注释： 集群内部做性能监控的一些相关组件，跟核心工作机制是没有关系的
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 创建 RPCServer， 绑定的 RPC 端口是 8485
             */
    ...
    public List<String> getClusterIds() {
        return journalsById.values().stream().map(j -> j.getStorage().getClusterID())
                .filter(cid -> !Strings.isNullOrEmpty(cid)).distinct().collect(Collectors.toList());
    }
    ...
    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： JournalNode 启动的入口
     *  如果你之后，发现某个类是 Tool 的子类，那不用想了，他里面必定有 run() 方法，核心逻辑就在 run 中
     */
    public static void main(String[] args) throws Exception {
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 运行 JournalNode 的 run() 方法
             *  ToolRunner.run() 方法的内部，其实就是调用 Tool 的 run()
             *  Tool = 第一个参数 = new JournalNode()
             */
            System.exit(ToolRunner.run(new JournalNode(), args));

//            JournalNode j = new JournalNode();
//            j.run(args);
        } catch (Throwable e) {
    ...
    public Boolean canRollBack(String journalId, StorageInfo storage, StorageInfo prevStorage, int targetLayoutVersion,
        ...
        return getOrCreateJournal(journalId, nameServiceId, StartupOption.ROLLBACK).canRollBack(storage, prevStorage,
                targetLayoutVersion
        );
    }
    ...
    protected InetSocketAddress getHttpServerBindAddress(Configuration configuration) {
        ...
        // TODO_MA 马中华 注释： dfs.journalnode.http-bind-host = 8480
