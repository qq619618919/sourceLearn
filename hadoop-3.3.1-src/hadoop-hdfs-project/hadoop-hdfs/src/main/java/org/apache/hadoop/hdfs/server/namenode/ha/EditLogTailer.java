...
/**
 * // TODO_MA 马中华 注释： EditLogTailer 代表一个线程，它定期从编辑日志中读取并将其中包含的事务应用于给定的 FSNamesystem。
 * EditLogTailer represents a thread which periodically reads from edits
 * journals and applies the transactions contained within to a given FSNamesystem.
 */
...
public class EditLogTailer {
    ...
    public EditLogTailer(FSNamesystem namesystem, Configuration conf) {
        ...
        logRollPeriodMs = conf.getTimeDuration(DFSConfigKeys.DFS_HA_LOGROLL_PERIOD_KEY,
                DFSConfigKeys.DFS_HA_LOGROLL_PERIOD_DEFAULT, TimeUnit.SECONDS, TimeUnit.MILLISECONDS
        );
        ...
        if (logRollPeriodMs >= 0) {
            ...
            LOG.info(
                    "Not going to trigger log rolls on active node because " + DFSConfigKeys.DFS_HA_LOGROLL_PERIOD_KEY + " is negative.");
        }

        sleepTimeMs = conf.getTimeDuration(DFSConfigKeys.DFS_HA_TAILEDITS_PERIOD_KEY,
                DFSConfigKeys.DFS_HA_TAILEDITS_PERIOD_DEFAULT, TimeUnit.SECONDS, TimeUnit.MILLISECONDS
        );
        long maxSleepTimeMsTemp = conf.getTimeDuration(DFSConfigKeys.DFS_HA_TAILEDITS_PERIOD_BACKOFF_MAX_KEY,
                DFSConfigKeys.DFS_HA_TAILEDITS_PERIOD_BACKOFF_MAX_DEFAULT, TimeUnit.SECONDS, TimeUnit.MILLISECONDS
        );
        if (maxSleepTimeMsTemp > 0 && maxSleepTimeMsTemp < sleepTimeMs) {
            LOG.warn(
                    "{} was configured to be {} ms, but this is less than {}." + "Disabling backoff when tailing edit logs.",
                    DFSConfigKeys.DFS_HA_TAILEDITS_PERIOD_BACKOFF_MAX_KEY, maxSleepTimeMsTemp,
        ...
        rollEditsTimeoutMs = conf.getTimeDuration(DFSConfigKeys.DFS_HA_TAILEDITS_ROLLEDITS_TIMEOUT_KEY,
                DFSConfigKeys.DFS_HA_TAILEDITS_ROLLEDITS_TIMEOUT_DEFAULT, TimeUnit.SECONDS, TimeUnit.MILLISECONDS
        );
        ...
        if (maxRetries <= 0) {
            LOG.error(
                    "Specified a non-positive number of retries for the number of retries for the " + "namenode connection when manipulating the edit log (" + DFSConfigKeys.DFS_HA_TAILEDITS_ALL_NAMESNODES_RETRY_KEY + "), setting to default: " + DFSConfigKeys.DFS_HA_TAILEDITS_ALL_NAMESNODES_RETRY_DEFAULT);
            maxRetries = DFSConfigKeys.DFS_HA_TAILEDITS_ALL_NAMESNODES_RETRY_DEFAULT;
        }

        inProgressOk = conf.getBoolean(DFSConfigKeys.DFS_HA_TAILEDITS_INPROGRESS_KEY,
                DFSConfigKeys.DFS_HA_TAILEDITS_INPROGRESS_DEFAULT
        );

        this.maxTxnsPerLock = conf.getLong(DFS_HA_TAILEDITS_MAX_TXNS_PER_LOCK_KEY,
    ...
    // TODO_MA 马中华 注释： 拉取 journal 上的日志文件，到 StandbyNameNode 合并到内存中
    // TODO_MA 马中华 注释： 就是为了保持当前 StandbyNameNode 的状态 和 Active NN 的状态几乎一致
    @VisibleForTesting
    public long doTailEdits() throws IOException, InterruptedException {
        ...
        try {
            // TODO_MA 马中华 注释： 得到最近进行同步的 最大 txid
            ...
            // for concern, so we don't catch them here. Simple errors reading from disk are ignored.
            long editsLoaded = 0;
            try {
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 通过 输入流 读取 journal 上的最近一段  新数据
                 */
            ...
            // TODO_MA 马中华 注释： 记录最近一次拉取同步之后，得到的 新的 最大 txid
        ...
        private void doWork() {
            ...
            while (shouldRun) {
                ...
                try {
                    ...
                    // name system lock will be acquired to further block even the block state updates.
                    ...
                    try {
                        ...
                        /*************************************************
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 从 JournalNode 中拉取 Edits 日志文件
                         */
                        editsTailed = doTailEdits();
                    } finally {
                        namesystem.cpUnlock();
                ...
                // TODO_MA 马中华 注释： 计算等待时间， 让当前线程 while 等待
                try {
