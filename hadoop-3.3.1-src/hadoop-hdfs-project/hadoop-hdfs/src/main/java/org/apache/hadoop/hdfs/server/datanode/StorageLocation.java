...
public class StorageLocation implements Checkable<StorageLocation.CheckContext, VolumeCheckResult>, Comparable<StorageLocation> {
    ...
    public void makeBlockPoolDir(String blockPoolID, Configuration conf) throws IOException {
        ...
        if (storageType == StorageType.PROVIDED) {
            ...
            Storage.LOG.info(
                    "Skipping creating directory for block pool " + blockPoolID + " for PROVIDED storage location " + this);
            return;
        }

        LocalFileSystem localFS = FileSystem.getLocal(conf);

        // TODO_MA 马中华 注释： current 目录的默认权限： dfs.datanode.data.dir.perm = 700
         ...
         *  注释： 检查 current 目录是否存在，如果不存在，则创建
         */
