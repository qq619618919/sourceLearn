 ...
 * // TODO_MA 马中华 注释： Quorum 集群 的 JournalManager 具体实现
 */
...
public class QuorumJournalManager implements JournalManager {
    ...
    private final LogThrottlingHelper selectInputStreamLogHelper = new LogThrottlingHelper(
            SELECT_INPUT_STREAM_LOG_INTERVAL_MS);
    ...
    QuorumJournalManager(Configuration conf, URI uri, NamespaceInfo nsInfo, String nameServiceId,
         ...
         *  注释： 一个 JournalNode 创建一个 IPCLoggerChannel
         */
        ...
        this.inProgressTailingEnabled = conf.getBoolean(DFSConfigKeys.DFS_HA_TAILEDITS_INPROGRESS_KEY,
        ...
        this.timeoutMs = (int) conf.getTimeDuration(DFSConfigKeys.DFS_QJM_OPERATIONS_TIMEOUT,
                DFSConfigKeys.DFS_QJM_OPERATIONS_TIMEOUT_DEFAULT, TimeUnit.MILLISECONDS
        );

        int connectTimeoutMs = conf.getInt(DFSConfigKeys.DFS_QJOURNAL_HTTP_OPEN_TIMEOUT_KEY,
                DFSConfigKeys.DFS_QJOURNAL_HTTP_OPEN_TIMEOUT_DEFAULT
        );
        int readTimeoutMs = conf.getInt(DFSConfigKeys.DFS_QJOURNAL_HTTP_READ_TIMEOUT_KEY,
...
//                DFSConfigKeys.DFS_NAMENODE_DOUBLEEDITS_BUFFERSIZE_VALUE));
    ...
    protected List<AsyncLogger> createLoggers(AsyncLogger.Factory factory) throws IOException {
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 创建的具体实现是： IPCLoggerChannel
         *  一个 JournalNode 创建一个 IPCLoggerChannel
         */
    ...
    Map<AsyncLogger, NewEpochResponseProto> createNewUniqueEpoch() throws IOException {
        ...
        Map<AsyncLogger, GetJournalStateResponseProto> lastPromises = loggers.waitForWriteQuorum(loggers.getJournalState(),
        ...
        for (Map.Entry<AsyncLogger, PrepareRecoveryResponseProto> e : prepareResponses.entrySet()) {
            ...
            if (resp.hasLastCommittedTxId() && resp.getLastCommittedTxId() > logToSync.getEndTxId()) {
                throw new AssertionError(
                        "Decided to synchronize log to " + logToSync + " but logger " + logger + " had seen txid " + resp.getLastCommittedTxId() + " committed");
            }
        }
    ...
    static List<AsyncLogger> createLoggers(Configuration conf, URI uri, NamespaceInfo nsInfo, AsyncLogger.Factory factory,
        ...
        if (addrs.size() % 2 == 0) {
            LOG.warn(
                    "Quorum journal URI '" + uri + "' has an even number " + "of Journal Nodes specified. This is not recommended!");
        }
    ...
    public void setOutputBufferCapacity(int size) {
        int ipcMaxDataLength = conf.getInt(CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH,
                CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH_DEFAULT
        );
        if (size >= ipcMaxDataLength) {
            throw new IllegalArgumentException(
                    "Attempted to use QJM output buffer " + "capacity (" + size + ") greater than the IPC max data length (" + CommonConfigurationKeys.IPC_MAXIMUM_DATA_LENGTH + " = " + ipcMaxDataLength + "). This will cause journals to reject edits.");
        }
    ...
    public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxnId,
    ...
    public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxnId, boolean inProgressOk,
        ...
        if (inProgressOk && inProgressTailingEnabled) {
            ...
            try {
                ...
                LOG.warn(
                        "Encountered exception while tailing edits >= " + fromTxnId + " via RPC; falling back to streaming.",
                        ioe
                );
            }
        }
     ...
     */
    private void selectRpcInputStreams(Collection<EditLogInputStream> streams, long fromTxnId,
                                       boolean onlyDurableTxns) throws IOException {
        QuorumCall<AsyncLogger, GetJournaledEditsResponseProto> q = loggers.getJournaledEdits(fromTxnId, maxTxnsPerRpc);
        Map<AsyncLogger, GetJournaledEditsResponseProto> responseMap = loggers.waitForWriteQuorum(q,
                selectInputStreamsTimeoutMs, "selectRpcInputStreams"
        );
        ...
        int maxAllowedTxns = !onlyDurableTxns ? highestTxnCount : responseCounts.get(
                responseCounts.size() - loggers.getMajoritySize());
        if (maxAllowedTxns == 0) {
        ...
        if (logAction.shouldLog()) {
            LOG.info(
                    "Selected loggers with >= " + maxAllowedTxns + " transactions " + "starting from lowest txn ID " + logAction.getStats(
                            0).getMin() + LogThrottlingHelper.getLogSupressionMessage(logAction));
        }
            ...
            for (RemoteEditLog remoteLog : manifest.getLogs()) {
                ...
                if (onlyDurableTxns && inProgressOk && remoteLog.isInProgress()) {
                    ...
                    if (endTxId < remoteLog.getStartTxId()) {
                        LOG.warn(
                                "Found endTxId (" + endTxId + ") that is less than " + "the startTxId (" + remoteLog.getStartTxId() + ") - setting it to startTxId.");
                        endTxId = remoteLog.getStartTxId();
                    }
                }

                EditLogInputStream elis = EditLogFileInputStream.fromUrl(connectionFactory, url, remoteLog.getStartTxId(),
