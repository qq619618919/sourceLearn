...
public class CacheManager {
    ...
    CacheManager(FSNamesystem namesystem, Configuration conf, BlockManager blockManager) {
        ...
        this.cachedBlocks = enabled ? new LightWeightGSet<CachedBlock, CachedBlock>(
                LightWeightGSet.computeCapacity(cachedBlocksPercent, "cachedBlocks")) : new LightWeightGSet<>(0);
    }
    ...
    private static long validateExpiryTime(CacheDirectiveInfo info,
        ...
        if (relExpiryTime > maxRelativeExpiryTime) {
            throw new InvalidRequestException(
                    "Expiration " + expiry.toString() + " exceeds the max relative expiration time of " + maxRelativeExpiryTime + " ms.");
        }
    ...
    private CacheDirectiveStats computeNeeded(String path, short replication) {
        ...
        return new CacheDirectiveStats.Builder().setBytesNeeded(requestedBytes * replication).setFilesCached(requestedFiles)
                .build();
    }
    ...
    public CacheDirectiveInfo addDirective(CacheDirectiveInfo info, FSPermissionChecker pc,
    ...
    public void modifyDirective(CacheDirectiveInfo info, FSPermissionChecker pc,
                                EnumSet<CacheFlag> flags) throws IOException {
        assert namesystem.hasWriteLock();
    ...
    public final void processCacheReport(final DatanodeID datanodeID, final List<Long> blockIds) throws IOException {
        if (!enabled) {
            LOG.debug("Ignoring cache report from {} because {} = false. " + "number of blocks: {}", datanodeID,
