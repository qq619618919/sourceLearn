...
public class IPCLoggerChannel implements AsyncLogger {
    ...
    public IPCLoggerChannel(Configuration conf, NamespaceInfo nsInfo, String journalId, String nameServiceId,
                            InetSocketAddress addr) {
        this.conf = conf;
        this.nsInfo = nsInfo;
    ...
    protected ExecutorService createParallelExecutor() {
        int numThreads = conf.getInt(DFSConfigKeys.DFS_QJOURNAL_PARALLEL_READ_NUM_THREADS_KEY,
                DFSConfigKeys.DFS_QJOURNAL_PARALLEL_READ_NUM_THREADS_DEFAULT
        );
        return new ThreadPoolExecutor(1, numThreads, 60L, TimeUnit.SECONDS, new LinkedBlockingQueue<>(),
    ...
    public ListenableFuture<Void> sendEdits(final long segmentTxId, final long firstTxnId, final int numTxns,
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 每往一个 JournalNode 节点发送一条日志，都通过提交一个任务到线程池中执行完成
             */
            ret = singleThreadExecutor.submit(new Callable<Void>() {
                ...
                public Void call() throws IOException {
                    ...
                    try {
                        /*************************************************
                         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                         *  注释： 发送日志给 JournalNodeRpcServer
                         *  getProxy() = JournalNodeRpcServer 服务实例的 一个代理对象
                         */
                        getProxy().journal(createReqInfo(), segmentTxId, firstTxnId, numTxns, data);
                    } catch (IOException e) {
                        QuorumJournalManager.LOG.warn(
                                "Remote journal " + IPCLoggerChannel.this + " failed to " + "write txns " + firstTxnId + "-" + (firstTxnId + numTxns - 1) + ". Will try to write to this JN again after the next " + "log roll.",
                                e
                        );
                        ...
                        if (rpcTime / 1000 > WARN_JOURNAL_MILLIS_THRESHOLD) {
                            QuorumJournalManager.LOG.warn(
                                    "Took " + (rpcTime / 1000) + "ms to send a batch of " + numTxns + " edits (" + data.length + " bytes) to " + "remote journal " + IPCLoggerChannel.this);
                        }
                    }
                    synchronized (IPCLoggerChannel.this) {
    ...
    private void heartbeatIfNecessary() throws IOException {
        if (lastHeartbeatStopwatch.now(
                TimeUnit.MILLISECONDS) > HEARTBEAT_INTERVAL_MILLIS || !lastHeartbeatStopwatch.isRunning()) {
            try {
    ...
    private synchronized void reserveQueueSpace(int size) throws LoggerTooFarBehindException {
        ...
        if (queuedEditsSizeBytes + size > queueSizeLimitBytes && queuedEditsSizeBytes > 0) {
            QuorumJournalManager.LOG.warn(
                    "Pending edits to " + IPCLoggerChannel.this + " is going to exceed limit size: " + queueSizeLimitBytes + ", current queued edits size: " + queuedEditsSizeBytes + ", will silently drop " + size + " bytes of edits!");
    ...
    public ListenableFuture<Void> startLogSegment(final long txid, final int layoutVersion) {
        return singleThreadExecutor.submit(new Callable<Void>() {
            ...
            public Void call() throws IOException {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 发送 RPC 请求给 JournalNode RPCServer
                 */
                ...
                synchronized (IPCLoggerChannel.this) {
                    if (outOfSync) {
                        ...
                        QuorumJournalManager.LOG.info(
                                "Restarting previously-stopped writes to " + IPCLoggerChannel.this + " in segment starting at txid " + txid);
                    }
                }
                return null;
            }
    ...
    public ListenableFuture<RemoteEditLogManifest> getEditLogManifest(final long fromTxnId, final boolean inProgressOk) {
        return parallelExecutor.submit(new Callable<RemoteEditLogManifest>() {
            ...
            public RemoteEditLogManifest call() throws IOException {
                GetEditLogManifestResponseProto ret = getProxy().getEditLogManifest(journalId, nameServiceId, fromTxnId,
                        inProgressOk
                );
