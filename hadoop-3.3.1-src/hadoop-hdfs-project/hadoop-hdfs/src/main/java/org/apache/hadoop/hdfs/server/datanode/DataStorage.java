...
public class DataStorage extends Storage {
     ...
     *  注释： 在联邦机制中，一个 BlockPool 对应到一个 BlockPoolSliceStorage 管理类
     *  注意： 每个配置的存储目录中，都有这个快池目录
     */
    ...
    private static boolean createStorageID(StorageDirectory sd, int lv, Configuration conf) {
        ...
        final boolean haveValidStorageId = DataNodeLayoutVersion.supports(
                LayoutVersion.Feature.ADD_DATANODE_AND_STORAGE_UUIDS, lv) && DatanodeStorage.isValidStorageId(
                sd.getStorageUuid());
    ...
    public static boolean createStorageID(StorageDirectory sd, boolean regenerateStorageIds, Configuration conf) {
        ...
        if (sd.getStorageLocation() != null && sd.getStorageLocation().getStorageType() == StorageType.PROVIDED) {
            ...
            sd.setStorageUuid(
                    conf.get(DFSConfigKeys.DFS_PROVIDER_STORAGEUUID, DFSConfigKeys.DFS_PROVIDER_STORAGEUUID_DEFAULT));
            return false;
        }
        if (oldStorageID == null || regenerateStorageIds) {
            sd.setStorageUuid(DatanodeStorage.generateUuid());
            LOG.info("Generated new storageID {} for directory {} {}", sd.getStorageUuid(), sd.getRoot(),
    ...
    private StorageDirectory loadStorageDirectory(DataNode datanode, NamespaceInfo nsInfo, StorageLocation location,
        ...
        try {
            ...
            switch (curState) {
                ...
                case NOT_FORMATTED: // format
                    LOG.info("Storage directory with location {} is not formatted for " + "namespace {}. Formatting...",
                            location, nsInfo.getNamespaceID()
                    );
     ...
     */
    public VolumeBuilder prepareVolume(DataNode datanode, StorageLocation location,
        ...
        for (NamespaceInfo nsInfo : nsInfos) {
            ...
            final List<StorageDirectory> dirs = bpStorage.loadBpStorageDirectories(nsInfo, location, StartupOption.HOTSWAP,
     ...
     */
    @VisibleForTesting
    synchronized List<StorageDirectory> addStorageLocations(DataNode datanode, NamespaceInfo nsInfo,
    ...
    private List<StorageLocation> loadDataStorage(DataNode datanode, NamespaceInfo nsInfo,
        ...
        // TODO_MA 马中华 注释： 返回结果容器： List<StorageLocation>
        return success;
    }

    private List<StorageDirectory> loadBlockPoolSliceStorage(DataNode datanode, NamespaceInfo nsInfo,
                                                             Collection<StorageLocation> dataDirs, StartupOption startOpt,
                                                             ExecutorService executor) throws IOException {
        ...
        for (StorageLocation dataDir : dataDirs) {
            ...
            try {
                 ...
                 *  注释：
                 */
                final List<StorageDirectory> dirs = bpStorage.recoverTransitionRead(nsInfo, dataDir, startOpt, sdCallables,
                        datanode.getConf()
                );
        ...
        if (props.getProperty("datanodeUuid") != null) {
            ...
            if (getDatanodeUuid() == null) {
                ...
                throw new InconsistentFSStateException(sd.getRoot(),
                        "Root " + sd.getRoot() + ": DatanodeUuid=" + dnUuid + ", does not match " + getDatanodeUuid() + " from other" + " StorageDirectory."
                );
            }
        }
    }
    ...
    public boolean isPreUpgradableLayout(StorageDirectory sd) throws IOException {
        ...
        try (RandomAccessFile oldFile = new RandomAccessFile(oldF, "rws"); FileLock oldLock = oldFile.getChannel()
                .tryLock()) {
            if (null == oldLock) {
     ...
     */
    private boolean doTransition(StorageDirectory sd, NamespaceInfo nsInfo, StartupOption startOpt,
                                 List<Callable<StorageDirectory>> callables, Configuration conf) throws IOException {
        ...
        if (!federationSupported && getNamespaceID() != nsInfo.getNamespaceID()) {
            throw new IOException("Incompatible namespaceIDs in " + sd.getRoot()
                    .getCanonicalPath() + ": namenode namespaceID = " + nsInfo.getNamespaceID() + "; datanode namespaceID = " + getNamespaceID());
        }

        // For version that supports federation, validate clusterID
        if (federationSupported && !getClusterID().equals(nsInfo.getClusterID())) {
            throw new IOException("Incompatible clusterIDs in " + sd.getRoot()
                    .getCanonicalPath() + ": namenode clusterID = " + nsInfo.getClusterID() + "; datanode clusterID = " + getClusterID());
        }
        ...
        throw new IOException(
                "BUG: The stored LV = " + this.getLayoutVersion() + " is newer than the supported LV = " + HdfsServerConstants.DATANODE_LAYOUT_VERSION);
    }
     ...
     * @param sd  storage directory
     */
    void doUpgradePreFederation(final StorageDirectory sd, final NamespaceInfo nsInfo,
        ...
        LOG.info("Upgrading storage directory {}.\n old LV = {}; old CTime = {}" + ".\n new LV = {}; new CTime = {}",
                sd.getRoot(), oldLV, this.getCTime(), HdfsServerConstants.DATANODE_LAYOUT_VERSION, nsInfo.getCTime()
        );
    ...
    private void doUpgrade(final StorageDirectory sd, final NamespaceInfo nsInfo, final File prevDir, final File tmpDir,
                           final File bbwDir, final File toDir, final int oldLV, Configuration conf) throws IOException {
    ...
    void upgradeProperties(StorageDirectory sd, Configuration conf) throws IOException {
        createStorageID(sd, layoutVersion, conf);
        LOG.info("Updating layout version from {} to {} for storage {}", layoutVersion,
    ...
    private void cleanupDetachDir(File detachDir) throws IOException {
        if (!DataNodeLayoutVersion.supports(LayoutVersion.Feature.APPEND_RBW_DIR,
                layoutVersion
        ) && detachDir.exists() && detachDir.isDirectory()) {

            if (FileUtil.list(detachDir).length != 0) {
                throw new IOException(
                        "Detached directory " + detachDir + " is not empty. Please manually move each file under this " + "directory to the finalized directory if the finalized " + "directory tree does not have the file.");
    ...
    void doRollback(StorageDirectory sd, NamespaceInfo nsInfo) throws IOException {
        ...
        if (!prevDir.exists()) {
            if (DataNodeLayoutVersion.supports(LayoutVersion.Feature.FEDERATION,
                ...
                writeProperties(sd);
                LOG.info("Layout version rolled back to {} for storage {}", HdfsServerConstants.DATANODE_LAYOUT_VERSION,
                        sd.getRoot()
                );
            }
            return;
        ...
        if (!(prevInfo.getLayoutVersion() >= HdfsServerConstants.DATANODE_LAYOUT_VERSION && prevInfo.getCTime() <= nsInfo.getCTime()))  // cannot rollback
            throw new InconsistentFSStateException(sd.getRoot(),
    ...
    void doFinalize(StorageDirectory sd) throws IOException {
        ...
        LOG.info("Finalizing upgrade for storage directory {}.\n   cur LV = {}; " + "cur CTime = {}", dataDirPath,
                this.getLayoutVersion(), this.getCTime()
        );
     ...
     */
    private static void linkAllBlocks(File fromDir, File fromBbwDir, File toDir, int diskLayoutVersion,
    ...
    static void linkBlocks(File fromDir, File toDir, String subdir, int oldLV, HardLink hl,
                           Configuration conf) throws IOException {
    ...
    private static void linkBlocks(File from, File to, int oldLV, HardLink hl, Configuration conf) throws IOException {
        ...
        if (oldLV > DataNodeLayoutVersion.Feature.BLOCKID_BASED_LAYOUT_32_by_32.getInfo().getLayoutVersion() && to.getName()
                .equals(STORAGE_DIR_FINALIZED)) {
        ...
        final int numLinkWorkers = conf.getInt(DFSConfigKeys.DFS_DATANODE_BLOCK_ID_LAYOUT_UPGRADE_THREADS_KEY,
    ...
    static ArrayList<LinkArgs> findDuplicateEntries(ArrayList<LinkArgs> all) {
        ...
        Collections.sort(all, new Comparator<LinkArgs>() {
            ...
            public int compare(LinkArgs a, LinkArgs b) {
                return ComparisonChain.start().compare(a.blockFile(), b.blockFile()).compare(a.src(), b.src())
    ...
    static void linkBlocksHelper(File from, File to, HardLink hl, boolean upgradeToIdBasedLayout, File blockRoot,
                                 List<LinkArgs> idBasedLayoutSingleLinks, Map<File, File> pathCache) throws IOException {
        if (!from.exists()) {
            return;
        }
        if (!from.isDirectory()) {
        ...
        if (otherNames != null) {
            for (int i = 0; i < otherNames.length; i++) {
                linkBlocksHelper(new File(from, otherNames[i]), new File(to, otherNames[i]), hl, upgradeToIdBasedLayout,
