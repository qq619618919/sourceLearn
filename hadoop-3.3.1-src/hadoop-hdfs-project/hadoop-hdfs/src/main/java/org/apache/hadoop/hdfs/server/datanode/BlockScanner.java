...
public class BlockScanner {
     ...
     * // TODO_MA 马中华 注释： 一个 BlockScanner 有多个 VolumeScanner
     */
    ...
    static class Conf {
        ...
        Conf(Configuration conf) {
            ...
            this.skipRecentAccessed = conf.getBoolean(DFS_BLOCK_SCANNER_SKIP_RECENT_ACCESSED,
                    DFS_BLOCK_SCANNER_SKIP_RECENT_ACCESSED_DEFAULT
            );
            if (allowUnitTestSettings) {
                this.resultHandler = (Class<? extends ScanResultHandler>) conf.getClass(
                        INTERNAL_VOLUME_SCANNER_SCAN_RESULT_HANDLER, ScanResultHandler.class);
            } else {
    ...
    public BlockScanner(DataNode datanode, Configuration conf) {
        ...
        setJoinVolumeScannersTimeOutMs(conf.getLong(DFS_BLOCK_SCANNER_VOLUME_JOIN_TIMEOUT_MSEC_KEY,
    ...
    synchronized void markSuspectBlock(String storageId, ExtendedBlock block) {
        if (!isEnabled()) {
            LOG.debug("Not scanning suspicious block {} on {}, because the block " + "scanner is disabled.", block,
                    storageId
            );
            return;
        }
    ...
    public static class Servlet extends HttpServlet {
        ...
        public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
            ...
            if (!blockScanner.isEnabled()) {
                ...
                buffer.append(
                        "Periodic block scanner is not running. " + "Please check the datanode log if this is unexpected.");
            } else {
