 ...
 * synchronized.
 * // TODO_MA 马中华 注释： JournalManager 的普通本地文件系统 具体实现类
 */
...
//    private int outputBufferCapacity = 512 * 1024 * 2;
    ...
    synchronized public EditLogOutputStream startLogSegment(long txid, int layoutVersion) throws IOException {
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 获取 inprogress editlog 文件
             *  current 目录下的 edits_inprogress_startTxId 文件
             */
            currentInProgress = NNStorage.getInProgressEditsFile(sd, txid);

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 生成 EditLogFileOutputStream 输出流
             */
            EditLogOutputStream stm = new EditLogFileOutputStream(conf, currentInProgress, outputBufferCapacity);

            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： Create empty edits logs file.
             */
            stm.create(layoutVersion);

            // TODO_MA 马中华 注释： 返回输出流
            return stm;
        } catch (IOException e) {
    ...
    synchronized public void finalizeLogSegment(long firstTxId, long lastTxId) throws IOException {
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释：edits_inprogress file 重命名成 edits_firstTxId_lastTxId 文件
             */
    ...
    private boolean isStaleInProgressLog(long minTxIdToKeep, EditLogFile log) {
        return log.isInProgress() && !log.getFile().equals(currentInProgress) && log.getFirstTxId() >= minTxIdToKeep &&
    ...
    synchronized public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxnId,
                                                boolean inProgressOk) throws IOException {
        selectInputStreams(streams, fromTxnId, inProgressOk, false);
    }

    @Override
    synchronized public void selectInputStreams(Collection<EditLogInputStream> streams, long fromTxId, boolean inProgressOk,
                                                boolean onlyDurableTxns) throws IOException {
        List<EditLogFile> elfs = matchEditLogs(sd.getCurrentDir());
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    this + ": selecting input streams starting at " + fromTxId + (inProgressOk ? " (inProgress ok) " : " (excluding inProgress) ") + "from among " + elfs.size() + " candidate file(s)");
        }
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释：
    ...
    static void addStreamsToCollectionFromFiles(Collection<EditLogFile> elfs, Collection<EditLogInputStream> streams, long fromTxId,
        ...
        for (EditLogFile elf : elfs) {
            ...
            if (elf.lastTxId < fromTxId) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug(
                            "passing over " + elf + " because it ends at " + elf.lastTxId + ", but we only care about transactions " + "as new as " + fromTxId);
                }
                continue;
            }
            /*************************************************
             ...
             *  注释：
             */
            EditLogFileInputStream elfis = new EditLogFileInputStream(elf.getFile(), elf.getFirstTxId(), elf.getLastTxId(),
                    elf.isInProgress()
            );
