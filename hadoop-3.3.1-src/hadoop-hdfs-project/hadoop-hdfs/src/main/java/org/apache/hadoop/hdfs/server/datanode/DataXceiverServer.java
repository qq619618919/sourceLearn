...
class DataXceiverServer implements Runnable {
    ...
    DataXceiverServer(PeerServer peerServer, Configuration conf, DataNode datanode) {
        ...
        Preconditions.checkArgument(this.maxXceiverCount >= 1,
                DFSConfigKeys.DFS_DATANODE_MAX_RECEIVER_THREADS_KEY + " should not be less than 1."
        );
        ...
        long bandwidthPerSec = conf.getLongBytes(DFSConfigKeys.DFS_DATANODE_DATA_TRANSFER_BANDWIDTHPERSEC_KEY,
         ...
         *  注释：
         */
        bandwidthPerSec = conf.getLongBytes(DFSConfigKeys.DFS_DATANODE_DATA_WRITE_BANDWIDTHPERSEC_KEY,
    ...
    public void run() {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： DataXceiverServer 主要用于监听并接收流式请求， 然后创建并启动 DataXceiver 对象
         *  在 datanode 的内部，会启动 DataXceiverServer 等待客户端的链接请求
         *  当接收到链接请求，则建立 TCP 链接，通过 NIO 通信，封装成 DataXceiver 线程来对这个客户端提供服务
         */
        while (datanode.shouldRun && !datanode.shutdownForUpgrade) {
            try {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 启动 DataXceiver 线程等待接收数据
                 *  DataXceiver 专门为 客户端提供服务的 线程
                 */
