...
class BPOfferService {
     ...
     * // TODO_MA 马中华 注释： 名称空间信息
     */
    NamespaceInfo bpNSInfo;

    /**
     * The registration information for this block pool.
     * This is assigned after the second phase of the handshake.
     * // TODO_MA 马中华 注释： 当前 Datanode 对 NameNode 的注册信息
     */
...
//    }
    ...
    BPOfferService(final String nameserviceId, List<String> nnIds, List<InetSocketAddress> nnAddrs,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动 BPServiceActor
         */
        for (int i = 0; i < nnAddrs.size(); ++i) {

            // TODO_MA 马中华 注释： 创建 BPServiceActor
            this.bpServices.add(new BPServiceActor(nameserviceId, nnIds.get(i), nnAddrs.get(i), lifelineNnAddrs.get(i), this));
        }
    }

    void refreshNNList(String serviceId, List<String> nnIds, ArrayList<InetSocketAddress> addrs,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 如果有新增加的 namenode， 都启动对应的 BPServiceActor
         */
        Set<InetSocketAddress> addedNNs = Sets.difference(newAddrs, oldAddrs);
        for (InetSocketAddress addedNN : addedNNs) {
            BPServiceActor actor = new BPServiceActor(serviceId, nnIds.get(addrs.indexOf(addedNN)), addedNN,
    ...
    private void notifyNamenodeBlock(ExtendedBlock block, BlockStatus status, String delHint, String storageUuid,
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 如果新增，或者删除了数据块，则通过 BPServiceActor 汇报给 NameNode
         *  1、RECEIVED_BLOCK
         *  2、RECEIVING_BLOCK
         *  3、DELETED_BLOCK
         */
    ...
    private void checkBlock(ExtendedBlock block) {
        ...
        Preconditions.checkArgument(block.getBlockPoolId().equals(bpId), "block belongs to BP %s instead of BP %s",
    ...
    void start() {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 启动 BPOfferService 的每个 BPServiceActor
         *  BPServiceActor 对接集群中的一个 namenoe
         */
        for (BPServiceActor actor : bpServices) {
            actor.start();
        }
    ...
    void verifyAndSetNamespaceInfo(BPServiceActor actor, NamespaceInfo nsInfo) throws IOException {
        ...
        try {
            ...
            // TODO_MA 马中华 注释： 如果对应的 blockPool 在该 DataNode 中不存在，则进行初始化
    ...
    DatanodeRegistration createRegistration() {
        ...
        try {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 创建一个注册对象： DatanodeRegistration
             */
    ...
    void updateActorStatesFromHeartbeat(BPServiceActor actor, NNHAStatusHeartbeat nnHaState) {
        ...
        try {
            ...
            // TODO_MA 马中华 注释： 该 actor 链接的 namenode 是否为 active namenode
            ...
            if (nnClaimsActive && !bposThinksActive) {
                ...
                if (!isMoreRecentClaim) {
                    ...
                    // txid.
                    LOG.warn(
                            "NN " + actor + " tried to claim ACTIVE state at txid=" + txid + " but there was already a more recent claim at txid=" + lastActiveClaimTxId);
                    return;
                } else {
                    if (bpServiceToActive == null) {
                        ...
                        LOG.info(
                                "Namenode " + actor + " taking over ACTIVE state from " + bpServiceToActive + " at higher txid=" + txid);
                    }
                    bpServiceToActive = actor;
                }
    ...
    boolean processCommandFromActor(DatanodeCommand cmd, BPServiceActor actor) throws IOException {
        ...
        try {
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： 处理因为心跳接收到的来自于 active namenode 的命令
             */
    ...
    private boolean processCommandFromActive(DatanodeCommand cmd, BPServiceActor actor) throws IOException {
        ...
        switch (cmd.getAction()) {
            ...
            case DatanodeProtocol.DNA_CACHE:
                LOG.info("DatanodeCommand action: DNA_CACHE for " + blockIdCmd.getBlockPoolId() + " of [" + blockIdArrayToString(
                        blockIdCmd.getBlockIds()) + "]");
                dn.getFSDataset().cache(blockIdCmd.getBlockPoolId(), blockIdCmd.getBlockIds());
                break;
            case DatanodeProtocol.DNA_UNCACHE:
                LOG.info("DatanodeCommand action: DNA_UNCACHE for " + blockIdCmd.getBlockPoolId() + " of [" + blockIdArrayToString(
                        blockIdCmd.getBlockIds()) + "]");
            ...
            case DatanodeProtocol.DNA_FINALIZE:
                ...
                assert getBlockPoolId().equals(
                        bp) : "BP " + getBlockPoolId() + " received DNA_FINALIZE " + "for other block pool " + bp;
            ...
            case DatanodeProtocol.DNA_BALANCERBANDWIDTHUPDATE:
                ...
                if (bandwidth > 0) {
                    ...
                    LOG.info(
                            "Updating balance throttler bandwidth from " + dxcs.balanceThrottler.getBandwidth() + " bytes/s " + "to: " + bandwidth + " bytes/s.");
