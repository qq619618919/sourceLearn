...
public class DFSNetworkTopology extends NetworkTopology {
    ...
    public static DFSNetworkTopology getInstance(Configuration conf) {
        ...
        // TODO_MA 马中华 注释： DFSNetworkTopology
        DFSNetworkTopology nt = ReflectionUtils.newInstance(
                conf.getClass(DFSConfigKeys.DFS_NET_TOPOLOGY_IMPL_KEY, DFSConfigKeys.DFS_NET_TOPOLOGY_IMPL_DEFAULT,
                        DFSNetworkTopology.class
                ), conf);

        // TODO_MA 马中华 注释： DFSNetworkTopology 初始化
     ...
     * @return the chosen node
     */
    public Node chooseRandomWithStorageTypeTwoTrial(final String scope, final Collection<Node> excludedNodes,
                                                    StorageType type) {
        netlock.readLock().lock();
        try {
            String searchScope;
            ...
            // next do a two-trial search first trial, call the old method, inherited from NetworkTopology
            /*************************************************
     ...
     */
    @VisibleForTesting
    Node chooseRandomWithStorageType(final String scope, String excludedScope, final Collection<Node> excludedNodes,
                                     StorageType type) {
