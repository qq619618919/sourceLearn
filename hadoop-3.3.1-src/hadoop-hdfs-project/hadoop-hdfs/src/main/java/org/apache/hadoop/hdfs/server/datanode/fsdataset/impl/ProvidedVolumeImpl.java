...
class ProvidedVolumeImpl extends FsVolumeImpl {
    ...
    static class ProvidedBlockPoolSlice {
        ...
        ProvidedBlockPoolSlice(String bpid, ProvidedVolumeImpl volume, Configuration conf) {
            ...
            Class<? extends BlockAliasMap> fmt = conf.getClass(DFSConfigKeys.DFS_PROVIDED_ALIASMAP_CLASS,
        ...
        void fetchVolumeMap(ReplicaMap volumeMap, RamDiskReplicaTracker ramDiskReplicaMap,
                            FileSystem remoteFS) throws IOException {
            ...
            for (FileRegion region : reader) {
                if (containsBlock(providedVolume.baseURI, region.getProvidedStorageLocation().getPath().toUri())) {
                    ...
                    ReplicaInfo newReplica = new ReplicaBuilder(ReplicaState.FINALIZED).setBlockId(
                                    region.getBlock().getBlockId()).setPathPrefix(blockPrefixPath).setPathSuffix(blockSuffix)
                            .setOffset(region.getProvidedStorageLocation().getOffset())
                            .setLength(region.getBlock().getNumBytes())
                            .setGenerationStamp(region.getBlock().getGenerationStamp()).setPathHandle(pathHandle)
                            .setFsVolume(providedVolume).setConf(conf).setRemoteFS(remoteFS).build();
                    ...
                    if (oldReplica == null) {
                        ...
                        LOG.warn(
                                "A block with id " + newReplica.getBlockId() + " exists locally. Skipping PROVIDED replica");
                    }
                }
            }
        }
        ...
        public void compileReport(Collection<ScanInfo> report,
                                  ReportCompiler reportCompiler) throws IOException, InterruptedException {
    ...
    ProvidedVolumeImpl(FsDatasetImpl dataset, String storageID, StorageDirectory sd, FileIoProvider fileIoProvider,
    ...
    @Override
    public ReplicaInPipeline append(String bpid, ReplicaInfo replicaInfo, long newGS,
