 ...
 *  注释： 等待删除的数据块副本集合
 *  InvalidateBlocks 类用于保存等待删除的数据块副本集合。
 *  它使用 HashMap 保存了 Datanode（使用 DatanodeInfo 标识） 到该 Datanode 上所有等待删除的副本集合的映射。
 *  这里使用 LightWeightHashSet 对象保存一个 Datanode 上所有等待删除的副本集合，
 *  LightWeightHashSet 是 Hadoop 定义的占用较少内存的Hashset 的实现。
 */
...
class InvalidateBlocks {
    ...
    private void printBlockDeletionTime() {
        BlockManager.LOG.info("{} is set to {}", DFSConfigKeys.DFS_NAMENODE_STARTUP_DELAY_BLOCK_DELETION_SEC_KEY,
