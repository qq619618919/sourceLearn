...
public class NamenodeWebHdfsMethods {
    ...
    protected void init(final UserGroupInformation ugi, final DelegationParam delegation, final UserParam username,
        ...
        if (useIpcCallq == null) {
            ...
            useIpcCallq = conf.getBoolean(DFSConfigKeys.DFS_WEBHDFS_USE_IPC_CALLQ,
                    DFSConfigKeys.DFS_WEBHDFS_USE_IPC_CALLQ_DEFAULT
            );
        }

        if (LOG.isTraceEnabled()) {
            LOG.trace("HTTP " + op.getValue()
                    .getType() + ": " + op + ", " + path + ", ugi=" + ugi + ", " + username + ", " + doAsUser + Param.toSortedString(
                    ", ", parameters));
        }
    ...
    private <T> T doAs(final UserGroupInformation ugi,
                       final PrivilegedExceptionAction<T> action) throws IOException, InterruptedException {
    ...
    static DatanodeInfo chooseDatanode(final NameNode namenode, final String path, final HttpOpParam.Op op,
                                       final long openOffset, final long blocksize, final String excludeDatanodes,
            ...
            for (String host : StringUtils.getTrimmedStringCollection(excludeDatanodes)) {
                ...
                if (idx != -1) {
                    excludeNode = bm.getDatanodeManager()
                            .getDatanodeByXferAddr(host.substring(0, idx), Integer.parseInt(host.substring(idx + 1)));
                } else {
    ...
    public Token<? extends TokenIdentifier> generateDelegationToken(final UserGroupInformation ugi,
    ...
    private URI redirectURI(ResponseBuilder rb, final NameNode namenode, final UserGroupInformation ugi,
                            final DelegationParam delegation, final UserParam username, final DoAsParam doAsUser,
                            final String path, final HttpOpParam.Op op, final long openOffset, final long blocksize,
        ...
        if (dn == null) {
            throw new IOException(
                    "Failed to find datanode, suggest to check cluster" + " health. excludeDatanodes=" + excludeDatanodes);
        }
    ...
    public Response putRoot(@Context final UserGroupInformation ugi,
                            ...
                            @QueryParam(UnmaskedPermissionParam.NAME) @DefaultValue(UnmaskedPermissionParam.DEFAULT) final UnmaskedPermissionParam unmaskedPermission,
                            ...
                            @QueryParam(StoragePolicyParam.NAME) @DefaultValue(StoragePolicyParam.DEFAULT) final StoragePolicyParam policyName,
                            @QueryParam(ECPolicyParam.NAME) @DefaultValue(ECPolicyParam.DEFAULT) final ECPolicyParam ecpolicy,
                            @QueryParam(NameSpaceQuotaParam.NAME) @DefaultValue(NameSpaceQuotaParam.DEFAULT) final NameSpaceQuotaParam namespaceQuota,
                            @QueryParam(StorageSpaceQuotaParam.NAME) @DefaultValue(StorageSpaceQuotaParam.DEFAULT) final StorageSpaceQuotaParam storagespaceQuota,
                            @QueryParam(StorageTypeParam.NAME) @DefaultValue(StorageTypeParam.DEFAULT) final StorageTypeParam storageType) throws IOException, InterruptedException {
        return put(ugi, delegation, username, doAsUser, ROOT, op, destination, owner, group, permission, unmaskedPermission,
                overwrite, bufferSize, replication, blockSize, modificationTime, accessTime, renameOptions, createParent,
                delegationTokenArgument, aclPermission, xattrName, xattrValue, xattrSetFlag, snapshotName, oldSnapshotName,
                excludeDatanodes, createFlagParam, noredirect, policyName, ecpolicy, namespaceQuota, storagespaceQuota,
                storageType
        );
    }
        ...
        for (Param param : params) {
            if (param.getValue() == null || param.getValueString() == null || param.getValueString().isEmpty()) {
                throw new IllegalArgumentException(
                        "Required param " + param.getName() + " for op: " + op.getValueString() + " is null or empty");
            }
        }
    }
    ...
    public Response put(@Context final UserGroupInformation ugi,
                        ...
                        @QueryParam(StorageSpaceQuotaParam.NAME) @DefaultValue(StorageSpaceQuotaParam.DEFAULT) final StorageSpaceQuotaParam storagespaceQuota,
                        @QueryParam(StorageTypeParam.NAME) @DefaultValue(StorageTypeParam.DEFAULT) final StorageTypeParam storageType) throws IOException, InterruptedException {
        init(ugi, delegation, username, doAsUser, path, op, destination, owner, group, permission, unmaskedPermission,
        ...
        return doAs(ugi, new PrivilegedExceptionAction<Response>() {
            ...
            public Response run() throws IOException, URISyntaxException {
                return put(ugi, delegation, username, doAsUser, path.getAbsolutePath(), op, destination, owner, group,
                        permission, unmaskedPermission, overwrite, bufferSize, replication, blockSize, modificationTime,
                        accessTime, renameOptions, createParent, delegationTokenArgument, aclPermission, xattrName,
                        xattrValue, xattrSetFlag, snapshotName, oldSnapshotName, excludeDatanodes, createFlagParam,
                        noredirect, policyName, ecpolicy, namespaceQuota, storagespaceQuota, storageType
                );
            }
        });
    }

    protected Response put(final UserGroupInformation ugi, final DelegationParam delegation, final UserParam username,
                           final DoAsParam doAsUser, final String fullpath, final PutOpParam op,
                           final DestinationParam destination, final OwnerParam owner, final GroupParam group,
                           final PermissionParam permission, final UnmaskedPermissionParam unmaskedPermission,
                           ...
                           final ModificationTimeParam modificationTime, final AccessTimeParam accessTime,
                           final RenameOptionSetParam renameOptions, final CreateParentParam createParent,
                           final TokenArgumentParam delegationTokenArgument, final AclPermissionParam aclPermission,
                           final XAttrNameParam xattrName, final XAttrValueParam xattrValue,
                           ...
                           final StoragePolicyParam policyName, final ECPolicyParam ecpolicy,
                           final NameSpaceQuotaParam namespaceQuota, final StorageSpaceQuotaParam storagespaceQuota,
                           final StorageTypeParam storageType) throws IOException, URISyntaxException {
        ...
        switch (op.getValue()) {
            case CREATE: {
                final NameNode namenode = (NameNode) context.getAttribute("name.node");
                final URI uri = redirectURI(null, namenode, ugi, delegation, username, doAsUser, fullpath, op.getValue(),
                        -1L, blockSize.getValue(conf), exclDatanodes.getValue(), permission, unmaskedPermission, overwrite,
                        bufferSize, replication, blockSize, createParent, createFlagParam
                );
            ...
            case MKDIRS: {
                FsPermission masked = unmaskedPermission.getValue() == null ? permission.getDirFsPermission() : FsCreateModes.create(
            ...
            case CREATESYMLINK: {
                ...
                cp.createSymlink(destination.getValue(), fullpath, PermissionParam.getDefaultSymLinkFsPermission(),
            ...
            case SETXATTR: {
                ...
                cp.setXAttr(fullpath, XAttrHelper.buildXAttr(xattrName.getXAttrName(), xattrValue.getXAttrValue()),
            ...
            case SETQUOTABYSTORAGETYPE:
                ...
                cp.setQuota(fullpath, HdfsConstants.QUOTA_DONT_SET, storagespaceQuota.getValue(),
    ...
    public Response postRoot(@Context final UserGroupInformation ugi,
                             ...
                             @QueryParam(NoRedirectParam.NAME) @DefaultValue(NoRedirectParam.DEFAULT) final NoRedirectParam noredirect) throws IOException, InterruptedException {
    ...
    protected Response post(final UserGroupInformation ugi, final DelegationParam delegation, final UserParam username,
                            ...
                            final ExcludeDatanodesParam excludeDatanodes, final NewLengthParam newLength,
                            final NoRedirectParam noredirectParam) throws IOException, URISyntaxException {
        final ClientProtocol cp = getRpcClientProtocol();
        ...
        switch (op.getValue()) {
            case APPEND: {
                final NameNode namenode = (NameNode) context.getAttribute("name.node");
                final URI uri = redirectURI(null, namenode, ugi, delegation, username, doAsUser, fullpath, op.getValue(),
                        -1L, -1L, excludeDatanodes.getValue(), bufferSize
                );
                if (!noredirectParam.getValue()) {
                    return Response.temporaryRedirect(uri).type(MediaType.APPLICATION_OCTET_STREAM).build();
                } else {
    ...
    public Response getRoot(@Context final UserGroupInformation ugi,
                            ...
                            @QueryParam(NoRedirectParam.NAME) @DefaultValue(NoRedirectParam.DEFAULT) final NoRedirectParam noredirect,
                            @QueryParam(StartAfterParam.NAME) @DefaultValue(StartAfterParam.DEFAULT) final StartAfterParam startAfter) throws IOException, InterruptedException {
        return get(ugi, delegation, username, doAsUser, ROOT, op, offset, length, renewer, bufferSize, xattrNames,
                xattrEncoding, excludeDatanodes, fsAction, snapshotName, oldSnapshotName, tokenKind, tokenService,
    ...
    public Response get(@Context final UserGroupInformation ugi,
        ...
        init(ugi, delegation, username, doAsUser, path, op, offset, length, renewer, bufferSize, xattrEncoding,
                excludeDatanodes, fsAction, snapshotName, oldSnapshotName, tokenKind, tokenService, startAfter
        );
        ...
        return doAs(ugi, new PrivilegedExceptionAction<Response>() {
            ...
            public Response run() throws IOException, URISyntaxException {
                return get(ugi, delegation, username, doAsUser, path.getAbsolutePath(), op, offset, length, renewer,
                        bufferSize, xattrNames, xattrEncoding, excludeDatanodes, fsAction, snapshotName, oldSnapshotName,
    ...
    protected Response get(final UserGroupInformation ugi, final DelegationParam delegation, final UserParam username,
                           ...
                           final SnapshotNameParam snapshotName, final OldSnapshotNameParam oldSnapshotName,
                           final TokenKindParam tokenKind, final TokenServiceParam tokenService,
                           final NoRedirectParam noredirectParam,
                           final StartAfterParam startAfter) throws IOException, URISyntaxException {
        final Configuration conf = (Configuration) context.getAttribute(JspHelper.CURRENT_CONF);
        final ClientProtocol cp = getRpcClientProtocol();
        ...
        switch (op.getValue()) {
            case OPEN: {
                ...
                final URI uri = redirectURI(rb, namenode, ugi, delegation, username, doAsUser, fullpath, op.getValue(),
                        offset.getValue(), -1L, excludeDatanodes.getValue(), offset, length, bufferSize
                );
                if (!noredirectParam.getValue()) {
                    return rb.status(Status.TEMPORARY_REDIRECT).location(uri).type(MediaType.APPLICATION_OCTET_STREAM)
                            .build();
                } else {
                    final String js = JsonUtil.toJsonString("Location", uri);
            ...
            case GETFILEBLOCKLOCATIONS: {
                ...
                LocatedBlocks locatedBlocks = getRpcClientProtocol().getBlockLocations(fullpath, offsetValue,
            ...
            case GET_BLOCK_LOCATIONS: {
                final long offsetValue = offset.getValue();
                final Long lengthValue = length.getValue();
                final LocatedBlocks locatedblocks = cp.getBlockLocations(fullpath, offsetValue,
                        lengthValue != null ? lengthValue : Long.MAX_VALUE
                );
            ...
            case GETXATTRS: {
                ...
                List<XAttr> xAttrs = cp.getXAttrs(fullpath,
            ...
            case GETSNAPSHOTDIFF: {
                SnapshotDiffReport diffReport = cp.getSnapshotDiffReport(fullpath, oldSnapshotName.getValue(),
    ...
    private static DirectoryListing getDirectoryListing(final ClientProtocol cp, final String p,
    ...
    private static StreamingOutput getListingStream(final ClientProtocol cp, final String p) throws IOException {
        ...
        return new StreamingOutput() {
            ...
            public void write(final OutputStream outstream) throws IOException {
                ...
                out.println(
                        "{\"" + FileStatus.class.getSimpleName() + "es\":{\"" + FileStatus.class.getSimpleName() + "\":[");
                ...
                try {
                    ...
                    ugi.doAs(new PrivilegedExceptionAction<Void>() {
                        ...
                        public Void run() throws IOException {
                            long n = 0;
                            for (DirectoryListing dirList = firstDirList; ; dirList = getDirectoryListing(cp, p,
                                    dirList.getLastName()
                            )) {
    ...
    public Response deleteRoot(@Context final UserGroupInformation ugi,
                               ...
                               @QueryParam(SnapshotNameParam.NAME) @DefaultValue(SnapshotNameParam.DEFAULT) final SnapshotNameParam snapshotName) throws IOException, InterruptedException {
    ...
    protected Response delete(final UserGroupInformation ugi, final DelegationParam delegation, final UserParam username,
                              final DoAsParam doAsUser, final String fullpath, final DeleteOpParam op,
                              final RecursiveParam recursive, final SnapshotNameParam snapshotName) throws IOException {
        final ClientProtocol cp = getRpcClientProtocol();
