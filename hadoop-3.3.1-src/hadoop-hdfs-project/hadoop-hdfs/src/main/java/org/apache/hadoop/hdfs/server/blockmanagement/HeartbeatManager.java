...
class HeartbeatManager implements DatanodeStatistics {
     ...
     * // TODO_MA 马中华 注释： 注册了的 DataNode 的集合
     */
    ...
    HeartbeatManager(final Namesystem namesystem, final BlockManager blockManager, final Configuration conf) {
        ...
        boolean avoidStaleDataNodesForWrite = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_AVOID_STALE_DATANODE_FOR_WRITE_KEY,
                DFSConfigKeys.DFS_NAMENODE_AVOID_STALE_DATANODE_FOR_WRITE_DEFAULT
        );

        // TODO_MA 马中华 注释： 检查时间 5min
        ...
        // TODO_MA 马中华 注释： 连续未收到心跳的最大时间 30s
        long staleInterval = conf.getLong(DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_INTERVAL_KEY,
                DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_INTERVAL_DEFAULT
        );// 30s

        enableLogStaleNodes = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_ENABLE_LOG_STALE_DATANODE_KEY,
        ...
        if (avoidStaleDataNodesForWrite && staleInterval < recheckInterval) {
            ...
            LOG.info(
                    "Setting heartbeat recheck interval to " + staleInterval + " since " + DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_INTERVAL_KEY + " is less than " + DFSConfigKeys.DFS_NAMENODE_HEARTBEAT_RECHECK_INTERVAL_KEY);
        } else {
    ...
    synchronized void updateHeartbeat(final DatanodeDescriptor node, StorageReport[] reports, long cacheCapacity,
         ...
         *  注释：
         */
        blockManager.updateHeartbeat(node, reports, cacheCapacity, cacheUsed, xceiverCount, failedVolumes,
                volumeFailureSummary
        );
        stats.add(node);
    }

    synchronized void updateLifeline(final DatanodeDescriptor node, StorageReport[] reports, long cacheCapacity,
                                     long cacheUsed, int xceiverCount, int failedVolumes,
                                     VolumeFailureSummary volumeFailureSummary) {
        stats.subtract(node);
        ...
        blockManager.updateHeartbeatState(node, reports, cacheCapacity, cacheUsed, xceiverCount, failedVolumes,
                volumeFailureSummary
        );
        stats.add(node);
    }
    ...
    synchronized void startMaintenance(final DatanodeDescriptor node) {
        if (!node.isAlive()) {
            ...
            if (node.isDecommissioned()) {
                ...
                LOG.info(
                        "MinReplicationToBeInMaintenance is set to zero. " + node + " is put in maintenance state" + " immediately.");
                node.setInMaintenance();
            } else {
    ...
    void heartbeatCheck() {
        ...
        while (!allAlive) {
            ...
            synchronized (this) {
                ...
                // TODO_MA 马中华 注释： 遍历每个 DataNode 执行检查
                for (DatanodeDescriptor d : datanodes) {
                    ...
                    // TODO_MA 马中华 注释： 判断是否是 死亡节点, 如果 datanode 的上一次心跳时间举例现在超过了 630s
                    if (dead == null && dm.isDatanodeDead(d)) {

                        // TODO_MA 马中华 注释： 统计死亡datanode节点信息 +1
    ...
    private class Monitor implements Runnable {
        ...
        public void run() {
            ...
            /*************************************************
             * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
             *  注释： while 循环： 每隔 5s 执行一次心跳检查
             */
            while (namesystem.isRunning()) {

                // TODO_MA 马中华 注释： 重启 秒表
                ...
                try {
                    ...
                    // TODO_MA 马中华 注释： 如果连续 30s 都没有收到心跳，则启动检查
                    if (lastHeartbeatCheck + heartbeatRecheckInterval < now) {

                        // TODO_MA 马中华 注释： 启动心跳检查，以后之后的一些处理
                        heartbeatCheck();

                        // TODO_MA 马中华 注释： 记录最近一次检查的时间
