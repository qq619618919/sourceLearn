...
public class ImageServlet extends HttpServlet {
    ...
    private SortedSet<ImageUploadRequest> currentlyDownloadingCheckpoints = Collections.<ImageUploadRequest>synchronizedSortedSet(
            new TreeSet<ImageUploadRequest>());
            ...
            UserGroupInformation.getCurrentUser().doAs(new PrivilegedExceptionAction<Void>() {
                ...
                public Void run() throws Exception {
                    if (parsedParams.isGetImage()) {
                        ...
                        if (parsedParams.shouldFetchLatest()) {
                            ...
                            imageFile = nnImage.getStorage()
                                    .getFsImage(txid, EnumSet.of(NameNodeFile.IMAGE, NameNodeFile.IMAGE_ROLLBACK));
                        }
                        if (imageFile == null) {
                ...
                private void serveFile(File file) throws IOException {
                    ...
                    try {
                        ...
                        // send file
                        DataTransferThrottler throttler = parsedParams.isBootstrapStandby ? getThrottlerForBootstrapStandby(
                                conf) : getThrottler(conf);
    ...
    private void validateRequest(ServletContext context, Configuration conf, HttpServletRequest request,
        ...
        if (UserGroupInformation.isSecurityEnabled() && !isValidRequestor(context, request.getUserPrincipal().getName(), conf)) {
            ...
            sendError(response, HttpServletResponse.SC_FORBIDDEN, errorMsg);
            LOG.warn("Received non-NN/SNN/administrator request for image or edits from " + request.getUserPrincipal()
                    .getName() + " at " + request.getRemoteHost());
            throw new IOException(errorMsg);
        }
        ...
        if (theirStorageInfoString != null && !myStorageInfoString.equals(theirStorageInfoString)) {
            ...
            sendError(response, HttpServletResponse.SC_FORBIDDEN, errorMsg);
            LOG.warn(
                    "Received an invalid request file transfer request " + "from a secondary with storage info " + theirStorageInfoString);
            throw new IOException(errorMsg);
        }
    }
    ...
    public static DataTransferThrottler getThrottler(Configuration conf) {
        long transferBandwidth = conf.getLongBytes(DFSConfigKeys.DFS_IMAGE_TRANSFER_RATE_KEY,
    ...
    public static DataTransferThrottler getThrottlerForBootstrapStandby(Configuration conf) {
        long transferBandwidth = conf.getLongBytes(DFSConfigKeys.DFS_IMAGE_TRANSFER_BOOTSTRAP_STANDBY_RATE_KEY,
    ...
    static boolean isValidRequestor(ServletContext context, String remoteUser, Configuration conf) throws IOException {
        ...
        try {
            validRequestors.add(
                    SecurityUtil.getServerPrincipal(conf.get(DFSConfigKeys.DFS_SECONDARY_NAMENODE_KERBEROS_PRINCIPAL_KEY),
        ...
        try {
            ...
            if (checkRecentImageEnableObj != null) {
                if (checkRecentImageEnableObj instanceof Boolean) {
                    ...
                    LOG.error(
                            "Expecting boolean obj for setting checking recent image, " + "but got " + checkRecentImageEnableObj.getClass() + ". This is " + "unexpected! Setting to default.");
                ...
                public Void run() throws Exception {
                    ...
                    if (state != HAServiceProtocol.HAServiceState.ACTIVE && state != HAServiceProtocol.HAServiceState.OBSERVER) {
                        ...
                        sendError(response, HttpServletResponse.SC_EXPECTATION_FAILED,
                                "Nameode " + request.getLocalAddr() + " is currently not in a state which can " + "accept uploads of new fsimages. State: " + state
                        );
                        return null;
                    }
                    ...
                    if (larger.size() > 0) {
                        sendError(response, HttpServletResponse.SC_CONFLICT,
                                "Another checkpointer is already in the process of uploading a" + " checkpoint made up to transaction ID " + larger.last()
                        );
                        return null;
                    }
                    ...
                    if (!currentlyDownloadingCheckpoints.add(imageRequest)) {
                        sendError(response, HttpServletResponse.SC_CONFLICT,
                                "Either current namenode is checkpointing or another" + " checkpointer is already in the process of " + "uploading a checkpoint made at transaction ID " + txid
                        );
                        return null;
                    }
                    ...
                    if (checkRecentImageEnable && NameNodeFile.IMAGE.equals(
                            parsedParams.getNameNodeFile()) && timeDelta < checkpointPeriod && txid - lastCheckpointTxid < checkpointTxnCount) {
