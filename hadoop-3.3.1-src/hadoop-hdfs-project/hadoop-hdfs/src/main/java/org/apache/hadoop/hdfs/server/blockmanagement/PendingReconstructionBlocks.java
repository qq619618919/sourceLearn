...
/***************************************************
 * // TODO_MA 马中华 注释： PendingReconstructionBlocks 对所有获得更强冗余的块进行簿记。
 * PendingReconstructionBlocks does the bookkeeping of all blocks that gains stronger redundancy.
 *
 ...
 * 3)  a thread that periodically identifies reconstruction-requests that never made it.
 *
 ***************************************************/
class PendingReconstructionBlocks {
    ...
    // It might take anywhere between 5 to 10 minutes before a request is timed out.
    ...
    class PendingReconstructionMonitor implements Runnable {
        ...
        public void run() {
            ...
            // TODO_MA 马中华 注释： 每隔 5 分钟执行一次 pendingReconstructionCheck
            while (fsRunning) {
         ...
         * // TODO_MA 马中华 注释： 遍历所有项并检测超时项
         */
        void pendingReconstructionCheck() {

            synchronized (pendingReconstructions) {

                // TODO_MA 马中华 注释： 遍历 pendingReconstructions
                ...
                while (iter.hasNext()) {
                    ...
                    if (now > pendingBlock.getTimeStamp() + timeout) {
                        ...
                        // TODO_MA 马中华 注释： 如果超时了，则加入到 timedOutItems 集合
                        synchronized (timedOutItems) {
    ...
    void metaSave(PrintWriter out) {
        synchronized (pendingReconstructions) {
            ...
            for (Map.Entry<BlockInfo, PendingBlockInfo> entry : pendingReconstructions.entrySet()) {
                ...
                out.println(block + " StartTime: " + new Time(
                        pendingBlock.timeStamp) + " NumReconstructInProgress: " + pendingBlock.getNumReplicas());
            }
        }
    }
