...
public abstract class Receiver implements DataTransferProtocol {
     ...
     *  注释： Receiver 组件负责接收数据，所以核心组件是 数据输入流 DataInputStream
     */
    ...
    protected final Op readOp() throws IOException {
        ...
        if (version != DataTransferProtocol.DATA_TRANSFER_VERSION) {
            throw new IOException(
                    "Version Mismatch (Expected: " + DataTransferProtocol.DATA_TRANSFER_VERSION + ", Received: " + version + " )");
        }
        return Op.read(in);
    }
        ...
        try {
            ...
            readBlock(PBHelperClient.convert(proto.getHeader().getBaseHeader().getBlock()),
                    ...
                    proto.getOffset(), proto.getLen(), proto.getSendChecksums(), (proto.hasCachingStrategy() ? getCachingStrategy(
                            proto.getCachingStrategy()) : CachingStrategy.newDefaultStrategy()
                    )
            );
        } finally {
    ...
    private void opWriteBlock(DataInputStream in) throws IOException {
        ...
        try {
            ...
            writeBlock(PBHelperClient.convert(proto.getHeader().getBaseHeader().getBlock()),
                    ...
                    PBHelperClient.convert(proto.getHeader().getBaseHeader().getToken()), proto.getHeader().getClientName(),
                    targets, PBHelperClient.convertStorageTypes(proto.getTargetStorageTypesList(), targets.length),
                    PBHelperClient.convert(proto.getSource()), fromProto(proto.getStage()), proto.getPipelineSize(),
                    proto.getMinBytesRcvd(), proto.getMaxBytesRcvd(), proto.getLatestGenerationStamp(),
                    fromProto(proto.getRequestedChecksum()), (proto.hasCachingStrategy() ? getCachingStrategy(
                            proto.getCachingStrategy()) : CachingStrategy.newDefaultStrategy()
                    ), (proto.hasAllowLazyPersist() ? proto.getAllowLazyPersist() : false),
                    (proto.hasPinning() ? proto.getPinning() : false),
                    (PBHelperClient.convertBooleanList(proto.getTargetPinningsList())), proto.getStorageId(),
    ...
    private void opTransferBlock(DataInputStream in) throws IOException {
        ...
        try {
            ...
            final StorageType[] targetStorageTypes = PBHelperClient.convertStorageTypes(proto.getTargetStorageTypesList(),
                    targets.length
            );
            transferBlock(block, PBHelperClient.convert(proto.getHeader().getBaseHeader().getToken()),
                    proto.getHeader().getClientName(), targets, targetStorageTypes,
                    proto.getTargetStorageIdsList().toArray(new String[0])
            );
        } finally {
            if (traceScope != null) traceScope.close();
    ...
    private void opRequestShortCircuitFds(DataInputStream in) throws IOException {
        ...
        try {
            requestShortCircuitFds(PBHelperClient.convert(proto.getHeader().getBlock()),
                    PBHelperClient.convert(proto.getHeader().getToken()), slotId, proto.getMaxVersion(),
    ...
    private void opReplaceBlock(DataInputStream in) throws IOException {
        ...
        try {
            ...
            replaceBlock(PBHelperClient.convert(proto.getHeader().getBlock()),
                    PBHelperClient.convertStorageType(proto.getStorageType()), PBHelperClient.convert(proto.getHeader().getToken()),
                    proto.getDelHint(), PBHelperClient.convert(proto.getSource()), proto.getStorageId()
            );
        } finally {
            if (traceScope != null) traceScope.close();
        }
    ...
    private void opBlockChecksum(DataInputStream in) throws IOException {
        ...
        try {
            blockChecksum(PBHelperClient.convert(proto.getHeader().getBlock()),
                    PBHelperClient.convert(proto.getHeader().getToken()), PBHelperClient.convert(proto.getBlockChecksumOptions())
            );
        } finally {
            if (traceScope != null) traceScope.close();
        }
    ...
    private void opStripedBlockChecksum(DataInputStream dis) throws IOException {
        ...
        TraceScope traceScope = continueTraceSpan(proto.getHeader(), proto.getClass().getSimpleName());
        StripedBlockInfo stripedBlockInfo = new StripedBlockInfo(PBHelperClient.convert(proto.getHeader().getBlock()),
                PBHelperClient.convert(proto.getDatanodes()), PBHelperClient.convertTokens(proto.getBlockTokensList()),
