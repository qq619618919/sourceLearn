...
public class DatanodeManager {
     ...
     * <p>
     * Done by storing a set of {@link DatanodeDescriptor} objects, sorted by storage id.
     * In order to keep the storage map consistent it tracks all storages ever registered with the namenode.
     * A descriptor corresponding to a specific storage id can be
     * <ul>
     * <li>added to the map if it is a new storage id;</li>
     * <li>updated with a new datanode started as a replacement for the old one with the same storage id; and </li>
     * <li>removed if and only if an existing datanode is restarted to serve a different storage id.</li>
     * </ul> <br>
     * <p>
     * Mapping: StorageID -> DatanodeDescriptor
     * // TODO_MA 马中华 注释： 维护注册 DataNode 数据的 Map 数据结构
     * // TODO_MA 马中华 注释： 维护 StorageId(一个 String uuid) -> DatanodeDescriptor 的映射关系
     */
    ...
    // TODO_MA 马中华 注释： 维护 host -> DatanodeDescriptor 的映射关系。
     ...
     * temporarily disabled when a high percentage of the nodes are marked as stale.
     */
    ...
    DatanodeManager(final BlockManager blockManager, final Namesystem namesystem,
        ...
        // TODO_MA 马中华 注释： dfs.use.dfs.network.topology = true
        this.useDfsNetworkTopology = conf.getBoolean(DFSConfigKeys.DFS_USE_DFS_NETWORK_TOPOLOGY_KEY,
                DFSConfigKeys.DFS_USE_DFS_NETWORK_TOPOLOGY_DEFAULT
        );

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 机架管理器 = DFSNetworkTopology
         */
        ...
        // TODO_MA 马中华 注释： dfs.datanode.peer.stats.enabled = false
        this.dataNodePeerStatsEnabled = conf.getBoolean(DFSConfigKeys.DFS_DATANODE_PEER_STATS_ENABLED_KEY,
                DFSConfigKeys.DFS_DATANODE_PEER_STATS_ENABLED_DEFAULT
        );

        // TODO_MA 马中华 注释： dfs.datanode.fileio.profiling.sampling.percentage = 0
        this.dataNodeDiskStatsEnabled = Util.isDiskStatsEnabled(
        ...
        // TODO_MA 马中华 注释： 如果开启了，则创建 SlowPeerTracker 用来跟踪慢速 DataNode
        this.slowPeerTracker = dataNodePeerStatsEnabled ? new SlowPeerTracker(conf, timer) : null;

        // TODO_MA 马中华 注释： dfs.namenode.block-placement-policy.exclude-slow-nodes.enabled = false
        this.excludeSlowNodesEnabled = conf.getBoolean(DFS_NAMENODE_BLOCKPLACEMENTPOLICY_EXCLUDE_SLOW_NODES_ENABLED_KEY,
                DFS_NAMENODE_BLOCKPLACEMENTPOLICY_EXCLUDE_SLOW_NODES_ENABLED_DEFAULT
        );
        this.maxSlowPeerReportNodes = conf.getInt(DFSConfigKeys.DFS_NAMENODE_MAX_SLOWPEER_COLLECT_NODES_KEY,
                DFSConfigKeys.DFS_NAMENODE_MAX_SLOWPEER_COLLECT_NODES_DEFAULT
        );
        this.slowPeerCollectionInterval = conf.getTimeDuration(DFSConfigKeys.DFS_NAMENODE_SLOWPEER_COLLECT_INTERVAL_KEY,
                DFSConfigKeys.DFS_NAMENODE_SLOWPEER_COLLECT_INTERVAL_DEFAULT, TimeUnit.MILLISECONDS
        );
        ...
        // TODO_MA 马中华 注释： dfs.datanode.https.address = 0.0.0.0:9865
        this.defaultInfoSecurePort = NetUtils.createSocketAddr(conf.getTrimmed(DFSConfigKeys.DFS_DATANODE_HTTPS_ADDRESS_KEY,
                DFSConfigKeys.DFS_DATANODE_HTTPS_ADDRESS_DEFAULT
        )).getPort();
        // TODO_MA 马中华 注释： dfs.datanode.ipc.address = 0.0.0.0:9867
        this.defaultIpcPort = NetUtils.createSocketAddr(
                        conf.getTrimmed(DFSConfigKeys.DFS_DATANODE_IPC_ADDRESS_KEY, DFSConfigKeys.DFS_DATANODE_IPC_ADDRESS_DEFAULT))
                .getPort();

        // TODO_MA 马中华 注释： 默认实现： dfs.namenode.hosts.provider.classname = HostFileManager
        this.hostConfigManager = ReflectionUtils.newInstance(
                conf.getClass(DFSConfigKeys.DFS_NAMENODE_HOSTS_PROVIDER_CLASSNAME_KEY, HostFileManager.class,
                        HostConfigManager.class
                ), conf);
        try {
            // TODO_MA 马中华 注释： dfs.hosts.exclude 和 dfs.hosts
            this.hostConfigManager.refresh();
        } catch (IOException e) {
            LOG.error("error reading hosts files: ", e);
        }

        // TODO_MA 马中华 注释： net.topology.node.switch.mapping.impl = ScriptBasedMapping
        this.dnsToSwitchMapping = ReflectionUtils.newInstance(
                conf.getClass(DFSConfigKeys.NET_TOPOLOGY_NODE_SWITCH_MAPPING_IMPL_KEY, ScriptBasedMapping.class,
                        DNSToSwitchMapping.class
                ), conf);

        // TODO_MA 马中华 注释： dfs.namenode.reject-unresolved-dn-topology-mapping = false
        ...
        // TODO_MA 马中华 注释： dfs.heartbeat.interval = 3s, HDFS 默认主从节点的心跳间隔时间
        heartbeatIntervalSeconds = conf.getTimeDuration(DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_KEY,
                DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_DEFAULT, TimeUnit.SECONDS
        );

        // TODO_MA 马中华 注释： heartbeat 检查间隔时间，默认 5min
        heartbeatRecheckInterval = conf.getInt(DFSConfigKeys.DFS_NAMENODE_HEARTBEAT_RECHECK_INTERVAL_KEY,
                DFSConfigKeys.DFS_NAMENODE_HEARTBEAT_RECHECK_INTERVAL_DEFAULT
        ); // 5 minutes
        // TODO_MA 马中华 注释： 10次连续心跳没有收到 + 2次 heartbeat 检查，总共 630s
        ...
        // TODO_MA 马中华 注释： 删除 block 的限制为 dfs.block.invalidate.limit = 1000 与 20 次心跳间隔 之间的最大值.
        // TODO_MA 马中华 注释： 也就是说如果心跳间隔小与 50秒一次的话, 那么限制数量为1000, 如果心跳间隔大于 50s ,那么就是 20*心跳间隔的时间.
        final int configuredBlockInvalidateLimit = conf.getInt(DFSConfigKeys.DFS_BLOCK_INVALIDATE_LIMIT_KEY,
        ...
        LOG.info(
                DFSConfigKeys.DFS_BLOCK_INVALIDATE_LIMIT_KEY + ": configured=" + configuredBlockInvalidateLimit + ", counted=" + countedBlockInvalidateLimit + ", effected=" + blockInvalidateLimit);
        ...
        LOG.info(
                DFSConfigKeys.DFS_NAMENODE_DATANODE_REGISTRATION_IP_HOSTNAME_CHECK_KEY + "=" + checkIpHostnameInRegistration);

        this.avoidStaleDataNodesForRead = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_AVOID_STALE_DATANODE_FOR_READ_KEY,
                DFSConfigKeys.DFS_NAMENODE_AVOID_STALE_DATANODE_FOR_READ_DEFAULT
        );
        this.readConsiderLoad = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERLOAD_KEY,
                DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERLOAD_DEFAULT
        );
        this.readConsiderStorageType = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERSTORAGETYPE_KEY,
                DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERSTORAGETYPE_DEFAULT
        );
        if (readConsiderLoad && readConsiderStorageType) {
            LOG.warn(
                    "{} and {} are incompatible and only one can be enabled. " + "Both are currently enabled. {} will be ignored.",
                    DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERLOAD_KEY,
                    DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERSTORAGETYPE_KEY,
                    DFSConfigKeys.DFS_NAMENODE_READ_CONSIDERSTORAGETYPE_KEY
            );
        }
        this.avoidStaleDataNodesForWrite = conf.getBoolean(DFSConfigKeys.DFS_NAMENODE_AVOID_STALE_DATANODE_FOR_WRITE_KEY,
        ...
        Preconditions.checkArgument((ratioUseStaleDataNodesForWrite > 0 && ratioUseStaleDataNodesForWrite <= 1.0f),
                DFSConfigKeys.DFS_NAMENODE_USE_STALE_DATANODE_FOR_WRITE_RATIO_KEY + " = '" + ratioUseStaleDataNodesForWrite + "' is invalid. " + "It should be a positive non-zero float value, not greater than 1.0f."
        );
    ...
    private static long getStaleIntervalFromConf(Configuration conf, long heartbeatExpireInterval) {
        ...
        Preconditions.checkArgument(staleInterval > 0,
                DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_INTERVAL_KEY + " = '" + staleInterval + "' is invalid. " + "It should be a positive non-zero value."
        );

        final long heartbeatIntervalSeconds = conf.getTimeDuration(DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_KEY,
                DFSConfigKeys.DFS_HEARTBEAT_INTERVAL_DEFAULT, TimeUnit.SECONDS
        );
        ...
        if (staleInterval < minStaleInterval) {
            LOG.warn(
                    "The given interval for marking stale datanode = " + staleInterval + ", which is less than " + DFSConfigKeys.DFS_NAMENODE_STALE_DATANODE_MINIMUM_INTERVAL_DEFAULT + " heartbeat intervals. This may cause too frequent changes of " + "stale states of DataNodes since a heartbeat msg may be missing " + "due to temporary short-term failures. Reset stale interval to " + minStaleInterval + ".");
            staleInterval = minStaleInterval;
        }
        if (staleInterval > heartbeatExpireInterval) {
            LOG.warn(
                    "The given interval for marking stale datanode = " + staleInterval + ", which is larger than heartbeat expire interval " + heartbeatExpireInterval + ".");
        }
    ...
    public void sortLocatedBlocks(final String targetHost, final List<LocatedBlock> locatedBlocks) {
        Comparator<DatanodeInfo> comparator = avoidStaleDataNodesForRead ? new DFSUtil.ServiceAndStaleComparator(
                staleInterval) : new DFSUtil.ServiceComparator();
        // sort located block
    ...
    private void sortLocatedBlock(final LocatedBlock lb, String targetHost, Comparator<DatanodeInfo> comparator) {
        ...
        if (nonDatanodeReader) {
            networktopology.sortByDistanceUsingNetworkLocation(client, lb.getLocations(), activeLen,
    ...
    public DatanodeStorageInfo[] getDatanodeStorageInfos(DatanodeID[] datanodeID, String[] storageIDs, String format,
                                                         Object... args) throws UnregisteredNodeException {
        ...
        if (datanodeID.length != storageIDs.length) {
            ...
            final String err = (storageIDs.length == 0 ? "Missing storageIDs: It is likely that the HDFS client," + " who made this call, is running in an older version of Hadoop" + "(pre-2.0.0-alpha)  which does not support storageIDs." : "Length mismatched: storageIDs.length=" + storageIDs.length + " != "
    ...
    void removeDeadDatanode(final DatanodeID nodeID, boolean removeBlocksFromBlockMap) {
        ...
        if (d != null && isDatanodeDead(d)) {
            NameNode.stateChangeLog.info(
                    "BLOCK* removeDeadDatanode: lost heartbeat from " + d + ", removeBlocksFromBlockMap " + removeBlocksFromBlockMap);
    ...
    void addDatanode(final DatanodeDescriptor node) {
        ...
        // TODO_MA 马中华 注释： 完成注册，将该注册的 Datanode put 到 datanodeMap 中
        // TODO_MA 马中华 注释： datanodeMap 这是正经的注册集合
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 加入机架拓扑
         *  networktopology 做机架管理的 Rack  Host
         */
        networktopology.add(node); // may throw InvalidTopologyException

        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 加入主机 和 datanode 的映射
         *  hostname, datanode , ip 的映射
         */
    ...
    private void wipeDatanode(final DatanodeID node) {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    getClass().getSimpleName() + ".wipeDatanode(" + node + "): storage " + key + " is removed from datanodeMap.");
        }
    }
    ...
    private String resolveNetworkLocationWithFallBackToDefaultLocation(DatanodeID node) {
        ...
        try {
            ...
            LOG.error(
                    "Unresolved topology mapping. Using " + NetworkTopology.DEFAULT_RACK + " for host " + node.getHostName());
    ...
    private List<String> getNetworkDependenciesWithDefault(DatanodeInfo node) {
        ...
        try {
            ...
            LOG.error(
                    "Unresolved dependency mapping for host " + node.getHostName() + ". Continuing with an empty dependency list");
    ...
    public void registerDatanode(
            DatanodeRegistration nodeReg) throws DisallowedDatanodeException, UnresolvedTopologyException {
        ...
        try {
            ...
            // TODO_MA 马中华 注释： 如果不在白名单中，则不允许注册
            ...
            // TODO_MA 马中华 注释： 获取数据结构中，可能存在的 DataNode 描述信息
            // TODO_MA 马中华 注释： 如果存在的话，证明之前注册过了
            DatanodeDescriptor nodeS = getDatanode(nodeReg.getDatanodeUuid());
            DatanodeDescriptor nodeN = host2DatanodeMap.getDatanodeByXferAddr(nodeReg.getIpAddr(), nodeReg.getXferPort());

            // TODO_MA 马中华 注释： 该 DataNode 使用了不同的 StorageID 来注册
            if (nodeN != null && nodeN != nodeS) {
                NameNode.LOG.info("BLOCK* registerDatanode: " + nodeN);
                // nodeN previously served a different data storage,
                // which is not served by anybody anymore.
                // TODO_MA 马中华 注释： 移除之前注册的 信息
                removeDatanode(nodeN);
                // physically remove node from datanodeMap
                // TODO_MA 马中华 注释： 从 datanodeMap 物理删除节点
                wipeDatanode(nodeN);
                nodeN = null;
            }

            // TODO_MA 马中华 注释： 如果之前存在，已经注册过了
            if (nodeS != null) {
                if (nodeN == nodeS) {
                     ...
                     */
                    NameNode.stateChangeLog.info(
                            "BLOCK* registerDatanode: " + nodeS + " is replaced by " + nodeReg + " with the same storageID " + nodeReg.getDatanodeUuid());
                }
            ...
            try {
                ...
                /*************************************************
                 * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                 *  注释： 向 BlockReportLeaseManager 执行 DataNode 的注册
                 */
                blockManager.getBlockReportLeaseManager().register(nodeDescr);

                // also treat the registration message as a heartbeat
                // no need to update its timestamp because its is done when the descriptor is created
                // TODO_MA 马中华 注释： 加入心跳管理机制
     ...
     * host files. If a node isn't allowed, hostConfigManager.isIncluded returns false and the node can't be used.
     * If a node is allowed and the desired admin operation is defined, it will transition to the desired admin state.
     * If a node is allowed and upgrade domain is defined, the upgrade domain will be set on the node.
     * To use maintenance mode or upgrade domain, set DFS_NAMENODE_HOSTS_PROVIDER_CLASSNAME_KEY to
    ...
    private void refreshDatanodes() {
        ...
        // TODO_MA 马中华 注释： 遍历每个 DatanodeDescriptor
        for (DatanodeDescriptor node : copy.values()) {

            // TODO_MA 马中华 注释： DatanNode 表示不可以链接到 NameNode
            // Check if not include.
            if (!hostConfigManager.isIncluded(node)) {
                // TODO_MA 马中华 注释：  在撤销节点时， 先在exclude文件中添加， 撤销结束后再从include文件中删除
    ...
    public List<DatanodeDescriptor> getDatanodeListForReport(final DatanodeReportType type) {
        ...
        synchronized (this) {
            ...
            for (DatanodeDescriptor dn : datanodeMap.values()) {
                ...
                if (((listLiveNodes && !isDead) || (listDeadNodes && isDead) || (listDecommissioningNodes && isDecommissioning) || (listEnteringMaintenanceNodes && isEnteringMaintenance) || (listInMaintenanceNodes && isInMaintenance)) && hostConfigManager.isIncluded(
                        dn)) {
                    nodes.add(dn);
                }
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug(
                    "getDatanodeListForReport with " + "includedNodes = " + hostConfigManager.getIncludes() + ", excludedNodes = " + hostConfigManager.getExcludes() + ", foundNodes = " + foundNodes + ", nodes = " + nodes);
        }
        return nodes;
    }
    ...
    private void addCacheCommands(String blockPoolId, DatanodeDescriptor nodeinfo, List<DatanodeCommand> cmds) {
        ...
        if (shouldSendCachingCommands && ((nowMs - nodeinfo.getLastCachingDirectiveSentTimeMs()) >= timeBetweenResendingCachingDirectivesMs)) {
            DatanodeCommand pendingCacheCommand = getCacheCommand(nodeinfo.getPendingCached(), DatanodeProtocol.DNA_CACHE,
                    blockPoolId
            );
    ...
    public DatanodeCommand[] handleHeartbeat(DatanodeRegistration nodeReg, StorageReport[] reports, final String blockPoolId,
                                             long cacheCapacity, long cacheUsed, int xceiverCount, int maxTransfers,
                                             int failedVolumes, VolumeFailureSummary volumeFailureSummary,
                                             @Nonnull SlowPeerReports slowPeers,
                                             @Nonnull SlowDiskReports slowDisks) throws IOException {

        // TODO_MA 马中华 注释： 获取注册对象
        ...
        // TODO_MA 马中华 注释： 后面都是计算，看看是否有一些跟 数据块管理有关的工作需要这个 datanode 去做
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： 开始构建各种 Command 返回给 DataNode
         */
     ...
     */
    public void handleLifeline(DatanodeRegistration nodeReg, StorageReport[] reports, String blockPoolId, long cacheCapacity,
        ...
        heartbeatManager.updateLifeline(nodeinfo, reports, cacheCapacity, cacheUsed, xceiverCount, failedVolumes,
                volumeFailureSummary
        );
    }
