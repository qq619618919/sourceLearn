...
public class SaslDataTransferServer {
    ...
    /**
     * // TODO_MA 马中华 注释： 代表服务器从对等方接收 SASL 协商。
    ...
    public IOStreamPair receive(Peer peer, OutputStream underlyingOut, InputStream underlyingIn, int xferPort,
        ...
        if (dnConf.getEncryptDataTransfer()) {
            ...
            LOG.debug("SASL server skipping handshake in unsecured configuration for " + "peer = {}, datanodeId = {}", peer,
                    datanodeId
            );
            ...
            LOG.debug("SASL server skipping handshake in secured configuration for " + "peer = {}, datanodeId = {}", peer,
                    datanodeId
            );
            return new IOStreamPair(underlyingIn, underlyingOut);
        }
            ...
            LOG.debug(
                    "SASL server skipping handshake in secured configuration with no SASL " + "protection configured for peer = {}, datanodeId = {}",
                    peer, datanodeId
            );
            return new IOStreamPair(underlyingIn, underlyingOut);
        }
     ...
     * @throws IOException for any error
     */
    private IOStreamPair getEncryptedStreams(Peer peer, OutputStream underlyingOut,
    ...
    private byte[] getEncryptionKeyFromUserName(String userName) throws IOException {
        ...
        if (nameComponents.length != 3) {
            throw new IOException(
                    "Provided name '" + userName + "' has " + nameComponents.length + " components instead of the expected 3.");
        }
     ...
     * @throws IOException for any error
     */
    private IOStreamPair doSaslHandshake(Peer peer, OutputStream underlyingOut, InputStream underlyingIn,
        ...
        try {
                ...
                if (LOG.isDebugEnabled()) {
                    if (cipherOption == null) {
                        ...
                        if (cipherSuites != null && !cipherSuites.isEmpty()) {
                            ...
                            LOG.debug("Server accepts cipher suites {}, " + "but client {} does not accept any of them",
                                    cipherSuites, peer.getRemoteAddressString()
                            );
                        }
                    } else {
                        LOG.debug("Server using cipher suite {} with client {}", cipherOption.getCipherSuite().getName(),
            ...
            // stream pair.
            return cipherOption != null ? createStreamPair(dnConf.getConf(), cipherOption, underlyingOut, underlyingIn,
                    true
