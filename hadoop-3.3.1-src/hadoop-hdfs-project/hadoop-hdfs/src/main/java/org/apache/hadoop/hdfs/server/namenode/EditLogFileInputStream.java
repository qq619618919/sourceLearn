 ...
 * reads edits from a file. That file may be either on the local disk or accessible via a URL.
 */
...
public class EditLogFileInputStream extends EditLogInputStream {
    ...
    public static EditLogInputStream fromByteString(ByteString bytes, long startTxId, long endTxId, boolean inProgress) {
        return new EditLogFileInputStream(new ByteStringLog(bytes, String.format("ByteStringEditLog[%d, %d]", startTxId, endTxId)), startTxId,
                endTxId, inProgress);
    }
    ...
    private EditLogFileInputStream(LogSource log, long firstTxId, long lastTxId, boolean isInProgress) {
        ...
        // TODO_MA 马中华 注释： 最大值 50 * 1024 * 1024  ==> 50M
    ...
    private void init(boolean verifyLayoutVersion) throws LogHeaderCorruptException, IOException {
        ...
        try {
            ...
            if (NameNodeLayoutVersion.supports(LayoutVersion.Feature.ADD_LAYOUT_FLAGS,
                    ...
                    if ((txId >= lastTxId) && (lastTxId != HdfsServerConstants.INVALID_TXID)) {
                        ...
                        if (skipAmt > 0) {
                            if (LOG.isDebugEnabled()) {
                                LOG.debug(
                                        "skipping " + skipAmt + " bytes at the end " + "of edit log  '" + getName() + "': reached txid " + txId + " out of " + lastTxId);
                            }
                            tracker.clearLimit();
    ...
    static int readLogVersion(DataInputStream in, boolean verifyLayoutVersion) throws IOException, LogHeaderCorruptException {
        ...
        if (verifyLayoutVersion && (logVersion < HdfsServerConstants.NAMENODE_LAYOUT_VERSION || // future version
