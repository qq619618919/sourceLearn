...
class BlockReportLeaseManager {
    ...
    BlockReportLeaseManager(Configuration conf) {
        this(conf.getInt(DFSConfigKeys.DFS_NAMENODE_MAX_FULL_BLOCK_REPORT_LEASES,
    ...
    public synchronized long requestLease(DatanodeDescriptor dn) {
        ...
        if (node == null) {
            LOG.warn("DN {} ({}) requested a lease even though it wasn't yet " + "registered.  Registering now.", dn.getDatanodeUuid(),
                    dn.getXferAddr());
        ...
        if (node.leaseId != 0) {
            ...
            LOG.debug("Removing existing BR lease 0x{} for DN {} in order to " + "issue a new one.", Long.toHexString(node.leaseId),
                    dn.getDatanodeUuid());
        }
        remove(node);
        ...
        if (numPending >= maxPending) {
            if (LOG.isDebugEnabled()) {
                ...
                LOG.debug("Can't create a new BR lease for DN {}, because " + "numPending equals maxPending at {}.  Current leases: {}",
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Created a new BR lease 0x{} for DN {}.  numPending = {}", Long.toHexString(node.leaseId), dn.getDatanodeUuid(),
                    numPending);
        }
        return node.leaseId;
    }
    ...
    public synchronized boolean checkLease(DatanodeDescriptor dn, long monotonicNowMs, long id) {
        ...
        if (node.leaseId == 0) {
            LOG.warn("BR lease 0x{} is not valid for DN {}, because the DN " + "is not in the pending set.", Long.toHexString(id),
                    dn.getDatanodeUuid());
            return false;
        }
        ...
        if (id != node.leaseId) {
            LOG.warn("BR lease 0x{} is not valid for DN {}.  Expected BR lease 0x{}.", Long.toHexString(id), dn.getDatanodeUuid(),
