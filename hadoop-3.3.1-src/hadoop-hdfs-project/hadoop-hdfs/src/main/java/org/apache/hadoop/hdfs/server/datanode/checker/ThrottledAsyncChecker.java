...
public class ThrottledAsyncChecker<K, V> implements AsyncChecker<K, V> {
    ...
    public ThrottledAsyncChecker(final Timer timer, final long minMsBetweenChecks, final long diskCheckTimeout,
    ...
    public synchronized Optional<ListenableFuture<V>> schedule(Checkable<K, V> target, K context) {
        ...
        if (result != null) {
            ...
            if (msSinceLastCheck < minMsBetweenChecks) {
                LOG.debug("Skipped checking {}. Time since last check {}ms " + "is less than the min gap {}ms.", target,
                        msSinceLastCheck, minMsBetweenChecks
                );
                return Optional.empty();
            }
        }
