...
/**
 * // TODO_MA 马中华 注释： NameNode 中的存储目录管理器
...
public class NNStorage extends Storage implements Closeable, StorageErrorReporter {
    ...
    void attemptRestoreRemovedStorage() {
        ...
        synchronized (this.restorationLock) {
            LOG.info("NNStorage.attemptRestoreRemovedStorage: check removed(failed) " + "storage. removedStorages size = {}",
                    removedStorageDirs.size()
            );
            for (StorageDirectory sd : this.removedStorageDirs) {
                File root = sd.getRoot();
                LOG.info("currently disabled dir {}; type={} ;canwrite={}", root.getAbsolutePath(), sd.getStorageDirType(),
    ...
    synchronized void setStorageDirectories(Collection<URI> fsNameDirs, Collection<URI> fsEditsDirs,
        ...
        for (URI dirName : fsNameDirs) {
            ...
            if (dirName.getScheme().compareTo("file") == 0) {
                ...
                StorageDirectory sd = new StorageDirectory(new File(dirName.getPath()), dirType,
                        sharedEditsDirs.contains(dirName), new FsPermission(
                        conf.get(DFSConfigKeys.DFS_NAMENODE_NAME_DIR_PERMISSION_KEY,
    ...
    static long readTransactionIdFile(StorageDirectory sd) throws IOException {
        // TODO_MA 马中华 注释： 读取到 current 目录下 seen_txid 文件中的内容
    ...
    private void format(StorageDirectory sd) throws IOException {
        ...
        // TODO_MA 马中华 注释： 先清空 current 目录，然后再创建
        sd.clearDirectory(); // create currrent dir

        // TODO_MA 马中华 注释： 写各种属性值到 VERSION 文件
        writeProperties(sd);

        // TODO_MA 马中华 注释： 创建 seen_txid 文件，并且 将 txid 写入 seen_txid 文件
    ...
    public void format(NamespaceInfo nsInfo) throws IOException {
        Preconditions.checkArgument(
                nsInfo.getLayoutVersion() == 0 || nsInfo.getLayoutVersion() == HdfsServerConstants.NAMENODE_LAYOUT_VERSION,
        ...
        for (Iterator<StorageDirectory> it = dirIterator(); it.hasNext(); ) {
            ...
            // TODO_MA 马中华 注释： 迭代出一个 StorageDirectory
            StorageDirectory sd = it.next();
    ...
    public static String getInProgressEditsFileName(long startTxId) {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： current 目录下的 edits_inprogress_startTxId 文件
         *  // TODO_MA 马中华 注释： edits_inprogress_startTxId
         */
    ...
    static File getInProgressEditsFile(StorageDirectory sd, long startTxId) {
         ...
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： current 目录下的 edits_inprogress_startTxId 文件
         *  file:///home/bidata/data/hadoopdata/dfs/name/current/  这个目录，存储了 当前 namenode 的所有磁盘元数据
         *  edits_inprogress_startTxId
         */
        ...
        if (startOpt == StartupOption.UPGRADE || startOpt == StartupOption.UPGRADEONLY) {
            ...
            if (!NameNodeLayoutVersion.supports(LayoutVersion.Feature.FEDERATION, layoutVersion)) {
                ...
                if (startOpt.getClusterId() != null && !startOpt.getClusterId().equals(getClusterID())) {
                    LOG.warn("Clusterid mismatch - current clusterid: {}, Ignoring " + "given clusterid: {}", getClusterID(),
    ...
    static String newBlockPoolID() throws UnknownHostException {
        ...
        // TODO_MA 马中华 注释： BlockPoolID 生成规则
    ...
    FSImageStorageInspector readAndInspectDirs(EnumSet<NameNodeFile> fileTypes,
        ...
        // (ie edits_<txnid>) then use the new inspector, which will ignore the old format dirs.
    ...
    public void updateNameDirSize() {
        ...
        /*************************************************
         * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
         *  注释： dirIterator() 方法返回的就是 List<StorageDirectory> storageDirs 的迭代器
         */
        for (Iterator<StorageDirectory> it = dirIterator(); it.hasNext(); ) {
            StorageDirectory sd = it.next();
            if (!sd.isShared()) {
