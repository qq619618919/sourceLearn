...
public class FSEditLogLoader {
     ...
     * along.
     */
    long loadFSEdits(EditLogInputStream edits, long expectedStartingTxId, long maxTxnsToRead, StartupOption startOpt,
        ...
        try {
            ...
            if (preLogAction.shouldLog()) {
                FSImage.LOG.info(
                        "Start loading edits file " + edits.getName() + " maxTxnsToRead = " + maxTxnsToRead + LogThrottlingHelper.getLogSupressionMessage(
                                preLogAction));
            }
            /*************************************************
            ...
            LogAction postLogAction = loadEditsLogHelper.record("post", endTime, numEdits, edits.length(),
                    endTime - startTime
            );
            if (postLogAction.shouldLog()) {
                FSImage.LOG.info(
                        "Loaded {} edits file(s) (the last named {}) of " + "total size {}, total edits {}, total load time {} ms",
                        postLogAction.getCount(), edits.getName(), postLogAction.getStats(1).getSum(),
                        postLogAction.getStats(0).getSum(), postLogAction.getStats(2).getSum()
                );
            }
    ...
    /*************************************************
     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
     *  注释： 通过输入流，不能的读取 editlog 然后 apply 到 standby 的本地内存中
     */
    long loadEditRecords(EditLogInputStream in, boolean closeOnExit, long expectedStartingTxId, long maxTxnsToRead,
        ...
        try {
            while (true) {
                try {
                    ...
                    if (op.hasTransactionId()) {
                        if (op.getTransactionId() > expectedTxId) {
                            MetaRecoveryContext.editLogLoaderPrompt(
                                    "There appears " + "to be a gap in the edit log.  We expected txid " + expectedTxId + ", but got txid " + op.getTransactionId() + ".",
                                    recovery, "ignoring missing " + " transaction IDs"
                            );
                        } else if (op.getTransactionId() < expectedTxId) {
                            MetaRecoveryContext.editLogLoaderPrompt(
                                    "There appears " + "to be an out-of-order edit in the edit log.  We " + "expected txid " + expectedTxId + ", but got txid " + op.getTransactionId() + ".",
                    ...
                    /*************************************************
                     * TODO_MA 马中华 https://blog.csdn.net/zhongqi2513
                     *  注释： 执行日志同步到 standby 本地 内存中
                     */
                    try {
                        if (LOG.isTraceEnabled()) {
                            LOG.trace(
                                    "op=" + op + ", startOpt=" + startOpt + ", numEdits=" + numEdits + ", totalEdits=" + totalEdits);
                        }
                    ...
                    // TODO_MA 马中华 注释：  lastAppliedTxId + 1
                    if (op.hasTransactionId()) {
                    ...
                    if (op.hasTransactionId()) {
                        ...
                        if (now - lastLogTime > REPLAY_TRANSACTION_LOG_INTERVAL) {
                            ...
                            LOG.info(
                                    "replaying edit log: " + deltaTxId + "/" + numTxns + " transactions completed. (" + percent + "%)");
                            lastLogTime = now;
                        }
                    }
    ...
    private long applyEditLogOp(FSEditLogOp op, FSDirectory fsDir, StartupOption startOpt, int logVersion,
        ...
        switch (op.opCode) {
            case OP_ADD: {
                ...
                if (FSNamesystem.LOG.isDebugEnabled()) {
                    FSNamesystem.LOG.debug(
                            op.opCode + ": " + path + " numblocks : " + addCloseOp.blocks.length + " clientHolder " + addCloseOp.clientName + " clientMachine " + addCloseOp.clientMachine);
                }
                ...
                if (oldFile == null) { // this is OP_ADD on a new file (case 1)
                    ...
                    newFile = FSDirWriteFileOp.addFileForEditLog(fsDir, inodeId, iip.getExistingINodes(),
                            iip.getLastLocalName(), addCloseOp.permissions, addCloseOp.aclEntries, addCloseOp.xAttrs,
                            replication, addCloseOp.mtime, addCloseOp.atime, addCloseOp.blockSize, true,
                    ...
                    if (!oldFile.isUnderConstruction()) {
                        ...
                        LocatedBlock lb = FSDirAppendOp.prepareFileForAppend(fsNamesys, iip, addCloseOp.clientName,
                ...
                ErasureCodingPolicy ecPolicy = FSDirErasureCodingOp.unprotectedGetErasureCodingPolicy(
            ...
            case OP_APPEND: {
                ...
                if (FSNamesystem.LOG.isDebugEnabled()) {
                    FSNamesystem.LOG.debug(
                            op.opCode + ": " + path + " clientName " + appendOp.clientName + " clientMachine " + appendOp.clientMachine + " newBlock " + appendOp.newBlock);
                }
                INodesInPath iip = fsDir.getINodesInPath(path, DirOp.WRITE);
                INodeFile file = INodeFile.valueOf(iip.getLastINode(), path);
                if (!file.isUnderConstruction()) {
                    LocatedBlock lb = FSDirAppendOp.prepareFileForAppend(fsNamesys, iip, appendOp.clientName,
            ...
            case OP_ADD_BLOCK: {
                ...
                if (FSNamesystem.LOG.isDebugEnabled()) {
                    FSNamesystem.LOG.debug(
                            op.opCode + ": " + path + " new block id : " + addBlockOp.getLastBlock().getBlockId());
                }
                INodesInPath iip = fsDir.getINodesInPath(path, DirOp.READ);
                INodeFile oldFile = INodeFile.valueOf(iip.getLastINode(), path);
            ...
            case OP_SET_QUOTA_BY_STORAGETYPE: {
                ...
                final INodesInPath iip = fsDir.getINodesInPath(src, DirOp.WRITE);
                FSDirAttrOp.unprotectedSetQuota(fsDir, iip, HdfsConstants.QUOTA_DONT_SET, setQuotaByStorageTypeOp.dsQuota,
            ...
            case OP_GET_DELEGATION_TOKEN: {
                ...
                fsNamesys.getDelegationTokenSecretManager()
                        .addPersistedDelegationToken(getDelegationTokenOp.token, getDelegationTokenOp.expiryTime);
                break;
            }
            case OP_RENEW_DELEGATION_TOKEN: {
                RenewDelegationTokenOp renewDelegationTokenOp = (RenewDelegationTokenOp) op;
                fsNamesys.getDelegationTokenSecretManager()
                        .updatePersistedTokenRenewal(renewDelegationTokenOp.token, renewDelegationTokenOp.expiryTime);
                break;
            }
            ...
            case OP_CREATE_SNAPSHOT: {
                ...
                String path = fsNamesys.getSnapshotManager()
                        .createSnapshot(fsDir.getFSNamesystem().getLeaseManager(), iip, snapshotRoot,
                                createSnapshotOp.snapshotName, createSnapshotOp.mtime
                        );
                if (toAddRetryCache) {
            ...
            case OP_DELETE_SNAPSHOT: {
                ...
                fsNamesys.getSnapshotManager().deleteSnapshot(iip, deleteSnapshotOp.snapshotName,
                        new INode.ReclaimContext(fsNamesys.dir.getBlockStoragePolicySuite(), collectedBlocks, removedINodes,
                                null
            ...
            case OP_SET_XATTR: {
                ...
                FSDirXAttrOp.unprotectedSetXAttrs(fsDir, iip, setXAttrOp.xAttrs,
            ...
            case OP_TRUNCATE: {
                ...
                FSDirTruncateOp.unprotectedTruncate(fsNamesys, iip, truncateOp.clientName, truncateOp.clientMachine,
    ...
    private static String formatEditLogReplayError(EditLogInputStream in, long recentOpcodeOffsets[], long txid) {
        ...
        sb.append("Error replaying edit log at offset " + in.getPosition()).append(".  Expected transaction ID was ")
                .append(txid);
    ...
    private void addNewBlock(AddBlockOp op, INodeFile file, ErasureCodingPolicy ecPolicy) throws IOException {
        ...
        if (pBlock != null) { // the penultimate block is not null
            ...
            if (oldLastBlock.getBlockId() != pBlock.getBlockId() || oldLastBlock.getGenerationStamp() != pBlock.getGenerationStamp()) {
                throw new IOException(
                        "Mismatched block IDs or generation stamps for the old last block of file " + op.getPath() + ", the old last block is " + oldLastBlock + ", and the block read from editlog is " + pBlock);
            }
    ...
    private void updateBlocks(FSDirectory fsDir, BlockListUpdatingOp op, INodesInPath iip, INodeFile file,
        ...
        for (int i = 0; i < oldBlocks.length && i < newBlocks.length; i++) {
            ...
            if (oldBlock.getBlockId() != newBlock.getBlockId() || (oldBlock.getGenerationStamp() != newBlock.getGenerationStamp() && !(isGenStampUpdate && isLastBlock))) {
                throw new IOException(
                        "Mismatched block IDs or generation stamps, " + "attempting to replace block " + oldBlock + " with " + newBlock + " as block # " + i + "/" + newBlocks.length + " of " + path);
            }
    ...
    static EditLogValidation scanEditLog(EditLogInputStream in, long maxTxIdToScan) {
        ...
        while (true) {
            ...
            try {
                ...
                FSImage.LOG.warn(
                        "Caught exception after scanning through " + numValid + " ops from " + in + " while determining its valid length. Position was " + lastPos,
                        t
                );
                in.resync();
                ...
                if (in.getPosition() <= lastPos) {
                    FSImage.LOG.warn(
                            "After resync, the position, {} is not greater " + "than the previous position {}. Skipping remainder of this log.",
