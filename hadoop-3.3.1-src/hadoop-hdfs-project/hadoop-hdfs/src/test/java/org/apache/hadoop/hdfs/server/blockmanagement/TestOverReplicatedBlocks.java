...
public class TestOverReplicatedBlocks {
    ...
    public void testProcesOverReplicateBlock() throws Exception {
        ...
        try {
            ...
            File scanCursor = new File(new File(MiniDFSCluster.getFinalizedDir(cluster.getInstanceStorageDir(0, 0),
                    cluster.getNamesystem().getBlockPoolId()
            ).getParent()).getParent(), "scanner.cursor");
            ...
            final DatanodeID corruptDataNode = InternalDataNodeTestUtils.getDNRegistrationForBP(
                ...
                synchronized (hm) {
                    ...
                    for (DatanodeDescriptor datanode : hm.getDatanodes()) {
                        if (!corruptMachineName.equals(datanode.getXferAddr())) {
                            ...
                            datanode.updateHeartbeat(BlockManagerTestUtil.getStorageReportsForDatanode(datanode), 0L, 0L, 0,
                                    0, null
                            );
                        }
                    }
    ...
    public void testChooseReplicaToDelete() throws Exception {
        ...
        try {
            ...
            DatanodeRegistration dnReg = InternalDataNodeTestUtils.getDNRegistrationForBP(lastDN,
            ...
            assertEquals("Replicas on node " + lastDNid + " should have been deleted", SMALL_FILE_LENGTH / SMALL_BLOCK_SIZE,
                    dnBlocks
            );
            namesystem.readUnlock();
            for (BlockLocation location : locs)
                assertEquals("Block should still have 4 replicas", 4, location.getNames().length);
        } finally {
            if (fs != null) fs.close();
    ...
    public void testInvalidateOverReplicatedBlock() throws Exception {
        ...
        try {
            ...
            assertEquals("Expected only one live replica for the block", 1,
                    bm.countNodes(bm.getStoredBlock(block.getLocalBlock())).liveReplicas()
            );
        } finally {
            cluster.shutdown();
        }
