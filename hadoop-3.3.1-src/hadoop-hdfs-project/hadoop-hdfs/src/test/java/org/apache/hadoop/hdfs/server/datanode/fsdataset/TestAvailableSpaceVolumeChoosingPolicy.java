...
public class TestAvailableSpaceVolumeChoosingPolicy {
    ...
    private static void initPolicy(VolumeChoosingPolicy<FsVolumeSpi> policy, float preferencePercent) {
        ...
        conf.setLong(DFS_DATANODE_AVAILABLE_SPACE_VOLUME_CHOOSING_POLICY_BALANCED_SPACE_THRESHOLD_KEY, 1024 * 1024); // 1MB
        conf.setFloat(DFS_DATANODE_AVAILABLE_SPACE_VOLUME_CHOOSING_POLICY_BALANCED_SPACE_PREFERENCE_FRACTION_KEY,
                preferencePercent
        );
    ...
    public void testRR() throws Exception {
        @SuppressWarnings("unchecked") final AvailableSpaceVolumeChoosingPolicy<FsVolumeSpi> policy = ReflectionUtils.newInstance(
                AvailableSpaceVolumeChoosingPolicy.class, null);
    ...
    public void testAvailableSpaceChanges() throws Exception {
        ...
        // than the threshold of 1MB.
        volumes.add(Mockito.mock(FsVolumeSpi.class));
        Mockito.when(volumes.get(1).getAvailable()).thenReturn(1024L * 1024L * 3).thenReturn(1024L * 1024L * 3)
                .thenReturn(1024L * 1024L * 3).thenReturn(1024L * 1024L * 1); // After the third check, return 1MB.
    ...
    public void doRandomizedTest(float preferencePercent, int lowSpaceVolumes, int highSpaceVolumes) throws Exception {
        ...
        GenericTestUtils.assertValueNear((long) (lowAvailableSpaceVolumeSelected * expectedSelectionRatio),
                highAvailableSpaceVolumeSelected, RANDOMIZED_ALLOWED_ERROR
        );
    }
