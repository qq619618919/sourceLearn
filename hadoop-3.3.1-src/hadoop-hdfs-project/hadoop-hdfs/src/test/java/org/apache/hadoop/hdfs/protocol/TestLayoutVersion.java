...
public class TestLayoutVersion {
    ...
    public void testRelease203() {
        assertTrue(
                NameNodeLayoutVersion.supports(LayoutVersion.Feature.DELEGATION_TOKEN, Feature.RESERVED_REL20_203.getInfo().getLayoutVersion()));
    }
    ...
    public void testRelease204() {
        assertTrue(
                NameNodeLayoutVersion.supports(LayoutVersion.Feature.DELEGATION_TOKEN, Feature.RESERVED_REL20_204.getInfo().getLayoutVersion()));
    }
    ...
    public void testNameNodeFeatureMinimumCompatibleLayoutVersions() {
        ...
        EnumSet<NameNodeLayoutVersion.Feature> compatibleFeatures = EnumSet.of(NameNodeLayoutVersion.Feature.TRUNCATE,
                NameNodeLayoutVersion.Feature.APPEND_NEW_BLOCK, NameNodeLayoutVersion.Feature.QUOTA_BY_STORAGE_TYPE,
                NameNodeLayoutVersion.Feature.ERASURE_CODING, NameNodeLayoutVersion.Feature.EXPANDED_STRING_TABLE,
                NameNodeLayoutVersion.Feature.SNAPSHOT_MODIFICATION_TIME);
        for (LayoutFeature f : compatibleFeatures) {
            assertEquals(String.format("Expected minimum compatible layout version " + "%d for feature %s.", baseLV, f), baseLV,
        ...
        for (LayoutFeature f : features) {
            if (!compatibleFeatures.contains(f)) {
                assertEquals(String.format("Expected feature %s to have minimum " + "compatible layout version set to itself.", f),
                        f.getInfo().getLayoutVersion(), f.getInfo().getMinimumCompatibleLayoutVersion());
            }
        }
    }
    ...
    public void testNameNodeFeatureMinimumCompatibleLayoutVersionAscending() {
        ...
        for (LayoutFeature f : EnumSet.allOf(NameNodeLayoutVersion.Feature.class)) {
            if (prevF != null) {
                assertTrue(String.format("Features %s and %s not listed in order of " + "minimum compatible layout version.", prevF, f),
                        f.getInfo().getMinimumCompatibleLayoutVersion() <= prevF.getInfo().getMinimumCompatibleLayoutVersion());
            } else {
                prevF = f;
            }
        }
    ...
    public void testNameNodeFeatureMinimumCompatibleLayoutVersionOutOfOrder() {
        ...
        when(f.getInfo()).thenReturn(new FeatureInfo(ancestorF.getLayoutVersion() - 1, ancestorF.getLayoutVersion(),
                ancestorF.getMinimumCompatibleLayoutVersion() + 1, "Invalid feature.", false));
    ...
    public void testCurrentMinimumCompatibleLayoutVersion() {
        ...
        assertEquals(
                "The minimum compatible layout version has changed.  " + "Downgrade to prior versions is no longer possible.  Please either " + "restore compatibility, or if the incompatibility is intentional, " + "then update this assertion.",
                expectedMinCompatLV, actualMinCompatLV);
    }
    ...
    private void validateFeatureList(LayoutFeature f) {
        ...
        for (LayoutFeature feature : ancestorSet) {
            assertTrue("LV " + lv + " does nto support " + feature + " supported by the ancestor LV " + info.getAncestorLayoutVersion(),
