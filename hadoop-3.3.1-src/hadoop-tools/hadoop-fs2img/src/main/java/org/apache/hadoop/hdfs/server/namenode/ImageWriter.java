...
public class ImageWriter implements Closeable {
    ...
    private final FileSummary.Builder summary = FileSummary.newBuilder().setOndiskVersion(ONDISK_VERSION)
    ...
    public ImageWriter(Options opts) throws IOException {
        ...
        if (null == opts.outStream) {
            ...
            outfs = (fs instanceof LocalFileSystem) ? ((LocalFileSystem) fs).getRaw() : fs;
            ...
            try (NNStorage stor = new NNStorage(opts.getConf(), Arrays.asList(tmp.toUri()), Arrays.asList(tmp.toUri()))) {
                ...
                if (info.getLayoutVersion() != LAYOUT_VERSION) {
                    throw new IllegalStateException(
                            "Incompatible layout " + info.getLayoutVersion() + " (expected " + LAYOUT_VERSION + ")");
                }
        ...
        raw = new TrackedOutputStream<>(new DigestOutputStream(new BufferedOutputStream(out), digest));
        ...
        ugis = null == opts.ugis ? ReflectionUtils.newInstance(opts.ugisClass, opts.getConf()) : opts.ugis;
        BlockAliasMap<FileRegion> fmt = null == opts.blocks ? ReflectionUtils.newInstance(opts.aliasMap,
                opts.getConf()
        ) : opts.blocks;
        blocks = fmt.getWriter(null, blockPoolID);
        blockIds = null == opts.blockIds ? ReflectionUtils.newInstance(opts.blockIdsClass, opts.getConf()) : opts.blockIds;
    ...
    void endSection(OutputStream out, SectionName name) throws IOException {
        ...
        summary.addSections(FileSummary.Section.newBuilder().setName(name.toString()) // not strictly correct, but name not visible
                .setOffset(curSec).setLength(length));
        curSec += length;
    }

    void writeNameSystemSection() throws IOException {
        NameSystemSection.Builder b = NameSystemSection.newBuilder().setGenstampV1(1000).setGenstampV1Limit(0).setGenstampV2(1001)
                .setLastAllocatedBlockId(blockIds.lastId()).setTransactionId(0);
    ...
    static class TrackedOutputStream<T extends OutputStream> extends FilterOutputStream {
    ...
    public static class Options implements Configurable {
        ...
        public void setConf(Configuration conf) {
            ...
            ugisClass = conf.getClass(UGI_CLASS, SingleUGIResolver.class, UGIResolver.class);
            aliasMap = conf.getClass(DFSConfigKeys.DFS_PROVIDED_ALIASMAP_CLASS, NullBlockAliasMap.class, BlockAliasMap.class);
            blockIdsClass = conf.getClass(BLOCK_RESOLVER_CLASS, FixedBlockResolver.class, BlockResolver.class);
            clusterID = "";
            blockPoolID = "";
        }
