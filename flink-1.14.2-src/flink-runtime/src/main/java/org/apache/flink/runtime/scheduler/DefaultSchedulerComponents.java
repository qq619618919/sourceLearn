...
public class DefaultSchedulerComponents {
    ...
    static DefaultSchedulerComponents createSchedulerComponents(
        ...
        return createPipelinedRegionSchedulerComponents(jobType, jobMasterConfiguration, slotPool, slotRequestTimeout);
    }
    ...
    private static DefaultSchedulerComponents createPipelinedRegionSchedulerComponents(
        ...
        final PhysicalSlotRequestBulkChecker bulkChecker = PhysicalSlotRequestBulkCheckerImpl.createFromSlotPool(slotPool,
                SystemClock.getInstance()
        );
        final PhysicalSlotProvider physicalSlotProvider = new PhysicalSlotProviderImpl(slotSelectionStrategy, slotPool);
        final ExecutionSlotAllocatorFactory allocatorFactory = new SlotSharingExecutionSlotAllocatorFactory(physicalSlotProvider,
                jobType == JobType.STREAMING,
                bulkChecker,
                slotRequestTimeout
        );
        return new DefaultSchedulerComponents(new PipelinedRegionSchedulingStrategy.Factory(),
                bulkChecker::start,
                allocatorFactory
        );
    }
    ...
    private static SlotSelectionStrategy selectSlotSelectionStrategy(
        ...
        return configuration.getBoolean(CheckpointingOptions.LOCAL_RECOVERY) ? PreviousAllocationSlotSelectionStrategy.create(
                locationPreferenceSlotSelectionStrategy) : locationPreferenceSlotSelectionStrategy;
    }
}
