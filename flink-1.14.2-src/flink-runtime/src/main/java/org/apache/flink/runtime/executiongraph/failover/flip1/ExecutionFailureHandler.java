...
public class ExecutionFailureHandler {
     ...
     *         restarting delay
     */
    public ExecutionFailureHandler(final SchedulingTopology schedulingTopology,
     ...
     */
    public FailureHandlingResult getFailureHandlingResult(ExecutionVertexID failedTask,
                                                          Throwable cause,
                                                          long timestamp) {
        return handleFailure(failedTask,
                cause,
                timestamp,
    ...
    private FailureHandlingResult handleFailure(@Nullable final ExecutionVertexID failingExecutionVertexId,
        ...
        if (isUnrecoverableError(cause)) {
            return FailureHandlingResult.unrecoverable(failingExecutionVertexId,
        ...
        if (restartBackoffTimeStrategy.canRestart()) {
            ...
            return FailureHandlingResult.restartable(failingExecutionVertexId,
                    cause,
                    timestamp,
                    verticesToRestart,
    ...
    public static boolean isUnrecoverableError(Throwable cause) {
        Optional<Throwable> unrecoverableError = ThrowableClassifier.findThrowableOfThrowableType(cause,
