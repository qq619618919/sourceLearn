...
public class RestartPipelinedRegionFailoverStrategy implements FailoverStrategy {
    ...
    public Set<ExecutionVertexID> getTasksNeedingRestart(ExecutionVertexID executionVertexId, Throwable cause) {
        ...
        Optional<PartitionException> dataConsumptionException = ExceptionUtils.findThrowable(cause,
                PartitionException.class
        );
        if (dataConsumptionException.isPresent()) {
            resultPartitionAvailabilityChecker.markResultPartitionFailed(dataConsumptionException
                    .get()
                    .getPartitionId()
                    .getPartitionId());
        }
        ...
        if (dataConsumptionException.isPresent()) {
            resultPartitionAvailabilityChecker.removeResultPartitionFromFailedState(dataConsumptionException
                    .get()
                    .getPartitionId()
                    .getPartitionId());
        }

        LOG.info("{} tasks should be restarted to recover the failed task {}. ",
                tasksToRestart.size(),
                executionVertexId
        );
    ...
    private Set<SchedulingPipelinedRegion> getRegionsToRestart(SchedulingPipelinedRegion failedRegion) {
        ...
        while (!regionsToVisit.isEmpty()) {
            ...
            for (IntermediateResultPartitionID consumedPartitionId : getConsumedPartitionsToVisit(regionToRestart,
                    visitedConsumedResultGroups
            )) {
                if (!resultPartitionAvailabilityChecker.isAvailable(consumedPartitionId)) {
                    ...
                    SchedulingPipelinedRegion producerRegion = topology.getPipelinedRegionOfVertex(consumedPartition
                            .getProducer()
                            .getId());
            ...
            for (ExecutionVertexID consumerVertexId : getConsumerVerticesToVisit(regionToRestart,
                    visitedConsumerVertexGroups
            )) {
    ...
    private Iterable<IntermediateResultPartitionID> getConsumedPartitionsToVisit(SchedulingPipelinedRegion regionToRestart,
    ...
    private Iterable<ExecutionVertexID> getConsumerVerticesToVisit(SchedulingPipelinedRegion regionToRestart,
    ...
    private static class RegionFailoverResultPartitionAvailabilityChecker implements ResultPartitionAvailabilityChecker {
        ...
        public boolean isAvailable(IntermediateResultPartitionID resultPartitionID) {
            return !failedPartitions.contains(resultPartitionID) && resultPartitionAvailabilityChecker.isAvailable(
                    resultPartitionID);
        }
