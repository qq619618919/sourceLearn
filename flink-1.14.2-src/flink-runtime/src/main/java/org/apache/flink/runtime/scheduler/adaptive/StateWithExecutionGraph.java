...
abstract class StateWithExecutionGraph implements State {
    ...
    StateWithExecutionGraph(Context context,
        ...
        FutureUtils.assertNoException(executionGraph.getTerminationFuture().thenAcceptAsync(jobStatus -> {
            if (jobStatus.isGloballyTerminalState()) {
                context.runIfState(this, () -> onGloballyTerminalState(jobStatus));
            }
    ...
    SerializedInputSplit requestNextInputSplit(JobVertexID vertexID,
    ...
    ExecutionState requestPartitionState(IntermediateDataSetID intermediateResultId,
    ...
    void reportCheckpointMetrics(ExecutionAttemptID executionAttemptID,
                                 long checkpointId,
                                 CheckpointMetrics checkpointMetrics) {
        executionGraphHandler.reportCheckpointMetrics(executionAttemptID, checkpointId, checkpointMetrics);
    }
    ...
    KvStateLocation requestKvStateLocation(JobID jobId,
                                           String registrationName) throws FlinkJobNotFoundException, UnknownKvStateLocation {
        return kvStateHandler.requestKvStateLocation(jobId, registrationName);
    }

    void notifyKvStateRegistered(JobID jobId,
    ...
    void notifyKvStateUnregistered(JobID jobId,
                                   JobVertexID jobVertexId,
                                   KeyGroupRange keyGroupRange,
    ...
    CompletableFuture<String> triggerSavepoint(String targetDirectory, boolean cancelJob) {
        ...
        if (checkpointCoordinator == null) {
            throw new IllegalStateException(String.format("Job %s is not a streaming job.", executionGraph.getJobID()));
        } else if (targetDirectory == null && !checkpointCoordinator
                .getCheckpointStorage()
                .hasDefaultSavepointLocation()) {
            logger.info("Trying to cancel job {} with savepoint, but no savepoint directory configured.",
        ...
        logger.info("Triggering {}savepoint for job {}.", cancelJob ? "cancel-with-" : "", executionGraph.getJobID());
        ...
        return checkpointCoordinator
                ...
                .handleAsync((path, throwable) -> {
                    if (throwable != null) {
                        ...
                        logger.info("Savepoint stored in {}. Now cancelling {}.", path, executionGraph.getJobID());
                        cancel();
                    }
                    return path;
                }, context.getMainThreadExecutor());
    ...
    CompletableFuture<CoordinationResponse> deliverCoordinationRequestToCoordinator(OperatorID operatorId,
        ...
        return operatorCoordinatorHandler.deliverCoordinationRequestToCoordinator(operatorId, request);
    }
     ...
     */
    abstract boolean updateTaskExecutionState(TaskExecutionStateTransition taskExecutionStateTransition);
