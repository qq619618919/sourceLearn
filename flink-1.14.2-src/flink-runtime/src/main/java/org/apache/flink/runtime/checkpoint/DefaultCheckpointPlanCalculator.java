...
public class DefaultCheckpointPlanCalculator implements CheckpointPlanCalculator {
    ...
    public DefaultCheckpointPlanCalculator(JobID jobId,
    ...
    private CheckpointPlan calculateWithAllTasksRunning() {
        List<Execution> executionsToTrigger = sourceTasks
                .stream()
        ...
        return new DefaultCheckpointPlan(Collections.unmodifiableList(executionsToTrigger),
    ...
    private CheckpointPlan calculateAfterTasksFinished() {
            ...
            for (int i = 0; i < jobVertex.getTaskVertices().length; ++i) {
                ...
                if (taskRunningStatus.get(task.getParallelSubtaskIndex())) {
                    ...
                    if (someTasksMustBeTriggered) {
                        boolean hasRunningPrecedentTasks = hasRunningPrecedentTasks(task,
                                prevJobEdges,
        ...
        return new DefaultCheckpointPlan(Collections.unmodifiableList(tasksToTrigger),
                Collections.unmodifiableList(tasksToWaitFor),
    ...
    private boolean someTasksMustBeTriggered(Map<JobVertexID, BitSet> runningTasksByVertex,
    ...
    private boolean hasActiveUpstreamVertex(DistributionPattern distribution, BitSet upstreamRunningTasks) {
        return (distribution == DistributionPattern.ALL_TO_ALL && upstreamRunningTasks.cardinality() > 0) || (
                distribution == DistributionPattern.POINTWISE
                        && upstreamRunningTasks.cardinality() == upstreamRunningTasks.size()
        );
    }

    private boolean hasRunningPrecedentTasks(ExecutionVertex vertex,
                                             List<JobEdge> prevJobEdges,
