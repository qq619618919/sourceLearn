...
public class FineGrainedSlotManager implements SlotManager {
    ...
    public FineGrainedSlotManager(ScheduledExecutor scheduledExecutor,
    ...
    public void start(ResourceManagerId newResourceManagerId,
        ...
        taskManagerTimeoutsCheck = scheduledExecutor.scheduleWithFixedDelay(() -> mainThreadExecutor.execute(this::checkTaskManagerTimeouts),
                0L,
    ...
    public void processResourceRequirements(ResourceRequirements resourceRequirements) {
        checkInit();
        if (resourceRequirements.getResourceRequirements().isEmpty() && resourceTracker.isRequirementEmpty(
    ...
    public boolean registerTaskManager(final TaskExecutorConnection taskExecutorConnection,
        ...
        checkInit();
        LOG.info("Registering task executor {} under {} at the slot manager.",
        ...
        if (taskManagerTracker.getRegisteredTaskManager(taskExecutorConnection.getInstanceID()).isPresent()) {
            ...
            Optional<PendingTaskManagerId> matchedPendingTaskManagerOptional = initialSlotReport.hasAllocatedSlot() ? Optional.empty() : findMatchingPendingTaskManager(
                    totalResourceProfile,
                    defaultSlotResourceProfile
            );

            if (!matchedPendingTaskManagerOptional.isPresent() && isMaxTotalResourceExceededAfterAdding(
                    totalResourceProfile)) {
                LOG.info("Releasing task manager {}. The max total resource limitation <{}, {}> is reached.",
                        taskExecutorConnection.getResourceID(),
                        maxTotalCpu,
            ...
            if (matchedPendingTaskManagerOptional.isPresent()) {
                ...
                allocateSlotsForRegisteredPendingTaskManager(pendingTaskManager,
                        taskExecutorConnection.getInstanceID()
                );
    ...
    private void allocateSlotsForRegisteredPendingTaskManager(PendingTaskManagerId pendingTaskManagerId,
                                                              InstanceID instanceId) {
        Map<JobID, Map<InstanceID, ResourceCounter>> allocations = taskManagerTracker
    ...
    private Optional<PendingTaskManagerId> findMatchingPendingTaskManager(ResourceProfile totalResourceProfile,
                                                                          ResourceProfile defaultSlotResourceProfile) {
        Collection<PendingTaskManager> matchedPendingTaskManagers = taskManagerTracker.getPendingTaskManagersByTotalAndDefaultSlotResourceProfile(
                totalResourceProfile,
                defaultSlotResourceProfile
        );

        Optional<PendingTaskManagerId> matchedPendingTaskManagerIdsWithAllocatedSlots = matchedPendingTaskManagers
                .stream()
                .map(PendingTaskManager::getPendingTaskManagerId)
                .filter((pendingTaskManagerId) -> !taskManagerTracker
                        .getPendingAllocationsOfPendingTaskManager(pendingTaskManagerId)
                        .isEmpty())
                .findAny();
        ...
        if (matchedPendingTaskManagerIdsWithAllocatedSlots.isPresent()) {
            ...
            return matchedPendingTaskManagers.stream().map(PendingTaskManager::getPendingTaskManagerId).findAny();
        }
    }
    ...
    public boolean unregisterTaskManager(InstanceID instanceId, Exception cause) {
        ...
        if (taskManagerTracker.getRegisteredTaskManager(instanceId).isPresent()) {
            Set<AllocationID> allocatedSlots = new HashSet<>(taskManagerTracker
    ...
    private void checkResourceRequirementsWithDelay() {
        if (requirementsCheckFuture == null || requirementsCheckFuture.isDone()) {
            ...
            }), requirementsCheckDelay.toMilliseconds(), TimeUnit.MILLISECONDS);
        }
    }
    ...
    private void checkResourceRequirements() {
        ...
        Map<JobID, Collection<ResourceRequirement>> missingResources = resourceTracker.getMissingResources();
        ...
        final ResourceAllocationResult result = resourceAllocationStrategy.tryFulfillRequirements(missingResources,
                taskManagerTracker
        );
        ...
        final Set<PendingTaskManagerId> failAllocations = allocateTaskManagersAccordingTo(result.getPendingTaskManagersToAllocate());

        // Record slot allocation of pending task managers
        final Map<PendingTaskManagerId, Map<JobID, ResourceCounter>> pendingResourceAllocationResult = new HashMap<>(
        ...
        for (PendingTaskManagerId pendingTaskManagerId : failAllocations) {
            unfulfillableJobs.addAll(result.getAllocationsOnPendingResources().get(pendingTaskManagerId).keySet());
        }
            ...
            for (Map.Entry<InstanceID, ResourceCounter> tmEntry : jobEntry.getValue().entrySet()) {
                ...
                for (Map.Entry<ResourceProfile, Integer> slotEntry : tmEntry.getValue().getResourcesWithCount()) {
                    for (int i = 0; i < slotEntry.getValue(); ++i) {
                        allocationFutures.add(slotStatusSyncer.allocateSlot(instanceID,
                                jobID,
    ...
    public Map<WorkerResourceSpec, Integer> getRequiredResources() {
        return taskManagerTracker
                ...
                .stream()
                .map(pendingTaskManager -> WorkerResourceSpec.fromTotalResourceProfile(pendingTaskManager.getTotalResourceProfile(),
    ...
    private Collection<TaskManagerInfo> getTimeOutTaskManagers() {
        ...
        return taskManagerTracker
                ...
                .stream()
                .filter(taskManager -> taskManager.isIdle()
                        && currentTime - taskManager.getIdleSince() >= taskManagerTimeout.toMilliseconds())
                .collect(Collectors.toList());
    }
    ...
    private boolean allocateResource(PendingTaskManager pendingTaskManager) {
        if (isMaxTotalResourceExceededAfterAdding(pendingTaskManager.getTotalResourceProfile())) {
            LOG.info("Could not allocate {}. Max total resource limitation <{}, {}> is reached.",
                    pendingTaskManager,
                    maxTotalCpu,
                    maxTotalMem.toHumanReadableString()
            );
        ...
        if (!resourceActions.allocateResource(WorkerResourceSpec.fromTotalResourceProfile(pendingTaskManager.getTotalResourceProfile(),
                pendingTaskManager.getNumSlots()
        ))) {
    ...
    public long getTaskManagerIdleSince(InstanceID instanceId) {
        return taskManagerTracker.getRegisteredTaskManager(instanceId).map(TaskManagerInfo::getIdleSince).orElse(0L);
    }
    ...
    private boolean isMaxTotalResourceExceededAfterAdding(ResourceProfile newResource) {
        final ResourceProfile totalResourceAfterAdding = newResource
