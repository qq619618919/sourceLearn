...
public class ResourceProfile implements Serializable {
     ...
     */
    private ResourceProfile(final CPUResource cpuCores,
        ...
        this.extendedResources = checkNotNull(extendedResources)
                .entrySet()
                .stream()
        ...
        if (cpuCores.getValue().compareTo(other.cpuCores.getValue()) >= 0
            ...
            for (Map.Entry<String, ExternalResource> resource : other.extendedResources.entrySet()) {
                if (!extendedResources.containsKey(resource.getKey())
                        || extendedResources.get(resource.getKey()).getValue().compareTo(resource.getValue().getValue())
                        < 0) {
                    return false;
                }
            }
    ...
    public boolean equals(Object obj) {
        if (obj == this) {
            ...
            return Objects.equals(this.cpuCores, that.cpuCores) && Objects.equals(taskHeapMemory, that.taskHeapMemory)
                    && Objects.equals(taskOffHeapMemory, that.taskOffHeapMemory) && Objects.equals(managedMemory,
                    that.managedMemory
            ) && Objects.equals(networkMemory, that.networkMemory) && Objects.equals(extendedResources,
    ...
    public ResourceProfile merge(final ResourceProfile other) {
        ...
        other.extendedResources.forEach((String name, ExternalResource resource) -> {
            resultExtendedResource.compute(name,
                    (ignored, oldResource) -> oldResource == null ? resource : oldResource.merge(resource)
            );
        });

        return new ResourceProfile(cpuCores.merge(other.cpuCores),
    ...
    public ResourceProfile subtract(final ResourceProfile other) {
        ...
        checkArgument(allFieldsNoLessThan(other), "Try to subtract an unmatched resource profile from this one.");

        Map<String, ExternalResource> resultExtendedResource = new HashMap<>(extendedResources);

        other.extendedResources.forEach((String name, ExternalResource resource) -> resultExtendedResource.compute(name,
                (ignored, oldResource) -> oldResource.subtract(resource)
        ));

        return new ResourceProfile(cpuCores.subtract(other.cpuCores),
    ...
    public ResourceProfile multiply(final int multiplier) {
        ...
        Map<String, ExternalResource> resultExtendedResource = extendedResources
                .entrySet()
                .stream()
                .map(entry -> Tuple2.of(entry.getKey(), entry.getValue().multiply(multiplier)))
                .collect(Collectors.toMap(tuple -> tuple.f0, tuple -> tuple.f1));

        return new ResourceProfile(cpuCores.multiply(multiplier),
    ...
    public String toString() {
        ...
        return "ResourceProfile{" + getResourceString() + (extendedResources.isEmpty() ? "" : (", "
                + ExternalResourceUtils.generateExternalResourcesString(extendedResources.values())
        )
        ) + '}';
    }
    ...
    public static class Builder {
        ...
        public ResourceProfile build() {
            return new ResourceProfile(cpuCores,
                    taskHeapMemory,
                    taskOffHeapMemory,
                    managedMemory,
                    networkMemory,
