...
class WaitingForResources implements State, ResourceConsumer {
    ...
    WaitingForResources(Context context,
                        Logger log,
    ...
    WaitingForResources(Context context,
        ...
        Preconditions.checkArgument(!desiredResources.isEmpty(), "Desired resources must not be empty");

        Preconditions.checkArgument(!resourceStabilizationTimeout.isNegative(),
        ...
        if (!initialResourceAllocationTimeout.isNegative()) {
            resourceTimeoutFuture = context.runIfState(this, this::resourceTimeout, initialResourceAllocationTimeout);
        }
    ...
    private void checkDesiredOrSufficientResourcesAvailable() {
        ...
        if (context.hasSufficientResources()) {
            if (resourceStabilizationDeadline == null) {
                resourceStabilizationDeadline = Deadline.fromNowWithClock(resourceStabilizationTimeout, clock);
            }
            /*************************************************
    ...
    private void resourceTimeout() {
        log.debug("Initial resource allocation timeout triggered: Creating ExecutionGraph with available resources.");
        createExecutionGraphWithAvailableResources();
    }
    ...
    interface Context {
         ...
         */
        ArchivedExecutionGraph getArchivedExecutionGraph(JobStatus jobStatus, @Nullable Throwable cause);
    ...
    static class Factory implements StateFactory<WaitingForResources> {
        ...
        public WaitingForResources getState() {
            return new WaitingForResources(context,
                    log,
                    desiredResources,
                    initialResourceAllocationTimeout,
                    resourceStabilizationTimeout
