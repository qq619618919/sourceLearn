...
/**
 * // TODO_MA 马中华 注释： MemoryStateBackend 已经废弃，新的是 HashMapStateBackend
 ...
 * }</pre>
 *
 * // TODO_MA 马中华 注释： Task State 存储在 TaskManager 的 JVM heap memory 中
 * <p>This state backend holds the working state in the memory (JVM heap) of the TaskManagers.
 *
 * // TODO_MA 马中华 注释： checkpoint 之后，将 state 存储在 JobManager's memory
 ...
 *
 * The MemoryStateBackend is consequently a FileSystem-based backend that can work
...
public class MemoryStateBackend extends AbstractFileStateBackend implements ConfigurableStateBackend {
     ...
     *         are always asynchronous now.
     */
    public MemoryStateBackend(@Nullable String checkpointPath,
    ...
    private MemoryStateBackend(MemoryStateBackend original, ReadableConfig configuration, ClassLoader classLoader) {
        ...
        latencyTrackingConfigBuilder = original.latencyTrackingConfigBuilder.configure(configuration);
    }
    ...
    public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(Environment env,
        ...
        HeapPriorityQueueSetFactory priorityQueueSetFactory = new HeapPriorityQueueSetFactory(keyGroupRange,
                numberOfKeyGroups,
                128
        );
    ...
    public String toString() {
        return "MemoryStateBackend (data in heap memory / checkpoints to JobManager) " + "(checkpoints: '"
                + getCheckpointPath() + "', savepoints: '" + getSavepointPath() + ", maxStateSize: " + maxStateSize
                + ")";
    }
}
