...
/**
 * // TODO_MA 马中华 注释： 1、task state 存储在 TaskManager 的内存中
 * // TODO_MA 马中华 注释： 2、job state 存储在 CheckpointStorage 配置中
 ...
 *
 * // TODO_MA 马中华 注释： 工作状态保存在 TaskManager 堆上。如果一个 TaskManager 同时执行多个任务（如果 TaskManager
 * // TODO_MA 马中华 注释： 有多个槽，或者如果使用槽共享），那么所有任务的聚合状态需要适合该 TaskManager 的内存。
...
public class HashMapStateBackend extends AbstractStateBackend implements ConfigurableStateBackend {
    ...
    @Override
    public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(Environment env, JobID jobID, String operatorIdentifier,
        ...
        HeapPriorityQueueSetFactory priorityQueueSetFactory = new HeapPriorityQueueSetFactory(keyGroupRange, numberOfKeyGroups,
                128
        );
        ...
        // TODO_MA 马中华 注释： HeapKeyedStateBackend
        return new HeapKeyedStateBackendBuilder<>(kvStateRegistry, keySerializer, env.getUserCodeClassLoader()
                .asClassLoader(), numberOfKeyGroups, keyGroupRange, env.getExecutionConfig(), ttlTimeProvider,
                latencyTrackingStateConfig, stateHandles, getCompressionDecorator(env.getExecutionConfig()), localRecoveryConfig,
                priorityQueueSetFactory, true, cancelStreamRegistry
        ).build();
    }
