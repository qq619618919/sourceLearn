...
public class DefaultResourceTracker implements ResourceTracker {
    ...
    public void notifyLostResource(JobID jobId, ResourceProfile resourceProfile) {
        ...
        if (tracker != null) {
            ...
            LOG.trace("Received notification for job {} having lost resource {}, but no such job was tracked.",
                    jobId,
                    resourceProfile
            );
        }
    ...
    public boolean isRequirementEmpty(JobID jobId) {
        return Optional.ofNullable(trackers.get(jobId)).map(JobScopedResourceTracker::isRequirementEmpty).orElse(true);
    }
