...
public class DispatcherResourceManagerComponent implements AutoCloseableAsync {
    ...
    DispatcherResourceManagerComponent(@Nonnull DispatcherRunner dispatcherRunner,
    ...
    private void handleUnexpectedResourceManagerTermination() {
        resourceManagerService.getTerminationFuture().whenComplete((ignored, throwable) -> {
            if (isRunning.get()) {
                fatalErrorHandler.onFatalError(new FlinkException("Unexpected termination of ResourceManagerService.",
                        throwable
                ));
            }
        });
     ...
     */
    public CompletableFuture<Void> stopApplication(final ApplicationStatus applicationStatus,
                                                   final @Nullable String diagnostics) {
        return internalShutdown(() -> resourceManagerService.deregisterApplication(applicationStatus, diagnostics));
    }
    ...
    private CompletableFuture<Void> internalShutdown(final Supplier<CompletableFuture<?>> additionalShutdownAction) {
        if (isRunning.compareAndSet(true, false)) {
            final CompletableFuture<Void> closeWebMonitorAndAdditionalShutdownActionFuture = FutureUtils.composeAfterwards(
                    webMonitorEndpoint.closeAsync(),
                    additionalShutdownAction
            );

            return FutureUtils.composeAfterwards(closeWebMonitorAndAdditionalShutdownActionFuture,
