...
public final class SchedulerUtils {
    ...
    public static CompletedCheckpointStore createCompletedCheckpointStoreIfCheckpointingIsEnabled(JobGraph jobGraph,
                                                                                                  Configuration configuration,
                                                                                                  ClassLoader userCodeLoader,
                                                                                                  CheckpointRecoveryFactory checkpointRecoveryFactory,
                                                                                                  Logger log) throws JobExecutionException {
        final JobID jobId = jobGraph.getJobID();
        if (DefaultExecutionGraphBuilder.isCheckpointingEnabled(jobGraph)) {
            try {
                // TODO_MA 马中华 注释：
                return createCompletedCheckpointStore(configuration,
                        userCodeLoader,
    ...
    @VisibleForTesting
    static CompletedCheckpointStore createCompletedCheckpointStore(Configuration jobManagerConfig,
                                                                   ClassLoader classLoader,
                                                                   CheckpointRecoveryFactory recoveryFactory,
                                                                   Logger log,
                                                                   JobID jobId) throws Exception {

        // TODO_MA 马中华 注释： 要保留的最大的成功 checkpoint 的数量
        ...
        if (maxNumberOfCheckpointsToRetain <= 0) {
            ...
            // TODO_MA 马中华 注释： 默认为 1
            maxNumberOfCheckpointsToRetain = CheckpointingOptions.MAX_RETAINED_CHECKPOINTS.defaultValue();
        }

        return recoveryFactory.createRecoveredCompletedCheckpointStore(jobId,
                maxNumberOfCheckpointsToRetain,
                classLoader
        );
    }

    public static CheckpointIDCounter createCheckpointIDCounterIfCheckpointingIsEnabled(JobGraph jobGraph,
                                                                                        CheckpointRecoveryFactory checkpointRecoveryFactory) throws JobExecutionException {
        final JobID jobId = jobGraph.getJobID();
        if (DefaultExecutionGraphBuilder.isCheckpointingEnabled(jobGraph)) {
            try {
                ...
                throw new JobExecutionException(jobId,
                        "Failed to initialize high-availability checkpoint id counter",
                        e
                );
            }
        } else {
