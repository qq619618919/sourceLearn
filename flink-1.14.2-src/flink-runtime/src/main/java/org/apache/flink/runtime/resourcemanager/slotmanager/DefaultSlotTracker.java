...
public class DefaultSlotTracker implements SlotTracker {
    ...
    private final SlotStatusStateReconciler slotStatusStateReconciler = new SlotStatusStateReconciler(this::transitionSlotToFree,
    ...
    public boolean notifySlotStatus(Iterable<SlotStatus> slotStatuses) {
        ...
        for (SlotStatus slotStatus : slotStatuses) {
            anyStatusChanged |= slotStatusStateReconciler.executeStateTransition(slots.get(slotStatus.getSlotID()),
                    slotStatus.getJobID()
            );
        }
    ...
    static class SlotStatusStateReconciler {
        ...
        @VisibleForTesting
        SlotStatusStateReconciler(Consumer<DeclarativeTaskManagerSlot> toFreeSlot,
        ...
        public boolean executeStateTransition(DeclarativeTaskManagerSlot slot, JobID jobId) {
            final SlotState reportedSlotState = jobId == null ? SlotState.FREE : SlotState.ALLOCATED;
    ...
    private static class MultiSlotStatusUpdateListener implements SlotStatusUpdateListener {
        ...
        @Override
        public void notifySlotStatusChange(TaskManagerSlotInformation slot,
                                           SlotState previous,
                                           SlotState current,
                                           JobID jobId) {
