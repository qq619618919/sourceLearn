...
public class ResourceManagerRuntimeServicesConfiguration {
    ...
    public static ResourceManagerRuntimeServicesConfiguration fromConfiguration(Configuration configuration,
        ...
        try {
            ...
            throw new ConfigurationException("Could not parse the resource manager's job timeout " + "value "
                    + ResourceManagerOptions.JOB_TIMEOUT + '.', e);
        }

        final WorkerResourceSpec defaultWorkerResourceSpec = defaultWorkerResourceSpecFactory.createDefaultWorkerResourceSpec(
                configuration);
        final SlotManagerConfiguration slotManagerConfiguration = SlotManagerConfiguration.fromConfiguration(
                configuration,
                defaultWorkerResourceSpec
        );

        // TODO_MA 马中华 注释： cluster.fine-grained-resource-management.enabled = 默认false
        final boolean enableFineGrainedResourceManagement = ClusterOptions.isFineGrainedResourceManagementEnabled(
                configuration);
