...
public class DefaultSlotStatusSyncer implements SlotStatusSyncer {
    ...
    @Override
    public void initialize(TaskManagerTracker taskManagerTracker,
    ...
    public CompletableFuture<Void> allocateSlot(InstanceID instanceId,
        ...
        Preconditions.checkState(taskManager.isPresent(),
        ...
        LOG.info("Starting allocation of slot {} from {} for job {} with resource profile {}.",
                allocationId,
                resourceId,
                jobId,
                resourceProfile
        ...
        CompletableFuture<Acknowledge> requestFuture = gateway.requestSlot(SlotID.getDynamicSlotID(resourceId),
                jobId,
                allocationId,
                resourceProfile,
                targetAddress,
        ...
        FutureUtils.assertNoException(requestFuture.handleAsync((Acknowledge acknowledge, Throwable throwable) -> {
            ...
            if (acknowledge != null) {
                ...
                taskManagerTracker.notifySlotStatus(allocationId,
                        jobId,
                        instanceId,
                        resourceProfile,
                        SlotState.ALLOCATED
    ...
    public void freeSlot(AllocationID allocationId) {
        ...
        final Optional<TaskManagerSlotInformation> slotOptional = taskManagerTracker.getAllocatedOrPendingSlot(
                allocationId);
    ...
    public boolean reportSlotStatus(InstanceID instanceId, SlotReport slotReport) {
        ...
        for (TaskManagerSlotInformation slot : new HashSet<>(taskManager.get().getAllocatedSlots().values())) {
            ...
            if (!reportedAllocationIds.contains(slot.getAllocationId()) && slot.getState() == SlotState.ALLOCATED) {
    ...
    private boolean syncAllocatedSlotStatus(SlotStatus slotStatus, TaskManagerInfo taskManager) {
        ...
        final ResourceProfile resourceProfile = Preconditions.checkNotNull(slotStatus.getResourceProfile());
        ...
        if (taskManager.getAllocatedSlots().containsKey(allocationId)) {
            if (taskManager.getAllocatedSlots().get(allocationId).getState() == SlotState.PENDING) {
                // Allocation Complete
                final TaskManagerSlotInformation slot = taskManager.getAllocatedSlots().get(allocationId);
            ...
            Preconditions.checkState(!taskManagerTracker.getAllocatedOrPendingSlot(allocationId).isPresent(),
