...
public class DefaultExecutionGraph implements ExecutionGraph, InternalExecutionGraphAccessor {
    ...
    public DefaultExecutionGraph(JobInformation jobInformation,
                                 ...
                                 VertexParallelismStore vertexParallelismStore) throws IOException {
        ...
        this.jobInformationOrBlobKey = BlobWriter.serializeAndTryOffload(jobInformation,
        ...
        this.resultPartitionAvailabilityChecker = new ExecutionGraphResultPartitionAvailabilityChecker(this::createResultPartitionId,
                partitionTracker
        );
    ...
    public void enableCheckpointing(CheckpointCoordinatorConfiguration chkConfig,
        ...
        checkpointCoordinatorTimer = Executors.newSingleThreadScheduledExecutor(new DispatcherThreadFactory(Thread
                .currentThread()
                .getThreadGroup(), "Checkpoint Timer"));

        // create the coordinator that triggers and commits checkpoints and holds the state
        checkpointCoordinator = new CheckpointCoordinator(jobInformation.getJobId(),
                chkConfig,
    ...
    private CheckpointPlanCalculator createCheckpointPlanCalculator(boolean enableCheckpointsAfterTasksFinish) {
        return new DefaultCheckpointPlanCalculator(getJobID(),
    ...
    @Override
    public IntermediateResultPartition getResultPartitionOrThrow(final IntermediateResultPartitionID id) {
    ...
    private static SerializedValue<OptionalFailure<Object>> serializeAccumulator(String name,
                                                                                 OptionalFailure<Accumulator<?, ?>> accumulator) {
        try {
                    ...
                    allTerminal.whenComplete((Void value, Throwable throwable) -> {
                        if (throwable != null) {
                            transitionState(JobStatus.CANCELLING,
                                    JobStatus.FAILED,
                                    new FlinkException("Could not cancel job " + getJobName()
                                            + " because not all execution job vertices could be cancelled.", throwable)
                            );
                        } else {
    ...
    public void suspend(Throwable suspensionCause) {
        ...
        if (state.isTerminalState()) {
            ...
            final ArrayList<CompletableFuture<Void>> executionJobVertexTerminationFutures = new ArrayList<>(
            ...
            final ConjunctFuture<Void> jobVerticesTerminationFuture = FutureUtils.waitForAll(
                    executionJobVertexTerminationFutures);
            ...
            throw new IllegalStateException(String.format("Could not suspend because transition from %s to %s failed.",
                    state,
                    JobStatus.SUSPENDED
            ));
        }
    ...
    void failGlobalIfExecutionIsStillRunning(Throwable cause, ExecutionAttemptID failingAttempt) {
        ...
        if (failedExecution != null && (failedExecution.getState() == ExecutionState.RUNNING
            ...
            LOG.debug("The failing attempt {} belongs to an already not" + " running task thus won't fail the job",
                    failingAttempt
            );
        }
    }
    ...
    public ArchivedExecutionConfig getArchivedExecutionConfig() {
        ...
        try {
            ExecutionConfig executionConfig = jobInformation
    ...
    private void maybeReleasePartitionGroupsFor(final Execution attempt) {
        ...
        if (attempt.getState() == ExecutionState.FINISHED) {
            final List<ConsumedPartitionGroup> releasablePartitionGroups = partitionGroupReleaseStrategy.vertexFinished(
                    finishedExecutionVertex);
    ...
    private void releasePartitionGroups(final List<ConsumedPartitionGroup> releasablePartitionGroups) {
        ...
        if (releasablePartitionGroups.size() > 0) {
            ...
            final List<ResultPartitionID> releasablePartitionIds = releasablePartitionGroups
                    .stream()
    ...
    ResultPartitionID createResultPartitionId(final IntermediateResultPartitionID resultPartitionId) {
        final SchedulingResultPartition schedulingResultPartition = getSchedulingTopology().getResultPartition(
                resultPartitionId);
        ...
        checkState(subtaskIndex < taskVertices.length,
    ...
    public void registerExecution(Execution exec) {
        ...
        if (previous != null) {
            failGlobal(new Exception(
                    "Trying to register execution " + exec + " for already used ID " + exec.getAttemptId()));
        }
    }
    ...
    public void deregisterExecution(Execution exec) {
        ...
        if (contained != null && contained != exec) {
            failGlobal(new Exception(
                    "De-registering execution " + exec + " failed. Found for same ID execution " + contained));
        }
    }
    ...
    @Override
    public void notifyExecutionChange(final Execution execution, final ExecutionState newExecutionState) {
    ...
    @Override
    public void notifySchedulerNgAboutInternalTaskFailure(final ExecutionAttemptID attemptId,
                                                          final Throwable t,
