...
class Executing extends StateWithExecutionGraph implements ResourceConsumer {
    ...
    Executing(ExecutionGraph executionGraph,
    ...
    @Override
    public void cancel() {
        context.goToCanceling(getExecutionGraph(), getExecutionGraphHandler(), getOperatorCoordinatorHandler());
    }
    ...
    private void handleAnyFailure(Throwable cause) {
        ...
        if (failureResult.canRestart()) {
            ...
            context.goToRestarting(getExecutionGraph(),
            ...
            context.goToFailing(getExecutionGraph(),
                    getExecutionGraphHandler(),
                    getOperatorCoordinatorHandler(),
    ...
    boolean updateTaskExecutionState(TaskExecutionStateTransition taskExecutionState) {
        ...
        if (successfulUpdate) {
            if (taskExecutionState.getExecutionState() == ExecutionState.FAILED) {
                ...
                handleAnyFailure(cause == null ? new FlinkException(
                        "Unknown failure cause. Probably related to FLINK-21376.") : cause);
            }
        }
    ...
    CompletableFuture<String> stopWithSavepoint(@Nullable final String targetDirectory, boolean terminate) {
        final ExecutionGraph executionGraph = getExecutionGraph();

        StopWithSavepointTerminationManager.checkStopWithSavepointPreconditions(executionGraph.getCheckpointCoordinator(),
                targetDirectory,
        ...
        final CompletableFuture<String> savepointFuture = executionGraph
                .getCheckpointCoordinator()
                .triggerSynchronousSavepoint(terminate, targetDirectory)
                .thenApply(CompletedCheckpoint::getExternalPointer);
        return context.goToStopWithSavepoint(executionGraph,
                getExecutionGraphHandler(),
                getOperatorCoordinatorHandler(),
                schedulingProvider,
                savepointFuture
    ...
    static class Factory implements StateFactory<Executing> {
        ...
        Factory(ExecutionGraph executionGraph,
                ExecutionGraphHandler executionGraphHandler,
                OperatorCoordinatorHandler operatorCoordinatorHandler,
                Logger log,
                Context context,
