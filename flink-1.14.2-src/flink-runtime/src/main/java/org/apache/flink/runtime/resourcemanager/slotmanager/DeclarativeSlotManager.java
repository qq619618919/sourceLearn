...
public class DeclarativeSlotManager implements SlotManager {
    ...
    public DeclarativeSlotManager(ScheduledExecutor scheduledExecutor,
         ...
         *  注释：
         */
        taskExecutorManagerFactory = (executor, resourceActions) -> new TaskExecutorManager(slotManagerConfiguration.getDefaultWorkerResourceSpec(),
    ...
    private void maybeReclaimInactiveSlots(JobID jobId) {
        if (!resourceTracker.getAcquiredResources(jobId).isEmpty()) {
            final Collection<TaskExecutorConnection> taskExecutorsWithAllocatedSlots = slotTracker.getTaskExecutorsWithAllocatedSlotsForJob(
                    jobId);
    ...
    private void checkResourceRequirements() {
         ...
         *  注释：
         */
        ResourceCounter pendingSlots = ResourceCounter.withResources(taskExecutorManager
                .getPendingTaskManagerSlots()
                .stream()
                .collect(Collectors.groupingBy(PendingTaskManagerSlot::getResourceProfile,
    ...
    private ResourceCounter tryAllocateSlotsForJob(JobID jobId, Collection<ResourceRequirement> missingResources) {
        ...
        for (ResourceRequirement resourceRequirement : missingResources) {
             ...
             *  注释：
             */
            int numMissingSlots = internalTryAllocateSlots(jobId,
                    jobMasterTargetAddresses.get(jobId),
                    resourceRequirement
            );
            if (numMissingSlots > 0) {
                outstandingRequirements = outstandingRequirements.add(resourceRequirement.getResourceProfile(),
                        numMissingSlots
                );
            }
        }
    ...
    private void allocateSlot(TaskManagerSlotInformation taskManagerSlot,
        ...
        LOG.debug("Starting allocation of slot {} for job {} with resource profile {}.",
                slotId,
                jobId,
                resourceProfile
        );
        ...
        CompletableFuture<Void> slotAllocationResponseProcessingFuture = requestFuture.handleAsync((Acknowledge acknowledge, Throwable throwable) -> {
            ...
            if (acknowledge != null) {
                ...
                if (throwable instanceof SlotOccupiedException) {
                    ...
                    LOG.debug("Tried allocating slot {} for job {}, but it was already allocated for job {}.",
                            slotId,
                            jobId,
                            exception.getJobId()
                    );
        ...
        for (Map.Entry<ResourceProfile, Integer> missingResource : missingResources) {
            ...
            for (int i = 0; i < missingResource.getValue(); i++) {
                ...
                if (!matchingResult.isSuccessfulMatching()) {
                    ...
                    if (!allocationResult.isSuccessfulAllocating() && sendNotEnoughResourceNotifications) {
                        LOG.warn("Could not fulfill resource requirements of job {}. Free slots: {}",
                                jobId,
                                slotTracker.getFreeSlots().size()
                        );
                        resourceActions.notifyNotEnoughResourcesAvailable(jobId,
    ...
    private WorkerAllocationResult tryAllocateWorkerAndReserveSlot(ResourceProfile profile,
                                                                   ResourceCounter pendingSlots) {
