...
public final class RestServerEndpointConfiguration {
    ...
    private RestServerEndpointConfiguration(final String restAddress,
    ...
    public static RestServerEndpointConfiguration fromConfiguration(Configuration config) throws ConfigurationException {
        ...
        final Path uploadDir = Paths.get(config.getString(WebOptions.UPLOAD_DIR, config.getString(WebOptions.TMP_DIR)),
                "flink-web-upload"
        );

        final int maxContentLength = config.getInteger(RestOptions.SERVER_MAX_CONTENT_LENGTH);

        final Map<String, String> responseHeaders = Collections.singletonMap(HttpHeaders.Names.ACCESS_CONTROL_ALLOW_ORIGIN,
         ...
         *  注释：
         */
        return new RestServerEndpointConfiguration(restAddress,
                restBindAddress,
                portRangeDefinition,
                sslHandlerFactory,
                uploadDir,
