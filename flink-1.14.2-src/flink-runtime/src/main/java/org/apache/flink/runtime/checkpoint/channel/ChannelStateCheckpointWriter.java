...
class ChannelStateCheckpointWriter {
    ...
    ChannelStateCheckpointWriter(String taskName,
                                 int subtaskIndex,
    ...
    void writeInput(InputChannelInfo info, Buffer buffer) throws Exception {
        write(inputChannelOffsets, info, buffer, !allInputsReceived, "ChannelStateCheckpointWriter#writeInput");
    }

    void writeOutput(ResultSubpartitionInfo info, Buffer buffer) throws Exception {
        write(resultSubpartitionOffsets, info, buffer, !allOutputsReceived, "ChannelStateCheckpointWriter#writeOutput");
    }

    private <K> void write(Map<K, StateContentMetaInfo> offsets,
                           K key,
                           Buffer buffer,
        ...
        try {
            ...
            runWithChecks(() -> {
                ...
                offsets.computeIfAbsent(key, unused -> new StateContentMetaInfo()).withDataAdded(offset, size);
    ...
    private void doComplete(boolean precondition,
                            RunnableWithException complete,
                            RunnableWithException... callbacks) throws Exception {
    ...
    private <I, H extends AbstractChannelStateHandle<I>> void complete(StreamStateHandle underlying,
    ...
    private <I, H extends AbstractChannelStateHandle<I>> H createHandle(HandleFactory<I, H> handleFactory,
                                                                        StreamStateHandle underlying,
                                                                        I channelInfo,
                                                                        StateContentMetaInfo contentMetaInfo) throws IOException {
        Optional<byte[]> bytes = underlying.asBytesIfInMemory(); // todo: consider restructuring channel state and
        // removing this method:
        // https://issues.apache.org/jira/browse/FLINK-17972
        if (bytes.isPresent()) {
            StreamStateHandle extracted = new ByteStreamStateHandle(randomUUID().toString(),
                    serializer.extractAndMerge(bytes.get(), contentMetaInfo.getOffsets())
            );
            return handleFactory.create(subtaskIndex,
                    channelInfo,
                    extracted,
