...
public class SourceCoordinator<SplitT extends SourceSplit, EnumChkT> implements OperatorCoordinator {
    ...
    public SourceCoordinator(String operatorName,
    ...
    public void subtaskReady(int subtask, SubtaskGateway gateway) {
        ...
        runInEventLoop(() -> context.subtaskReady(gateway), "making event gateway to subtask %d available", subtask);
    }
    ...
    public void checkpointCoordinator(long checkpointId, CompletableFuture<byte[]> result) {
        runInEventLoop(() -> {
            ...
            try {
                ...
                result.completeExceptionally(new CompletionException(String.format(
    ...
    private void runInEventLoop(final ThrowingRunnable<Throwable> action,
    ...
    static <EnumChkT> byte[] writeCheckpointBytes(final EnumChkT enumeratorCheckpoint,
                                                  final SimpleVersionedSerializer<EnumChkT> enumeratorCheckpointSerializer) throws Exception {

        try (ByteArrayOutputStream baos = new ByteArrayOutputStream(); DataOutputStream out = new DataOutputViewStreamWrapper(
                baos)) {

            writeCoordinatorSerdeVersion(out);
            out.writeInt(enumeratorCheckpointSerializer.getVersion());
            byte[] serialziedEnumChkpt = enumeratorCheckpointSerializer.serialize(enumeratorCheckpoint);
    ...
    private EnumChkT deserializeCheckpoint(byte[] bytes) throws Exception {
        try (ByteArrayInputStream bais = new ByteArrayInputStream(bytes); DataInputStream in = new DataInputViewStreamWrapper(
                bais)) {
