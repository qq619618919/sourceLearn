...
public class HighAvailabilityServicesUtils {
    ...
    public static HighAvailabilityServices createAvailableOrEmbeddedServices(Configuration config,
                                                                             Executor executor,
        ...
        switch (highAvailabilityMode) {
            ...
            case ZOOKEEPER:
                ...
                return new ZooKeeperHaServices(ZooKeeperUtils.startCuratorFramework(config, fatalErrorHandler),
                        executor,
                        config,
                        blobStoreService
                );
    ...
    public static HighAvailabilityServices createHighAvailabilityServices(Configuration configuration,
                                                                          Executor executor,
        ...
        switch (highAvailabilityMode) {
            ...
            case NONE:
                ...
                final String resourceManagerRpcUrl = rpcSystemUtils.getRpcUrl(hostnamePort.f0,
                        hostnamePort.f1,
                        RpcServiceUtils.createWildcardName(ResourceManager.RESOURCE_MANAGER_NAME),
                        addressResolution,
                        configuration
                );
                final String dispatcherRpcUrl = rpcSystemUtils.getRpcUrl(hostnamePort.f0,
                        hostnamePort.f1,
            ...
            case ZOOKEEPER:
                 ...
                 *  注释：
                 */
                return new ZooKeeperHaServices(ZooKeeperUtils.startCuratorFramework(configuration, fatalErrorHandler),
                        executor,
                        configuration,
                        blobStoreService
                );
    ...
    public static ClientHighAvailabilityServices createClientHAService(Configuration configuration,
        ...
        switch (highAvailabilityMode) {
            case NONE:
                final String webMonitorAddress = getWebMonitorAddress(configuration,
    ...
    public static Tuple2<String, Integer> getJobManagerAddress(Configuration configuration) throws ConfigurationException {
        ...
        if (port <= 0 || port >= 65536) {
            throw new ConfigurationException(
                    "Invalid value for '" + JobManagerOptions.PORT + "' (port of the JobManager actor system) : " + port
    ...
    public static Path getClusterHighAvailableStoragePath(Configuration configuration) {
        ...
        // TODO_MA 马中华 注释： high-availability.storageDir
        ...
        if (isNullOrWhitespaceOnly(storagePath)) {
            throw new IllegalConfigurationException(
                    "Configuration is missing the mandatory parameter: " + HighAvailabilityOptions.HA_STORAGE_PATH);
        }

        // TODO_MA 马中华 注释： base path
        final Path path;
        try {
            path = new Path(storagePath);
        } catch (Exception e) {
            throw new IllegalConfigurationException(
                    "Invalid path for highly available storage (" + HighAvailabilityOptions.HA_STORAGE_PATH.key() + ')',
                    e
            );
        }

        // TODO_MA 马中华 注释： high-availability.cluster-id
        final String clusterId = configuration.getValue(HighAvailabilityOptions.HA_CLUSTER_ID);

        // TODO_MA 马中华 注释： clsuter path
    ...
    private static HighAvailabilityServices createCustomHAServices(Configuration config,
                                                                   Executor executor) throws FlinkException {
    ...
    private static HighAvailabilityServicesFactory loadCustomHighAvailabilityServicesFactory(String highAvailabilityServicesFactoryClassName) throws FlinkException {
        ...
        return InstantiationUtil.instantiate(highAvailabilityServicesFactoryClassName,
