...
public enum ProgramOptionsUtils {
    ...
    public static boolean containsPythonDependencyOptions(CommandLine line) {
        return line.hasOption(PYFILES_OPTION.getOpt()) || line.hasOption(PYREQUIREMENTS_OPTION.getOpt())
                || line.hasOption(PYARCHIVE_OPTION.getOpt()) || line.hasOption(PYEXEC_OPTION.getOpt())
    ...
    public static ProgramOptions createPythonProgramOptions(CommandLine line) throws CliArgsException {
        try {
            ...
            Class<?> pythonProgramOptionsClazz = Class.forName("org.apache.flink.client.cli.PythonProgramOptions",
                    false,
                    classLoader
            );
    ...
    private static ClassLoader getPythonClassLoader() {
        try {
            return new URLClassLoader(new URL[]{PackagedProgramUtils.getPythonJar()},
                    Thread.currentThread().getContextClassLoader()
            );
        } catch (RuntimeException e) {
            LOG.warn("An attempt to load the flink-python jar from the \"opt\" directory failed, "
    ...
    public static void configurePythonExecution(Configuration configuration,
                                                PackagedProgram packagedProgram) throws Exception {
        ...
        ClassLoader classLoader = getPythonClassLoader();
        Class<?> pythonDependencyUtilsClazz = Class.forName("org.apache.flink.python.util.PythonDependencyUtils",
                false,
                classLoader
        );
        Method mergeMethod = pythonDependencyUtilsClazz.getDeclaredMethod("merge",
                Configuration.class,
                Configuration.class
        );
