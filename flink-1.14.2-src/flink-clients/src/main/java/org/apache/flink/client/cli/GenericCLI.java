...
public class GenericCLI implements CustomCommandLine {
    ...
    private final Option executorOption = new Option("e",
            "executor",
            true,
    ...
    private final Option targetOption = new Option("t",
            "target",
            true,
            "The deployment target for the given application, which is equivalent " + "to the \""
                    + DeploymentOptions.TARGET.key() + "\" config option. For the \"run\" action the "
                    + "currently available targets are: " + getExecutorFactoryNames()
                    + ". For the \"run-application\" action" + " the currently available targets are: "
    ...
    public boolean isActive(CommandLine commandLine) {
        return configuration.getOptional(DeploymentOptions.TARGET).isPresent()
                || commandLine.hasOption(executorOption.getOpt()) || commandLine.hasOption(targetOption.getOpt());
    }
