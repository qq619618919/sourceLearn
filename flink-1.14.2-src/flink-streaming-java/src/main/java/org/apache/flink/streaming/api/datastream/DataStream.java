...
public class DataStream<T> {
        ...
        for (DataStream<T> newStream : streams) {
            if (!getType().equals(newStream.getType())) {
                throw new IllegalArgumentException(
                        "Cannot union streams of different types: " + getType() + " and " + newStream.getType());
            }
    ...
    /**
     * // TODO_MA 马中华 注释： 通过将（可能的）不同类型的 {@link DataStream} 输出相互连接来创建一个新的 {@link ConnectedStreams}。
     * Creates a new {@link ConnectedStreams} by connecting {@link DataStream} outputs of (possible)
     * different types with each other.
     *
     * // TODO_MA 马中华 注释： 使用此运算符连接的 DataStreams 可与 CoFunctions 一起使用以应用联合转换。
    ...
    public <R> BroadcastConnectedStream<T, R> connect(BroadcastStream<R> broadcastStream) {
        return new BroadcastConnectedStream<>(environment,
                this,
    ...
    private <K> DataStream<T> partitionCustom(Partitioner<K> partitioner, Keys<T> keys) {
        KeySelector<T, K> keySelector = KeySelectorUtil.getSelectorForOneKey(keys,
                partitioner,
                getType(),
                getExecutionConfig()
        );
    ...
    /**
     * // TODO_MA 马中华 注释： 设置 {@link DataStream} 的分区，以便将输出元素广播到下一个操作的每个并行实例。
    ...
    /**
     * // TODO_MA 马中华 注释： 设置 {@link DataStream} 的分区，使输出元素均匀随机混洗到下一个操作。
    ...
    /**
     * // TODO_MA 马中华 注释： 设置 {@link DataStream} 的分区，以便将输出元素转发到下一个操作的本地子任务。
    ...
    /**
     * // TODO_MA 马中华 注释： round-robin fashion. 轮询
     ...
     *
     * // TODO_MA 马中华 注释： 例如，如果上游操作的并行度为 2，下游操作的并行度为 4，
     * // TODO_MA 马中华 注释： 那么一个上游操作会将元素分发给两个下游操作，而另一个上游操作将分发给其他两个下游操作。
     ...
     *
     * // TODO_MA 马中华 注释： 另一方面，如果下游操作的并行度为 2，而上游操作的并行度为 4，
     * // TODO_MA 马中华 注释： 那么两个上游操作将分配给一个下游操作，而另外两个上游操作将分配给其他下游操作。
     ...
     *
     * @return The DataStream with rescale partitioning set.  even  odd
     */
    @PublicEvolving
     ...
     *
     * Use this setting with care since it might cause a serious performance bottleneck in the application.
     *
     * @return The DataStream with shuffle partitioning set.
     */
    @PublicEvolving
    ...
    public <R> SingleOutputStreamOperator<R> map(MapFunction<T, R> mapper) {
        ...
        TypeInformation<R> outType = TypeExtractor.getMapReturnTypes(clean(mapper),
                getType(),
    ...
    ClientAndIterator<T> executeAndCollectWithClient(String jobExecutionName) throws Exception {
        ...
        CollectResultIterator<T> iterator = new CollectResultIterator<>(operator.getOperatorIdFuture(),
                serializer,
                accumulatorName,
