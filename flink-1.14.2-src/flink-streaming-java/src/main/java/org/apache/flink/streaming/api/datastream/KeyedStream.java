...
public class KeyedStream<T, KEY> extends DataStream<T> {
     ...
     */
    @Internal
    KeyedStream(DataStream<T> stream,
    ...
    private TypeInformation<KEY> validateKeyType(TypeInformation<KEY> keyType) {
        ...
        if (!unsupportedTypes.isEmpty()) {
            throw new InvalidProgramException(
                    "Type " + keyType + " cannot be used as key. Contained " + "UNSUPPORTED key types: "
    ...
    private boolean validateKeyTypeIsHashable(TypeInformation<?> type) {
        try {
            return (type instanceof PojoTypeInfo) ? !type
                    .getTypeClass()
                    .getMethod("hashCode")
                    .getDeclaringClass()
    ...
    private static boolean isArrayType(TypeInformation<?> type) {
        return type instanceof PrimitiveArrayTypeInfo || type instanceof BasicArrayTypeInfo
    ...
    @Override
    protected <R> SingleOutputStreamOperator<R> doTransform(final String operatorName,
    ...
    public <R> SingleOutputStreamOperator<R> process(KeyedProcessFunction<KEY, T, R> keyedProcessFunction) {
        ...
        TypeInformation<R> outType = TypeExtractor.getUnaryOperatorReturnType(keyedProcessFunction,
    ...
    public static class IntervalJoin<T1, T2, KEY> {
        ...
        public IntervalJoined<T1, T2, KEY> between(Time lowerBound, Time upperBound) {
            ...
            return new IntervalJoined<>(streamOne,
                    streamTwo,
    ...
    public static class IntervalJoined<IN1, IN2, KEY> {
        ...
        public <OUT> SingleOutputStreamOperator<OUT> process(ProcessJoinFunction<IN1, IN2, OUT> processJoinFunction) {
            ...
            final TypeInformation<OUT> outputType = TypeExtractor.getBinaryOperatorReturnType(processJoinFunction,
        ...
        public <OUT> SingleOutputStreamOperator<OUT> process(ProcessJoinFunction<IN1, IN2, OUT> processJoinFunction,
            ...
            final ProcessJoinFunction<IN1, IN2, OUT> cleanedUdf = left
                    .getExecutionEnvironment()
                    .clean(processJoinFunction);

            final IntervalJoinOperator<KEY, IN1, IN2, OUT> operator = new IntervalJoinOperator<>(lowerBound,
                    upperBound,
                    lowerBoundInclusive,
                    upperBoundInclusive,
    ...
    public QueryableStateStream<KEY, T> asQueryableState(String queryableStateName) {
        ValueStateDescriptor<T> valueStateDescriptor = new ValueStateDescriptor<>(UUID.randomUUID().toString(),
                getType()
        );
    ...
    public QueryableStateStream<KEY, T> asQueryableState(String queryableStateName,
        ...
        return new QueryableStateStream<>(queryableStateName,
                stateDescriptor,
