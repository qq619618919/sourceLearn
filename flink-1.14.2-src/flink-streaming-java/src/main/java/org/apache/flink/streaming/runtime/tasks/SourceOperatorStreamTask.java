...
public class SourceOperatorStreamTask<T> extends StreamTask<T, SourceOperator<T, ?>> {
    ...
    public void init() throws Exception {
        ...
        if (operatorChain.isFinishedOnRestore()) {
            ...
            input = new StreamTaskExternallyInducedSourceInput<>(sourceOperator,
        ...
        output = new AsyncDataOutputToOutput<T>(operatorChain.getMainOperatorOutput(),
    ...
    private CompletableFuture<Boolean> triggerStopWithSavepointWithDrainAsync(CheckpointMetaData checkpointMetaData,
                                                                              CheckpointOptions checkpointOptions) {
        ...
        return assertTriggeringCheckpointExceptions(operatorFinished.thenCompose((ignore) -> super.triggerCheckpointAsync(
                checkpointMetaData,
                checkpointOptions
    ...
    private void triggerCheckpointForExternallyInducedSource(long checkpointId) {
        final CheckpointOptions checkpointOptions = CheckpointOptions.forConfig(CheckpointType.CHECKPOINT,
    ...
    public static class AsyncDataOutputToOutput<T> implements DataOutput<T> {
        ...
        public AsyncDataOutputToOutput(Output<StreamRecord<T>> output,
