...
public class DataStreamSink<T> {
    ...
    protected DataStreamSink(DataStream<T> inputStream, StreamSink<T> operator) {
        this.transformation = (PhysicalTransformation<T>) new LegacySinkTransformation<>(inputStream.getTransformation(),
                "Unnamed",
                operator,
    ...
    protected DataStreamSink(DataStream<T> inputStream, Sink<T, ?, ?, ?> sink) {
        transformation = (PhysicalTransformation<T>) new SinkTransformation<>(inputStream.getTransformation(),
                sink,
                "Unnamed",
                inputStream.getExecutionEnvironment().getParallelism()
        );
