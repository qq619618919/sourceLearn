...
public class SourceStreamTask<OUT, SRC extends SourceFunction<OUT>, OP extends StreamSource<OUT, SRC>> extends StreamTask<OUT, OP> {
    ...
    private CompletableFuture<Boolean> triggerStopWithSavepointWithDrainAsync(CheckpointMetaData checkpointMetaData,
                                                                              CheckpointOptions checkpointOptions) {
        mainMailboxExecutor.execute(() -> stopOperatorForStopWithSavepointWithDrain(checkpointMetaData.getCheckpointId()),
                "stop legacy source for stop-with-savepoint --drain"
        );
        return assertTriggeringCheckpointExceptions(sourceThread
                        .getCompletionFuture()
                        .thenCompose(ignore -> super.triggerCheckpointAsync(checkpointMetaData, checkpointOptions)),
