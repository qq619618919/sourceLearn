...
public class CheckpointConfig implements java.io.Serializable {
    ...
    private long checkpointIdOfIgnoredInFlightData = DEFAULT_CHECKPOINT_ID_OF_IGNORED_IN_FLIGHT_DATA;

    /** The delay from the start of checkpoint after which AC switches to UC. */
    private Duration alignedCheckpointTimeout = ExecutionCheckpointingOptions.ALIGNED_CHECKPOINT_TIMEOUT.defaultValue();
    ...
    public void setCheckpointInterval(long checkpointInterval) {
        if (checkpointInterval < MINIMAL_CHECKPOINT_TIME) {
            throw new IllegalArgumentException(String.format("Checkpoint interval must be larger than or equal to %s ms",
    ...
    public void setCheckpointTimeout(long checkpointTimeout) {
        if (checkpointTimeout < MINIMAL_CHECKPOINT_TIME) {
            throw new IllegalArgumentException(String.format("Checkpoint timeout must be larger than or equal to %s ms",
                    MINIMAL_CHECKPOINT_TIME
            ));
        }
     ...
     *
     * // TODO_MA 马中华 注释： 这个方法内部的 参数解析，是在做所有 跟 checkpoint 有关的参数解析
     */
