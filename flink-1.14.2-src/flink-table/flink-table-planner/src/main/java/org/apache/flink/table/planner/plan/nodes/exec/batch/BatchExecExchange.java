...
public class BatchExecExchange extends CommonExecExchange implements BatchExecNode<RowData> {
    ...
    public String getDescription() {
        ...
        if (requiredDistribution.getType() == InputProperty.DistributionType.HASH) {
            ...
            String[] fieldNames = Arrays
                    .stream(hashDistribution.getKeys())
    ...
    protected Transformation<RowData> translateToPlanInternal(PlannerBase planner) {
        ...
        switch (distributionType) {
            ...
            case HASH:
                ...
                String[] fieldNames = Arrays
                        ...
                        .toArray(String[]::new);
                partitioner = new BinaryHashPartitioner(HashCodeGenerator.generateRowHash(new CodeGeneratorContext(
                        planner.getTableConfig()), inputEdge.getOutputType(), "HashPartitioner", keys), fieldNames);
                parallelism = ExecutionConfig.PARALLELISM_DEFAULT;
                break;
            default:
        ...
        final StreamExchangeMode exchangeMode = getBatchStreamExchangeMode(planner.getConfiguration(),
                requiredExchangeMode
        );
        final Transformation<RowData> transformation = new PartitionTransformation<>(inputTransform,
                partitioner,
                exchangeMode
        );
