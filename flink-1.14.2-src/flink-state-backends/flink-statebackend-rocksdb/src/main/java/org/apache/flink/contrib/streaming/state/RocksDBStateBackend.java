...
/**
 * // TODO_MA 马中华 注释： RocksDBStateBackend 被废弃了，使用 EmbeddedRocksDBStateBackend
...
public class RocksDBStateBackend extends AbstractManagedMemoryStateBackend implements CheckpointStorage, ConfigurableStateBackend {
    ...
    private RocksDBStateBackend(RocksDBStateBackend original, ReadableConfig config, ClassLoader classLoader) {
        ...
        this.checkpointStreamBackend = originalStreamBackend instanceof ConfigurableStateBackend ? ((ConfigurableStateBackend) originalStreamBackend).configure(
                config,
                classLoader
    ...
    public <K> AbstractKeyedStateBackend<K> createKeyedStateBackend(Environment env,
        ...
        return rocksDBStateBackend.createKeyedStateBackend(env,
                jobID,
                operatorIdentifier,
                keySerializer,
                numberOfKeyGroups,
    ...
    public OperatorStateBackend createOperatorStateBackend(Environment env,
        ...
        return rocksDBStateBackend.createOperatorStateBackend(env,
                operatorIdentifier,
                stateHandles,
                cancelStreamRegistry
        );
    ...
    public void setPriorityQueueStateType(PriorityQueueStateType priorityQueueStateType) {
        rocksDBStateBackend.setPriorityQueueStateType(LegacyEnumBridge.convert(priorityQueueStateType));
    }
    ...
    public String toString() {
        return "RocksDBStateBackend{" + "checkpointStreamBackend=" + checkpointStreamBackend
                + ", localRocksDbDirectories=" + Arrays.toString(rocksDBStateBackend.getDbStoragePaths())
                + ", enableIncrementalCheckpointing=" + rocksDBStateBackend.isIncrementalCheckpointsEnabled()
                + ", numberOfTransferThreads=" + rocksDBStateBackend.getNumberOfTransferThreads() + ", writeBatchSize="
