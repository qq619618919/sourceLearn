...
public class DataStreamBatchExecutionITCase {
    ...
    public void batchKeyedNonKeyedTwoInputOperator() throws Exception {
        ...
        DataStream<Tuple2<String, Integer>> keyedInput = env
                .fromElements(Tuple2.of("regular2", 4),
        ...
        DataStream<Tuple2<String, Integer>> regularInput = env
                .fromElements(Tuple2.of("regular4", 4),
        ...
        DataStream<String> result = keyedInput
                .keyBy(in -> in.f0)
                .connect(regularInput)
                .transform("operator", BasicTypeInfo.STRING_TYPE_INFO, new TwoInputIdentityOperator());
        ...
        try (CloseableIterator<String> resultIterator = result.executeAndCollect()) {
            ...
            assertThat(results,
                    equalTo(Arrays.asList("(regular4,4)",
                            "(regular3,3)",
                            "(regular3,2)",
                            "(regular4,1)",
                            "(regular1,2)",
    ...
    public void batchNonKeyedKeyedTwoInputOperator() throws Exception {
        ...
        DataStream<String> result = regularInput
    ...
    public void batchKeyedBroadcastExecution() throws Exception {
        ...
        DataStream<Tuple2<String, Integer>> bcInput = env
                .fromElements(Tuple2.of("bc1", 1), Tuple2.of("bc2", 2), Tuple2.of("bc3", 3))
                .assignTimestampsAndWatermarks(WatermarkStrategy
                        .<Tuple2<String, Integer>>forMonotonousTimestamps()
                        .withTimestampAssigner((in, ts) -> in.f1));

        DataStream<Tuple2<String, Integer>> regularInput = env
                .fromElements(Tuple2.of("regular1", 1),
                        Tuple2.of("regular1", 2),
        ...
        try (CloseableIterator<String> resultIterator = result.executeAndCollect()) {
            List<String> results = CollectionUtil.iteratorToList(resultIterator);
            assertThat(results,
                    equalTo(Arrays.asList("(regular1,1): [bc2=bc2, bc1=bc1, bc3=bc3]",
    ...
    public void batchMixedKeyedAndNonKeyedTwoInputOperator() throws Exception {
        ...
        DataStream<Tuple2<String, Integer>> bcInput = env
                .fromElements(Tuple2.of("bc3", 3), Tuple2.of("bc2", 2), Tuple2.of("bc1", 1))
                .assignTimestampsAndWatermarks(WatermarkStrategy
                        .<Tuple2<String, Integer>>forMonotonousTimestamps()
                        .withTimestampAssigner((in, ts) -> in.f1))
                .broadcast();
        ...
        TwoInputTransformation<Tuple2<String, Integer>, Tuple2<String, Integer>, String> twoInputTransformation = new TwoInputTransformation<>(
        ...
        try (CloseableIterator<String> resultIterator = result.executeAndCollect()) {
            List<String> results = CollectionUtil.iteratorToList(resultIterator);
            assertThat(results,
                    equalTo(Arrays.asList("(regular1,1): [bc3, bc2, bc1]",
    ...
    public void batchMixedKeyedAndNonKeyedMultiInputOperator() throws Exception {
        ...
        DataStream<Tuple2<String, Integer>> bc1Input = env
                .fromElements(Tuple2.of("bc3", 3), Tuple2.of("bc2", 2))
                .assignTimestampsAndWatermarks(WatermarkStrategy
                        .<Tuple2<String, Integer>>forMonotonousTimestamps()
                        .withTimestampAssigner((in, ts) -> in.f1))
                .broadcast();
        DataStream<Tuple2<String, Integer>> bc2Input = env
                .fromElements(Tuple2.of("bc1", 1))
                .assignTimestampsAndWatermarks(WatermarkStrategy
                        .<Tuple2<String, Integer>>forMonotonousTimestamps()
                        .withTimestampAssigner((in, ts) -> in.f1))
                .broadcast();
        ...
        KeyedMultipleInputTransformation<String> multipleInputTransformation = new KeyedMultipleInputTransformation<>(
                "operator",
        ...
        multipleInputTransformation.addInput(regularInput.getTransformation(),
                input -> ((Tuple2<String, Integer>) input).f0
        );
    ...
    static final MapStateDescriptor<String, String> STATE_DESCRIPTOR = new MapStateDescriptor<>("bc-input",
            StringSerializer.INSTANCE,
            StringSerializer.INSTANCE
    );

    static final ValueStateDescriptor<String> KEYED_STATE_DESCRIPTOR = new ValueStateDescriptor<>("keyed-state",
            StringSerializer.INSTANCE
    );

    static final ListStateDescriptor<String> LIST_STATE_DESCRIPTOR = new ListStateDescriptor<>("bc-list-input",
            StringSerializer.INSTANCE
    );
    ...
    private static class TestKeyedBroadcastFunction extends KeyedBroadcastProcessFunction<String, Tuple2<String, Integer>, Tuple2<String, Integer>, String> {
        ...
        @Override
        public void processBroadcastElement(Tuple2<String, Integer> value,
                                            Context ctx,
                                            Collector<String> out) throws Exception {
