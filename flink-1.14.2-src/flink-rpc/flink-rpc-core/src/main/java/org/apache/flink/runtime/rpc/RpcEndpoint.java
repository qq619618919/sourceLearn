...
/**
 * // TODO_MA 马中华 注释： RPC 端点的基类。
 * Base class for RPC endpoints.
 *
 * // TODO_MA 马中华 注释： 提供远程过程调用的分布式组件必须扩展 RPC 端点基类。
 * Distributed components which offer remote procedure calls have to extend the RPC endpoint base class.
 *
 * // TODO_MA 马中华 注释： RPC 端点由 {@link RpcService} 支持。
 * An RPC endpoint is backed by an {@link RpcService}.
 *
 ...
 * // TODO_MA 马中华 注释： 和 spark 中的 RpcEndpoint 是一个东西
    ...
    public <C extends RpcGateway> C getSelfGateway(Class<C> selfGatewayType) {
        if (selfGatewayType.isInstance(rpcServer)) {
            ...
            throw new RuntimeException(
                    "RpcEndpoint does not implement the RpcGateway interface of type " + selfGatewayType + '.');
        }
    }
