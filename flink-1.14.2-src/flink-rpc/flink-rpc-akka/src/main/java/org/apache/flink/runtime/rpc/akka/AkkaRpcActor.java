...
class AkkaRpcActor<T extends RpcEndpoint & RpcGateway> extends AbstractActor {
    ...
    AkkaRpcActor(final T rpcEndpoint,
        ...
        this.rpcEndpointTerminationResult = RpcEndpointTerminationResult.failure(new AkkaRpcException(String.format(
    ...
    public void postStop() throws Exception {
        ...
        if (rpcEndpointTerminationResult.isSuccess()) {
            ...
            log.info("The RpcEndpoint {} failed.",
                    rpcEndpoint.getEndpointId(),
    ...
    private void handleMessage(final Object message) {
        if (state.isRunning()) {
            ...
            sendErrorIfSender(new AkkaRpcException(String.format(
    ...
    protected void handleRpcMessage(Object message) {
        if (message instanceof RunAsync) {
            ...
            sendErrorIfSender(new AkkaUnknownMessageException(
                    "Received unknown message " + message + " of type " + message.getClass().getSimpleName() + '.'));
        }
    }
    ...
    private void handleHandshakeMessage(RemoteHandshakeMessage handshakeMessage) {
        if (!isCompatibleVersion(handshakeMessage.getVersion())) {
            sendErrorIfSender(new AkkaHandshakeException(String.format(
    ...
    private void handleRpcInvocation(RpcInvocation rpcInvocation) {
        ...
        try {
            ...
            RpcConnectionException rpcException = new RpcConnectionException(
        ...
        if (rpcMethod != null) {
            try {
                ...
                if (rpcMethod.getReturnType().equals(Void.TYPE)) {
                    ...
                    runWithContextClassLoader(() -> capturedRpcMethod.invoke(rpcEndpoint, rpcInvocation.getArgs()),
                            flinkClassLoader
                    );
                } else {
                    final Object result;
                    try {
                        result = runWithContextClassLoader(() -> capturedRpcMethod.invoke(rpcEndpoint,
    ...
    private void sendSyncResponse(Object response, String methodName) {
        if (isRemoteSender(getSender())) {
            Either<AkkaRpcSerializedValue, AkkaRpcException> serializedResult = serializeRemoteResultAndVerifySize(
                    response,
                    methodName
            );
    ...
    private Either<AkkaRpcSerializedValue, AkkaRpcException> serializeRemoteResultAndVerifySize(Object result,
                                                                                                String methodName) {
        try {
            ...
            if (resultSize > maximumFramesize) {
                return Either.Right(new AkkaRpcException(
                        "The method " + methodName + "'s result size " + resultSize + " exceeds the maximum size "
                                + maximumFramesize + " ."));
            } else {
    ...
    interface State {
        ...
        default String invalidStateTransitionMessage(State targetState) {
            return String.format("AkkaRpcActor is currently in state %s and cannot go into state %s.",
                    this,
                    targetState
            );
        }
    ...
    enum StartedState implements State {
        ...
        public State terminate(AkkaRpcActor<?> akkaRpcActor, ClassLoader flinkClassLoader) {
            ...
            try {
                ...
                terminationFuture = FutureUtils.completedExceptionally(new AkkaRpcException(String.format(
            ...
            terminationFuture.whenComplete((ignored, throwable) -> akkaRpcActor.stop(RpcEndpointTerminationResult.of(
                    throwable)));
    ...
    enum StoppedState implements State {
        ...
        public State start(AkkaRpcActor<?> akkaRpcActor, ClassLoader flinkClassLoader) {
            ...
            try {
                runWithContextClassLoader(() -> akkaRpcActor.rpcEndpoint.internalCallOnStart(), flinkClassLoader);
            } catch (Throwable throwable) {
                akkaRpcActor.stop(RpcEndpointTerminationResult.failure(new AkkaRpcException(String.format(
