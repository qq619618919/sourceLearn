...
class AkkaInvocationHandler implements InvocationHandler, AkkaBasedEndpoint, RpcServer {
    ...
    AkkaInvocationHandler(String address,
                          String hostname,
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        if (declaringClass.equals(AkkaBasedEndpoint.class) || declaringClass.equals(Object.class)
                || declaringClass.equals(RpcGateway.class) || declaringClass.equals(StartStoppable.class)
                || declaringClass.equals(MainThreadExecutable.class) || declaringClass.equals(RpcServer.class)) {
            result = method.invoke(this, args);
        } else if (declaringClass.equals(FencedRpcGateway.class)) {
            throw new UnsupportedOperationException(
                    "AkkaInvocationHandler does not support the call FencedRpcGateway#" + method.getName()
    ...
    public void scheduleRunAsync(Runnable runnable, long delayMillis) {
        ...
        if (isLocal) {
            ...
            throw new RuntimeException("Trying to send a Runnable to a remote actor at " + rpcEndpoint.path()
    ...
    public <V> CompletableFuture<V> callAsync(Callable<V> callable, Time callTimeout) {
        if (isLocal) {
            @SuppressWarnings("unchecked") CompletableFuture<V> resultFuture = (CompletableFuture<V>) ask(new CallAsync(
                    callable), callTimeout);

            return resultFuture;
        } else {
            throw new RuntimeException("Trying to send a Callable to a remote actor at " + rpcEndpoint.path()
                    + ". This is not supported.");
        }
    }
    ...
    private Object invokeRpc(Method method, Object[] args) throws Exception {
        ...
        final RpcInvocation rpcInvocation = createRpcInvocationMessage(method.getDeclaringClass().getSimpleName(),
                methodName,
                parameterTypes,
                args
        );
        ...
        if (Objects.equals(returnType, Void.TYPE)) {
            ...
            if (Objects.equals(returnType, CompletableFuture.class)) {
                ...
                try {
                    ...
                    throw new RpcException("Failure while obtaining synchronous RPC result.",
     ...
     */
    protected RpcInvocation createRpcInvocationMessage(final String declaringClassName,
        ...
        if (isLocal) {
            ...
            try {
                RemoteRpcInvocation remoteRpcInvocation = new RemoteRpcInvocation(declaringClassName,
                        methodName,
                        parameterTypes,
                        args
                );
                ...
                if (remoteRpcInvocation.getSize() > maximumFramesize) {
                    throw new IOException(String.format("The rpc invocation size %d exceeds the maximum akka framesize.",
                ...
                if (isRpcTimeout(parameterAnnotations[i])) {
                    if (args[i] instanceof Time) {
                        ...
                        throw new RuntimeException(
                                "The rpc timeout parameter must be of type " + Time.class.getName() + ". The type "
                                        + args[i].getClass().getName() + " is not supported.");
                    }
                }
            }
        }
    ...
    protected CompletableFuture<?> ask(Object message, Time timeout) {
        final CompletableFuture<?> response = AkkaFutureUtils.toJava(Patterns.ask(rpcEndpoint,
                message,
    ...
    static Object deserializeValueIfNeeded(Object o, Method method) {
        if (o instanceof AkkaRpcSerializedValue) {
            try {
                ...
                throw new CompletionException(new RpcException(
    ...
    static Throwable resolveTimeoutException(Throwable exception,
        ...
        if (AkkaRpcServiceUtils.isRecipientTerminatedException(exception)) {
            ...
            newException = new TimeoutException(String.format("Invocation of [%s] at recipient [%s] timed out.",
                    rpcInvocation,
                    recipient
            ));
        }
