...
public final class ActorSystemScheduledExecutorAdapter implements ScheduledExecutor {
    ...
    public ScheduledFuture<?> scheduleAtFixedRate(@Nonnull Runnable command,
        ...
        Cancellable cancellable = actorSystem
                .scheduler()
    ...
    public void execute(@Nonnull Runnable command) {
        actorSystem.dispatcher().execute(ClassLoadingUtils.withContextClassLoader(command, flinkClassLoader));
    }

    private Cancellable internalSchedule(Runnable runnable, long delay, TimeUnit unit) {
        return actorSystem
                .scheduler()
                .scheduleOnce(new FiniteDuration(delay, unit),
