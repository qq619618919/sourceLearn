...
public class FencedAkkaInvocationHandler<F extends Serializable> extends AkkaInvocationHandler implements FencedMainThreadExecutable, FencedRpcGateway<F> {
    ...
    public FencedAkkaInvocationHandler(String address,
                                       String hostname,
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        ...
        if (declaringClass.equals(FencedMainThreadExecutable.class) || declaringClass.equals(FencedRpcGateway.class)) {
    ...
    public void runAsyncWithoutFencing(Runnable runnable) {
        ...
        if (isLocal) {
            ...
            throw new RuntimeException("Trying to send a Runnable to a remote actor at " + getActorRef().path()
    ...
    public <V> CompletableFuture<V> callAsyncWithoutFencing(Callable<V> callable, Time timeout) {
        ...
        if (isLocal) {
            ...
            CompletableFuture<V> resultFuture = (CompletableFuture<V>) AkkaFutureUtils.toJava(Patterns.ask(getActorRef(),
    ...
    private <P> FencedMessage<F, P> fenceMessage(P message) {
        if (isLocal) {
            ...
            if (message instanceof Serializable) {
                @SuppressWarnings("unchecked")
                FencedMessage<F, P> result = (FencedMessage<F, P>) new RemoteFencedMessage<>(fencingTokenSupplier.get(),
