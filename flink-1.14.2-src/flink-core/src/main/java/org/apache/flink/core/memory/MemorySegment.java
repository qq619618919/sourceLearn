...
public final class MemorySegment {
    ...
    public void get(int index, byte[] dst, int offset, int length) {
        ...
        if (index >= 0 && pos <= addressLimit - length) {
            ...
            throw new IndexOutOfBoundsException(String.format("pos: %d, length: %d, index: %d, offset: %d",
                    pos,
                    length,
                    index,
                    offset
    ...
    public void copyToUnsafe(int offset, Object target, int targetPointer, int numBytes) {
        ...
        if (thisPointer + numBytes > addressLimit) {
            throw new IndexOutOfBoundsException(String.format("offset=%d, numBytes=%d, address=%d",
                    offset,
                    numBytes,
                    this.address
            ));
