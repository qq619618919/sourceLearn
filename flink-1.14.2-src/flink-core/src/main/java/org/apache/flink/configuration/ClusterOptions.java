...
public class ClusterOptions {
    ...
    public static final ConfigOption<Boolean> HALT_ON_FATAL_ERROR = key("cluster.processes.halt-on-fatal-error")
            .booleanType()
            .defaultValue(false)
            .withDescription(Description
                    .builder()
                    .text("Whether processes should halt on fatal errors instead of performing a graceful shutdown. "
    ...
    @Documentation.Section(Documentation.Sections.EXPERT_CLUSTER)
    public static final ConfigOption<UserSystemExitMode> INTERCEPT_USER_SYSTEM_EXIT = key(
            "cluster.intercept-user-system-exit")
            .enumType(UserSystemExitMode.class)
            .defaultValue(UserSystemExitMode.DISABLED)
            .withDescription(Description
                    .builder()
                    .text("Flag to check user code exiting system by terminating JVM (e.g., System.exit()). ")
                    .text("Note that this configuration option can interfere with %s: "
                                    + "In intercepted user-code, a call to System.exit() will not cause the JVM to halt, when %s is configured.",
                            code(HALT_ON_FATAL_ERROR.key()),
                            code(THROW.name())
                    )
                    .build());

    @Documentation.Section(Documentation.Sections.EXPERT_SCHEDULING)
    public static final ConfigOption<Boolean> ENABLE_FINE_GRAINED_RESOURCE_MANAGEMENT = ConfigOptions
            .key("cluster.fine-grained-resource-management.enabled")
            .booleanType()
            .defaultValue(false)
            .withDescription("Defines whether the cluster uses fine-grained resource management.");

    @Documentation.Section(Documentation.Sections.EXPERT_SCHEDULING)
    public static final ConfigOption<Boolean> FINE_GRAINED_SHUFFLE_MODE_ALL_BLOCKING = ConfigOptions
            .key("fine-grained.shuffle-mode.all-blocking")
            .booleanType()
            .defaultValue(false)
            .withDescription(
    ...
    @Documentation.Section(Documentation.Sections.EXPERT_CLUSTER)
    public static final ConfigOption<UncaughtExceptionHandleMode> UNCAUGHT_EXCEPTION_HANDLING = ConfigOptions
            .key("cluster.uncaught-exception-handling")
            .enumType(UncaughtExceptionHandleMode.class)
            .defaultValue(UncaughtExceptionHandleMode.LOG)
            .withDescription(String.format("Defines whether cluster will handle any uncaught exceptions "
    ...
    public static boolean isAdaptiveSchedulerEnabled(Configuration configuration) {
        // TODO_MA 马中华 注释： jobmanager.scheduler = NG
