...
public class ExecutionOptions {
    ...
    public static final ConfigOption<RuntimeExecutionMode> RUNTIME_MODE = ConfigOptions
    ...
    public static final ConfigOption<BatchShuffleMode> BATCH_SHUFFLE_MODE = ConfigOptions
            ...
            .withDescription(Description
                    .builder()
                    .text("Defines how data is exchanged between tasks in batch '%s' if the shuffling "
                                    + "behavior has not been set explicitly for an individual exchange.",
                            text(RUNTIME_MODE.key())
                    )
                    .linebreak()
                    .text("With pipelined exchanges, upstream and downstream tasks run simultaneously. "
                            ...
                            + "back-pressures the sender. The streaming mode always uses this " + "exchange.")
                    .linebreak()
                    .text("With blocking exchanges, upstream and downstream tasks run in stages. "
                            ...
                            + "job as it does not need to run upstream and downstream " + "tasks simultaneously.")
                    .build());
    ...
    public static final ConfigOption<Duration> BUFFER_TIMEOUT = ConfigOptions
            ...
            .withDescription(Description
                    ...
                    .list(text("A positive value triggers flushing periodically by that interval"),
                            text("0 triggers flushing after every record thus minimizing latency"),
                            text("-1 ms triggers flushing only when the output buffer is full thus maximizing "
                                    + "throughput")
                    )
                    .build());

    @Documentation.ExcludeFromDocumentation("This is an expert option, that we do not want to expose in the documentation")
    public static final ConfigOption<Boolean> SORT_INPUTS = ConfigOptions
            .key("execution.sorted-inputs.enabled")
            .booleanType()
            .defaultValue(true)
            .withDescription("A flag to enable or disable sorting inputs of keyed operators. "
    ...
    public static final ConfigOption<MemorySize> SORTED_INPUTS_MEMORY = ConfigOptions
            ...
            .withDescription("Sets the managed memory size for sorting inputs of keyed operators in "
