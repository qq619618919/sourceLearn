...
public class HiveSourceBuilder {
     ...
     */
    public HiveSourceBuilder(@Nonnull JobConf jobConf,
    ...
    public <T> HiveSource<T> buildWithBulkFormat(BulkFormat<T, HiveSourceSplit> bulkFormat) {
        ...
        if (isStreamingSource()) {
            Preconditions.checkState(partitions == null, "setPartitions shouldn't be called in streaming mode");
            if (partitionKeys.isEmpty()) {
                FileSystemConnectorOptions.PartitionOrder partitionOrder = configuration.get(
                        STREAMING_SOURCE_PARTITION_ORDER);
                if (partitionOrder != FileSystemConnectorOptions.PartitionOrder.CREATE_TIME) {
                    throw new UnsupportedOperationException(
                            "Only '" + FileSystemConnectorOptions.PartitionOrder.CREATE_TIME
                ...
                partitions = Collections.singletonList(HiveTablePartition.ofTable(HiveConfUtils.create(jobConf),
                        hiveVersion,
            ...
            Duration monitorInterval = configuration.get(STREAMING_SOURCE_MONITOR_INTERVAL)
                    == null ? DEFAULT_SCAN_MONITOR_INTERVAL : configuration.get(STREAMING_SOURCE_MONITOR_INTERVAL);
            checkArgument(!(monitorInterval.isNegative() || monitorInterval.isZero()), "monitorInterval must be > 0");
            ...
            if (!partitionKeys.isEmpty()) {
                ...
                fetcherContext = new HiveTableSource.HiveContinuousPartitionFetcherContext(tablePath,
            ...
            partitions = HivePartitionUtils.getAllPartitions(jobConf, hiveVersion, tablePath, partitionKeys, null);
        }

        FileSplitAssigner.Provider splitAssigner = continuousSourceSettings == null
                || partitionKeys.isEmpty() ? DEFAULT_SPLIT_ASSIGNER : SimpleSplitAssigner::new;
        return new HiveSource<>(new Path[1],
                new HiveSourceFileEnumerator.Provider(partitions != null ? partitions : Collections.emptyList(),
                        new JobConfWrapper(jobConf)
                ),
                splitAssigner,
                bulkFormat,
    ...
    private static void validateScanConfigurations(Map<String, String> configurations) {
        String partitionInclude = configurations.getOrDefault(STREAMING_SOURCE_PARTITION_INCLUDE.key(),
                STREAMING_SOURCE_PARTITION_INCLUDE.defaultValue()
        );
        Preconditions.checkArgument("all".equals(partitionInclude),
                String.format("The only supported '%s' is 'all' in hive table scan, but is '%s'",
    ...
    private boolean isStreamingSource() {
        return Boolean.parseBoolean(tableOptions.getOrDefault(STREAMING_SOURCE_ENABLE.key(),
    ...
    private RowType getProducedRowType() {
        ...
        if (projectedFields == null) {
            ...
            producedSchema = TableSchema
                    .builder()
                    .fields(Arrays.stream(projectedFields).mapToObj(i -> fullNames[i]).toArray(String[]::new),
    ...
    private BulkFormat<RowData, HiveSourceSplit> createDefaultBulkFormat() {
        return LimitableBulkFormat.create(new HiveBulkFormatAdapter(new JobConfWrapper(jobConf),
                partitionKeys,
                fullSchema.getFieldNames(),
                fullSchema.getFieldDataTypes(),
                hiveVersion,
