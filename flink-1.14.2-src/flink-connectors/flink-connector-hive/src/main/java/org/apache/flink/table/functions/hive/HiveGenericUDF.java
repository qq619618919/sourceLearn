...
public class HiveGenericUDF extends HiveScalarFunction<GenericUDF> {
    ...
    public void openInternal() {
        ...
        for (int i = 0; i < deferredObjects.length; i++) {
            deferredObjects[i] = new DeferredObjectAdapter(argInspectors[i], argTypes[i].getLogicalType(), hiveShim);
        }
    }
    ...
    public Object evalInternal(Object[] args) {
        ...
        try {
            Object result = returnInspector instanceof ConstantObjectInspector ? ((ConstantObjectInspector) returnInspector).getWritableConstantValue() : function.evaluate(
                    deferredObjects);
    ...
    protected DataType inferReturnType() throws UDFArgumentException {
        ...
        ObjectInspector resultObjectInspector = createFunction().initializeAndFoldConstants(argumentInspectors);
