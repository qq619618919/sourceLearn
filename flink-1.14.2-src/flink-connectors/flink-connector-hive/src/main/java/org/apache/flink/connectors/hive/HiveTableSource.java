...
public class HiveTableSource implements ScanTableSource, SupportsPartitionPushDown, SupportsProjectionPushDown, SupportsLimitPushDown {
    ...
    public HiveTableSource(JobConf jobConf, ReadableConfig flinkConf, ObjectPath tablePath, CatalogTable catalogTable) {
        ...
        this.hiveVersion = Preconditions.checkNotNull(jobConf.get(HiveCatalogFactoryOptions.HIVE_VERSION.key()),
    ...
    protected DataStream<RowData> getDataStream(StreamExecutionEnvironment execEnv) {
        HiveSourceBuilder sourceBuilder = new HiveSourceBuilder(jobConf,
                flinkConf,
                tablePath,
                hiveVersion,
                catalogTable
        ).setProjectedFields(projectedFields).setLimit(limit);

        if (isStreamingSource()) {
            return toDataStreamSource(execEnv, sourceBuilder.buildWithDefaultBulkFormat());
        } else {
            List<HiveTablePartition> hivePartitionsToRead = getAllPartitions(jobConf,
                    hiveVersion,
                    tablePath,
            ...
            int parallelism = new HiveParallelismInference(tablePath, flinkConf)
                    .infer(() -> HiveSourceFileEnumerator.getNumFiles(hivePartitionsToRead, jobConf),
                            () -> HiveSourceFileEnumerator.createInputSplits(0, hivePartitionsToRead, jobConf).size()
                    )
                    .limit(limit);
            return toDataStreamSource(execEnv,
                    sourceBuilder.setPartitions(hivePartitionsToRead).buildWithDefaultBulkFormat()
            ).setParallelism(parallelism);
        }
    }

    private DataStreamSource<RowData> toDataStreamSource(StreamExecutionEnvironment execEnv,
                                                         HiveSource<RowData> hiveSource) {
        return execEnv.fromSource(hiveSource,
    ...
    protected boolean isStreamingSource() {
        return Boolean.parseBoolean(catalogTable
                .getOptions()
    ...
    public static class HiveContinuousPartitionFetcherContext<T extends Comparable<T>> extends HivePartitionFetcherContextBase<Partition> implements HiveContinuousPartitionContext<Partition, T> {
        ...
        public HiveContinuousPartitionFetcherContext(ObjectPath tablePath,
                                                     HiveShim hiveShim,
        ...
        public Optional<Partition> getPartition(List<String> partValues) throws TException {
            try {
                return Optional.of(metaStoreClient.getPartition(tablePath.getDatabaseName(),
